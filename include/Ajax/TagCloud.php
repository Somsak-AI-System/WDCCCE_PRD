...
if($ajaxaction == "SAVETAG")
{
	...
	if($tagfields != "")
	{
		...
		{	
			//$freetag->tag_object($userid,$crmid,$tagfields,$module,$list_max_tags);
			$response = $freetag->tag_object($userid,$crmid,$tagfields,$module,$list_max_tags);
			if($response == false){
				echo ":#:MAXTAG"; 
			}else{
...
}elseif($ajaxaction == 'DELETETAG')
{
	if(is_numeric($_REQUEST['tagid']))
	{
		...
		global $adb;
		$query="delete from aicrm_freetagged_objects where tag_id=? and object_id=?";
