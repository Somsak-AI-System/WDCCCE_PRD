...
*/
	...
	function getListViewNavigation($currentRecordId){
		...
		if($currentModule == 'Documents'){
			$sql = "select folderid from aicrm_notes where notesid=?";
		...
		if($reUseData === false){
			...
			if($currentModule=='Documents' && !empty($folderId)){
				...
				if ($findOrderByPosition > 0) {
					...
					$list_query = substr($list_query, 0, $findOrderByPosition). " AND aicrm_notes.folderid=$folderId " . $orderByClause;
				} else {
					$list_query .= " AND aicrm_notes.folderid=$folderId";
				}
			}
			if($start !=1){
	...
	function getRequestCurrentPage($currentModule, $query, $viewid, $queryMode = false) {
		...
		if(!$queryMode) {
			...
			$_SESSION['nav_start']= intval($start);
		}
		return $start;
	}
