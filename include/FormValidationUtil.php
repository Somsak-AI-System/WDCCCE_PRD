...
function getDBValidationData($tablearray, $tabid='') {
    ...
    $fieldres =  $adb->pquery(
      "SELECT aicrm_field.fieldlabel,aicrm_field.fieldname,aicrm_field.typeofdata,aicrm_field.block,aicrm_field.sequence,aicrm_field.uitype
FROM aicrm_field 
inner join aicrm_blocks on aicrm_blocks.blockid =  aicrm_field.block
WHERE aicrm_field.displaytype IN (1,3) AND aicrm_field.presence in (0,2) AND aicrm_field.tabid =? order by aicrm_blocks.sequence asc , aicrm_field.sequence  asc", Array($tabid));
    /**/
...
function getDBValidationDataUitype($tablearray, $tabid='') {
  if($tabid != '') {    
    global $adb, $mod_strings;
    $fieldModuleName = getTabModuleName($tabid);
    $fieldres =  $adb->pquery(
...
WHERE aicrm_field.displaytype IN (1,3) AND aicrm_field.presence in (0,2) AND aicrm_field.tabid =? order by aicrm_blocks.sequence asc , aicrm_field.sequence  asc", Array($tabid));
    ...
    while($fieldrow = $adb->fetch_array($fieldres)) {
      $fieldname = $fieldrow['fieldname'];
      $uitypedata= $fieldrow['uitype'];
      $fieldinfos[$fieldname] = Array('FieldUitype' => $uitypedata);
    }
    return $fieldinfos;
  } else {
    //  TODO: Call the old API defined below in the file?
...
function getDBValidationData_510($tablearray,$tabid='')
{
  ...
  for($i=0;$i<$numValues;$i++)
  {
  	...
  	if(in_array("emails",$tablearray))
  	{
    	...
    	{
			$sql .= "select fieldlabel,fieldname,typeofdata from aicrm_field where tablename=? and tabid=10 and aicrm_field.presence in (0,2) and displaytype <> 2 union ";
			array_push($params, $tablearray[$i]);	
     	}
   		else
    	{
   			$sql  .= "select fieldlabel,fieldname,typeofdata from aicrm_field where tablename=? and tabid=10 and aicrm_field.presence in (0,2) and displaytype <> 2 ";
    		array_push($params, $tablearray[$i]);	
		}
  	}
  	else
  	{
    		if($numValues > 1 && $i != $numValues-1)
    		{
      			$sql .= "select fieldlabel,fieldname,typeofdata from aicrm_field where tablename=? $tab_con and displaytype in (1,3) and aicrm_field.presence in (0,2) union ";
    			array_push($params, $tablearray[$i]);	
			}
    		else
    		{
      			$sql  .= "select fieldlabel,fieldname,typeofdata from aicrm_field where tablename=? $tab_con and displaytype in (1,3) and aicrm_field.presence in (0,2)";
    			array_push($params, $tablearray[$i]);	
			}
  	}
  }
