...
if(!empty($_REQUEST['selectedColumnsString'])) {
	...
	if(!empty($merge_criteria_cols)) {
		...
		$adb->pquery("DELETE FROM aicrm_user2mergefields WHERE tabid=? AND userid=?", array($tabid, $current_user->id));
	
		// Update the new merge field selections
		foreach($merge_criteria_cols as $merge_fieldid) {
			$adb->pquery("INSERT INTO aicrm_user2mergefields (userid, tabid, fieldid, visible) VALUES (?,?,?,?)", 
