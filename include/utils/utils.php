...
function get_user_array($add_blank=true, $status="Active", $assigned_user="",$private="")
{
	...
	if($user_array == null)
	{
		...
		$temp_result = Array();
		// Including deleted aicrm_users for now.
		if (empty($status)) {
				$query = "SELECT id, user_name from aicrm_users";
				$params = array();
		}
		else {
				if($private == 'private')
...
function get_group_array($add_blank=true, $status="Active", $assigned_user="",$private="")
{
	...
	if($group_array == null)
	{
		...
		// Including deleted aicrm_users for now.
		$log->debug("Sharing is Public. All aicrm_users should be listed");
		$query = "SELECT groupid, groupname from aicrm_groups";
		$params = array();		
		...
		if($private == 'private'){
			...
			if(count($current_user_groups) != 0) {
				$query .= " OR aicrm_groups.groupid in (".generateQuestionMarks($current_user_groups).")";
				array_push($params, $current_user_groups);
			}
			$log->debug("Sharing is Private. Only the current user should be listed");
			$query .= " union select aicrm_group2role.groupid as groupid,aicrm_groups.groupname as groupname from aicrm_group2role inner join aicrm_groups on aicrm_groups.groupid=aicrm_group2role.groupid inner join aicrm_role on aicrm_role.roleid=aicrm_group2role.roleid where aicrm_role.parentrole like ?";
			array_push($params, $current_user_parent_role_seq."::%");
			
			if(count($current_user_groups) != 0) {
				$query .= " union select aicrm_groups.groupid as groupid,aicrm_groups.groupname as groupname from aicrm_groups inner join aicrm_group2rs on aicrm_groups.groupid=aicrm_group2rs.groupid where aicrm_group2rs.roleandsubid in (".generateQuestionMarks($parent_roles).")";
				array_push($params, $parent_roles);
			}
					
			$query .= " union select sharedgroupid as groupid,aicrm_groups.groupname as groupname from aicrm_tmp_write_group_sharing_per inner join aicrm_groups on aicrm_groups.groupid=aicrm_tmp_write_group_sharing_per.sharedgroupid where aicrm_tmp_write_group_sharing_per.userid=?";
			array_push($params, $current_user->id);
			
			$query .= " and aicrm_tmp_write_group_sharing_per.tabid=?";
...
/**
 * Create javascript to set the cursor focus to specific aicrm_field in a form
 * when the screen is rendered.  The aicrm_field name is currently hardcoded into the
 * the function.
 * Portions created by SugarCRM are Copyright (C) SugarCRM, Inc.
 * All Rights Reserved.
 * Contributor(s): ______________________________________..
...
//TODO Clint 5/20 - Make this function more generic so that it can take in the target form and aicrm_field names as variables
$the_script = <<<EOQ
<script type="text/javascript" language="JavaScript">
<!-- Begin
function set_focus() {
	if (document.forms.length > 0) {
		for (i = 0; i < document.forms.length; i++) {
			for (j = 0; j < document.forms[i].elements.length; j++) {
				var aicrm_field = document.forms[i].elements[j];
				if ((aicrm_field.type == "text" || aicrm_field.type == "textarea" || aicrm_field.type == "password") &&
						!field.disabled && (aicrm_field.name == "first_name" || aicrm_field.name == "name")) {
				aicrm_field.focus();
                    if (aicrm_field.type == "text") {
                        aicrm_field.select();
                    }
					break;
	    		}
...
function getTabname($tabid)
{
	...
	$log->debug("Entering getTabname(".$tabid.") method ...");
        ...
        global $adb;
	$sql = "select tablabel from aicrm_tab where tabid=?";
...
function getTabModuleName($tabid)
{
	...
	if($tabname === false) {
		if (file_exists('tabdata.php') && (filesize('tabdata.php') != 0)) {
			...
			$log->info("tab id is ".$tabid);
	        global $adb;
	        $sql = "select name from aicrm_tab where tabid=?";
	        $result = $adb->pquery($sql, array($tabid));
...
{	//echo "555";exit;
	...
	if($cachedModuleFields === false) {
		...
		FROM aicrm_field WHERE tabid in (" . generateQuestionMarks($tabid) . ") order by fieldlabel ASC";
	...
	$log->debug("Exiting getColumnFields method ...");
   	//echo "<pre>";print_r($column_fld);echo "</pre>"; exit;
	return $column_fld;	
}
...
function getUserEmail($userid)
{
	...
	$log->info("in getUserEmail ".$userid);
        ...
        if($userid != '')
        {
                $sql = "select email1 from aicrm_users where id=?";
...
function getUserId_Ol($username)
{
	...
	global $adb;
	$sql = "select id from aicrm_users where user_name=?";
...
function getActionid($action)
{
	...
	{
		$query="select * from aicrm_actionmapping where actionname=?";
...
function getActionname($actionid)
{
	...
	else
	{
		...
		$query="select * from aicrm_actionmapping where actionid=? and securitycheck=0";
...
function getUserId($record)
{
	...
	global $adb;
        $user_id=$adb->query_result($adb->pquery("select * from aicrm_crmentity where crmid = ?", array($record)),0,'smownerid');
...
function getRecordOwnerId($record)
{
	...
	global $adb;
	$ownerArr=Array();
	$query="select smownerid from aicrm_crmentity where crmid = ?";
	$result=$adb->pquery($query, array($record));
	if($adb->num_rows($result) > 0)
	{
		$ownerId=$adb->query_result($result,0,'smownerid');
		$sql_result = $adb->pquery("select count(*) as count from aicrm_users where id = ?",array($ownerId));
...
function insertProfile2field($profileid)
{
	...
	$fld_result = $adb->pquery("select * from aicrm_field where generatedtype=1 and displaytype in (1,2,3) and aicrm_field.presence in (0,2) and tabid != 29", array());
        ...
        for($i=0; $i<$num_rows; $i++)
        {
                 ...
                 $adb->pquery("insert into aicrm_profile2field values (?,?,?,?,?)", $params);
	}
...
function insert_def_org_field()
{
	...
	$fld_result = $adb->pquery("select * from aicrm_field where generatedtype=1 and displaytype in (1,2,3) and aicrm_field.presence in (0,2) and tabid != 29", array());
        ...
        for($i=0; $i<$num_rows; $i++)
        {
                 ...
                 $adb->pquery("insert into aicrm_def_org_field values (?,?,?,?)", $params);
	}
...
function getProfile2FieldList($fld_module, $profileid)
{
	...
	$log->debug("Entering getProfile2FieldList(".$fld_module.",". $profileid.") method ...");
        $log->info("in getProfile2FieldList ".$fld_module. ' aicrm_profile id is  '.$profileid);

	global $adb;
	$tabid = getTabid($fld_module);
	
	$query = "select aicrm_profile2field.visible,aicrm_field.* from aicrm_profile2field inner join aicrm_field on aicrm_field.fieldid=aicrm_profile2field.fieldid where aicrm_profile2field.profileid=? and aicrm_profile2field.tabid=? and aicrm_field.presence in (0,2)";
...
function getProfile2FieldPermissionList($fld_module, $profileid)
{
    ...
    if($return_data === false) {
		...
		$query = "SELECT aicrm_profile2field.visible, aicrm_field.fieldlabel, aicrm_field.uitype, 
			aicrm_field.fieldid, aicrm_field.displaytype, aicrm_field.typeofdata 
			FROM aicrm_profile2field INNER JOIN aicrm_field ON aicrm_field.fieldid=aicrm_profile2field.fieldid 
			WHERE aicrm_profile2field.profileid=? and aicrm_profile2field.tabid=? and aicrm_field.presence in (0,2)";
    	...
    	for($i=0; $i<$adb->num_rows($result); $i++) {
			$return_data[]=array(
				...
				$adb->query_result($result,$i,"visible"), // From aicrm_profile2field.visible
...
function getProfile2AllFieldList($mod_array,$profileid)
{
	global $log;
     ...
     $log->info("in getProfile2AllFieldList aicrm_profile id is " .$profileid);
...
function getDefOrgFieldList($fld_module)
{
	...
	$query = "select aicrm_def_org_field.visible,aicrm_field.* from aicrm_def_org_field inner join aicrm_field on aicrm_field.fieldid=aicrm_def_org_field.fieldid where aicrm_def_org_field.tabid=? and aicrm_field.presence in (0,2)";
	$qparams = array($tabid);
	$result = $adb->pquery($query, $qparams);
...
function ChangeStatus($status,$activityid,$activity_mode='')
 {
	...
	$log->debug("Entering ChangeStatus(".$status.",".$activityid.",".$activity_mode."='') method ...");
        $log->info("in ChangeStatus ".$status. ' aicrm_activityid is  '.$activityid);

        global $adb;
        if ($activity_mode == 'Task')
        {
                $query = "Update aicrm_activity set status=? where activityid = ?";
        }
        elseif ($activity_mode == 'Events')
        {
                $query = "Update aicrm_activity set eventstatus=? where activityid = ?";
        }
		if($query) {
...
function getDBInsertDateValue($value)
{
    ...
    else if(preg_match("/^(0[1-9]|[1-2][0-9]|3[0-1])-(0[1-9]|1[0-2])-[0-9]{4}$/",$value)){
        list($d,$m,$y) = split('-',$value);
    }else if(preg_match("/^(0[1-9]|1[0-2])-(0[1-9]|[1-2][0-9]|3[0-1])-[0-9]{4}$/",$value)){
...
function getUnitPrice($productid, $module='Products')
{
	...
	if($module == 'Services') {
    	$query = "select unit_price from aicrm_service where serviceid=?";
	} else {
    	$query = "select unit_price from aicrm_products where productid=?";		
	}
...
function getContactImageName($id)
{
	...
	$log->debug("Entering getContactImageName(".$id.") method ...");
        global $adb;
        $query = "select imagename from aicrm_contactdetails where contactid=?";
        $result = $adb->pquery($query, array($id));
        $image_name = $adb->query_result($result,0,"imagename");
...
function getInventoryTotal($return_module,$id)
{
	...
	{
		$query ="select aicrm_products.productname,aicrm_products.unit_price,aicrm_products.qtyinstock,aicrm_seproductsrel.* from aicrm_products inner join aicrm_seproductsrel on aicrm_seproductsrel.productid=aicrm_products.productid where crmid=?";
	}
	elseif($return_module == "Products")
	{
		$query="select aicrm_products.productid,aicrm_products.productname,aicrm_products.unit_price,aicrm_products.qtyinstock,aicrm_crmentity.* from aicrm_products inner join aicrm_crmentity on aicrm_crmentity.crmid=aicrm_products.productid where aicrm_crmentity.deleted=0 and productid=?";
	}
	$result = $adb->pquery($query, array($id));
	$num_rows=$adb->num_rows($result);
	$total=0;
...
function updateProductQty($product_id, $upd_qty)
{
	...
	global $adb;
	$query= "update aicrm_products set qtyinstock=? where productid=?";
...
function getEmailParentsList($module,$id,$focus = false)
{
    ...
    if($focus === false) {
        ...
        $res = $adb->pquery("select * from aicrm_field where tabid = ? and fieldname= ? and aicrm_field.presence in (0,2)", array(getTabid($module), $fieldname));
...
function getPoStatus($po_id)
{
	...
	global $log;
        ...
        global $adb;
        $sql = "select postatus from aicrm_purchaseorder where purchaseorderid=?";
...
function addToProductStock($productId,$qty)
{
	...
	$sql = "UPDATE aicrm_products set qtyinstock=? where productid=?";
...
function addToProductDemand($productId,$qty)
{
	...
	$updQty=$qtyInStck + $qty;
	$sql = "UPDATE aicrm_products set qtyindemand=? where productid=?";
	$adb->pquery($sql, array($updQty, $productId));
...
function getProductQtyInStock($product_id)
{
	...
	$log->debug("Entering getProductQtyInStock(".$product_id.") method ...");
        global $adb;
        $query1 = "select qtyinstock from aicrm_products where productid=?";
...
function getProductQtyInDemand($product_id)
{
	...
	$log->debug("Entering getProductQtyInDemand(".$product_id.") method ...");
        global $adb;
        $query1 = "select qtyindemand from aicrm_products where productid=?";
        $result = $adb->pquery($query1, array($product_id));
...
/**     Function to get the aicrm_table name from 'field' aicrm_table for the input aicrm_field based on the module
 *      @param  : string $module - current module value
 *      @param  : string $fieldname - aicrm_fieldname to which we want the aicrm_tablename
 *      @return : string $tablename - aicrm_tablename in which $fieldname is a column, which is retrieved from 'field' aicrm_table per $module basis
 */
function getTableNameForField($module,$fieldname)
{
	...
	$sql = "select tablename from aicrm_field where tabid in (". generateQuestionMarks($tabid) .") and aicrm_field.presence in (0,2) and columnname like ?";
...
function getEmailsRelatedAccounts($record_id)
{
	...
	global $adb;
	$query = "select aicrm_seactivityrel.crmid from aicrm_seactivityrel inner join aicrm_crmentity on aicrm_crmentity.crmid=aicrm_seactivityrel.crmid where aicrm_crmentity.setype='Accounts' and activityid=?";
	$result = $adb->pquery($query, array($record_id));
...
function getEmailsRelatedLeads($record_id)
{
	...
	global $adb;
	$query = "select aicrm_seactivityrel.crmid from aicrm_seactivityrel inner join aicrm_crmentity on aicrm_crmentity.crmid=aicrm_seactivityrel.crmid where aicrm_crmentity.setype='Leads' and activityid=?";
	$result = $adb->pquery($query, array($record_id));
...
function getHelpDeskRelatedAccounts($record_id)
{
	...
	global $adb;
        $query="select accountid from aicrm_troubletickets inner join aicrm_crmentity on aicrm_crmentity.crmid=aicrm_troubletickets.accountid where ticketid=? and aicrm_crmentity.setype='Accounts'";
        $result=$adb->pquery($query, array($record_id));
...
function getQuotesRelatedAccounts($record_id)
{
	...
	global $adb;
        $query="select accountid from aicrm_quotes where quoteid=?";
        $result=$adb->pquery($query, array($record_id));
        $accountid=$adb->query_result($result,0,'accountid');
...
function getQuotesRelatedPotentials($record_id)
{
	...
	global $adb;
        $query="select potentialid from aicrm_quotes where quoteid=?";
        $result=$adb->pquery($query, array($record_id));
...
function getSalesOrderRelatedAccounts($record_id)
{
	...
	global $adb;
        $query="select accountid from aicrm_salesorder where salesorderid=?";
        $result=$adb->pquery($query, array($record_id));
        $accountid=$adb->query_result($result,0,'accountid');
...
function getSalesOrderRelatedPotentials($record_id)
{
	...
	global $adb;
        $query="select potentialid from aicrm_salesorder where salesorderid=?";
        $result=$adb->pquery($query, array($record_id));
        $potid=$adb->query_result($result,0,'potentialid');
...
function getSalesOrderRelatedQuotes($record_id)
{
	...
	global $adb;
        $query="select quoteid from aicrm_salesorder where salesorderid=?";
        $result=$adb->pquery($query, array($record_id));
...
function getInvoiceRelatedAccounts($record_id)
{
	...
	global $adb;
        $query="select accountid from aicrm_invoice where invoiceid=?";
        $result=$adb->pquery($query, array($record_id));
        $accountid=$adb->query_result($result,0,'accountid');
...
function getInvoiceRelatedSalesOrder($record_id)
{
	...
	global $adb;
        $query="select salesorderid from aicrm_invoice where invoiceid=?";
        $result=$adb->pquery($query, array($record_id));
...
/**   Function to get the Graph and aicrm_table format for a particular date
...
function Graph_n_table_format($period_type,$date_value)
{
	...
	$log->debug("Entering Graph_n_table_format(".$period_type.",".$date_value.") method ...");
        ...
        if($period_type=="month")   //to get the aicrm_table format dates
        {
...
function getUserImageName($id)
{
	...
	global $adb;
	$query = "select imagename from aicrm_users where id=?";
	$result = $adb->pquery($query, array($id));
	$image_name = $adb->query_result($result,0,"imagename");
...
function getUserImageNames()
{
	...
	global $adb;
	$query = "select imagename from aicrm_users where deleted=0";
...
function useInternalMailer() {
	...
	return $adb->query_result($adb->pquery("select int_mailer from aicrm_mail_accounts where user_id=?", array($current_user->id)),0,"int_mailer");
}
...
function getAccessPickListValues($module)
{
	...
	$id = getTabid($module);
	$query = "select fieldname,columnname,fieldid,fieldlabel,tabid,uitype from aicrm_field where tabid = ? and uitype in ('15','33','55') and aicrm_field.presence in (0,2)";
	$result = $adb->pquery($query, array($id));
	...
	for($i=0;$i < $adb->num_rows($result);$i++)
	{
		...
		if (count($roleids) > 1)
		{
			$mulsel="select distinct $fieldname from aicrm_$fieldname inner join aicrm_role2picklist on aicrm_role2picklist.picklistvalueid = aicrm_$fieldname.picklist_valueid where roleid in (\"". implode($roleids,"\",\"") ."\") and picklistid in (select picklistid from aicrm_$fieldname) order by sortid asc";
		}
		else
		{
			$mulsel="select distinct $fieldname from aicrm_$fieldname inner join aicrm_role2picklist on aicrm_role2picklist.picklistvalueid = aicrm_$fieldname.picklist_valueid where roleid ='".$roleid."' and picklistid in (select picklistid from aicrm_$fieldname) order by sortid asc";
		}
...
function getRecordValues($id_array,$module) {
	...
	global $app_strings;
	$tabid=getTabid($module);	
	$query="select fieldname,fieldlabel,uitype from aicrm_field where tabid=? and fieldname  not in ('createdtime','modifiedtime') and aicrm_field.presence in (0,2) and uitype not in('4')";
...
{	
	...
	if( stristr($_REQUEST['action'],'ImportStep') || ($_REQUEST['action'] == $_REQUEST['module'].'Ajax' && $_REQUEST['current_action'] == 'ImportSteplast'))
	{
		if($module == 'Contacts')
		{
			...
			$nquery="select aicrm_contactdetails.contactid as recordid, aicrm_contactdetails.contact_no as recordno, aicrm_users_last_import.deleted,$table_cols 
					...
					INNER JOIN aicrm_contactsubdetails ON aicrm_contactaddress.contactaddressid = aicrm_contactsubdetails.contactsubscriptionid
					...
					INNER JOIN (select $select_clause from aicrm_contactdetails t
							INNER JOIN aicrm_crmentity crm ON crm.crmid=t.contactid
							INNER JOIN aicrm_contactaddress addr ON t.contactid = addr.contactaddressid
							INNER JOIN aicrm_contactsubdetails subd ON addr.contactaddressid = subd.contactsubscriptionid
							LEFT JOIN aicrm_contactscf tcf ON t.contactid = tcf.contactid 
    						LEFT JOIN aicrm_account acc ON acc.accountid=t.accountid
							LEFT JOIN aicrm_customerdetails custd ON custd.customerid=t.contactid
							WHERE crm.deleted=0 group by $select_clause  HAVING COUNT(*)>1) as temp
						ON ".get_on_clause($field_values,$ui_type_arr,$module)."
					WHERE aicrm_crmentity.deleted=0 $sec_parameter ORDER BY $table_cols,aicrm_contactdetails.contactid ASC";
        ...
        else if($module == 'SmartSms')
		{
			...
			$nquery="select aicrm_smartsms.smartsmsid as recordid, aicrm_users_last_import.deleted,$table_cols 
					FROM aicrm_smartsms 
					...
					LEFT JOIN aicrm_groups ON aicrm_groups.groupid = aicrm_crmentity.smownerid
					LEFT JOIN aicrm_users ON aicrm_users.id = aicrm_crmentity.smownerid
					LEFT JOIN aicrm_users_last_import ON aicrm_users_last_import.bean_id=aicrm_smartsms.smartsmsid 
					INNER JOIN (select $select_clause from aicrm_smartsms t 
							INNER JOIN aicrm_crmentity crm ON crm.crmid=t.smartsmsid 
							LEFT JOIN aicrm_smartsmscf tcf ON tcf.smartsmsid=t.smartsmsid 
							WHERE crm.deleted=0  group by $select_clause HAVING COUNT(*)>1) as temp 
						ON ".get_on_clause($field_values,$ui_type_arr,$module)." 
				WHERE aicrm_crmentity.deleted=0 $sec_parameter ORDER BY $table_cols,aicrm_smartsms.smartsmsid ASC";
                //echo $nquery;
		}
		...
		else if($module == 'Accounts')
		{
			...
			$select_clause = $ret_arr['sel_clause'];
			$on_clause = $ret_arr['on_clause'];	
			$nquery="SELECT aicrm_account.accountid AS recordid, aicrm_account.account_no as recordno ,aicrm_users_last_import.deleted,".$table_cols."
				FROM aicrm_account
				...
				LEFT JOIN aicrm_users_last_import ON aicrm_users_last_import.bean_id=aicrm_account.accountid
				LEFT JOIN aicrm_groups ON aicrm_groups.groupid = aicrm_crmentity.smownerid
				LEFT JOIN aicrm_users ON aicrm_users.id = aicrm_crmentity.smownerid
				INNER JOIN (select $select_clause from aicrm_account t
							INNER JOIN aicrm_crmentity crm ON crm.crmid=t.accountid
							INNER JOIN aicrm_accountbillads badd ON t.accountid = badd.accountaddressid
							INNER JOIN aicrm_accountshipads sadd ON t.accountid = sadd.accountaddressid
							LEFT JOIN aicrm_accountscf tcf ON t.accountid = tcf.accountid
							WHERE crm.deleted=0 group by $select_clause HAVING COUNT(*)>1) as temp 
					ON ".get_on_clause($field_values,$ui_type_arr,$module)."
				WHERE aicrm_crmentity.deleted=0 $sec_parameter ORDER BY $table_cols,aicrm_account.accountid ASC";
		...
		else if($module == 'Leads')
		{
			...
			$select_clause = $ret_arr['sel_clause'];
			$on_clause = $ret_arr['on_clause'];
			$nquery="select aicrm_leaddetails.leadid as recordid, aicrm_leaddetails.lead_no as recordno, aicrm_users_last_import.deleted,$table_cols 
					...
					LEFT JOIN aicrm_groups ON aicrm_groups.groupid = aicrm_crmentity.smownerid
					LEFT JOIN aicrm_users ON aicrm_users.id = aicrm_crmentity.smownerid
					LEFT JOIN aicrm_users_last_import ON aicrm_users_last_import.bean_id=aicrm_leaddetails.leadid 
					INNER JOIN (select $select_clause from aicrm_leaddetails t 
							INNER JOIN aicrm_crmentity crm ON crm.crmid=t.leadid 
							INNER JOIN aicrm_leadsubdetails subd ON subd.leadsubscriptionid = t.leadid 
							INNER JOIN aicrm_leadaddress addr ON addr.leadaddressid = subd.leadsubscriptionid
							LEFT JOIN aicrm_leadscf tcf ON tcf.leadid=t.leadid 
							WHERE crm.deleted=0 and t.converted = 0 group by $select_clause HAVING COUNT(*)>1) as temp 
						ON ".get_on_clause($field_values,$ui_type_arr,$module)." 
				WHERE aicrm_crmentity.deleted=0 AND aicrm_leaddetails.converted = 0 $sec_parameter ORDER BY $table_cols,aicrm_leaddetails.leadid ASC";
				
		}	

		else if($module == 'LeadManagement')
		{
			$ret_arr = get_special_on_clause($field_values);
			$select_clause = $ret_arr['sel_clause'];
			$on_clause = $ret_arr['on_clause'];
			$nquery="select aicrm_leadmanage.leadid as recordid, aicrm_users_last_import.deleted,$table_cols
			...
			LEFT JOIN aicrm_groups ON aicrm_groups.groupid = aicrm_crmentity.smownerid
			LEFT JOIN aicrm_users ON aicrm_users.id = aicrm_crmentity.smownerid
			LEFT JOIN aicrm_users_last_import ON aicrm_users_last_import.bean_id=aicrm_leadmanage.leadid
			INNER JOIN (select $select_clause from aicrm_leadmanage t
			INNER JOIN aicrm_crmentity crm ON crm.crmid=t.leadid
			INNER JOIN aicrm_leadmanagesubdetail subd ON subd.leadsubscriptionid = t.leadid
			INNER JOIN aicrm_leadmanageaddress addr ON addr.leadaddressid = subd.leadsubscriptionid
			LEFT JOIN aicrm_leadmanagecf tcf ON tcf.leadid=t.leadid
			WHERE crm.deleted=0 and t.converted = 0 group by $select_clause HAVING COUNT(*)>1) as temp
			ON ".get_on_clause($field_values,$ui_type_arr,$module)."
			WHERE aicrm_crmentity.deleted=0 AND aicrm_leadmanage.converted = 0 $sec_parameter ORDER BY $table_cols,aicrm_leadmanage.leadid ASC";
		...
		else if($module == 'Products')
		{
				...
				$nquery="SELECT aicrm_products.productid AS recordid, aicrm_products.product_no as recordno, aicrm_users_last_import.deleted,".$table_cols."
					FROM aicrm_products
					INNER JOIN aicrm_crmentity ON aicrm_crmentity.crmid=aicrm_products.productid
					LEFT JOIN aicrm_users_last_import ON aicrm_users_last_import.bean_id=aicrm_products.productid
					LEFT JOIN aicrm_productcf ON aicrm_productcf.productid = aicrm_products.productid
					INNER JOIN (select $select_clause from aicrm_products t
							INNER JOIN aicrm_crmentity crm ON crm.crmid=t.productid
							LEFT JOIN aicrm_productcf tcf ON tcf.productid=t.productid
							WHERE crm.deleted=0 group by $select_clause HAVING COUNT(*)>1) as temp
						ON ".get_on_clause($field_values,$ui_type_arr,$module)."
					WHERE aicrm_crmentity.deleted=0 ORDER BY $table_cols,aicrm_products.productid ASC";
		...
		else if($module == 'Sparepart')
		{
				...
				$nquery="SELECT aicrm_sparepart.sparepartid AS recordid, aicrm_sparepart.sparepart_no as recordno, aicrm_users_last_import.deleted,".$table_cols."
					FROM aicrm_sparepart
					INNER JOIN aicrm_crmentity ON aicrm_crmentity.crmid=aicrm_sparepart.sparepartid
					LEFT JOIN aicrm_users_last_import ON aicrm_users_last_import.bean_id=aicrm_sparepart.sparepartid
					LEFT JOIN aicrm_sparepartcf ON aicrm_sparepartcf.sparepartid = aicrm_sparepart.sparepartid
					INNER JOIN (select $select_clause from aicrm_sparepart t
							INNER JOIN aicrm_crmentity crm ON crm.crmid=t.sparepartid
							LEFT JOIN aicrm_sparepartcf tcf ON tcf.sparepartid=t.sparepartid
							WHERE crm.deleted=0 group by $select_clause HAVING COUNT(*)>1) as temp
						ON ".get_on_clause($field_values,$ui_type_arr,$module)."
					WHERE aicrm_crmentity.deleted=0 ORDER BY $table_cols,aicrm_sparepart.sparepartid ASC";
		...
		else if($module == 'Competitor')
		{
				...
				$nquery="SELECT aicrm_competitor.competitorid AS recordid, aicrm_competitor.competitor_no as recordno ,aicrm_users_last_import.deleted,".$table_cols."
					FROM aicrm_competitor
					INNER JOIN aicrm_crmentity ON aicrm_crmentity.crmid=aicrm_competitor.competitorid
					LEFT JOIN aicrm_users_last_import ON aicrm_users_last_import.bean_id=aicrm_competitor.competitorid
					LEFT JOIN aicrm_competitorcf ON aicrm_competitorcf.competitorid = aicrm_competitor.competitorid
					INNER JOIN (select $select_clause from aicrm_competitor t
							INNER JOIN aicrm_crmentity crm ON crm.crmid=t.competitorid
							LEFT JOIN aicrm_competitorcf tcf ON tcf.competitorid=t.competitorid
							WHERE crm.deleted=0 group by $select_clause HAVING COUNT(*)>1) as temp
						ON ".get_on_clause($field_values,$ui_type_arr,$module)."
					WHERE aicrm_crmentity.deleted=0 ORDER BY $table_cols,aicrm_competitor.competitorid ASC";
        ...
        else if($module == 'Errors')
        {
            ...
            $nquery="SELECT aicrm_errors.errorsid AS recordid, aicrm_errors.errors_no as recordno ,aicrm_users_last_import.deleted,".$table_cols."
					FROM aicrm_errors
					INNER JOIN aicrm_crmentity ON aicrm_crmentity.crmid=aicrm_errors.errorsid
					LEFT JOIN aicrm_users_last_import ON aicrm_users_last_import.bean_id=aicrm_errors.errorsid
					LEFT JOIN aicrm_errorscf ON aicrm_errorscf.errorsid = aicrm_errors.errorsid
					INNER JOIN (select $select_clause from aicrm_errors t
							INNER JOIN aicrm_crmentity crm ON crm.crmid=t.errorsid
							LEFT JOIN aicrm_errorscf tcf ON tcf.errorsid=t.errorsid
							WHERE crm.deleted=0 group by $select_clause HAVING COUNT(*)>1) as temp
						ON ".get_on_clause($field_values,$ui_type_arr,$module)."
					WHERE aicrm_crmentity.deleted=0 ORDER BY $table_cols,aicrm_errors.errorsid ASC";
        ...
        else if($module == 'Serial')
        {
            ...
            $nquery="SELECT aicrm_serial.serialid AS recordid,aicrm_serial.serial_no as recordno ,aicrm_users_last_import.deleted,".$table_cols."
					FROM aicrm_serial
					INNER JOIN aicrm_crmentity ON aicrm_crmentity.crmid=aicrm_serial.serialid
					LEFT JOIN aicrm_users_last_import ON aicrm_users_last_import.bean_id=aicrm_serial.serialid
					LEFT JOIN aicrm_serialcf ON aicrm_serialcf.serialid = aicrm_serial.serialid
					INNER JOIN (select $select_clause from aicrm_serial t
							INNER JOIN aicrm_crmentity crm ON crm.crmid=t.serialid
							LEFT JOIN aicrm_serialcf tcf ON tcf.serialid=t.serialid
							WHERE crm.deleted=0 group by $select_clause HAVING COUNT(*)>1) as temp
						ON ".get_on_clause($field_values,$ui_type_arr,$module)."
					WHERE aicrm_crmentity.deleted=0 ORDER BY $table_cols,aicrm_serial.serialid ASC";
        ...
        else if($module == 'Campaigns')
        {
            ...
            $nquery="SELECT aicrm_campaign.campaignid AS recordid,aicrm_campaign.campaign_no as recordno ,aicrm_users_last_import.deleted,".$table_cols."
					FROM aicrm_campaign
					INNER JOIN aicrm_crmentity ON aicrm_crmentity.crmid=aicrm_campaign.campaignid
					LEFT JOIN aicrm_users_last_import ON aicrm_users_last_import.bean_id=aicrm_campaign.campaignid
					LEFT JOIN aicrm_campaignscf ON aicrm_campaignscf.campaignid = aicrm_campaign.campaignid
					INNER JOIN (select $select_clause from aicrm_campaign t
							INNER JOIN aicrm_crmentity crm ON crm.crmid=t.campaignid
							LEFT JOIN aicrm_campaignscf tcf ON tcf.campaignid=t.campaignid
							WHERE crm.deleted=0 group by $select_clause HAVING COUNT(*)>1) as temp
						ON ".get_on_clause($field_values,$ui_type_arr,$module)."
					WHERE aicrm_crmentity.deleted=0 ORDER BY $table_cols,aicrm_campaign.campaignid ASC";
        ...
        else if($module == 'HelpDesk')
		{
			...
			$select_clause = $ret_arr['sel_clause'];
			$on_clause = $ret_arr['on_clause'];
			$nquery="SELECT aicrm_troubletickets.ticketid AS recordid, aicrm_troubletickets.ticket_no as recordno ,aicrm_users_last_import.deleted,".$table_cols."
				...
				LEFT JOIN aicrm_users_last_import ON aicrm_users_last_import.bean_id=aicrm_troubletickets.ticketid
				LEFT JOIN aicrm_attachments ON aicrm_attachments.attachmentsid=aicrm_crmentity.crmid
				LEFT JOIN aicrm_ticketcomments ON aicrm_ticketcomments.ticketid = aicrm_crmentity.crmid				
				INNER JOIN (select $select_clause from aicrm_troubletickets t
						INNER JOIN aicrm_crmentity crm ON crm.crmid=t.ticketid
						LEFT JOIN aicrm_account acc ON acc.accountid = t.accountid 
						LEFT JOIN aicrm_contactdetails contd ON contd.contactid = t.contactid
						LEFT JOIN aicrm_ticketcf tcf ON tcf.ticketid = t.ticketid
						WHERE crm.deleted=0 group by $select_clause HAVING COUNT(*)>1) as temp ON ".get_on_clause($field_values,$ui_type_arr,$module)."
				WHERE aicrm_crmentity.deleted=0 ". $sec_parameter ." ORDER BY $table_cols,aicrm_troubletickets.ticketid ASC";					
		}
		else if($module == 'Potentials')
		{
			...
			$select_clause = $ret_arr['sel_clause'];
			$on_clause = $ret_arr['on_clause'];
			$nquery="SELECT aicrm_potential.potentialid AS recordid,
				aicrm_users_last_import.deleted,".$table_cols."
				FROM aicrm_potential 
				INNER JOIN aicrm_crmentity ON aicrm_crmentity.crmid=aicrm_potential.potentialid
				LEFT JOIN aicrm_groups ON aicrm_groups.groupid = aicrm_crmentity.smownerid
				LEFT JOIN aicrm_users ON aicrm_users.id = aicrm_crmentity.smownerid
				LEFT JOIN aicrm_users_last_import ON aicrm_users_last_import.bean_id=aicrm_potential.potentialid
				LEFT JOIN aicrm_potentialscf ON aicrm_potentialscf.potentialid = aicrm_potential.potentialid 
				INNER JOIN (select $select_clause from aicrm_potential t
						INNER JOIN aicrm_crmentity crm ON crm.crmid=t.potentialid
						LEFT JOIN aicrm_potentialscf tcf ON tcf.potentialid=t.potentialid
						WHERE crm.deleted=0 group by $select_clause HAVING COUNT(*)>1) as temp
					ON ".get_on_clause($field_values,$ui_type_arr,$module)."
				WHERE aicrm_crmentity.deleted=0 $sec_parameter ORDER BY $table_cols,aicrm_potential.potentialid ASC";
	...
	else
	{
		...
		if($module == 'Contacts')
		{	
			$nquery = "SELECT aicrm_contactdetails.contactid AS recordid, aicrm_contactdetails.contact_no as recordno ,
					aicrm_users_last_import.deleted,".$table_cols."
					FROM aicrm_contactdetails
					INNER JOIN aicrm_crmentity ON aicrm_crmentity.crmid=aicrm_contactdetails.contactid
					INNER JOIN aicrm_contactaddress ON aicrm_contactdetails.contactid = aicrm_contactaddress.contactaddressid
		...
		{
			$nquery="SELECT aicrm_smartsms.smartsmsid AS recordid,
				aicrm_users_last_import.deleted,".$table_cols."
				FROM aicrm_smartsms
				INNER JOIN aicrm_crmentity ON aicrm_crmentity.crmid=aicrm_smartsms.smartsmsid
				LEFT JOIN aicrm_smartsmscf ON aicrm_smartsms.smartsmsid=aicrm_smartsmscf.smartsmsid
				LEFT JOIN aicrm_users_last_import ON aicrm_users_last_import.bean_id=aicrm_smartsms.smartsmsid
				LEFT JOIN aicrm_groups ON aicrm_groups.groupid = aicrm_crmentity.smownerid
				LEFT JOIN aicrm_users ON aicrm_users.id = aicrm_crmentity.smownerid
				INNER JOIN (SELECT $table_cols
		...
		{
			$nquery="SELECT aicrm_account.accountid AS recordid,aicrm_account.account_no AS recordno,
				aicrm_users_last_import.deleted,".$table_cols."
				FROM aicrm_account
				INNER JOIN aicrm_crmentity ON aicrm_crmentity.crmid=aicrm_account.accountid
				INNER JOIN aicrm_accountbillads ON aicrm_account.accountid = aicrm_accountbillads.accountaddressid
		...
		else if($module == 'LeadManagement')
		{
			...
			LEFT JOIN aicrm_groups ON aicrm_groups.groupid = aicrm_crmentity.smownerid
			LEFT JOIN aicrm_users ON aicrm_users.id = aicrm_crmentity.smownerid
			WHERE aicrm_crmentity.deleted=0 AND aicrm_leadmanage.converted = 0 $sec_parameter
			GROUP BY $table_cols HAVING COUNT(*)>1) as temp
			ON ".get_on_clause($field_values,$ui_type_arr,$module) ."
			WHERE aicrm_crmentity.deleted=0  AND aicrm_leadmanage.converted = 0 $sec_parameter ORDER BY $table_cols,aicrm_leadmanage.leadid ASC";
		...
		{
			$nquery = "SELECT aicrm_products.productid AS recordid,aicrm_products.product_no AS recordno,
				aicrm_users_last_import.deleted,".$table_cols."
				FROM aicrm_products
				INNER JOIN aicrm_crmentity ON aicrm_crmentity.crmid=aicrm_products.productid
				LEFT JOIN aicrm_users_last_import ON aicrm_users_last_import.bean_id=aicrm_products.productid
		...
		{
			$nquery = "SELECT aicrm_sparepart.sparepartid AS recordid,aicrm_sparepart.sparepart_no AS recordno,
				aicrm_users_last_import.deleted,".$table_cols."
				FROM aicrm_sparepart
				INNER JOIN aicrm_crmentity ON aicrm_crmentity.crmid=aicrm_sparepart.sparepartid
				LEFT JOIN aicrm_users_last_import ON aicrm_users_last_import.bean_id=aicrm_sparepart.sparepartid
        ...
        {
            $nquery = "SELECT aicrm_competitor.competitorid AS recordid, aicrm_competitor.competitor_no AS recordno,
				aicrm_users_last_import.deleted,".$table_cols."
				FROM aicrm_competitor
				INNER JOIN aicrm_crmentity ON aicrm_crmentity.crmid=aicrm_competitor.competitorid
				LEFT JOIN aicrm_users_last_import ON aicrm_users_last_import.bean_id=aicrm_competitor.competitorid
        ...
        {
            $nquery = "SELECT aicrm_errors.errorsid AS recordid,aicrm_errors.errors_no AS recordno,
				aicrm_users_last_import.deleted,".$table_cols."
				FROM aicrm_errors
				INNER JOIN aicrm_crmentity ON aicrm_crmentity.crmid=aicrm_errors.errorsid
				LEFT JOIN aicrm_users_last_import ON aicrm_users_last_import.bean_id=aicrm_errors.errorsid
        ...
        {
            $nquery = "SELECT aicrm_serial.serialid AS recordid,aicrm_serial.serial_no AS recordno,
				aicrm_users_last_import.deleted,".$table_cols."
				FROM aicrm_serial
				INNER JOIN aicrm_crmentity ON aicrm_crmentity.crmid=aicrm_serial.serialid
				LEFT JOIN aicrm_users_last_import ON aicrm_users_last_import.bean_id=aicrm_serial.serialid
        ...
        {
            $nquery = "SELECT aicrm_campaign.campaignid AS recordid,aicrm_campaign.campaign_no AS recordno,
				aicrm_users_last_import.deleted,".$table_cols."
				FROM aicrm_campaign
				INNER JOIN aicrm_crmentity ON aicrm_crmentity.crmid=aicrm_campaign.campaignid
				LEFT JOIN aicrm_users_last_import ON aicrm_users_last_import.bean_id=aicrm_campaign.campaignid
				LEFT JOIN aicrm_campaigncf ON aicrm_campaigncf.campaignid = aicrm_campaign.campaignid
				INNER JOIN (SELECT $table_cols
							FROM aicrm_campaign
							INNER JOIN aicrm_crmentity ON aicrm_crmentity.crmid = aicrm_campaign.campaignid
							LEFT JOIN aicrm_campaigncf ON aicrm_campaigncf.campaignid = aicrm_campaign.campaignid 
		...
		{
			$nquery = "SELECT aicrm_troubletickets.ticketid AS recordid,aicrm_troubletickets.ticket_no AS recordno,
				aicrm_users_last_import.deleted,".$table_cols."
				FROM aicrm_troubletickets
				INNER JOIN aicrm_crmentity ON aicrm_crmentity.crmid=aicrm_troubletickets.ticketid
				LEFT JOIN aicrm_ticketcf ON aicrm_ticketcf.ticketid = aicrm_troubletickets.ticketid
				...
				LEFT JOIN aicrm_contactdetails ON aicrm_contactdetails.contactid = aicrm_troubletickets.contactid
				LEFT JOIN aicrm_ticketcomments ON aicrm_ticketcomments.ticketid = aicrm_crmentity.crmid
				INNER JOIN (SELECT $table_cols FROM aicrm_troubletickets
							INNER JOIN aicrm_crmentity ON aicrm_crmentity.crmid = aicrm_troubletickets.ticketid
							LEFT JOIN aicrm_ticketcf ON aicrm_ticketcf.ticketid = aicrm_troubletickets.ticketid 
							LEFT JOIN aicrm_attachments ON aicrm_attachments.attachmentsid=aicrm_crmentity.crmid
							LEFT JOIN aicrm_contactdetails ON aicrm_contactdetails.contactid = aicrm_troubletickets.contactid
							LEFT JOIN aicrm_ticketcomments ON aicrm_ticketcomments.ticketid = aicrm_crmentity.crmid
							LEFT JOIN aicrm_groups ON aicrm_groups.groupid = aicrm_crmentity.smownerid
							LEFT JOIN aicrm_contactdetails contd ON contd.contactid = aicrm_troubletickets.contactid
				WHERE aicrm_crmentity.deleted=0 $sec_parameter
							GROUP BY ".$table_cols." HAVING COUNT(*)>1) as temp
				ON ".get_on_clause($field_values,$ui_type_arr,$module) ."
                                WHERE aicrm_crmentity.deleted=0 $sec_parameter ORDER BY $table_cols,aicrm_troubletickets.ticketid ASC";
...
function getDuplicateRecordsArr($module)
{
	...
	if($no_rows == 0)
	{
		if ($_REQUEST['action'] == 'FindDuplicateRecords')
		{
			...
			echo "<div style='border: 3px solid rgb(201 201 201); background-color: rgb(255, 255, 255); width: 55%; position: relative; z-index: 10000000;border-radius: 6px;'>
			
				<table border='0' cellpadding='5' cellspacing='0' width='98%'>
				<tbody><tr>
				<td rowspan='2' width='11%'><img src='" . aicrm_imageurl('empty.jpg', $theme) . "' ></td>
	...
	{			
		...
		$fld_values[$grp][$ii]['recordno'] = $result['recordno'];	
...
function get_special_on_clause($field_list)
{
	...
	foreach($field_array as $fld)
	{
		...
		if($tbl_name == 'aicrm_leadsubdetails' || $tbl_name == 'aicrm_leadmanagesubdetail' || $tbl_name == 'aicrm_contactsubdetails')
			$tbl_alias = "subd";
		else if($tbl_name == 'aicrm_leadaddress'  || $tbl_name == 'aicrm_leadmanageaddress' || $tbl_name == 'aicrm_contactaddress')
			$tbl_alias = "addr";
		else if($tbl_name == 'aicrm_account' && $spl_chk == 'Contacts')
			$tbl_alias = "acc";
		else if($tbl_name == 'aicrm_accountbillads')
			$tbl_alias = "badd";
		else if($tbl_name == 'aicrm_accountshipads')
			$tbl_alias = "sadd";
		else if($tbl_name == 'aicrm_crmentity')
			$tbl_alias = "crm";
		else if($tbl_name == 'aicrm_customerdetails')
			$tbl_alias = "custd";
		else if($tbl_name == 'aicrm_contactdetails' && spl_chk == 'HelpDesk')
			$tbl_alias = "contd";
...
function get_subquery_recordids($sub_query)
{
	...
	$module_id_array = Array("Accounts"=>"accountid","Contacts"=>"contactid","Leads"=>"leadid","LeadManagement"=>"leadid","Products"=>"productid","HelpDesk"=>"ticketid","Potentials"=>"potentialid");
...
function getFieldValues($module)
{
	...
	//$fld_table_arr = Array("aicrm_contactdetails.account_id"=>"aicrm_account.accountname");
	...
	$fieldname_query="select fieldname,fieldlabel,uitype,tablename,columnname from aicrm_field where fieldid in 
			(select fieldid from aicrm_user2mergefields WHERE tabid=? AND userid=? AND visible = ?) and aicrm_field.presence in (0,2)";
	$fieldname_result = $adb->pquery($fieldname_query, array($tabid, $current_user->id, 1));
	/*echo "select fieldname,fieldlabel,uitype,tablename,columnname from aicrm_field where fieldid in 
			(select fieldid from aicrm_user2mergefields WHERE tabid=$tabid AND userid=$current_user->id AND visible = 1) and aicrm_field.presence in (0,2)";*/
...
function getSecParameterforMerge($module)
{
	...
	if($is_admin == false && $profileGlobalPermission[1] == 1 && $profileGlobalPermission[2] == 1 && $defaultOrgSharingPermission[$tab_id] == 3)
	{
		if($module == "Products") {
			...
			if($module == "Accounts") {
				$sec_parameter .= " AND (aicrm_crmentity.smownerid IN (".$current_user->id.")
						...
						AND tabid=".$tab_id.")
						OR (aicrm_crmentity.smownerid in (0)
						AND (";

				if(sizeof($current_user_groups) > 0) {
					$sec_parameter .= " aicrm_groups.groupname IN (
									SELECT groupname
									FROM aicrm_groups
				...
				$sec_parameter .= " aicrm_groups.groupname IN (
				 	SELECT aicrm_groups.groupname
					FROM aicrm_tmp_read_group_sharing_per
					INNER JOIN aicrm_groups
						ON aicrm_groups.groupid = aicrm_tmp_read_group_sharing_per.sharedgroupid
					WHERE userid=".$current_user->id."
					AND tabid=".$tab_id.")))) ";
			}
		}
...
function transferUserCurrency($old_cur, $new_cur) {
	...
	$sql = "update aicrm_users set currency_id=? where currency_id=?";
...
function transferProductCurrency($old_cur, $new_cur) {
	...
	$prod_res = $adb->pquery("select productid from aicrm_products where currency_id = ?", array($old_cur));
	...
	if(count($prod_ids) > 0) {
		...
		for($i=0;$i<count($prod_ids);$i++) {
			...
			$query = "update aicrm_products set currency_id=?, unit_price=? where productid=?";
...
function transferPriceBookCurrency($old_cur, $new_cur) {
	...
	$pb_res = $adb->pquery("select pricebookid from aicrm_pricebook where currency_id = ?", array($old_cur));
...
function getCallerInfo($number){
	...
	$name['Contacts'] = "select contactid as id,concat(firstname,' ',lastname) as name from aicrm_contactdetails inner join aicrm_crmentity on crmid=contactid where deleted=0 and (phone = ? or mobile = ? or fax = ?)";//array('name'=>"concat(firstname,' ',lastname)", 'table'=>'aicrm_contactdetails', 'field'=>"phone,mobile,fax", 'id'=>'contactid');
	$name['Accounts'] = "select accountid as id, accountname as name from aicrm_account inner join aicrm_crmentity on crmid=accountid where deleted =0 and (phone = ? or otherphone = ? or fax = ?)";//array('name'=>"accountname", 'table'=>"aicrm_account", 'field'=>"phone, otherphone, fax", 'id'=>'accountid');
	$name['Leads'] = "select leadid as id,concat(firstname,' ',lastname) as name from aicrm_leaddetails inner join aicrm_crmentity on aicrm_crmentity.crmid=aicrm_leaddetails.leadid inner join aicrm_leadaddress on aicrm_leaddetails.leadid = aicrm_leadaddress.leadaddressid where aicrm_crmentity.deleted =0 and (aicrm_leadaddress.phone = ? or aicrm_leadaddress.mobile = ? or aicrm_leadaddress.fax = ?)";//array('name'=>"concat(firstname,' ',lastname)", 'table'=>"aicrm_leaddetails inner join aicrm_leadaddress on aicrm_leaddetails.leadid = aicrm_leadaddress.leadaddressid", 'field'=>"phone,mobile,fax", 'id'=>'leadid');
...
function get_tab_name_index($adb, $module){
	$tabid = getTabid($module);
	$sql = "select * from aicrm_tab_name_index where tabid = ?";
	$result = $adb->pquery($sql, array($tabid));
	$count = $adb->num_rows($result);
	$data = array();
...
function get_use_asterisk($id){
	...
	$sql = "select * from aicrm_asteriskextensions where userid = ?";
...
function addToCallHistory($userExtension, $callfrom, $callto, $status, $adb){
	$sql = "select * from aicrm_asteriskextensions where asterisk_extension=".$userExtension;
	...
	$sql = "insert into aicrm_crmentity values (?,?,?,?,?,?,?,?,?,?,?,?,?)";
	...
	if($status == 'outgoing'){
		...
		$sql = "select * from aicrm_asteriskextensions where asterisk_extension=?";
	...
	$sql = "insert into aicrm_pbxmanager (pbxmanagerid,callfrom,callto,timeofcall,status)values (?,?,?,?,?)";
...
function getSettingsBlocks(){
	global $adb;
	$sql = "select * from aicrm_settings_blocks order by sequence";
...
function getSettingsFields(){
	global $adb;
	$sql = "select * from aicrm_settings_field where blockid!=? and active=0 order by blockid,sequence";
...
function getModuleSequenceField($module) {
	...
	if (!empty($module)) {
		...
		if($cachedModuleFields === false) {
			...
			$seqColRes = $adb->pquery("SELECT fieldname, fieldlabel, columnname FROM aicrm_field WHERE uitype=? AND tabid=? and aicrm_field.presence in (0,2)", array('4', getTabid($module)));
...
function getFieldsResultForMerge($tabid) {
	...
	$sql = "SELECT fieldid,typeofdata FROM aicrm_field WHERE tabid = ? and aicrm_field.presence in (0,2)";
	$params = array($tabid);
...
function getRelationTables($module,$secmodule){
	...
	$ui10_query = $adb->pquery("SELECT aicrm_field.tablename AS tablename, aicrm_field.columnname AS columnname FROM aicrm_field INNER JOIN aicrm_fieldmodulerel ON aicrm_fieldmodulerel.fieldid = aicrm_field.fieldid WHERE (aicrm_fieldmodulerel.module=? AND aicrm_fieldmodulerel.relmodule=?) OR (aicrm_fieldmodulerel.module=? AND aicrm_fieldmodulerel.relmodule=?)",array($module,$secmodule,$secmodule,$module));
	...
	if(is_array($reltables) && !empty($reltables)){
		...
		$rel_array = array("aicrm_crmentityrel"=>array("crmid","relcrmid"),"".$primary_obj->table_name."" => "".$primary_obj->table_index."");
	}

	//echo '<pre>'; print_r($rel_array); echo '</pre>'; exit;
...
function com_vtGetModules($adb) {
	$sql="select distinct aicrm_field.tabid, name 
		...
		where aicrm_field.tabid not in(9,10,16,15,8,29) and aicrm_tab.presence = 0 and aicrm_tab.isentitytype=1";
...
function getRelatedInfo($id){
	global $adb;
	$data = array();
	$sql = "select related_to from aicrm_potential where potentialid=?";
	$result = $adb->pquery($sql, array($id));
	if($adb->num_rows($result)>0){
...
function getMailFields($tabid){
	global $adb;
	$fields = array();
	$result = $adb->pquery("SELECT tablename,fieldlabel,fieldname FROM aicrm_field WHERE tabid=? AND uitype IN (13,104)", array($tabid));
	if($adb->num_rows($result)>0){
...
function isRecordExists($recordId) {
	global $adb;
	$query = "SELECT crmid FROM aicrm_crmentity where crmid=? AND deleted=0";
