...
function get_options_array_seperate_key (&$label_list, &$key_list, $selected_key, $advsearch='false') {
	...
	foreach ($key_list as $option_key=>$option_value) {
		...
		// The aicrm_reported bug was only happening with one of the aicrm_users in the drop down.  It was being replaced by none.
...
function get_Job_No($event_id)
{
	global $log;
	$log->debug("Entering get_Job_No(".$event_id.") method ...");
	$log->info("in get_Job_No ".$event_id);

	global $adb;
	$sql = "select * from aicrm_jobs where jobid=?";
	$result = $adb->pquery($sql, array($event_id));
	$event_name = $adb->query_result($result,0,"job_no");
	$log->debug("Exiting get_Job_No method ...");
	return $event_name;
}

function get_Job_Name($event_id)
{
	global $log;
	$log->debug("Entering get_Job_Name(".$event_id.") method ...");
	$log->info("in get_Job_Name ".$event_id);

	global $adb;
	$sql = "select * from aicrm_jobs where jobid=?";
	$result = $adb->pquery($sql, array($event_id));
	$event_name = $adb->query_result($result,0,"job_name");
	$log->debug("Exiting get_Job_Name method ...");
	return $event_name;
}

function get_HelpDesk_No($event_id)
{
	global $log;
	$log->debug("Entering get_HelpDesk_No(".$event_id.") method ...");
	$log->info("in get_HelpDesk_No ".$event_id);

	global $adb;
	$sql = "select * from aicrm_troubletickets where ticketid=?";
	$result = $adb->pquery($sql, array($event_id));
	$event_name = $adb->query_result($result,0,"ticket_no");
	$log->debug("Exiting get_HelpDesk_No method ...");
	return $event_name;
}

function get_Projects_Name($event_id)
{
	global $log;
	$log->debug("Entering get_Projects_Name(".$event_id.") method ...");
	$log->info("in get_Projects_Name ".$event_id);

	global $adb;
	$sql = "select * from aicrm_projects where projectsid=?";
	$result = $adb->pquery($sql, array($event_id));
	$event_name = $adb->query_result($result,0,"projects_name");
	$log->debug("Exiting get_Projects_Name method ...");
	return $event_name;
}
function getSerial($serial_id)
{	
	global $log;
	$log->debug("Entering getSerial(".$serial_id.") method ...");
	$log->info("in getSerial ".$serial_id);

	global $adb;
	$sql = "select * from aicrm_serial where serialid=?";
	$result = $adb->pquery($sql, array($serial_id));
	$serial_name = $adb->query_result($result,0,"serial_name");
	$log->debug("Exiting getSerial method ...");
	return $serial_name;
}
function getSparepart($sparepartid)
{	
	global $log;
	$log->debug("Entering getSparepart(".$sparepartid.") method ...");
	$log->info("in getSparepart ".$sparepartid);
	global $adb;
	$sql = "select sparepart_no from aicrm_sparepart where sparepartid=?";
	
	$result = $adb->pquery($sql, array($sparepartid));
	$sparepart_name = $adb->query_result($result,0,"sparepart_no");
	$log->debug("Exiting getSparepart method ...");
	return $sparepart_name;
}

function geterrorno($errorid)
{	
	global $log;
	$log->debug("Entering geterrorno(".$errorid.") method ...");
	$log->info("in geterrorno ".$errorid);
	global $adb;
	$sql = "select errors_no from aicrm_errors where errorsid=?";
	
	$result = $adb->pquery($sql, array($errorid));
	$errors_no = $adb->query_result($result,0,"errors_no");
	$log->debug("Exiting geterrorno method ...");
	return $errors_no;
}
...
{	
	...
	global $log;
	$log->debug("Entering getjob(".$jobid.") method ...");
	$log->info("in getjob ".$jobid);
	global $adb;
	$sql = "select job_no from aicrm_jobs where jobid=?";
	
	$result = $adb->pquery($sql, array($jobid));
	$job_no = $adb->query_result($result,0,"job_no");
	$log->debug("Exiting getjob method ...");
	return $job_no;
}

function getinspection_template_name($inspectiontemplateid)
{
    global $log;
    $log->debug("Entering getinspection_template_name(".$inspectiontemplateid.") method ...");
    $log->info("in getinspection_template_name ".$inspectiontemplateid);
    global $adb;
    $sql = "select inspectiontemplate_name from aicrm_inspectiontemplate where inspectiontemplateid=?";

    $result = $adb->pquery($sql, array($inspectiontemplateid));
    $inspectiontemplate_name = $adb->query_result($result,0,"inspectiontemplate_name");
    $log->debug("Exiting getinspection_template_name method ...");
    return $inspectiontemplate_name;
}

function getcase($ticketid)
{
	global $log;
	$log->debug("Entering getcase(".$ticketid.") method ...");
	$log->info("in getcase ".$ticketid);

	global $adb;
	$sql = "select ticket_no from aicrm_troubletickets where ticketid=?";
    $result = $adb->pquery($sql, array($ticketid));
	$ticket_no = $adb->query_result($result,0,"ticket_no");
	$log->debug("Exiting getcase method ...");
	return $ticket_no;
}
...
{
	global $log;
	$log->debug("Entering getplant(".$plantid.") method ...");
	$log->info("in getplant ".$plantid);

	global $adb;
	$sql = "select plant_id from aicrm_plant where plantid=?";
    $result = $adb->pquery($sql, array($plantid));
	$plant_id = $adb->query_result($result,0,"plant_id");
	$log->debug("Exiting getplant method ...");
	return $plant_id;
}
...
{
	global $log;
	$log->debug("Entering getpromotion(".$promotionid.") method ...");
	$log->info("in getpromotion ".$promotionid);

	global $adb;
	$sql = "select promotion_name from aicrm_promotion where promotionid=?";
    $result = $adb->pquery($sql, array($promotionid));
	$promotion_name = $adb->query_result($result,0,"promotion_name");
	$log->debug("Exiting getpromotion method ...");
...
{
	global $log;
	$log->debug("Entering getactivity(".$activityid.") method ...");
	$log->info("in getactivity ".$activityid);

	global $adb;
	$sql = "select activitytype from aicrm_activity where activityid=?";
    $result = $adb->pquery($sql, array($activityid));
	$activitytype = $adb->query_result($result,0,"activitytype");
	$log->debug("Exiting getactivity method ...");
	return $activitytype;
}
...
{
	global $log;
	$log->debug("Entering getdealname(".$dealid.") method ...");
	$log->info("in getdealname ".$dealid);

	global $adb;
	$sql = "select deal_no from aicrm_deal where dealid=?";
    $result = $adb->pquery($sql, array($dealid));
	$deal_no = $adb->query_result($result,0,"deal_no");
	$log->debug("Exiting getdealname method ...");
	return $deal_no;
}

function getsalesinvoice($salesinvoiceid)
{
	global $log;
	$log->debug("Entering getsalesinvoice(".$salesinvoiceid.") method ...");
	$log->info("in getsalesinvoice ".$salesinvoiceid);

	global $adb;
	$sql = "select salesinvoice_no from aicrm_salesinvoice where salesinvoiceid=?";
    $result = $adb->pquery($sql, array($salesinvoiceid));
	$salesinvoice_no = $adb->query_result($result,0,"salesinvoice_no");
	$log->debug("Exiting getsalesinvoice method ...");
	return $salesinvoice_no;
}
function getcompetitorname($competitorid)
{
	global $log;
	$log->debug("Entering getcompetitorname(".$competitorid.") method ...");
	$log->info("in getcompetitorname ".$competitorid);

	global $adb;
	$sql = "select competitor_name from aicrm_competitor where competitorid=?";
    $result = $adb->pquery($sql, array($competitorid));
	$competitor_name = $adb->query_result($result,0,"competitor_name");
	$log->debug("Exiting getcompetitorname method ...");
	return $competitor_name;
}

function getpromotionvouchername($promotionvoucherid)
{
	global $log;
	$log->debug("Entering getpromotionvouchername(".$promotionvoucherid.") method ...");
	$log->info("in getpromotionvouchername ".$promotionvoucherid);

	global $adb;
	$sql = "select promotionvoucher_name from aicrm_promotionvoucher where promotionvoucherid=?";
    $result = $adb->pquery($sql, array($promotionvoucherid));
	$promotionvoucher_name = $adb->query_result($result,0,"promotionvoucher_name");
	$log->debug("Exiting getpromotionvouchername method ...");
	return $promotionvoucher_name;
}

function getsalesorderno($salesorderid)
{
	global $log;
	$log->debug("Entering getsalesorderno(".$salesorderid.") method ...");
	$log->info("in getsalesorderno ".$salesorderid);

	global $adb;
	$sql = "select salesorder_no from aicrm_salesorder where salesorderid=?";
    $result = $adb->pquery($sql, array($salesorderid));
	$salesorder_no = $adb->query_result($result,0,"salesorder_no");

	$log->debug("Exiting getsalesorderno method ...");
	return $salesorder_no;
}

function getpremuimproductno($premuimproductid)
{
	global $log;
	$log->debug("Entering getpremuimproductno(".$premuimproductid.") method ...");
	$log->info("in getpremuimproductno ".$premuimproductid);

	global $adb;
	$sql = "select premuimproduct_no from aicrm_premuimproduct where premuimproductid=?";
    $result = $adb->pquery($sql, array($premuimproductid));
	$premuimproduct_no = $adb->query_result($result,0,"premuimproduct_no");
	$log->debug("Exiting getpremuimproductno method ...");
	return $premuimproduct_no;
}

function getquotesno($quotesid)
{
	global $log;
	$log->debug("Entering getquotesno(".$quotesid.") method ...");
	$log->info("in getquotesno ".$quotesid);

	global $adb;
	$sql = "select quote_no from aicrm_quotes where quoteid=?";
    $result = $adb->pquery($sql, array($quotesid));
	$quote_no = $adb->query_result($result,0,"quote_no");
	$log->debug("Exiting getquotesno method ...");
	return $quote_no;
}
...
{
	global $log;
	$log->debug("Entering getservicerequest(".$servicerequestid.") method ...");
	$log->info("in getservicerequest ".$servicerequestid);

	global $adb;
	$sql = "select servicerequest_name from aicrm_servicerequest where servicerequestid=?";
    $result = $adb->pquery($sql, array($servicerequestid));
	$servicerequest_name = $adb->query_result($result,0,"servicerequest_name");
	$log->debug("Exiting getservicerequest method ...");
	return $servicerequest_name;
}

function getticket($ticketid)
{
	global $log;
	$log->debug("Entering getticket(".$ticketid.") method ...");
	$log->info("in getticket ".$ticketid);

	global $adb;
	$sql = "select title from aicrm_troubletickets where ticketid=?";
    $result = $adb->pquery($sql, array($ticketid));
	$ticket_title = $adb->query_result($result,0,"title");
	$log->debug("Exiting getticket method ...");
	return $ticket_title;
}

function getquestionnairetemplatename($questionnairetemplateid)
{
	global $log;
	$log->debug("Entering getquestionnairetemplatename(".$questionnairetemplateid.") method ...");
	$log->info("in getquestionnairetemplatename ".$questionnairetemplateid);

	global $adb;
	$sql = "select questionnairetemplate_name from aicrm_questionnairetemplate where questionnairetemplateid=?";
    $result = $adb->pquery($sql, array($questionnairetemplateid));
	$questionnairetemplate_name = $adb->query_result($result,0,"questionnairetemplate_name");
	$log->debug("Exiting getquestionnairetemplatename method ...");
	return $questionnairetemplate_name;
}

function getquestionnairename($questionnaireid)
{
	global $log;
	$log->debug("Entering getquestionnairename(".$questionnaireid.") method ...");
	$log->info("in getquestionnairename ".$questionnaireid);

	global $adb;
	$sql = "select questionnaire_name from aicrm_questionnaire where questionnaireid=?";
    $result = $adb->pquery($sql, array($questionnaireid));
	$questionnaire_name = $adb->query_result($result,0,"questionnaire_name");
	$log->debug("Exiting getquestionnairename method ...");
	return $questionnaire_name;
}

function getactivity_name($activityid)
{
	global $log;
	$log->debug("Entering getactivity_name(".$activityid.") method ...");
	$log->info("in getactivity_name ".$activityid);

	global $adb;
	$sql = "select salesvisit_name from aicrm_activity where aicrm_activity.activityid=?";
    $result = $adb->pquery($sql, array($activityid));
	$salesvisit_name = $adb->query_result($result,0,"salesvisit_name");
	$log->debug("Exiting getactivity_name method ...");
	return $salesvisit_name;
}

function getAccountNameRel($account_id)
{
	global $log;
	$log->debug("Entering getAccountNameRel(".$account_id.") method ...");
	$log->info("in getAccountNameRel ".$account_id);

	global $adb;
	$sql = "select accountname from aicrm_account where accountid=?";
    $result = $adb->pquery($sql, array($account_id));
	$accountname = $adb->query_result($result,0,"accountname");
	$log->debug("Exiting getAccountNameRel method ...");
	return $accountname;
}

function getAccountPrevName($crmid)
{
	global $log;
	$log->debug("Entering getAccountPrevName(".$crmid.") method ...");
	$log->info("in getAccountPrevName ".$crmid);
	global $adb;
	$sql = "select accountname from aicrm_account where accountid=?";
	$result = $adb->pquery($sql, array($crmid));
	$name = $adb->query_result($result,0,"accountname");
	$log->debug("Exiting getAccountPrevName method ...");
	return $name;
}
...
{
	global $log;
	$log->debug("Entering getProject(".$projectid.") method ...");
	$log->info("in getProject".$projectid);

	global $adb;
	$sql = "select projects_name from aicrm_projects where projectsid=?";
    $result = $adb->pquery($sql, array($projectid));
	$projects_no = $adb->query_result($result,0,"projects_name");
	$log->debug("Exiting getProject method ...");
	return $projects_no;
}
//Bas 07-02-2017

function getProjectNo($projectid)
{
	global $log;
	$log->debug("Entering getProjectNo(".$projectid.") method ...");
	$log->info("in getProjectNo".$projectid);

	global $adb;
	$sql = "select projects_no from aicrm_projects where projectsid=?";
    $result = $adb->pquery($sql, array($projectid));
	$projects_no = $adb->query_result($result,0,"projects_no");
	$log->debug("Exiting getProjectNo method ...");
	return $projects_no;
}

function getAccountNameField($account_id)
{
	global $log;
	$log->debug("Entering getAccountNameField(".$account_id.") method ...");
	$log->info("in getAccountNameField ".$account_id);
	...
	$log->debug("Exiting getAccountNameField method ...");
	return $accountname;
}

function getAccountcoderms($account_id)
{
	global $log;
	$log->debug("Entering getAccountNameField(".$account_id.") method ...");
	$log->info("in getAccountCodeField ".$account_id);
	...
	$log->debug("Exiting getAccountCodeField method ...");
	return $accountname;
}

function getContactcode($contact_id)
{
	global $log;
	$log->debug("Entering getContactcode(".$contact_id.") method ...");
	$log->info("in getContactcode ".$contact_id);

	global $adb;
	if($contact_id != ''){
		$sql = "select contactname from aicrm_contactdetails where contactid=?";
		$result = $adb->pquery($sql, array($contact_id));
		$contactname = $adb->query_result($result,0,"contactname");
	}
	$log->debug("Exiting getContactcode method ...");
	return $contactname;
}


function getContact_id($contact_id)
{
	global $log;
	$log->debug("Entering getContact_id(".$contact_id.") method ...");
	$log->info("in getContact_id ".$contact_id);
	...
	$log->debug("Exiting getContact_id method ...");
	return $contactname;
}
...
/** To get the Currency of the specified user
  ...
  * @returns  aicrm_currencyid :: Type integer
 */
function fetchCurrency($id)
{
	...
	if($currencyinfo === false) {
        global $adb;
        $sql = "select currency_id from aicrm_users where id=?";
...
/** Function to get the Currency name from the aicrm_currency_info
  * @param $currencyid -- aicrm_currencyid:: Type integer
...
function getCurrencyName($currencyid, $show_symbol=true)
{
	...
	if($currencyinfo === false) {
    	global $adb;
    	$sql1 = "select * from aicrm_currency_info where id= ?";
...
/**
 * Function to fetch the list of aicrm_groups from group aicrm_table
...
function get_group_options()
{
	...
	$sql = "select groupname,groupid from aicrm_groups";
...
function getTabid($module)
{
	...
	if($tabid === false) {
		...
		if(file_exists('tabdata.php') && (filesize('tabdata.php') != 0)) {
    	    ...
    	    global $adb;
			$sql = "select tabid from aicrm_tab where name=?";
...
function getFieldid($tabid, $fieldname, $onlyactive = true) {
	...
	if($fieldinfo === false) {
		$query  = "SELECT fieldid, fieldlabel, columnname, tablename, uitype, typeofdata, presence
			FROM aicrm_field WHERE tabid=? AND fieldname=?";
...
function getCVname($cvid)
{
        ...
        global $adb;
        $sql = "select viewname from aicrm_customview where cvid=?";
...
function getSalesEntityType($crmid)
{
	...
	global $adb;
	$sql = "select setype from aicrm_crmentity where crmid=?";
        $result = $adb->pquery($sql, array($crmid));
...
/**
 * Function to get the AccountName when a aicrm_account id is given
 * Takes the input as $acount_id - aicrm_account id
 * returns the aicrm_account name in string format.
 */
...
function getProductName($product_id)
{
	...
	global $adb;
	$sql = "select productname from aicrm_products where productid=?";
...
/**
 * Function to get the Potentail Name when a aicrm_potential id is given
 * Takes the input as $potential_id - aicrm_potential id
 * returns the aicrm_potential name in string format.
 */
...
function getPotentialName($potential_id)
{
	...
	if($potential_id != '')
	{
		$sql = "select potentialname from aicrm_potential where potentialid=?";
...
function getLeadName($lead_id)
{
	...
	if($lead_id != '')
	{
        	$sql = "select * from aicrm_leaddetails where leadid=?";
        	$result = $adb->pquery($sql, array($lead_id));
        	$firstname = $adb->query_result($result,0,"firstname");
        	$lastname = $adb->query_result($result,0,"lastname");
        	$lead_name = $firstname;
			// Asha: Check added for ticket 4788
			if (getFieldVisibilityPermission("Leads", $current_user->id,'lastname') == '0') {
				$lead_name .= ' '.$lastname;
			}
	}
...
function getLeadManagementName($lead_id)
{
	global $log;
	$log->debug("Entering getLeadManagementName(".$lead_id.") method ...");
	$log->info("in getLeadName ".$lead_id);
	...
	if($lead_id != '')
	{
		$sql = "select * from aicrm_leadmanage where leadid=?";
		$result = $adb->pquery($sql, array($lead_id));
		$firstname = $adb->query_result($result,0,"firstname");
		$lastname = $adb->query_result($result,0,"lastname");
		$lead_name = $lastname;
...
function getFullNameFromQResult($result, $row_count, $module)
{
	...
	$nam = textlength_check($firstname." ".$lastname);
...
/**
 * Function to get the Quote Name when a aicrm_vendor id is given
...
/**
 * Function to get the PriceBook Name when a aicrm_pricebook id is given
 * Takes the input as $pricebook_id - aicrm_pricebook id
...
function getPriceBookName($pricebookid)
{
	...
	$log->info("in getPriceBookName ".$pricebookid);
        global $adb;
        $sql = "select * from aicrm_pricebook where pricebookid=?";
...
function getGroupName($groupid)
{
    ...
    if($groupid != '')
	{
		$sql = "select groupname,groupid from aicrm_groups where groupid = ?";
...
function getUserFullName($userid)
{
	...
	if($userid != '')
	{
		$sql = "select first_name, last_name from aicrm_users where id=?";
		$result = $adb->pquery($sql, array($userid));
...
function getParentName($parent_id)
{
	...
	if($adb->query_result($result,'setype') == 'Accounts')
		$sql1="select accountname name from aicrm_account where accountid=?";
	else if($adb->query_result($result,'setype') == 'Contacts')
		$sql1="select concat( firstname, ' ', lastname ) name from aicrm_contactdetails where contactid=?";
 ...
 * param $focus - the module object contains the column aicrm_fields
 */
...
/** This function returns the default aicrm_currency information.
...
function getDisplayCurrency()
{
	...
	$log->debug("Entering getDisplayCurrency() method ...");
        $curr_array = Array();
        $sql1 = "select * from aicrm_currency_info where currency_status=? and deleted=0";
...
/** This function returns the conversion rate and aicrm_currency symbol
  * in array format for a given id.
  * param $id - aicrm_currency id.
  */
...
function getTermsandConditions()
{
    ...
    global $adb;
    $sql1 = "select * from aicrm_inventory_tandc";
...
function getTermsandConditions_en()
{
	global $log;
	$log->debug("Entering getTermsandConditions() method ...");
    global $adb;
    $sql1 = "select * from aicrm_inventory_tandc";
    $result = $adb->pquery($sql1, array());
    $tandc = $adb->query_result($result,1,"tandc");
	$log->debug("Exiting getTermsandConditions method ...");
    return $tandc;
}
 ...
 * Create select options in a dropdown list.  To be used inside
  *  a reminder select statement in a aicrm_activity form.
   * param $start - start value
   * param $end - end value
   * param $fldname - aicrm_field name
...
/** This function returns the List price of a given product in a given price book.
  ...
  * param $pbid - aicrm_pricebook id.
  */

function getPriceList($productid,$pbid)
{
	global $log;
	$log->debug("Entering getPriceList(".$productid.",".$pbid.") method ...");
    $log->info("in getPriceList productid ".$productid);

	global $adb;
	$query = "select listprice from aicrm_inventoryproductrel where id=? and productid=?";
	$result = $adb->pquery($query, array($pbid, $productid));
	$lp = $adb->query_result($result,0,'listprice');
	$log->debug("Exiting getPriceList method ...");
	return $lp;
}
...
function getListPrice($productid,$pbid)
{
	...
	global $adb;
	$query = "select listprice from aicrm_pricebookproductrel where pricebookid=? and productid=?";
	$result = $adb->pquery($query, array($pbid, $productid));
	$lp = $adb->query_result($result,0,'listprice');
...
/**
 * This function returns the aicrm_blocks and its related information for given module.
 * Input Parameter are $module - module name, $disp_view = display view (edit,detail or create),$mode - edit, $col_fields - * column aicrm_fields/
...
function getBlocks($module,$disp_view,$mode,$col_fields='',$info_type='')
{
	...
	if($module == 'Calendar') {
		$tabid = 16;
		$query="select blockid,blocklabel,show_title,display_status from aicrm_blocks where tabid=? and $disp_view=0 and visible = 0 and blockid not in (388) order by sequence";
	/*}elseif ($module == 'HelpDesk'){
        $query="select blockid,blocklabel,show_title,display_status from aicrm_blocks where tabid=? and $disp_view=0 and visible = 0 and blockid not in (702) order by sequence";*/
	}
	else{
    	$query="select blockid,blocklabel,show_title,display_status from aicrm_blocks where tabid=? and $disp_view=0 and visible = 0 order by sequence";
	}
	// echo $query; echo $tabid; exit;
	...
	if($mode == 'edit')
	{
		$display_type_check = 'aicrm_field.displaytype = 1';
	}elseif($mode == 'mass_edit')
	{
		$display_type_check = 'aicrm_field.displaytype = 1 AND aicrm_field.masseditable NOT IN (0,2)';
	}else
	{
		$display_type_check = 'aicrm_field.displaytype in (1,4)';
	}
	
	//retreive the aicrm_profileList from database
	...
	if($disp_view == "detail_view")
	{
		if($is_admin == true || $profileGlobalPermission[1] == 0 || $profileGlobalPermission[2] == 0 || $module == "Users" || $module == "Emails")
  		{
 			$sql = "SELECT aicrm_field.* FROM aicrm_field WHERE aicrm_field.tabid=? AND aicrm_field.block IN (". generateQuestionMarks($blockid_list) .") AND aicrm_field.displaytype IN (1,2,4) and aicrm_field.presence in (0,2) ORDER BY block,sequence";
  			$params = array($tabid, $blockid_list);
		}
  		else
  		{
  			$profileList = getCurrentUserProfileList();
 			$sql = "SELECT aicrm_field.* FROM aicrm_field INNER JOIN aicrm_profile2field ON aicrm_profile2field.fieldid=aicrm_field.fieldid INNER JOIN aicrm_def_org_field ON aicrm_def_org_field.fieldid=aicrm_field.fieldid WHERE aicrm_field.tabid=? AND aicrm_field.block IN (". generateQuestionMarks($blockid_list) .") AND aicrm_field.displaytype IN (1,2,4) and aicrm_field.presence in (0,2) AND aicrm_profile2field.visible=0 AND aicrm_def_org_field.visible=0 AND aicrm_profile2field.profileid IN (". generateQuestionMarks($profileList) .") GROUP BY aicrm_field.fieldid ORDER BY block,sequence";
	...
	else
	{
		if ($info_type != '')
		{
			if($is_admin==true || $profileGlobalPermission[1] == 0 || $profileGlobalPermission[2]== 0 || $module == 'Users' || $module == "Emails")
  			{
 				$sql = "SELECT aicrm_field.* FROM aicrm_field WHERE aicrm_field.tabid=? AND aicrm_field.block IN (". generateQuestionMarks($blockid_list) .") AND $display_type_check AND info_type = ? and aicrm_field.presence in (0,2) ORDER BY block,sequence";
  				$params = array($tabid, $blockid_list, $info_type);
			}
  			else
  			{
  				$profileList = getCurrentUserProfileList();
 				$sql = "SELECT aicrm_field.* FROM aicrm_field INNER JOIN aicrm_profile2field ON aicrm_profile2field.fieldid=aicrm_field.fieldid INNER JOIN aicrm_def_org_field ON aicrm_def_org_field.fieldid=aicrm_field.fieldid  WHERE aicrm_field.tabid=? AND aicrm_field.block IN (". generateQuestionMarks($blockid_list) .") AND $display_type_check AND info_type = ? AND aicrm_profile2field.visible=0 AND aicrm_def_org_field.visible=0 AND aicrm_profile2field.profileid IN (". generateQuestionMarks($profileList) .") and aicrm_field.presence in (0,2) GROUP BY aicrm_field.fieldid ORDER BY block,sequence";
		...
		else
		{
			if($is_admin==true || $profileGlobalPermission[1] == 0 || $profileGlobalPermission[2] == 0 || $module == 'Users' || $module == "Emails")
  			{
 				$sql = "SELECT aicrm_field.* FROM aicrm_field WHERE aicrm_field.tabid=? AND aicrm_field.block IN (". generateQuestionMarks($blockid_list).") AND $display_type_check  and aicrm_field.presence in (0,2) ORDER BY block,sequence";
				$params = array($tabid, $blockid_list);
			}
  			else
  			{
  				$profileList = getCurrentUserProfileList();
 				$sql = "SELECT aicrm_field.* FROM aicrm_field INNER JOIN aicrm_profile2field ON aicrm_profile2field.fieldid=aicrm_field.fieldid INNER JOIN aicrm_def_org_field ON aicrm_def_org_field.fieldid=aicrm_field.fieldid  WHERE aicrm_field.tabid=? AND aicrm_field.block IN (". generateQuestionMarks($blockid_list).") AND $display_type_check AND aicrm_profile2field.visible=0 AND aicrm_def_org_field.visible=0 AND aicrm_profile2field.profileid IN (". generateQuestionMarks($profileList).") and aicrm_field.presence in (0,2) GROUP BY aicrm_field.fieldid ORDER BY block,sequence";
				$params = array($tabid, $blockid_list, $profileList);
 				//Postgres 8 fixes
 				if( $adb->dbType == "pgsql")
 				    $sql = fixPostgresQuery( $sql, $log, 0);
...
function getBlockId($tabid,$label)
{
    ...
    global $adb;
    $blockid = '';
    $query = "select blockid from aicrm_blocks where tabid=? and blocklabel = ?";
...
/**
 * This function is used to get the Parent and Child aicrm_tab relation array.
 * Takes no parameter and get the data from parent_tabdata.php and aicrm_tabdata.php
...
function getHeaderArray()
{
	...
	//echo "<pre>"; print_r($relatedtabs); echo "</pre>"; exit;
	
	return $relatedtabs;
}

/**
 * This function is used to get the Parent Tab name for a given parent aicrm_tab id.
 * Takes the input parameter as $parenttabid - Parent aicrm_tab id
...
function getParentTabName($parenttabid)
{
	...
	{
		$sql = "select parenttab_label from aicrm_parenttab where parenttabid=?";
...
function getParentTabFromModule($module)
{
	...
	{
		$sql = "select aicrm_parenttab.* from aicrm_parenttab inner join aicrm_parenttabrel on aicrm_parenttabrel.parenttabid=aicrm_parenttab.parenttabid inner join aicrm_tab on aicrm_tab.tabid=aicrm_parenttabrel.tabid where aicrm_tab.name=?";
		$result = $adb->pquery($sql, array($module));
...
/**
 * This function is used to get the Parent Tab name for a given module.
 * Takes no parameter but gets the aicrm_parenttab value from form request
 * This returns value string type
 */
 ...
 * the current time and the modified time from the aicrm_crmentity aicrm_table and return the result as a string.
...
function updateInfo($id)
{
    ...
    global $adb;
    global $app_strings;
    $query='select modifiedtime from aicrm_crmentity where crmid = ?';
...
function getProductImages($id)
{
	...
	$script = '<script>var ProductImages = new Array(';
   	$i=0;
	$query='select imagename from aicrm_products where productid=?';
	$result = $adb->pquery($query, array($id));
 ...
 * It returns the array which has the total aicrm_users ,admin aicrm_users,and the non admin aicrm_users
 */
...
function UserCount()
{
	...
	global $adb;
	$result=$adb->pquery("select * from aicrm_users where deleted =0", array());
	$user_count=$adb->num_rows($result);
	$result=$adb->pquery("select * from aicrm_users where deleted =0 AND is_admin != 'on'", array());
...
function setObjectValuesFromRequest($focus)
{
	...
	foreach($focus->column_fields as $fieldname => $val)
	{
		if(isset($_REQUEST[$fieldname]))
		{
			if(is_array($_REQUEST[$fieldname])){
			    ...
			    if(in_array($fieldname, array('cf_4117'))){
                    $value = $_REQUEST[$fieldname];
                }else{
 ...
 /**
 * Function to write the tabid and name to a flat file aicrm_tabdata.txt so that the data
...
function create_tab_data_file()
{
	...
	$log->debug("Entering create_tab_data_file() method ...");
        $log->info("creating aicrm_tabdata file");
        global $adb;
		//$sql = "select * from aicrm_tab";
		// vtlib customization: Disabling the tab item based on presence
        $sql = "select * from aicrm_tab where presence in (0,2)";
		// END
	...
	$sql1="select * from aicrm_actionmapping";
	...
	$sql2="select * from aicrm_actionmapping where securitycheck=0";
...
else
{
 ...
 /**
 * Function to write the aicrm_parenttabid and name to a flat file parent_tabdata.txt so that the data
 * is obtained from the file instead of repeated queries
 * returns null
 */
...
function create_parenttab_data_file()
{
	...
	global $adb;
	$sql = "select parenttabid,parenttab_label from aicrm_parenttab where visible=0 order by sequence";
	$result = $adb->pquery($sql, array());
	$num_rows=$adb->num_rows($result);
	$result_array=Array();
	for($i=0;$i<$num_rows;$i++)
	...
	if (file_exists($filename)) {
		...
		if (is_writable($filename))
		{
			...
			foreach($result_array as $parid=>$parvalue)
			{
				$childArray=Array();
				// vtlib customization: Disabling the tab item based on presence
				$sql = "select * from aicrm_parenttabrel where parenttabid=?
					and tabid in (select tabid from aicrm_tab where presence in (0,2)) order by sequence";
				// END
...
function getQuickCreateModules()
{
	...
	$qc_query = "select distinct aicrm_tab.tablabel,aicrm_tab.name from aicrm_field inner join aicrm_tab on aicrm_tab.tabid = aicrm_field.tabid where quickcreate=0 and aicrm_tab.presence != 1 order by aicrm_tab.tablabel";
	// END
...
/**
 * This function is used to get the Quick create form aicrm_field parameters for a given module.
...
function QuickCreate($module)
{
	...
	{
		$quickcreate_query = "select * from aicrm_field where quickcreate in (0,2) and tabid = ? and aicrm_field.presence in (0,2) and displaytype != 2 order by quickcreatesequence";
		$params = array($tabid);
	}
	else
	{
		$profileList = getCurrentUserProfileList();
		$quickcreate_query = "SELECT aicrm_field.* FROM aicrm_field INNER JOIN aicrm_profile2field ON aicrm_profile2field.fieldid=aicrm_field.fieldid INNER JOIN aicrm_def_org_field ON aicrm_def_org_field.fieldid=aicrm_field.fieldid WHERE aicrm_field.tabid=? AND quickcreate in (0,2) AND aicrm_profile2field.visible=0 AND aicrm_def_org_field.visible=0  AND aicrm_profile2field.profileid IN (". generateQuestionMarks($profileList) .") and aicrm_field.presence in (0,2) and displaytype != 2 GROUP BY aicrm_field.fieldid ORDER BY quickcreatesequence";
...
function sendNotificationToOwner($module,$focus)
{
	...
	if($module == "Potentials" )
	{
		...
		else if($module == 'Accounts' || $module == 'Contacts'){
			if($module == 'Accounts'){
				$table_name = 'aicrm_account';
				$filed_id = 'accountid';
				$field_no = 'account_no';
			}else if($module == 'Contacts'){
				$table_name = 'aicrm_contactdetails';
				$filed_id = 'contactid';
				$field_no = 'contact_no';
			}

		require_once('include/email_alert/email_alert.php');
		$email_body =GetEmail($module,'header_alert.jpg',$focus->id,$filed_id);
		...
		$pquery = "select * from ".$table_name." where  ".$filed_id." = '".$focus->id."' ";
		$cf_res = $adb->pquery($pquery,'');
		$colum_no= $adb->query_result($cf_res, 0, $field_no);
		$subject = $module." No : ".$colum_no;
		if($user_emailid != '')
		{
			if($_REQUEST['mode'] != 'edit')
			{	
				$mail_status = send_mail($module,$user_emailid,$HELPDESK_SUPPORT_NAME,$HELPDESK_SUPPORT_EMAIL_ID,$subject, $email_body);
				//iconv("tis-620", "utf-8", $type );
			}
			else
			{
				$mail_status = send_mail($module,$user_emailid,$HELPDESK_SUPPORT_NAME,$HELPDESK_SUPPORT_EMAIL_ID,$subject,$email_body);
...
{
       global $adb,$app_strings,$current_user,$site_URL;
       $returnEntity=Array();
       $returnEntity=getEntityName_Tosendemail($module,Array($crmid));
       $user_create = getuser_create(Array($crmid));
       $mycrmid=$groupid;
	   ...
	   if (count($userIds) > 0) {
	       $groupqry="select email1,id,user_name from aicrm_users WHERE status='Active' AND id in(". generateQuestionMarks($userIds) .")";
	       $groupqry_res=$adb->pquery($groupqry, array($userIds));

	       $subject = 'Record Created '.$module.' -: '.$returnEntity[$crmid][$returnEntity[$crmid]['fistfield']] ;
	       for($z=0;$z < $adb->num_rows($groupqry_res);$z++)
	       {
	               //handle the mail send to aicrm_users
	               ...
	               $pmodule = 'Users';
		       	   $description = $app_strings['MSG_DEAR']." ".$tosender.",<br><br>".$user_create[$crmid]." Create<br>";
		       	   foreach ($returnEntity[$crmid] as $key => $value) {
		       	   		if($key != 'fistfield'){
		       	   			$description .= $key.': '.$value.'<br>';
		       	   		}
		       	   }
		       	   $description .= $app_strings['MSG_HAS_BEEN_CREATED_FOR']." ".$module."<br><br>";
		       	   $description .="Link <a href='".$site_URL."/index.php?action=DetailView&module=".$module."&record=".$crmid."&parenttab='>Click here</a><br><br>";
		       	   $description .= $app_strings['MSG_THANKS']."<br>".$app_strings['MSG_VTIGERTEAM'];
	               require_once('modules/Emails/mail.php');
				   $mail_status = send_mail('Emails',$emailadd,$current_user->user_name,'',$subject,$description,'','','all',$focus->id);
...
function getuser_create($ids_list){
	global $adb;
	if (count($ids_list) <= 0) {
		return array();
	}
	 $query = "select aicrm_users.user_name from aicrm_users Inner join aicrm_crmentity on aicrm_crmentity.smcreatorid = aicrm_users.id where aicrm_crmentity.crmid in (".generateQuestionMarks($ids_list).")";
	  $result = $adb->pquery($query, array($ids_list));
	  //print_r($result); exit;
	 ...
	 for ($i = 0; $i < $numrows; $i++)
	 {
		...
		$usercreate[$entity_id] = $adb->query_result($result,$i,'user_name');			
	 }
		 
	return $usercreate;
}

function getEntityName_Tosendemail($module, $ids_list)
{
	global $adb;
	global $log;
	...
	if($module != '')
	{
		 $query = "select tabid,fieldname,tablename,entityidfield from aicrm_entityname where modulename = ?";
		 ...
		 $query1 = " select fieldname,fieldlabel from aicrm_field where tabid = '".$tabid."' and fieldname in (". generateQuestionMarks($fieldlists).")";
		 $result1 = $adb->pquery($query1, $fieldlists);

		 $numrows1 = $adb->num_rows($result1);
		 for ($i = 0; $i < $numrows1; $i++)
		 {
		 	$field_record[$adb->query_result($result1,$i,'fieldname')] = $adb->query_result($result1,$i,'fieldlabel');
		 }
		  $field_record['fistfield'] = $field_record[$fieldlists[0]];

		 $query2 = " select $fieldsname from $tablename where $entityidfield in (". generateQuestionMarks($ids_list) .")";
		 $params2 = array($ids_list);
		 $result2 = $adb->pquery($query2, $params2);
		 $numrows2 = $adb->num_rows($result2);
		 for ($i = 0; $i < $numrows2; $i++)
		 {
			$entity_id = $ids_list[$i];
			for ($p = 0; $p < count($fieldlists); $p++){
				$entity_info[$entity_id][$field_record[$fieldlists[$p]]] = $adb->query_result($result2,$i,$fieldlists[$p]);
			}
			$entity_info[$entity_id]['fistfield']=$field_record['fistfield'];
		 }
...
function Button_Check($module)
{
	...
	$exportRestrictedModules = array('Leads', 'Accounts');
	$exportAllowedRoles = array('H2', 'H939');
	$shouldRestrictExportByRole = in_array($module, $exportRestrictedModules, true);
	$currentUserRoleId = isset($current_user->roleid) ? $current_user->roleid : '';

	foreach($permit_arr as $action => $perr){
		$tempPer=isPermitted($module,$action,'');
		if($action === 'Export' && $shouldRestrictExportByRole) {
			$tempPer = in_array($currentUserRoleId, $exportAllowedRoles, true) ? 'yes' : 'no';
		}
...
function getAllParenttabmoduleslist()
{
	...
	// vtlib customization: Disabling the tab item based on presence
	$query = 'select name,tablabel,parenttab_label,aicrm_tab.tabid from aicrm_parenttabrel inner join aicrm_tab on aicrm_parenttabrel.tabid = aicrm_tab.tabid inner join aicrm_parenttab on aicrm_parenttabrel.parenttabid = aicrm_parenttab.parenttabid and aicrm_tab.presence in (0,2) order by aicrm_parenttab.sequence, aicrm_parenttabrel.sequence';
	// END
...
/**
 * 	This function is used to get the Path in where we store the aicrm_files based on the module.
 *	@param string $module   - module name
 ...
 *	@param string $file_details  - aicrm_files array which contains all the uploaded file details
...
function getTemplateDetails($templateid)
{
        ...
        $returndata =  Array();
        $result = $adb->pquery("select * from aicrm_emailtemplates where templateid=?", array($templateid));
	...
	switch($parent_type)
	{
		case 'Accounts':
			if(is_array($fields["accounts"]))
			{
				...
				$query = "select $columnfields from aicrm_account inner join aicrm_accountscf where aicrm_account.accountid =? and aicrm_accountscf.accountid =?";
		...
		case 'Contacts':
			if(is_array($fields["contacts"]))
			{
				...
				$query = "select $columnfields from aicrm_contactdetails inner join aicrm_contactscf inner join aicrm_customerdetails on aicrm_customerdetails.customerid=aicrm_contactdetails.contactid and aicrm_customerdetails.customerid=aicrm_contactscf.contactid where aicrm_contactdetails.contactid=?";
				$result = $adb->pquery($query, array($id,$id));
		...
		case 'Leads':
			if(is_array($fields["leads"]))
			{
				...
				$query = "select $columnfields from aicrm_leaddetails inner join aicrm_leadscf where aicrm_leaddetails.leadid=? and aicrm_leadscf.leadid=?";
				$result = $adb->pquery($query, array($id,$id));
			...
			case 'LeadManagement':
				if(is_array($fields["leads"]))
				{
					...
					$columnfields = implode(',',$fields["leads"]);
					$query = "select $columnfields from aicrm_leadmanage inner join aicrm_leadmanagecf where aicrm_leadmanage.leadid=? and aicrm_leadmanagecf.leadid=?";
					$result = $adb->pquery($query, array($id,$id));
					foreach($fields["leads"] as $columnname)
					{
						$token_data = '$leads-'.$columnname.'$';
		...
		case 'Users':
			if(is_array($fields["users"]))
			{
				...
				$query = "select $columnfields from aicrm_users where id=?";
				$result = $adb->pquery($query, array($id));
...
{
	global $adb;
	$sql=" select * from aicrm_announcement inner join aicrm_users on aicrm_announcement.creatorid=aicrm_users.id";
	$sql.=" AND aicrm_users.is_admin='on' AND aicrm_users.status='Active' AND aicrm_users.deleted = 0";
	$result=$adb->pquery($sql, array());
	for($i=0;$i<$adb->num_rows($result);$i++)
	{
...
function getImportFieldsList($module)
{
	...
	$displaytype = " displaytype=1 and aicrm_field.presence in (0,2) ";

	$fieldnames = "";
	//For module basis we can add the list of fields for Import mapping
	if($module == "Leads" || $module == "LeadManagement" || $module == "Contacts")
	{
	...
	$query = "SELECT fieldname, fieldlabel FROM aicrm_field $where order by fieldlabel asc ";
...
function getTicketComments($ticketid)
{
        ...
        $commentlist = '';
        $sql = "select * from aicrm_ticketcomments where ticketid=?";
        $result = $adb->pquery($sql, array($ticketid));
        for($i=0;$i<$adb->num_rows($result);$i++)
        {
...
function getTicketDetails($id,$whole_date)
{
	 ...
	 $sql = "SELECT * FROM aicrm_ticketcf WHERE ticketid = ?";
	 $result = $adb->pquery($sql, array($id));
	 ...
	 foreach ($cffields as $cfOneField)
	 {
		 if ($cfOneField != 'ticketid')
		 {
			 ...
			 $sql = "SELECT fieldlabel FROM aicrm_field WHERE columnname = ? and aicrm_field.presence in (0,2)";
...
function getmail_contents_portalUser($request_array,$password,$type='')
{
	...
	//here id is hardcoded with 5. it is for support start notification in aicrm_notificationscheduler

	$query='select aicrm_emailtemplates.subject,aicrm_emailtemplates.body from aicrm_notificationscheduler inner join aicrm_emailtemplates on aicrm_emailtemplates.templateid=aicrm_notificationscheduler.notificationbody where schedulednotificationid=5';
...
function getUItype($module,$columnname)
{
	...
	$tabid=getTabid($module);
    global $adb;
    $sql = "select uitype from aicrm_field where tabid=? and columnname=?";
...
function is_emailId($entity_id)
{
	...
	if($module == 'Contacts')
	{
		$sql = "select email,yahooid from aicrm_contactdetails inner join aicrm_crmentity on aicrm_crmentity.crmid = aicrm_contactdetails.contactid where contactid = ?";
	...
	elseif($module == 'Leads')
	{
		$sql = "select email,yahooid from aicrm_leaddetails inner join aicrm_crmentity on aicrm_crmentity.crmid = aicrm_leaddetails.leadid where leadid = ?";
		$result = $adb->pquery($sql, array($entity_id));
		$email1 = $adb->query_result($result,0,"email");
		$email2 = $adb->query_result($result,0,"yahooid");
		if($email1 != "" || $email2 != "") {
...
function getCvIdOfAll($module)
{
	...
	$qry_res = $adb->pquery("select cvid from aicrm_customview where viewname='All' and entitytype=?", array($module));
...
function getTagCloudView($id=""){
	...
	if($id == ''){
		...
		$query = "select visible from aicrm_homestuff where userid=? and stufftype='Tag Cloud'";
...
function SaveTagCloudView($id="")
{
	...
	if($id == ''){
		...
		$query = "update aicrm_homestuff set visible = ? where userid=? and stufftype='Tag Cloud'";
 ...
 **	        -------------------+------------+----------------------+------------------+
 **		Potential Name     | I~O        | aicrm_quotes        | potentialid      |
 **
 **     Then put an entry in $new_field_details  like this:
 **
 **				"aicrm_quotes:potentialid"=>"V",
 **
...
function ChangeTypeOfData_Filter($table_name,$column_name,$type_of_data)
{
	...
	$new_field_details = Array(
		...
		"aicrm_contactdetails:accountid"=>"V",
		"aicrm_contactsubdetails:birthday"=>"D",
		"aicrm_contactdetails:email"=>"V",
		"aicrm_contactdetails:yahooid"=>"V",

		//Potential Related Fields
		"aicrm_potential:campaignid"=>"V",

		//Account Related Fields
		"aicrm_account:parentid"=>"V",
		"aicrm_account:email1"=>"V",
		"aicrm_account:email2"=>"V",

		//Lead Related Fields
		"aicrm_leaddetails:email"=>"V",
		"aicrm_leaddetails:yahooid"=>"V",

		//Documents Related Fields
		"aicrm_senotesrel:crmid"=>"V",

		//Calendar Related Fields
		"aicrm_seactivityrel:crmid"=>"V",
		"aicrm_seactivityrel:contactid"=>"V",
		"aicrm_recurringevents:recurringtype"=>"V",

		//HelpDesk Related Fields
		"aicrm_troubletickets:parent_id"=>"V",
		"aicrm_troubletickets:product_id"=>"V",

		//Product Related Fields
		"aicrm_products:discontinued"=>"C",
		"aicrm_products:vendor_id"=>"V",
		"aicrm_products:handler"=>"V",
		"aicrm_products:parentid"=>"V",

		//Faq Related Fields
		"aicrm_faq:product_id"=>"V",

		//Vendor Related Fields
		"aicrm_vendor:email"=>"V",

		//Quotes Related Fields
		"aicrm_quotes:potentialid"=>"V",
		"aicrm_quotes:inventorymanager"=>"V",
		"aicrm_quotes:accountid"=>"V",

		//Purchase Order Related Fields
		"aicrm_purchaseorder:vendorid"=>"V",
		"aicrm_purchaseorder:contactid"=>"V",

		//SalesOrder Related Fields
		"aicrm_salesorder:potentialid"=>"V",
		"aicrm_salesorder:quoteid"=>"V",
		"aicrm_salesorder:contactid"=>"V",
		"aicrm_salesorder:accountid"=>"V",

		//Invoice Related Fields
		"aicrm_invoice:salesorderid"=>"V",
		"aicrm_invoice:contactid"=>"V",
		"aicrm_invoice:accountid"=>"V",

		//Campaign Related Fields
		"aicrm_campaign:product_id"=>"V",

		//Related List Entries(For Report Module)
		"aicrm_activityproductrel:activityid"=>"V",
		"aicrm_activityproductrel:productid"=>"V",

		"aicrm_campaigncontrel:campaignid"=>"V",
		"aicrm_campaigncontrel:contactid"=>"V",

		"aicrm_campaignleadrel:campaignid"=>"V",
		"aicrm_campaignleadrel:leadid"=>"V",

		"aicrm_cntactivityrel:contactid"=>"V",
		"aicrm_cntactivityrel:activityid"=>"V",

		"aicrm_contpotentialrel:contactid"=>"V",
		"aicrm_contpotentialrel:potentialid"=>"V",

		"aicrm_crmentitynotesrel:crmid"=>"V",
		"aicrm_crmentitynotesrel:notesid"=>"V",

		"aicrm_leadacctrel:leadid"=>"V",
		"aicrm_leadacctrel:accountid"=>"V",

		"aicrm_leadcontrel:leadid"=>"V",
		"aicrm_leadcontrel:contactid"=>"V",

		"aicrm_leadpotrel:leadid"=>"V",
		"aicrm_leadpotrel:potentialid"=>"V",

		"aicrm_pricebookproductrel:pricebookid"=>"V",
		"aicrm_pricebookproductrel:productid"=>"V",

		"aicrm_seactivityrel:crmid"=>"V",
		"aicrm_seactivityrel:activityid"=>"V",

		"aicrm_senotesrel:crmid"=>"V",
		"aicrm_senotesrel:notesid"=>"V",

		"aicrm_seproductsrel:crmid"=>"V",
		"aicrm_seproductsrel:productid"=>"V",

		"aicrm_seticketsrel:crmid"=>"V",
		"aicrm_seticketsrel:ticketid"=>"V",

		"aicrm_vendorcontactrel:vendorid"=>"V",
		"aicrm_vendorcontactrel:contactid"=>"V",

		"aicrm_pricebook:currency_id"=>"V",
	);
...
function getEmailTemplateVariables(){
	...
	$module_cf_table = array('Accounts' => 'aicrm_accountscf',
	...
	foreach($modules_list as $index=>$module){
		...
		if ($default_fields != null) {
			foreach($default_fields as $index=>$field){
				if($field=='support_start_date' || $field=='support_end_date'){
					$tabname='aicrm_customerdetails';
				}
				$sql="select fieldlabel from aicrm_field where tablename=? and columnname=? and aicrm_field.presence in (0,2)";
		...
		// Set the custom fields of a given module for Email templates
			...
			foreach($custFields as $val=>$key){
				$sql="select fieldlabel from aicrm_field where tablename=? and columnname=? and uitype!=56 and aicrm_field.presence in (0,2)";
...
function getFieldTemplateVariables(){
    ...
    $modules_list = array('Accounts', 'Contacts', 'Leads' );
    $module_cf_table = array('Accounts' => 'Accounts',
        'Contacts' => 'Contacts',
        'Leads' => 'Leads' );
    ...
    foreach($modules_list as $index=>$module){
        ...
        $sql = "select aicrm_field.fieldlabel,aicrm_field.columnname ,aicrm_field.tablename from aicrm_field 
				Inner Join aicrm_tab on aicrm_tab.tabid = aicrm_field.tabid
				where aicrm_tab.name ='".$module."' and aicrm_field.quickcreate in (0,2) and aicrm_field.presence in (0,2) order by aicrm_field.fieldlabel";

        $default_fields = $myLibrary_mysqli->select($sql);
        //echo "<pre>"; print_r($default_fields); echo "</pre>"; exit;
        if($module == 'Accounts'){
            array_push($default_fields,array('fieldlabel'=>'Account ID','columnname'=>'accountid' ,'tablename'=>'aicrm_account'));
            array_push($default_fields,array('fieldlabel'=>'รหัสลูกค้า(RMS)' ,'columnname'=>'accountcode','tablename'=>'aicrm_account'));
        }elseif($module == 'Contacts'){
            array_push($default_fields,array('fieldlabel'=>'Contact ID' ,'columnname'=>'contactid','tablename'=>'aicrm_contactdetails'));
        }elseif($module == 'Leads'){
            array_push($default_fields,array('fieldlabel'=>'Lead ID' ,'columnname'=>'leadid','tablename'=>'aicrm_leaddetails'));
        }elseif($module == 'Opportunity'){
            array_push($default_fields,array('fieldlabel'=>'Opportunityid' ,'columnname'=>'opportunityid','tablename'=>'aicrm_opportunity'));
        }
        ...
        if (count($default_fields) > 0) {
            foreach($default_fields as $index=>$field){
                ...
                //$option=array(getTranslatedString($module).': '.getTranslatedString($field['fieldlabel']),"$".strtolower($module)."-".$field['tablename'].".".$field['columnname']."$");
                $option=array(getTranslatedString($module).': '.getTranslatedString($field['fieldlabel']),"$".$field['tablename'].".".$field['columnname']."$");
                $allFields[] = $option;
            }
        }
    ...
    $allFields[] = $option;
    $allOptions[] = $allFields;
    //echo "<pre>"; print_r($allOptions); echo "</pre>"; exit;
    return $allOptions;
}
...
{
	global $adb;
	$query = "select $tablename from aicrm_$tablename inner join aicrm_role2picklist on aicrm_role2picklist.picklistvalueid = aicrm_$tablename.picklist_valueid where roleid=? and picklistid in (select picklistid from aicrm_picklist) order by sortid";
...
{
	global $adb;
	$quer = "select activitytype from aicrm_activity where activityid=?";
...
function getOwnerName($id)
{
	...
	if($id != '') {
        $sql = "select concat( first_name, ' ', last_name ) as user_name from aicrm_users where id=?";
		$result = $adb->pquery($sql, array($id));
		$ownername = $adb->query_result($result,0,"user_name");
	}
	if($ownername == '') {
		$sql = "select groupname from aicrm_groups where groupid=?";
		$result = $adb->pquery($sql, array($id));
...
function getRecordems($account_id)
{
	global $adb, $log;
	$log->debug("Entering getRecordems(".$account_id.") method ...");
	$log->info("in getRecordems ".$account_id);

	if($account_id != '0') {
		$sql = "select accountcode from aicrm_account where accountid =?";
		$result = $adb->pquery($sql, array($account_id));
		$recordrms = $adb->query_result($result,0,"accountcode");
	}
	
	$log->debug("Exiting getRecordems method ...");
	return $recordrms;
}
...
function getSettingsBlockId($label) {
	...
	$blockid = '';
	$query = "select blockid from aicrm_settings_blocks where label = ?";
