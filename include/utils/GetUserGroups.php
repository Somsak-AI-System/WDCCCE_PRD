...
class GetUserGroups { 
	...
	/** to get all the parent aicrm_groups of the specified group
	...
	function getAllUserGroups($userid)
	{
		...
		$query="select * from aicrm_users2group where userid=?";
		...
		//Retreiving from the aicrm_user2role
		$query="select * from aicrm_group2role where roleid=?";
		...
		$query="select * from aicrm_group2rs where roleandsubid in (". generateQuestionMarks($parentRolelist) .")";
