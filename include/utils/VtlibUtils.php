...
if(!isset($__cache_aicrm_imagepath)) {
    $__cache_aicrm_imagepath = Array();
}
function aicrm_imageurl($imagename, $themename) {
	global $__cache_aicrm_imagepath;
	if($__cache_aicrm_imagepath[$imagename]) {
        $imagepath = $__cache_aicrm_imagepath[$imagename];
    } else {
		$imagepath = false;
        ...
        $__cache_aicrm_imagepath[$imagename] = $imagepath;
    }
	return $imagepath;
}
...
function vtlib_getModuleNameById($tabid) {
	global $adb;
	$sqlresult = $adb->pquery("SELECT name FROM aicrm_tab WHERE tabid = ?",array($tabid));
...
function vtlib_getModuleNameForSharing() {
	global $adb;
	$vtlib_sqlres = $adb->query("SELECT * from aicrm_tab WHERE ownedby = 0 
...
function vtlib_prefetchModuleActiveInfo($force = true) {
	...
	if($tabrows === false || $force) {
		global $adb;
		$tabres = $adb->query("SELECT * FROM aicrm_tab");
		$tabrows = array();	
		if($tabres) {
...
function vtlib_isModuleActive($module) {
	...
	if(!isset($__cache_module_activeinfo[$module])) {
		$tabres = $adb->pquery("SELECT presence FROM aicrm_tab WHERE name=?", array($module));
...
function vtlib_RecreateUserPrivilegeFiles() {
	global $adb;
	$userres = $adb->query('SELECT id FROM aicrm_users WHERE deleted = 0');
...
function vtlib_moduleAlwaysActive() {
	$modules = Array (
		...
		'Utilities', 'uploads', 'Import', 'System', 'com_aicrm_workflow', 'PickList',
	);
	return $modules;
}
...
function vtlib_toggleModuleAccess($module, $enable_disable) {
	...
	$adb->pquery("UPDATE aicrm_tab set presence = ? WHERE name = ?", array($enable_disable,$module));
	...
	global $aicrm_current_version;
	if(version_compare($aicrm_current_version, '5.0.4', '>')) {
...
function vtlib_getToggleModuleInfo() {
	...
	$sqlresult = $adb->query("SELECT name, presence, customized, isentitytype FROM aicrm_tab WHERE /*name NOT IN ('Users')*/ 1=1 AND presence IN (0,1) ORDER BY name");
...
function vtlib_getToggleLanguageInfo() {
	...
	$langinfo = Array();
	$sqlresult = $adb->query("SELECT * FROM aicrm_language");
	if($sqlresult) {
...
function vtlib_toggleLanguageAccess($langprefix, $enable_disable) {
	...
	$adb->pquery('UPDATE aicrm_language set active = ? WHERE prefix = ?', Array($enable_disable, $langprefix));
...
function vtlib_getFieldHelpInfo($module) {
	...
	if(in_array('helpinfo', $adb->getColumnNames('aicrm_field'))) {
		$result = $adb->pquery('SELECT fieldname,helpinfo FROM aicrm_field WHERE tabid=?', Array(getTabid($module)));
		...
		Array(
			...
			Array(
				...
				'related_tables' => Array( 
					'aicrm_accountbillads' => Array ('accountaddressid', 'aicrm_account', 'accountid'),					
					'aicrm_accountshipads' => Array ('accountaddressid', 'aicrm_account', 'accountid'), 
				),
			...
			Array(
				...
				'related_tables'=> Array( 'aicrm_account' => Array ('accountid' ) ),
			...
			Array(
				...
				'table_index' => 'leadid',
				'related_tables' => Array ( 
					'aicrm_leadsubdetails' => Array ( 'leadsubscriptionid', 'aicrm_leaddetails', 'leadid' ),
					'aicrm_leadaddress'    => Array ( 'leadaddressid', 'aicrm_leaddetails', 'leadid' ),		   	
				),
				'popup_fields'=> Array ('lastname'),
			),
			'LeadManagement' =>
				Array(
						'IsCustomModule'=>false,
						'table_name'  => 'aicrm_leadmanage',
						'table_index' => 'leadid',
						'related_tables' => Array (
								'aicrm_leadmanagesubdetail' => Array ( 'leadsubscriptionid', 'aicrm_leadmanage', 'leadid' ),
								'aicrm_leadmanageaddress'    => Array ( 'leadaddressid', 'aicrm_leadmanage', 'leadid' ),
						),
						'popup_fields'=> Array ('lastname'),
				),
			'Campaigns' =>
			...
			Array(
				...
				'popup_fields'=> Array('task_name')
			),
			'Documents'=>
			Array(
				'IsCustomModule'=>false,
				'table_name' => 'aicrm_notes',
				'table_index'=> 'notesid',
			),
			'Products'=>
			Array(
...
function vtlib_getPicklistValues_AccessibleToAll($field_columnname) {
	...
	$tablename = "aicrm_$columnname";

	// Gather all the roles (except H1 which is organization role)
	$roleres = $adb->query("SELECT roleid FROM aicrm_role WHERE roleid != 'H1'");
	...
	$picklistres = $adb->query(
		...
		INNER JOIN aicrm_role2picklist ON $tablename.picklist_valueid=aicrm_role2picklist.picklistvalueid
		WHERE roleid != 'H1'");
