...
function getMailServerInfo($user)
{
  ...
  global $adb;
        $sql = "select * from aicrm_mail_accounts where status=1 and user_id=?";
...
/** To get the Role of the specified user
  ...
  * @returns  aicrm_roleid :: Type String
 */
function fetchUserRole($userid)
{
  ...
  global $adb;
  $sql = "select roleid from aicrm_user2role where userid=?";
...
function fetchUserProfileId($userid)
{
  ...
  if($profileid === false) {
    ...
    $query  = "SELECT profileid FROM aicrm_role2profile WHERE roleid=(SELECT roleid FROM aicrm_user2role WHERE userid=?)";
...
function loadAllPerms()
  ...
  $log->debug("Entering loadAllPerms() method ...");
        ...
        $profiles = Array();
        $sql = "select distinct profileid from aicrm_profile2tab";
        ...
        {
                $sql = "select * from aicrm_profile2tab where profileid=?";
...
/** Function to get all the aicrm_tab permission for the specified aicrm_profile
...
/** Function to get all the aicrm_tab permission for the specified aicrm_profile other than tabid 15
  * @param $profileid -- Profile Id:: Type integer
  * @returns  TabPermission Array in the following format:
  * $tabPermission = Array($tabid1=>permission,
  *                        $tabid2=>permission, 
...
/** Function to get all the aicrm_tab standard action permission for the specified aicrm_profile
  * @param $profileid -- Profile Id:: Type integer
...
function getTabsActionPermission($profileid)
{
  ...
  $check = Array();
  $temp_tabid = Array();
  $sql1 = "select * from aicrm_profile2standardpermissions where profileid=? and tabid not in(16) order by(tabid)";
...
/** Function to get all the aicrm_tab utility action permission for the specified aicrm_profile
  * @param $profileid -- Profile Id:: Type integer
...
function getTabsUtilityActionPermission($profileid)
{
  ...
  $check = Array();
  $temp_tabid = Array();
  $sql1 = "select * from aicrm_profile2utility where profileid=? order by(tabid)";
  $result1 = $adb->pquery($sql1, array($profileid));
        $num_rows1 = $adb->num_rows($result1);
        for($i=0; $i<$num_rows1; $i++)
        {
...
function getDefaultSharingEditAction()
{
  ...
  $sql= "select * from aicrm_def_org_share where editstatus=0";
  $result = $adb->pquery($sql, array());
...
function getDefaultSharingAction()
{
  ...
  $sql= "select * from aicrm_def_org_share where editstatus in(0,1)";
  $result = $adb->pquery($sql, array());
  $permissionRow=$adb->fetch_array($result);
  do
  {
...
function getAllDefaultSharingAction()
{
  ...
  $copy=Array();
  //retreiving the standard permissions 
  $sql= "select * from aicrm_def_org_share";
  $result = $adb->pquery($sql, array());
  $num_rows=$adb->num_rows($result);
...
/** Function to create the aicrm_role
  * @param $roleName -- Role Name:: Type varchar
  * @param $parentRoleId -- Parent Role Id:: Type varchar
  * @param $roleProfileArray -- Profile to be associated with this aicrm_role:: Type Array
...
function createRole($roleName,$parentRoleId,$roleProfileArray)
{
  ...
  $roleid_no=$adb->getUniqueId("aicrm_role");
        $roleId='H'.$roleid_no;
  ...
  $query="insert into aicrm_role values(?,?,?,?)";
...
/** Function to update the aicrm_role
  * @param $roleName -- Role Name:: Type varchar
...
function updateRole($roleId,$roleName,$roleProfileArray)
{
  ...
  global $adb;
  $sql1 = "update aicrm_role set rolename=? where roleid=?";
    $adb->pquery($sql1, array($roleName, $roleId));
  //Updating the Role2Profile relation
  $sql2 = "delete from aicrm_role2profile where roleId=?";
...
/** Function to add the aicrm_role to aicrm_profile relation
  * @param $profileId -- Profile Id:: Type integer
  * @param $roleId -- Role Id:: Type varchar
  *
 */
function insertRole2ProfileRelation($roleId,$profileId)
{
  ...
  global $adb;
  $query="insert into aicrm_role2profile values(?,?)";
...
/** Function to get the aicrm_roleid from aicrm_rolename
  * @param $rolename -- Role Name:: Type varchar
...
$log->debug("Entering fetchRoleId(".$rolename.") method ...");
  ...
  global $adb;
  $sqlfetchroleid = "select roleid from aicrm_role where rolename=?";
...
/** Function to update user to aicrm_role mapping based on the userid
  * @param $roleid -- Role Id:: Type varchar
...
$log->debug("Entering updateUser2RoleMapping(".$roleid.",".$userid.") method ...");
  ...
  $sqlcheck = "select * from aicrm_user2role where userid=?";
  $resultcheck = $adb->pquery($sqlcheck, array($userid));
  if($adb->num_rows($resultcheck) == 1)
  {
    $sqldelete = "delete from aicrm_user2role where userid=?";
  $delparams = array($userid);
    $result_delete = $adb->pquery($sqldelete, $delparams);
  } 
  $sql = "insert into aicrm_user2role(userid,roleid) values(?,?)";
...
$log->debug("Entering updateUsers2GroupMapping(".$groupname.",".$userid.") method ...");
  global $adb;
  $sqldelete = "delete from aicrm_users2group where userid = ?";
  $delparams = array($userid);
  $result_delete = $adb->pquery($sqldelete, $delparams);
  
  $sql = "insert into aicrm_users2group(groupname,userid) values(?,?)";
  $params = array($groupname,$userid);
  $result = $adb->pquery($sql, $params);
  $log->debug("Exiting updateUsers2GroupMapping method ...");
}

/** Function to add user to aicrm_role mapping 
  * @param $roleid -- Role Id:: Type varchar
  * @param $userid User Id:: Type integer
  *
 */
...
function fetchWordTemplateList($module)
{
  ...
  $log->debug("Entering fetchWordTemplateList(".$module.") method ...");
    global $adb;
    $sql_word = "select templateid, filename from aicrm_wordtemplates where module =?" ; 
...
function fetchEmailTemplateInfo($templateName)
{
  ...
  global $adb;
    $sql= "select * from aicrm_emailtemplates where templatename=?";
...
/** Function to get the aicrm_role name from the aicrm_roleid 
  * @param $roleid -- Role Id:: Type varchar
...
function getRoleName($roleid)
{
  ...
  global $adb;
  $sql1 = "select * from aicrm_role where roleid=?";
...
/** Function to get the aicrm_profile name from the aicrm_profileid 
  * @param $profileid -- Profile Id:: Type integer
  * @returns $rolename -- Role Name:: Type varchar
  *
 */
function getProfileName($profileid)
{
  ...
  global $adb;
  $sql1 = "select * from aicrm_profile where profileid=?";
...
/** Function to get the aicrm_profile Description from the aicrm_profileid
  * @param $profileid -- Profile Id:: Type integer
  * @returns $rolename -- Role Name:: Type varchar
  *
 */
function getProfileDescription($profileid)
{
        ...
        global $adb;
        $sql1 = "select  description from aicrm_profile where profileid=?";
        $result = $adb->pquery($sql1, array($profileid));
...
function isPermitted($module,$actionname,$record_id='')
{
  ...
  //If no actionid, then allow action is aicrm_tab permission is available  
  if($actionid == '')
  {
  ...
  //Checking for aicrm_tab permission

 /* print_r($profileGlobalPermission[2]);
  exit;*/
  if($profileTabsPermission[$tabid] !=0)
  {
  ...
  //echo $actionid;
  //print_r($profileActionPermission[$tabid][$actionid] );exit;
  ...
  if($record_id != '')
  {
    // echo 'Record ID: '.$record_id. getTabOwnedBy($module).'<br />';
  ...
  if($others_permission_id == 0)
  {
    if($actionid == 1 || $actionid == 0)
    {
      ...
      if($module == 'Projects')
      {
        $query = "SELECT aicrm_projects.related_sales_person, aicrm_crmentity.smownerid FROM aicrm_projects
        INNER JOIN aicrm_crmentity ON aicrm_crmentity.crmid = aicrm_projects.projectsid
        WHERE aicrm_projects.projectsid=".$record_id;
        $rs = $adb->pquery($query, []);

        $sales_person = $adb->query_result($rs, 0, "related_sales_person");
        $owner = $adb->query_result($rs, 0, "smownerid");

        $owners = [$owner];
        if($sales_person != ''){
          $sales = explode(' |##| ', $sales_person);
          foreach($sales as $sale){
            $query  = "SELECT id FROM aicrm_users WHERE CONCAT(first_name, ' ', last_name) = '". $sale ."'";
            $rs = $adb->pquery($query, []);

            $owners[] = $adb->query_result($rs, 0, "id");
          }
        }
        
        if($permission == 'no' && in_array($current_user->id, $owners)){
          $permission = 'yes';
        }
      }
  ...
  elseif($others_permission_id == 3)
  {
    ...
    if($actionid == 3 || $actionid == 4)
    {
      ...
      if($module=="HelpDesk"){
        $query="select crmid from aicrm_crmentity where crmid='".$record_id."' and smcreatorid='".$current_user->id."'";
        $result = $adb->pquery($query,array());
        if($adb->query_result($result,0,"crmid")!=""){
          $permission='yes'; 
        }
      }
...
function isReadPermittedBySharing($module,$tabid,$actionid,$record_id)
{
  ...
  //echo "<pre>"; print_r($sharingModuleList); echo "</pre>"; exit;
  $recordOwnerArr=getRecordOwnerId($record_id);
  //echo "<pre>"; print_r($recordOwnerArr); echo "</pre>"; exit;
  foreach($recordOwnerArr as $type=>$id)
  {
    $ownertype=$type;
    $ownerid=$id;
...
function isAllowed_Outlook($module,$action,$user_id,$record_id)
{
  ...
  { 
    ...
    //Checking whether this aicrm_tab is allowed
...
/** Function to get the Profile Global Information for the specified aicrm_profileid  
  * @param $profileid -- Profile Id:: Type integer
...
$log->debug("Entering getProfileGlobalPermission(".$profileid.") method ...");
  global $adb;
  $sql = "select * from aicrm_profile2globalpermissions where profileid=?" ;
  $result = $adb->pquery($sql, array($profileid));
  $num_rows = $adb->num_rows($result);
...
/** Function to get the Profile Tab Permissions for the specified aicrm_profileid  
  * @param $profileid -- Profile Id:: Type integer
...
/** Function to get the Profile Action Permissions for the specified aicrm_profileid  
  * @param $profileid -- Profile Id:: Type integer
...
$log->debug("Entering getProfileActionPermission(".$profileid.") method ...");
  ...
  $check = Array();
  $temp_tabid = Array();  
  $sql1 = "select * from aicrm_profile2standardpermissions where profileid=?";
  $result1 = $adb->pquery($sql1, array($profileid));
        $num_rows1 = $adb->num_rows($result1);
        for($i=0; $i<$num_rows1; $i++)
        {
...
/** Function to get the Standard and Utility Profile Action Permissions for the specified aicrm_profileid  
  * @param $profileid -- Profile Id:: Type integer
  * @returns Profile Tabs Action Permission Array in the following format:
  *    $tabActionPermission = Array($tabid1=>Array(actionid1=>permission, actionid2=>permission,...,actionidn=>permission),
  *                        $tabid2=>Array(actionid1=>permission, actionid2=>permission,...,actionidn=>permission),
...
/** Function to create aicrm_profile 
...
$log->debug("Entering createProfile(".$profilename.",".$parentProfileId.",".$description.") method ...");
  ...
  $sql1 = "insert into aicrm_profile values(?,?,?)";
  $params1 = array('', $profilename, $description);
  $adb->pquery($sql1, $params1);

  //Retreiving the aicrm_profileid
  $sql2 = "select max(profileid) as current_id from aicrm_profile";
  $result2 = $adb->pquery($sql2, array());
  $current_profile_id = $adb->query_result($result2,0,'current_id');

  //Inserting values into aicrm_profile2globalpermissions
  $sql3 = "select * from aicrm_profile2globalpermissions where profileid=?";
  ...
  for($i=0; $i<$p2tab_rows; $i++)
  {
    ...
    $sql4="insert into aicrm_profile2globalpermissions values(?,?,?)";
    $params4 = array($current_profile_id, $act_id, $permissions);
    $adb->pquery($sql4, $params4);  
  }

  //Inserting values into Profile2tab aicrm_table
  $sql3 = "select * from aicrm_profile2tab where profileid=?";
  $params3 = array($parentProfileId);
  $result3= $adb->pquery($sql3, $params3);
  $p2tab_rows = $adb->num_rows($result3);
  for($i=0; $i<$p2tab_rows; $i++)
  {
    ...
    $sql4="insert into aicrm_profile2tab values(?,?,?)";
    $params4 = array($current_profile_id, $tab_id, $permissions);
    $adb->pquery($sql4, $params4);  
  }

  //Inserting values into Profile2standard aicrm_table
  $sql6 = "select * from aicrm_profile2standardpermissions where profileid=?";
  ...
  for($i=0; $i<$p2per_rows; $i++)
  {
    ...
    $sql7="insert into aicrm_profile2standardpermissions values(?,?,?,?)";
    $params7 = array($current_profile_id, $tab_id, $action_id, $permissions);
    $adb->pquery($sql7, $params7);  
  }

  //Inserting values into Profile2Utility aicrm_table
  $sql8 = "select * from aicrm_profile2utility where profileid=?";
  ...
  for($i=0; $i<$p2util_rows; $i++)
  {
    ...
    $sql9="insert into aicrm_profile2utility values(?,?,?,?)";
    $params9 = array($current_profile_id, $tab_id, $action_id, $permissions);
    $adb->pquery($sql9, $params9);  
  }

  //Inserting values into Profile2field aicrm_table
  $sql10 = "select * from aicrm_profile2field where profileid=?";
  ...
  for($i=0; $i<$p2field_rows; $i++)
  {
    ...
    $sql11="insert into aicrm_profile2field values(?,?,?,?,?)";
...
/** Function to delete aicrm_profile 
  * @param $transfer_profileid -- Profile Id to which the existing aicrm_role2profile relationships are to be transferred :: Type varchar
...
$log->debug("Entering deleteProfile(".$prof_id.",".$transfer_profileid.") method ...");
  global $adb;
  //delete from aicrm_profile2global permissions
  $sql4 = "delete from aicrm_profile2globalpermissions where profileid=?";
  $adb->pquery($sql4, array($prof_id));

  //deleting from aicrm_profile 2 aicrm_tab;
  $sql4 = "delete from aicrm_profile2tab where profileid=?";
  $adb->pquery($sql4, array($prof_id));

  //deleting from aicrm_profile2standardpermissions aicrm_table
  $sql5 = "delete from aicrm_profile2standardpermissions where profileid=?";
  $adb->pquery($sql5, array($prof_id));

  //deleting from aicrm_profile2field
  $sql6 ="delete from aicrm_profile2field where profileid=?";
  $adb->pquery($sql6, array($prof_id));

  //deleting from aicrm_profile2utility
  $sql7 ="delete from aicrm_profile2utility where profileid=?";
  $adb->pquery($sql7, array($prof_id));

  //updating aicrm_role2profile
        if(isset($transfer_profileid) && $transfer_profileid != '')
        {
                $sql8 = "select roleid from aicrm_role2profile where profileid=?";
                ...
                for($i=0;$i<$num_rows;$i++)
                {
                        ...
                        $sql = "select profileid from aicrm_role2profile where roleid=?";
                        $profresult=$adb->pquery($sql, array($roleid));
                        $num=$adb->num_rows($profresult);
                        if($num>1)
                        {
                                $sql10="delete from aicrm_role2profile where roleid=? and profileid=?";
                                $adb->pquery($sql10, array($roleid, $prof_id));
                        }
                        else
                        {
                                $sql8 = "update aicrm_role2profile set profileid=? where profileid=? and roleid=?";
  ...
  //delete from aicrm_profile aicrm_table;
  $sql9 = "delete from aicrm_profile where profileid=?";
  $adb->pquery($sql9, array($prof_id));
  $log->debug("Exiting deleteProfile method ...");  

}

/** Function to get all  the aicrm_role information 
  * @returns $allRoleDetailArray-- Array will contain the details of all the aicrm_roles. RoleId will be the key:: Type array
 */
...
$log->debug("Entering getAllRoleDetails() method ...");
  ...
  for($i=0; $i<$num_rows;$i++)
  {
    ...
    $query1="select * from aicrm_role where parentrole like ? and depth=?";
...
/** Function to get all  the aicrm_profile information 
  * @returns $allProfileInfoArray-- Array will contain the details of all the aicrm_profiles. Profile ID will be the key:: Type array
 */
function getAllProfileInfo()
{
  ...
  global $adb;
  $query="select * from aicrm_profile";
  $result = $adb->pquery($query, array());
  $num_rows=$adb->num_rows($result);
  $prof_details=Array();
  for($i=0;$i<$num_rows;$i++)
...
/** Function to get the aicrm_role information of the specified aicrm_role
...
/** Function to get the aicrm_role related aicrm_profiles
  * @param $roleid -- RoleId :: Type varchar 
...
function getRoleRelatedProfiles($roleId)
{
  ...
  global $adb;
  $query = "select aicrm_role2profile.*,aicrm_profile.profilename from aicrm_role2profile inner join aicrm_profile on aicrm_profile.profileid=aicrm_role2profile.profileid where roleid=?";
  $result = $adb->pquery($query, array($roleId));
  $num_rows=$adb->num_rows($result);
...
/** Function to get the aicrm_role related aicrm_users
  * @param $roleid -- RoleId :: Type varchar 
...
function getRoleUsers($roleId)
{
  ...
  global $adb;
  $query = "select aicrm_user2role.*,aicrm_users.user_name from aicrm_user2role inner join aicrm_users on aicrm_users.id=aicrm_user2role.userid where roleid=?";
  $result = $adb->pquery($query, array($roleId));
  $num_rows=$adb->num_rows($result);
...
/** Function to get the aicrm_role related user ids
  * @param $roleid -- RoleId :: Type varchar 
...
/** Function to get the aicrm_role and subordinate aicrm_users
  * @param $roleid -- RoleId :: Type varchar 
...
function getRoleAndSubordinateUsers($roleId)
{
  ...
  $query = "select aicrm_user2role.*,aicrm_users.user_name from aicrm_user2role inner join aicrm_users on aicrm_users.id=aicrm_user2role.userid inner join aicrm_role on aicrm_role.roleid=aicrm_user2role.roleid where aicrm_role.parentrole like ?";
...
/** Function to get the aicrm_role and subordinate user ids
  * @param $roleid -- RoleId :: Type varchar 
...
/** Function to get the aicrm_role and subordinate Information for the specified aicrm_roleId
  * @param $roleid -- RoleId :: Type varchar 
...
function getRoleAndSubordinatesInformation($roleId)
{
  ...
  $query="select * from aicrm_role where parentrole like ? order by parentrole asc";
...
/** Function to get the aicrm_role and subordinate aicrm_role ids
  * @param $roleid -- RoleId :: Type varchar 
...
/** Function to get delete the spcified aicrm_role 
  * @param $roleid -- RoleId :: Type varchar 
  * @param $transferRoleId -- RoleId to which aicrm_users of the aicrm_role that is being deleted are transferred:: Type varchar 
 */
function deleteRole($roleId,$transferRoleId)
{
  ...
  $log->debug("Entering deleteRole(".$roleId.",".$transferRoleId.") method ...");
        ...
        {
                $sql1 = "update aicrm_user2role set roleid=? where roleid=?";
                $adb->pquery($sql1, array($transferRoleId, $roleid));
                //Deleteing from aicrm_role2profile aicrm_table
                $sql2 = "delete from aicrm_role2profile where roleid=?";
                $adb->pquery($sql2, array($roleid));
                //delete handling for aicrm_groups
                $sql10 = "delete from aicrm_group2role where roleid=?";
                $adb->pquery($sql10, array($roleid));

                $sql11 = "delete from aicrm_group2rs where roleandsubid=?";
                $adb->pquery($sql11, array($roleid));

                //delete handling for sharing rules
                deleteRoleRelatedSharingRules($roleid);

                //delete from aicrm_role aicrm_table;
                $sql9 = "delete from aicrm_role where roleid=?";
                $adb->pquery($sql9, array($roleid));
        }
  $log->debug("Exiting deleteRole method ...");
}

/** Function to delete the aicrm_role related sharing rules
  * @param $roleid -- RoleId :: Type varchar
 */
function deleteRoleRelatedSharingRules($roleId)
{
  ...
  $log->debug("Entering deleteRoleRelatedSharingRules(".$roleId.") method ...");
        global $adb;
        $dataShareTableColArr=Array('aicrm_datashare_grp2role'=>'to_roleid',
                                    'aicrm_datashare_grp2rs'=>'to_roleandsubid',
                                    'aicrm_datashare_role2group'=>'share_roleid',
                                    'aicrm_datashare_role2role'=>'share_roleid::to_roleid',
                                    'aicrm_datashare_role2rs'=>'share_roleid::to_roleandsubid',
                                    'aicrm_datashare_rs2grp'=>'share_roleandsubid',
                                    'aicrm_datashare_rs2role'=>'share_roleandsubid::to_roleid',
                                    'aicrm_datashare_rs2rs'=>'share_roleandsubid::to_roleandsubid');
...
function deleteGroupRelatedSharingRules($grpId)
{
  ...
  $log->debug("Entering deleteGroupRelatedSharingRules(".$grpId.") method ...");
        ...
        global $adb;
        $dataShareTableColArr=Array('aicrm_datashare_grp2grp'=>'share_groupid::to_groupid',
                                    'aicrm_datashare_grp2role'=>'share_groupid',
                                    'aicrm_datashare_grp2rs'=>'share_groupid',
                                    'aicrm_datashare_role2group'=>'to_groupid',
                                    'aicrm_datashare_rs2grp'=>'to_groupid');
...
/** Function to get userid and username of all aicrm_users 
...
function getAllUserName()
{
  ...
  global $adb;
  $query="select * from aicrm_users where deleted=0";
  $result = $adb->pquery($query, array());
  $num_rows=$adb->num_rows($result);
  $user_details=Array();
  for($i=0;$i<$num_rows;$i++)
...
/** Function to get groupid and groupname of all aicrm_groups 
...
function getAllGroupName()
{
  ...
  global $adb;
  $query="select * from aicrm_groups";
  $result = $adb->pquery($query, array());
  $num_rows=$adb->num_rows($result);
  $group_details=Array();
  for($i=0;$i<$num_rows;$i++)
...
function getGroupDetails($id)
{
  ...
  global $adb;
  $query="select * from aicrm_groups where groupid = ?";
...
/** Function to get group information of all aicrm_groups 
...
{ 
  ...
  global $adb;
  $groupId=$adb->getUniqueId("aicrm_users");
  //Insert into group aicrm_table
  $query = "insert into aicrm_groups values(?,?,?)";
...
function insertGroupToRoleRelation($groupId,$roleId)
{
  ...
  global $adb;
  $query="insert into aicrm_group2role values(?,?)";
  $adb->pquery($query, array($groupId, $roleId));
  $log->debug("Exiting insertGroupToRoleRelation method ...");
}


/** Function to insert group to aicrm_role&subordinate relation 
  * @param $groupId -- Group Id :: Type integer 
...
function insertGroupToRsRelation($groupId,$rsId)
{
  ...
  global $adb;
  $query="insert into aicrm_group2rs values(?,?)";
...
function insertGroupToUserRelation($groupId,$userId)
{
  ...
  global $adb;
  $query="insert into aicrm_users2group values(?,?)";
...
/** Function to get the group related aicrm_roles of the specified group 
  * @param $groupId -- Group Id :: Type integer 
...
function getGroupRelatedRoles($groupId)
{
  ...
  global $adb;
  $roleGroupArr=Array();
  $query="select * from aicrm_group2role where groupid=?";
  $result = $adb->pquery($query, array($groupId));
  $num_rows=$adb->num_rows($result);
  for($i=0;$i<$num_rows;$i++)
  {
...
/** Function to get the group related aicrm_roles and subordinates of the specified group 
  * @param $groupId -- Group Id :: Type integer 
...
function getGroupRelatedRoleSubordinates($groupId)
{
  ...
  global $adb;
  $rsGroupArr=Array();
  $query="select * from aicrm_group2rs where groupid=?";
  $result = $adb->pquery($query, array($groupId));
  $num_rows=$adb->num_rows($result);
  for($i=0;$i<$num_rows;$i++)
  {
...
/** Function to get the group related aicrm_groups  
  * @param $groupId -- Group Id :: Type integer 
...
function getGroupRelatedGroups($groupId)
{
  ...
  global $adb;
  $groupGroupArr=Array();
  $query="select * from aicrm_group2grouprel where groupid=?";
  $result = $adb->pquery($query, array($groupId));
  $num_rows=$adb->num_rows($result);
  for($i=0;$i<$num_rows;$i++)
  {
...
/** Function to get the group related aicrm_users  
  * @param $userId -- User Id :: Type integer 
...
function getGroupRelatedUsers($groupId)
{
  ...
  global $adb;
  $userGroupArr=Array();
  $query="select * from aicrm_users2group where groupid=?";
  $result = $adb->pquery($query, array($groupId));
  $num_rows=$adb->num_rows($result);
  for($i=0;$i<$num_rows;$i++)
  {
...
function updateGroup($groupId,$groupName,$groupMemberArray,$description)
{
  ...
  global $adb;
  $query="update aicrm_groups set groupname=?, description=? where groupid=?";
...
function deleteGroup($groupId,$transferId)
{
  ...
  $query="delete from aicrm_groups where groupid=?";
...
function tranferGroupOwnership($groupId,$transferId)
{
  ...
  $query = "update aicrm_crmentity set smownerid=? where smownerid=?";
...
function deleteGroupRelatedGroups($groupId)
{
  ...
  global $adb;
  $query="delete from aicrm_group2grouprel where groupid=?";
  $adb->pquery($query, array($groupId));
  $log->debug("Exiting deleteGroupRelatedGroups method ...");
}


/** Function to delete group to aicrm_role relation of the  specified group  
  * @param $groupId -- Group Id :: Type integer 
 */
function deleteGroupRelatedRoles($groupId)
{
  ...
  global $adb;
  $query="delete from aicrm_group2role where groupid=?";
  $adb->pquery($query, array($groupId));
  $log->debug("Exiting deleteGroupRelatedRoles method ...");
}


/** Function to delete group to aicrm_role and subordinates relation of the  specified group  
  * @param $groupId -- Group Id :: Type integer 
 */
function deleteGroupRelatedRolesAndSubordinates($groupId)
{
  ...
  global $adb;
  $query="delete from aicrm_group2rs where groupid=?";
  $adb->pquery($query, array($groupId));
...
function deleteGroupRelatedUsers($groupId)
{
  ...
  global $adb;
  $query="delete from aicrm_users2group where groupid=?";
  $adb->pquery($query, array($groupId));
...
function getDefOrgShareActionName($share_action_id)
{
  ...
  global $adb;
  $query="select * from aicrm_org_share_action_mapping where share_action_id=?";
...
function getModuleSharingActionArray($tabid)
{
  ...
  $query = "select aicrm_org_share_action_mapping.share_action_name,aicrm_org_share_action2tab.share_action_id from aicrm_org_share_action2tab inner join aicrm_org_share_action_mapping on aicrm_org_share_action2tab.share_action_id=aicrm_org_share_action_mapping.share_action_id where aicrm_org_share_action2tab.tabid=?";
...
/** This function adds a organisation level sharing rule for the specified Module
  ...
  *   $shareEntityType -- The Entity Type may be aicrm_groups,roles,rs and aicrm_users - Datatype::String
  *   $toEntityType -- The Entity Type may be aicrm_groups,roles,rs and aicrm_users - Datatype::String
...
function addSharingRule($tabid,$shareEntityType,$toEntityType,$shareEntityId,$toEntityId,$sharePermission)
{
  ...
  global $adb;
  $shareid=$adb->getUniqueId("aicrm_datashare_module_rel");
  

  if($shareEntityType == 'groups' && $toEntityType == 'groups')
  {
    $type_string='GRP::GRP';
    $query = "insert into aicrm_datashare_grp2grp values(?,?,?,?)";
  }
  elseif($shareEntityType == 'groups' && $toEntityType == 'roles')
  {
    
    $type_string='GRP::ROLE';
    $query = "insert into aicrm_datashare_grp2role values(?,?,?,?)";
  }
  elseif($shareEntityType == 'groups' && $toEntityType == 'rs')
  {
    
    $type_string='GRP::RS';
    $query = "insert into aicrm_datashare_grp2rs values(?,?,?,?)";
  }
  elseif($shareEntityType == 'roles' && $toEntityType == 'groups')
  {
    
    $type_string='ROLE::GRP';
    $query = "insert into aicrm_datashare_role2group values(?,?,?,?)";
  }
  elseif($shareEntityType == 'roles' && $toEntityType == 'roles')
  {
    
    $type_string='ROLE::ROLE';
    $query = "insert into aicrm_datashare_role2role values(?,?,?,?)";
  }
  elseif($shareEntityType == 'roles' && $toEntityType == 'rs')
  {
    
    $type_string='ROLE::RS';
    $query = "insert into aicrm_datashare_role2rs values(?,?,?,?)";
  }
  elseif($shareEntityType == 'rs' && $toEntityType == 'groups')
  {
    
    $type_string='RS::GRP';
    $query = "insert into aicrm_datashare_rs2grp values(?,?,?,?)";
  }
  elseif($shareEntityType == 'rs' && $toEntityType == 'roles')
  {
    
    $type_string='RS::ROLE';
    $query = "insert into aicrm_datashare_rs2role values(?,?,?,?)";
  }
  elseif($shareEntityType == 'rs' && $toEntityType == 'rs')
  {
    
    $type_string='RS::RS';
    $query = "insert into aicrm_datashare_rs2rs values(?,?,?,?)";
  }
  $query1 = "insert into aicrm_datashare_module_rel values(?,?,?)";
...
function updateSharingRule($shareid,$tabid,$shareEntityType,$toEntityType,$shareEntityId,$toEntityId,$sharePermission)
{
  ...
  global $adb;
  $query2="select * from aicrm_datashare_module_rel where shareid=?";
  ...
  $query1 = "update aicrm_datashare_module_rel set relationtype=? where shareid=?";
...
function deleteSharingRule($shareid)
{
  ...
  $query3="delete from $tabname where shareid=?";
  $adb->pquery($query3, array($shareid));
  $query4="delete from aicrm_datashare_module_rel where shareid=?";
  $adb->pquery($query4, array($shareid));

  //deleting the releated module sharing permission
  $query5="delete from aicrm_datashare_relatedmodule_permission where shareid=?";
...
function getDataShareTableandColumnArray()
{
  ...
  $dataShareTableColArr=Array('aicrm_datashare_grp2grp'=>'share_groupid::to_groupid',
            'aicrm_datashare_grp2role'=>'share_groupid::to_roleid',
            'aicrm_datashare_grp2rs'=>'share_groupid::to_roleandsubid',
            'aicrm_datashare_role2group'=>'share_roleid::to_groupid',
            'aicrm_datashare_role2role'=>'share_roleid::to_roleid',
            'aicrm_datashare_role2rs'=>'share_roleid::to_roleandsubid',
            'aicrm_datashare_rs2grp'=>'share_roleandsubid::to_groupid',
            'aicrm_datashare_rs2role'=>'share_roleandsubid::to_roleid',
            'aicrm_datashare_rs2rs'=>'share_roleandsubid::to_roleandsubid');
...
function getDataShareTableName()
{
  ...
  $dataShareTableColArr=Array('GRP::GRP'=>'aicrm_datashare_grp2grp',
            'GRP::ROLE'=>'aicrm_datashare_grp2role',
            'GRP::RS'=>'aicrm_datashare_grp2rs',
            'ROLE::GRP'=>'aicrm_datashare_role2group',
            'ROLE::ROLE'=>'aicrm_datashare_role2role',
            'ROLE::RS'=>'aicrm_datashare_role2rs',
            'RS::GRP'=>'aicrm_datashare_rs2grp',
            'RS::ROLE'=>'aicrm_datashare_rs2role',
            'RS::RS'=>'aicrm_datashare_rs2rs');
 ...
 *  @returns The entity type. The entity type may be aicrm_groups or aicrm_roles or rs -- Type Varchar
 */
 ...
 *  @params $entityType --  The entity type may be aicrm_groups or aicrm_roles or rs -- Type Varchar
...
function getSharingRuleInfo($shareId)
{
  ...
  $query="select * from aicrm_datashare_module_rel where shareid=?";
...
function getRelatedSharingModules($tabid)
{
  ...
  $query="select * from aicrm_datashare_relatedmodules where tabid=?";
  $result=$adb->pquery($query, array($tabid));
  $num_rows=$adb->num_rows($result);
  for($i=0;$i<$num_rows;$i++)
  {
...
function addRelatedModuleSharingPermission($shareid,$tabid,$relatedtabid,$sharePermission)
{
  ...
  $query="insert into aicrm_datashare_relatedmodule_permission values(?,?,?)" ;
...
function updateRelatedModuleSharingPermission($shareid,$tabid,$relatedtabid,$sharePermission)
{
  ...
  global $adb;
  $relatedModuleSharingId=getRelatedModuleSharingId($tabid,$relatedtabid);
  $query="update aicrm_datashare_relatedmodule_permission set permission=? where shareid=? and datashare_relatedmodule_id=?";   
...
function getRelatedModuleSharingId($tabid,$related_tabid)
{
  ...
  global $adb;
  $query="select datashare_relatedmodule_id from aicrm_datashare_relatedmodules where tabid=? and relatedto_tabid=?";
...
function getRelatedModuleSharingPermission($shareid)
{
  ...
  $query="select aicrm_datashare_relatedmodules.*,aicrm_datashare_relatedmodule_permission.permission from aicrm_datashare_relatedmodules inner join aicrm_datashare_relatedmodule_permission on aicrm_datashare_relatedmodule_permission.datashare_relatedmodule_id=aicrm_datashare_relatedmodules.datashare_relatedmodule_id where aicrm_datashare_relatedmodule_permission.shareid=?";
  $result=$adb->pquery($query, array($shareid));
  $num_rows=$adb->num_rows($result);
  for($i=0;$i<$num_rows;$i++)
  {
...
/** This function is to retreive the aicrm_profiles associated with the  the specified user 
  * It takes the following input parameters:
  *     $userid -- The User Id:: Type Integer
  *This function will return the aicrm_profiles associated to the specified aicrm_users in an Array in the following format:
...
function getUserProfile($userId)
{
  ...
  $profArr=Array(); 
  $sql1 = "select profileid from aicrm_role2profile where roleid=?";
...
/** To retreive the global permission of the specifed user from the various aicrm_profiles associated with the user  
  * @param $userid -- The User Id:: Type Integer
...
/** To retreive the aicrm_tab permissions of the specifed user from the various aicrm_profiles associated with the user  
  * @param $userid -- The User Id:: Type Integer
  * @returns  user global permission  array in the following format:
...
/** To retreive the aicrm_tab acion permissions of the specifed user from the various aicrm_profiles associated with the user  
  * @param $userid -- The User Id:: Type Integer
  * @returns  user global permission  array in the following format:
...
/** To retreive the parent aicrm_role of the specified aicrm_role 
  * @param $roleid -- The Role Id:: Type varchar
  * @returns  parent aicrm_role array in the following format:
...
/** To retreive the subordinate aicrm_roles of the specified parent aicrm_role  
  * @param $roleid -- The Role Id:: Type varchar
  * @returns  subordinate aicrm_role array in the following format:
...
/** To retreive the subordinate aicrm_roles and aicrm_users of the specified parent aicrm_role  
  * @param $roleid -- The Role Id:: Type varchar
  * @returns  subordinate aicrm_role array in the following format:
  *     $subordinateRoleUserArray=(roleid1=>Array(userid1,userid2,userid3),
                               aicrm_roleid2=>Array(userid1,userid2,userid3)
                        |
            |
             aicrm_roleidn=>Array(userid1,userid2,userid3));
 */
...
function getReadSharingUsersList($module)
{
  ...
  $user_array=Array();
  $tabid=getTabid($module);
  $query = "select shareduserid from aicrm_tmp_read_user_sharing_per where userid=? and tabid=?";
...
function getReadSharingGroupsList($module)
{
  ...
  $grp_array=Array();
  $tabid=getTabid($module);
  $query = "select sharedgroupid from aicrm_tmp_read_group_sharing_per where userid=? and tabid=?";
  $result=$adb->pquery($query, array($current_user->id, $tabid));
  $num_rows=$adb->num_rows($result);
  for($i=0;$i<$num_rows;$i++)
  {
...
function getWriteSharingGroupsList($module)
{
  ...
  $grp_array=Array();
  $tabid=getTabid($module);
  $query = "select sharedgroupid from aicrm_tmp_write_group_sharing_per where userid=? and tabid=?";
  $result=$adb->pquery($query, array($current_user->id, $tabid));
  $num_rows=$adb->num_rows($result);
  for($i=0;$i<$num_rows;$i++)
  {
...
function getListViewSecurityParameter($module)
{
  ...
  {
    $sec_query .= " and (
            aicrm_crmentity.smownerid in($current_user->id) 
            ...
            or aicrm_crmentity.smownerid in(select shareduserid from aicrm_tmp_read_user_sharing_per where userid=".$current_user->id." and tabid=".$tabid.") 
            or (";
  ...
  {
    $sec_query .= " and (aicrm_crmentity.smownerid in($current_user->id) " .
        "or aicrm_crmentity.smownerid in(select aicrm_user2role.userid from aicrm_user2role inner join aicrm_users on aicrm_users.id=aicrm_user2role.userid inner join aicrm_role on aicrm_role.roleid=aicrm_user2role.roleid where aicrm_role.parentrole like '".$current_user_parent_role_seq."::%') " .
        "or aicrm_crmentity.smownerid in(select shareduserid from aicrm_tmp_read_user_sharing_per where userid=".$current_user->id." and tabid=".$tabid.") or (";
  ...
  elseif($module == 'Potentials')
  {
    ...
    if(vtlib_isModuleActive("Accounts")){
      $sec_query .= "or aicrm_potential.related_to in " .
            "(select crmid from aicrm_crmentity where setype='accounts' " .
            "and aicrm_crmentity.smownerid in(select shareduserid from aicrm_tmp_read_user_rel_sharing_per where userid=".$current_user->id." and tabid=".getTabid('Accounts')." and relatedtabid=".$tabid.")) " .
            "or aicrm_potential.related_to in " .
            "(select crmid from aicrm_crmentity inner join aicrm_groups on aicrm_groups.groupid=aicrm_crmentity.smownerid where setype='Accounts' " .
            "and aicrm_groups.groupid in(select aicrm_tmp_read_group_rel_sharing_per.sharedgroupid from aicrm_tmp_read_group_rel_sharing_per where userid=".$current_user->id." and tabid=".getTabid('Accounts')." and relatedtabid=".$tabid.")) ";
    }
    $sec_query .= " or (";
  ...
  {
    $sec_query .= " and aicrm_crmentity.smownerid=".$current_user->id." ";  
  }
  ...
  {     
      $sec_query .= " and (
      aicrm_crmentity.crmid in (
        select distinct(tbt_quotes_log.crmid) as  crmid
        from tbt_quotes_log
        where tbt_quotes_log.adduser  in ($current_user->id)
        union 
        select distinct(tbt_quotes_approve.crmid) as  crmid
        from tbt_quotes_approve
        where tbt_quotes_approve.userid in ($current_user->id)
      )
      or aicrm_crmentity.crmid in (
        select aicrm_crmentity.crmid
        from aicrm_crmentity
        inner join aicrm_users on aicrm_crmentity.smcreatorid = aicrm_users.id        
        where aicrm_crmentity.deleted = 0 
        and aicrm_crmentity.setype = 'Quotes'
        and aicrm_crmentity.smownerid in($current_user->id) 
        )
      or ";
      $sec_query .= " (aicrm_crmentity.smownerid in($current_user->id) " .
        "or aicrm_crmentity.smownerid in(select aicrm_user2role.userid from aicrm_user2role inner join aicrm_users on aicrm_users.id=aicrm_user2role.userid inner join aicrm_role on aicrm_role.roleid=aicrm_user2role.roleid where aicrm_role.parentrole like '".$current_user_parent_role_seq."::%') " .
        "or aicrm_crmentity.smownerid in(select shareduserid from aicrm_tmp_read_user_sharing_per where userid=".$current_user->id." and tabid=".$tabid.") or (";
  ...
  elseif($module == 'Competitorproduct')
  {
    $sec_query .= " and (aicrm_crmentity.smownerid in($current_user->id) " .
        "or aicrm_crmentity.smownerid in(select aicrm_user2role.userid from aicrm_user2role inner join aicrm_users on aicrm_users.id=aicrm_user2role.userid inner join aicrm_role on aicrm_role.roleid=aicrm_user2role.roleid where aicrm_role.parentrole like '".$current_user_parent_role_seq."::%') " .
        "or aicrm_crmentity.smownerid in(select shareduserid from aicrm_tmp_read_user_sharing_per where userid=".$current_user->id." and tabid=".$tabid.") or (";
  ...
  elseif($module == 'Inspectiontemplate')
  {
    $sec_query .= " and (aicrm_crmentity.smownerid in($current_user->id) " .
        "or aicrm_crmentity.smownerid in(select aicrm_user2role.userid from aicrm_user2role inner join aicrm_users on aicrm_users.id=aicrm_user2role.userid inner join aicrm_role on aicrm_role.roleid=aicrm_user2role.roleid where aicrm_role.parentrole like '".$current_user_parent_role_seq."::%') " .
        "or aicrm_crmentity.smownerid in(select shareduserid from aicrm_tmp_read_user_sharing_per where userid=".$current_user->id." and tabid=".$tabid.") or (";
  ...
  elseif($module == 'Projects')
  {
    ...
    $user_login = $current_user->first_name.' '.$current_user->last_name;
    $sec_query .= " OR (aicrm_projects.related_sales_person LIKE '%".$user_login."%' )";
    $sec_query .= " OR ( aicrm_crmentity.crmid IN (
      SELECT aicrm_crmentity.crmid
      FROM aicrm_crmentity
      INNER JOIN aicrm_projects ON aicrm_projects.projectsid = aicrm_crmentity.crmid
      INNER JOIN (
        SELECT CONCAT(aicrm_users.first_name, ' ', aicrm_users.last_name) AS 'fullname'
        FROM aicrm_users
        INNER JOIN aicrm_user2role ON aicrm_user2role.userid = aicrm_users.id
        WHERE aicrm_user2role.roleid IN (
          SELECT aicrm_role.roleid 
          FROM aicrm_role 
          WHERE aicrm_role.parentrole LIKE CONCAT('%', 
            (SELECT aicrm_role.roleid FROM aicrm_role
            INNER JOIN aicrm_user2role ON aicrm_user2role.roleid = aicrm_role.roleid
            INNER JOIN aicrm_users ON aicrm_users.id = aicrm_user2role.userid
            WHERE CONCAT(aicrm_users.first_name, ' ', aicrm_users.last_name) = '".$user_login."'), '%'
          ) 
          AND aicrm_role.roleid NOT IN (SELECT aicrm_role.roleid FROM aicrm_role
            INNER JOIN aicrm_user2role ON aicrm_user2role.roleid = aicrm_role.roleid
            INNER JOIN aicrm_users ON aicrm_users.id = aicrm_user2role.userid
            WHERE CONCAT(aicrm_users.first_name, ' ', aicrm_users.last_name) = '".$user_login."')
        )
      ) AS users ON aicrm_projects.related_sales_person LIKE CONCAT('%', users.fullname, '%')
      WHERE aicrm_crmentity.deleted = 0 
    ))";
  ...
  elseif($module == 'Promotionvoucher')
  {
    $sec_query .= " and (aicrm_crmentity.smownerid in($current_user->id) or aicrm_crmentity.smownerid in(select aicrm_user2role.userid from aicrm_user2role inner join aicrm_users on aicrm_users.id=aicrm_user2role.userid inner join aicrm_role on aicrm_role.roleid=aicrm_user2role.roleid where aicrm_role.parentrole like '".$current_user_parent_role_seq."::%') or aicrm_crmentity.smownerid in(select shareduserid from aicrm_tmp_read_user_sharing_per where userid=".$current_user->id." and tabid=".$tabid.") or ((";
    if(sizeof($current_user_groups) > 0)
    {
  ...
  elseif($module == 'Questionnairetemplate')
  {
    $sec_query .= " and (aicrm_crmentity.smownerid in($current_user->id) or aicrm_crmentity.smownerid in(select aicrm_user2role.userid from aicrm_user2role inner join aicrm_users on aicrm_users.id=aicrm_user2role.userid inner join aicrm_role on aicrm_role.roleid=aicrm_user2role.roleid where aicrm_role.parentrole like '".$current_user_parent_role_seq."::%') or aicrm_crmentity.smownerid in(select shareduserid from aicrm_tmp_read_user_sharing_per where userid=".$current_user->id." and tabid=".$tabid.") or ((";
    if(sizeof($current_user_groups) > 0)
    {
...
function getSecListViewSecurityParameter($module)
{
  ...
  elseif($module == 'Potentials')
  {
    $sec_query .= " and (aicrm_crmentity$module.smownerid in($current_user->id) or aicrm_crmentity$module.smownerid in(select aicrm_user2role.userid from aicrm_user2role inner join aicrm_users on aicrm_users.id=aicrm_user2role.userid inner join aicrm_role on aicrm_role.roleid=aicrm_user2role.roleid where aicrm_role.parentrole like '".$current_user_parent_role_seq."::%') or aicrm_crmentity$module.smownerid in(select shareduserid from aicrm_tmp_read_user_sharing_per where userid=".$current_user->id." and tabid=".$tabid.") or aicrm_potential.related_to in (select crmid from aicrm_crmentity where setype in ('Accounts', 'Contacts') and aicrm_crmentity.smownerid in(select shareduserid from aicrm_tmp_read_user_rel_sharing_per where userid=".$current_user->id." and tabid in (".getTabid('Accounts').", ".getTabid('Contacts').") and relatedtabid=".$tabid.")) ";
        
    if(vtlib_isModuleActive("Accounts")){
        "or aicrm_potential.related_to in (select crmid from aicrm_crmentity inner join aicrm_groups on aicrm_groups.groupid=aicrm_crmentity.smownerid where setype='Accounts' and aicrm_groups.groupid in (select aicrm_tmp_read_group_rel_sharing_per.sharedgroupid from aicrm_tmp_read_group_rel_sharing_per where userid=".$current_user->id." and tabid=".getTabid('Accounts')." and relatedtabid=".$tabid.")) ";
    }
    if(vtlib_isModuleActive("Contacts")){
        "or aicrm_potential.related_to in (select crmid from aicrm_crmentity inner join aicrm_groups on aicrm_groups.groupid=aicrm_crmentity.smownerid where setype='Contacts' and aicrm_groups.groupid in (select aicrm_tmp_read_group_rel_sharing_per.sharedgroupid from aicrm_tmp_read_group_rel_sharing_per where userid=".$current_user->id." and tabid=".getTabid('Contacts')." and relatedtabid=".$tabid.")) ";
    }
    $sec_query .= " or (";
        if(sizeof($current_user_groups) > 0){
          $sec_query .= " aicrm_groups$module.groupid in (". implode(",", $current_user_groups) .") or ";
  ...
  elseif($module == 'HelpDesk')
  {
    $sec_query .= " and (aicrm_crmentity$module.smownerid in($current_user->id) or aicrm_crmentity$module.smownerid in(select aicrm_user2role.userid from aicrm_user2role inner join aicrm_users on aicrm_users.id=aicrm_user2role.userid inner join aicrm_role on aicrm_role.roleid=aicrm_user2role.roleid where aicrm_role.parentrole like '".$current_user_parent_role_seq."::%') or aicrm_crmentity$module.smownerid in(select shareduserid from aicrm_tmp_read_user_sharing_per where userid=".$current_user->id." and tabid=".$tabid.") ";
    
    if(vtlib_isModuleActive("Accounts")){
        "or aicrm_troubletickets.parent_id in (select crmid from aicrm_crmentity where setype='Accounts' and aicrm_crmentity.smownerid in(select shareduserid from aicrm_tmp_read_user_rel_sharing_per where userid=".$current_user->id." and tabid=".getTabid('Accounts')." and relatedtabid=".$tabid.")) or aicrm_troubletickets.parent_id in (select crmid from aicrm_crmentity inner join aicrm_groups on aicrm_groups.groupid=aicrm_crmentity.smownerid where setype='Accounts' and aicrm_groups.groupid in(select aicrm_tmp_read_group_rel_sharing_per.sharedgroupid from aicrm_tmp_read_group_rel_sharing_per where userid=".$current_user->id." and tabid=".getTabid('Accounts')." and relatedtabid=".$tabid.")) ";
    }
  ...
  elseif($module == 'Calendar')
  {
    ...
    if(isset($shared_ids) && $shared_ids != '')
      $condition = " or (aicrm_crmentity$module.smownerid in($shared_ids) and aicrm_activity.visibility = 'Public')";
    else
      $condition = null;
    $sec_query .= " and (aicrm_crmentity$module.smownerid in($current_user->id) $condition or aicrm_crmentity$module.smownerid in(select aicrm_user2role.userid from aicrm_user2role inner join aicrm_users on aicrm_users.id=aicrm_user2role.userid inner join aicrm_role on aicrm_role.roleid=aicrm_user2role.roleid where aicrm_role.parentrole like '".$current_user_parent_role_seq."::%')";

    if(sizeof($current_user_groups) > 0)
    {
      $sec_query .= " or ((aicrm_groups$module.groupid in (". implode(",", $current_user_groups) .")))";
    }
    $sec_query .= ")";  
  }
  elseif($module == 'Quotes')
  {
    ...
    //Adding crterial for aicrm_account related aicrm_quotes sharing
    if(vtlib_isModuleActive("Accounts")){
     $sec_query .= " or aicrm_quotes.accountid in (select crmid from aicrm_crmentity where setype='Accounts' and aicrm_crmentity.smownerid in(select shareduserid from aicrm_tmp_read_user_rel_sharing_per where userid=".$current_user->id." and tabid=".getTabid('Accounts')." and relatedtabid=".$tabid.")) or aicrm_quotes.accountid in (select crmid from aicrm_crmentity inner join aicrm_groups on aicrm_groups.groupid=aicrm_crmentity.smownerid where setype='Accounts' and aicrm_groups.groupid in(select aicrm_tmp_read_group_rel_sharing_per.sharedgroupid from aicrm_tmp_read_group_rel_sharing_per where userid=".$current_user->id." and tabid=".getTabid('Accounts')." and relatedtabid=".$tabid."))";
    }
    //Adding crterial for aicrm_potential related aicrm_quotes sharing
    if(vtlib_isModuleActive("Potentials")){
     $sec_query .= " or aicrm_quotes.potentialid in (select crmid from aicrm_crmentity where setype='Potentials' and aicrm_crmentity.smownerid in(select shareduserid from aicrm_tmp_read_user_rel_sharing_per where userid=".$current_user->id." and tabid=".getTabid('Potentials')." and relatedtabid=".$tabid.")) or aicrm_quotes.potentialid in (select crmid from aicrm_crmentity inner join aicrm_groups on aicrm_groups.groupid=aicrm_crmentity.smownerid where setype='Potentials' and aicrm_groups.groupid in(select aicrm_tmp_read_group_rel_sharing_per.sharedgroupid from aicrm_tmp_read_group_rel_sharing_per where userid=".$current_user->id." and tabid=".getTabid('Potentials')." and relatedtabid=".$tabid."))";
    }
...
function get_current_user_access_groups($module)
{
  ...
  $query ="select groupname,groupid from aicrm_groups";
  $params = array();
...
function getGrpId($groupname)
{
  ...
  $result = $adb->pquery("select groupid from aicrm_groups where groupname=?", array($groupname));
  $groupid = $adb->query_result($result,0,'groupid');
  $log->debug("Exiting getGrpId method ...");
  return $groupid;
}

/** Function to check permission to access a aicrm_field for a given user
...
function getFieldVisibilityPermission($fld_module, $userid, $fieldname)
{
  ...
  if($fld_module == 'Calendar' && $fieldname == 'activity_no') {
    $fld_module = 'Events';
  }
  ...
  {
    //get aicrm_profile list using userid
...
function getColumnVisibilityPermission($userid,$columnname, $module)
{
  ...
  if($cacheFieldInfo === false) {
    $res = $adb->pquery("select fieldname from aicrm_field where tabid=? and columnname=? and aicrm_field.presence in (0,2)", array($tabid, $columnname));
...
/** Function to get the aicrm_field access module array 
  * @returns The aicrm_field Access module Array :: Type Array
  *
 */
function getFieldModuleAccessArray()
{
  ...
  $fldModArr=Array();
  $query = 'select distinct(name) from aicrm_profile2field inner join aicrm_tab on aicrm_tab.tabid=aicrm_profile2field.tabid';
  $result = $adb->pquery($query, array());
  $num_rows=$adb->num_rows($result);
  for($i=0;$i<$num_rows;$i++)
  {
...
{ 
  ...
  //echo "<pre>"; print_r($getTabModuleName); echo "</pre>";
  ...
  {
    //echo "<pre>"; print_r($tab_seq_array); echo "</pre>";
    foreach($tab_seq_array as $tabid=>$seq_value)
    {
      if($seq_value == 0)
      {
        //echo getTabModuleName($tabid); echo "<br>";
        $permittedModules[]=getTabModuleName($tabid);
      }
      //echo "<pre>"; print_r($permittedModules); echo "</pre>";

    } 
  }
  //echo "<pre>"; print_r($permittedModules); echo "</pre>"; exit;
...
/** Function to recalculate the Sharing Rules for all the aicrm_users 
  * This function will recalculate all the sharing rules for all the aicrm_users in the Organization and will write them in flat aicrm_files 
  *
 */
function RecalculateSharingRules()
{
  ...
  $query="select id from aicrm_users where deleted=0";
  //$query="select id from aicrm_users where deleted=0 and id = 9903 ";
...
function getSharingModuleList($eliminateModules=false)
{
  ...
  $query = "SELECT name FROM aicrm_tab WHERE presence=0 AND ownedby = 0 AND isentitytype = 1";  
...
function isCalendarPermittedBySharing($recordId)
{
  ...
  global $current_user;
  $permission = 'no';
  $query = "select * from aicrm_sharedcalendar where userid in(select smownerid from aicrm_activity inner join aicrm_crmentity on aicrm_crmentity.crmid=aicrm_activity.activityid where activityid=? and visibility='Public' and smownerid !=0) and sharedid=?";
...
function insertRole2Picklist($roleid,$parentroleid)
{
  ...
  $sql = "insert into aicrm_role2picklist select '".$roleid."',picklistvalueid,picklistid,sortid from aicrm_role2picklist where roleid=?";
...
function deleteGroupReportRelations($groupId)
{
  ...
  global $adb;
  $query="delete from aicrm_reportsharing where shareid=? and setype='groups'";
  $adb->pquery($query, array($groupId));
...
function get_username($id){
    global $log;
    $log->debug("Entering get_username(".$id.") method ...");
    global $adb;
    $query="select user_name from aicrm_users where id in (".$id.") ";
    $result = $adb->pquery($query, array());
