...
class PopulateComboValues
{
	...
	/** 
	 * To populate the default combo values for the combo aicrm_tables
	...
	function insertComboValues($values, $tableName,$picklistid)
	{
		...
		global $adb;
		//inserting the value in the aicrm_picklistvalues_seq for the getting uniqueID for each picklist values...
		$i=0;
		foreach ($values as $val => $cal)
		{
			$picklist_valueid = getUniquePicklistID();
			$id = $adb->getUniqueID('aicrm_'.$tableName);
			if($val != '')
			{
				$params = array($id, $val, 1, $picklist_valueid);
				$adb->pquery("insert into aicrm_$tableName values(?,?,?,?)", $params);
			}
			else
			{
	...
	/** 
	 * To populate the combo aicrm_tables at startup time
	 */
	...
	function create_tables () 
	{
		...
		$comboRes = $adb->query("SELECT distinct fieldname FROM aicrm_field WHERE uitype IN ('15') OR fieldname = 'salutationtype' and aicrm_field.presence in (0,2)");
		...
		for($i=0; $i<$noOfCombos; $i++)
		{
			...
			$picklist_qry = "insert into aicrm_picklist values(?,?)";
		...
		{
			$adb->pquery("update aicrm_".$picklistname." set PRESENCE=0", array());
		}
		foreach($noneditable_values as $picklistname => $value)
		{
			$adb->pquery("update aicrm_$value set PRESENCE=0 where $value=?", array($picklistname));
		}
	...
	function create_nonpicklist_tables ()
	{
		...
		$comboRes = $adb->query("SELECT distinct fieldname FROM aicrm_field WHERE uitype IN ('16','115','83') AND fieldname NOT IN ('hdnTaxType','email_flag') and aicrm_field.presence in (0,2)");
		$noOfCombos = $adb->num_rows($comboRes);
		for($i=0; $i<$noOfCombos; $i++)
		{
			$comTab = $adb->query_result($comboRes, $i, 'fieldname');
