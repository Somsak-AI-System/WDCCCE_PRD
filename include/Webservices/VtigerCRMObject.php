...
class VtigerCRMObject{
	...
	private function getObjectTypeId($objectName){
		...
		$sql = "select * from aicrm_tab where name=?;";
	...
	public function read($id){
		...
		$this->instance->retrieve_entity_info($id,'Events');
	...
	public function create($element){
		...
		//print_r($this->getTabName()); 	
		$this->instance->Save("Events");
		//$this->instance->Save($this->getTabName());
		$error = $adb->hasFailedTransaction();
		//echo "<pre>"; print_r($adb->hasFailedTransaction()); echo "</pre>"; exit;
		$adb->completeTransaction();
		return !$error;
	}
