 ...
 *************************************************************************************/
	...
	function vtws_sync($mtime,$elementType,$user){
		...
		$q= "select crmid,setype from aicrm_crmentity where modifiedtime >? and smownerid=? and deleted=0";
		...
		$q= "select crmid,setype,modifiedtime from aicrm_crmentity where modifiedtime >? and smownerid=? and deleted=1";
		$params = array($datetime,$user->id);
		if($typed){
			$q = $q." and setype=?";
			array_push($params,$elementType);
		...
		$q= "select max(modifiedtime) as modifiedtime from aicrm_crmentity where modifiedtime >? and smownerid=?";
		$params = array($datetime,$user->id);
		if($typed){
			$q = $q." and setype=?";
			array_push($params,$elementType);
