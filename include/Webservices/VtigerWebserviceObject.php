...
class VtigerWebserviceObject{
	...
	static function fromName($adb,$entityName){
		...
		if(!isset(self::$_fromNameCache[$entityName])) {
			$result = $adb->pquery("select * from aicrm_ws_entity where name=?",array($entityName));
			//print_r($entityName); exit;
			//echo "select * from aicrm_ws_entity where name=$entityName";
			if($result){
	...
	static function fromId($adb,$entityId){		
		...
		if(!isset(self::$_fromIdCache[$entityId])) {
			$result = $adb->pquery("select * from aicrm_ws_entity where id=?",array($entityId));
			if($result){
				$rowCount = $adb->num_rows($result);
				if($rowCount === 1){
					$rowData = $adb->query_result_rowdata($result,0);
