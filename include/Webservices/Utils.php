...
function vtws_getVtigerVersion(){
	global $adb;
	$query = 'select * from aicrm_version';
...
function getEmailFieldId($meta, $entityId,$fields){
	...
	$query="SELECT fieldid,fieldlabel,columnname FROM aicrm_field WHERE tabid=? 
...
function vtws_getModuleNameList(){
	...
	$sql = "select name from aicrm_tab where presence=0 and isentitytype=1 order by tabsequence";
...
function vtws_getWebserviceEntities(){
	...
	$sql = "select name,id,ismodule from aicrm_ws_entity";
	$res = $adb->pquery($sql, array());
	$moduleArray = Array();
	$entityArray = Array();
	while($row = $adb->fetchByAssoc($res)){
...
function vtws_isRecordOwnerUser($ownerId){
	global $adb;
	$result = $adb->pquery("select first_name from aicrm_users where id = ?",array($ownerId));
	$rowCount = $adb->num_rows($result);
...
function vtws_isRecordOwnerGroup($ownerId){
	global $adb;
	$result = $adb->pquery("select groupname from aicrm_groups where groupid = ?",array($ownerId));
	$rowCount = $adb->num_rows($result);
...
function vtws_addModuleTypeWebserviceEntity($moduleName,$filePath,$className){
	global $adb;	
	$checkres = $adb->pquery('SELECT id FROM aicrm_ws_entity WHERE name=? AND handler_path=? AND handler_class=?',
...
function vtws_addActorTypeName($entityId,$fieldNames,$indexColumn,$tableName){
	global $adb;
	$adb->pquery('insert into aicrm_ws_entity_name(entity_id,name_fields,index_field,table_name) values (?,?,?,?)',
