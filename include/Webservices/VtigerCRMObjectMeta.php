...
class VtigerCRMObjectMeta extends EntityMeta {
	...
	private function computeAccess(){
		...
		if($is_admin == true || $profileGlobalPermission[1] == 0 || $profileGlobalPermission[2] == 0){
			...
			$sql = "select * from aicrm_profile2globalpermissions where profileid in (".generateQuestionMarks($profileList).");";
			...
			$sql = 'select * from aicrm_profile2tab where profileid in ('.generateQuestionMarks($profileList).') and tabid = ?;';
			...
			$sql = "select * from aicrm_profile2standardpermissions where profileid in (".generateQuestionMarks($profileList).") and tabid=?";
			$result = $adb->pquery($sql,array($profileList,$this->getTabId()));
	...
	private function retrieveMetaForBlock($block){
		...
		require('user_privileges/user_privileges_'.$this->user->id.'.php');
		if($is_admin == true || $profileGlobalPermission[1] == 0 || $profileGlobalPermission[2] ==0){
				 	$sql = "select * from aicrm_field where tabid =? and block in (".generateQuestionMarks($block).") and displaytype in (1,2,3,4) and aicrm_field.presence in (0,2) group by columnname"; 
			...
			if (count($profileList) > 0) {
				$sql = "SELECT *
						...
						WHERE aicrm_field.tabid =? AND aicrm_profile2field.visible = 0 
						AND aicrm_profile2field.profileid IN (". generateQuestionMarks($profileList) .")
						AND aicrm_def_org_field.visible = 0 and aicrm_field.block in (".generateQuestionMarks($block).") and aicrm_field.displaytype in (1,2,3,4) and aicrm_field.presence in (0,2) group by columnname";
	...
	function getObjectEntityName($webserviceId){
		...
		if($this->objectName == 'Users'){
			$sql = "select user_name from aicrm_users where id=? and deleted=0";
			...
			$sql = "select * from aicrm_crmentity where crmid=? and deleted=0";
			$result = $adb->pquery($sql , array($id));
			if($result != null && isset($result)){
				if($adb->num_rows($result)>0){
	...
	function exists($recordId){
		...
		$sql = '';
		if($this->objectName == 'Users'){
			$sql = 'select * from aicrm_users where id=? and deleted=0';
		}else{
			$sql = "select * from aicrm_crmentity where crmid=? and deleted=0 and setype='".$this->getTabName()."'";
		}
	...
	public function getNameFields(){
		...
		$query = "select fieldname,tablename,entityidfield from aicrm_entityname where tabid = ?";
