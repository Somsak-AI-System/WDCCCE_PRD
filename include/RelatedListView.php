...
function GetRelatedList($module,$relatedmodule,$focus,$query,$button,$returnset,$id='',$edit_val='',$del_val='')
{
	...
	//echo $relatedmodule."<br>";
	// We do not have RelatedListView in Detail View mode of Calendar module. So need to skip it.
	if($relatedmodule!='TrackerUrl' && $relatedmodule!='EmailMarketing' && $relatedmodule!='SMSMarketing' && $relatedmodule!='Media'){
		//echo $relatedmodule;
		if ($module!= 'Calendar' ) {//echo $relatedmodule;
 			$focus->initSortByField($relatedmodule);
 		}
	//Retreive the list from Database
	//Appending the security parameter

		if($relatedmodule != 'Products' && $relatedmodule != 'PriceBook'  || ($relatedmodule != 'Calendar' &&  $module == 'Projects')) //Security fix by Don
		{
		...
		if($relatedmodule == 'Leads') {
			$query .= " AND aicrm_leaddetails.converted in ('1','0')";
		}
		...
		if($order_by == 'smownerid') {
			$query_order_by = "case when (aicrm_users.user_name not like '') then aicrm_users.user_name else aicrm_groups.groupname end ";
		...
		{
			//$count_query = "select count(*) as count, aicrm_activity.activitytype ".substr($query, stripos($query,'from'),strlen($query));
		...
		else
		{
			...
			elseif($relatedmodule == 'SmartSms' || $relatedmodule == 'Smartemail')
			{
				$listview_entries = getListViewEntries($focus, $relatedmodule,$list_result,$navigation_array,'relatedlist',$returnset,'EditView','Delete','','','','',true);
			}
			else
			{
	...
	}else if($relatedmodule=='TrackerUrl'){
		$count_query = mkCountQuery($query);
		$count_result = $adb->query($count_query);
		...
		if($noofrows == 0){
			...
			$id = vtlib_purify($_REQUEST['record']);
			$listview_header = array('Tracker Key','Tracker Name','URL','Action');
			if ($noofrows > 15)
			{
				$smarty->assign('SCROLLSTART','<div style="overflow:auto;height:315px;width:100%;">');
				$smarty->assign('SCROLLSTOP','</div>');
			...
			if($relatedmodule == 'TrackerUrl'){
				 $cf_res = $adb->pquery($query,"");
				 $data =Array();
				 $list_block = Array();
				 $rows = $adb->num_rows($cf_res);
				 
				 for($i=0;$i<$rows;$i++){
					$links_info='';
					$key="<a href='#' onclick=\"window.open('tracker_url.php?trackerid=".$adb->query_result($cf_res, $i, "id")."&campaignid=".$adb->query_result($cf_res, $i, "campaignid")."&userid=".$_SESSION["user_id"]."', 'list','resizable=no,left=300,top=150,width=400,height=200,toolbar=no,menubar=no,resizable=no,scrollbars=yes,status=no,location=no');\"> ".$adb->query_result($cf_res, $i, "tracker_key")."</a> ";
					$links_info .= "<a href='#' onclick=\"window.open('tracker_url.php?trackerid=".$adb->query_result($cf_res, $i, "id")."&campaignid=".$adb->query_result($cf_res, $i, "campaignid")."&userid=".$_SESSION["user_id"]."', 'list','resizable=no,left=300,top=150,width=400,height=200,toolbar=no,menubar=no,resizable=no,scrollbars=yes,status=no,location=no');\"> ".$app_strings["LNK_EDIT"]."</a> ";
					$links_info .=  " | ";
					$links_info .=	"<a href='javascript:confirmdelete(\"tracker_url_del.php?trackerid=".$adb->query_result($cf_res, $i, "id")."&campaignid=".$adb->query_result($cf_res, $i, "campaignid")."&userid=".$_SESSION["user_id"]."\")'>".$app_strings["LNK_DELETE"]."</a>";
										
					$data[0]=$key;
					$data[1]=$adb->query_result($cf_res, $i, "tracker_name");
					$data[2]=$adb->query_result($cf_res, $i, "tracker_url");
					$data[3]=$links_info;
  					$list_block[$adb->query_result($cf_res, $i, "id")] = $data;
					$data='';
				 }
				 $listview_entries= $list_block;				
			}		
	...
	if($result)
	{
		while($row = $adb->fetch_array($result))
		{
			if($row['activitytype'] == 'Attachments') {
				$query1="select setype,createdtime from aicrm_crmentity where crmid=?";
			...
			if(($setype != "Products Image") && ($setype != "Contacts Image")) 
			{
				...
				if((getFieldVisibilityPermission('Documents', $current_user->id, 'filename') == 0))
				{
					...
					$modulepermissionQuery = "select permissions from aicrm_profile2tab where tabid=8 and profileid= ?";
					...
					$folderQuery = 'select folderid,filelocationtype,filestatus,filename from aicrm_notes where notesid = ?';
					...
					$fileQuery = $adb->pquery("select attachmentsid from aicrm_seattachmentsrel where crmid = ?",array($row['crmid']));
				...
				$assignedToQuery = $adb->pquery('SELECT smownerid FROM aicrm_crmentity WHERE crmid = ?',array($row['crmid']));
...
function getHistory($parentmodule,$query,$id)
{
	...
	$query.= ' '."ORDER BY aicrm_activity.date_start DESC,aicrm_activity.time_start DESC";
	$result=$adb->query($query);
	$noofrows = $adb->num_rows($result);
...
function getPriceBookRelatedProducts($query,$focus,$returnset='')
{
	...
	for($i=0; $i<$numRows; $i++) {
		...
		if(isPermitted("PriceBooks","EditView","") == 'yes')
			$action .= '<img style="cursor:pointer;" src="'. aicrm_imageurl('editfield.gif', $theme).'" border="0" onClick="fnvshobj(this,\'editlistprice\'),editProductListPrice(\''.$entity_id.'\',\''.$pricebook_id.'\',\''.$listprice.'\')" alt="'.$app_strings["LBL_EDIT_BUTTON"].'" title="'.$app_strings["LBL_EDIT_BUTTON"].'"/>';
		if(isPermitted("PriceBooks","Delete","") == 'yes')
		{		
			if($action != "")
				$action .= '&nbsp;|&nbsp;';
			$action .= '<img src="'. aicrm_imageurl('delete.gif', $theme).'" onclick="if(confirm(\''.$app_strings['ARE_YOU_SURE'].'\')) deletePriceBookProductRel('.$entity_id.','.$pricebook_id.');" alt="'.$app_strings["LBL_DELETE"].'" title="'.$app_strings["LBL_DELETE"].'" style="cursor:pointer;" border="0">';	
		}
		if($action != "")		
			$entries[] = $action;
		$entries_list[] = $entries;
...
function CheckFieldPermission($fieldname,$module)
{
	...
	if($is_admin==false && $profileGlobalPermission[1] == 1 && $profileGlobalPermission[2] == 1 )
	{
		...
		$sql1= "SELECT fieldname FROM aicrm_field INNER JOIN aicrm_profile2field ON aicrm_profile2field.fieldid=aicrm_field.fieldid INNER JOIN aicrm_def_org_field ON aicrm_def_org_field.fieldid=aicrm_field.fieldid WHERE aicrm_field.tabid in(".generateQuestionMarks($tab_id).") AND fieldname=? AND aicrm_field.displaytype IN (1,2,3,4) AND aicrm_profile2field.visible=0 AND aicrm_def_org_field.visible=0 AND aicrm_profile2field.profileid IN (". generateQuestionMarks($profileList) .") and aicrm_field.presence in (0,2) GROUP BY aicrm_field.fieldid ORDER BY block,sequence";
...
function CheckColumnPermission($tablename, $columnname, $module)
{
	...
	$res = $adb->pquery("select fieldname from aicrm_field where tablename=? and columnname=? and aicrm_field.presence in (0,2)", array($tablename, $columnname));
