...
function getComboArray($combofieldNames)
{
	...
	foreach ($combofieldNames as $tableName => $arrayName)
	{
		...
		if(!is_admin($current_user))
		{
			...
			$sql = "select distinct $tableName from aicrm_$tableName  inner join aicrm_role2picklist on aicrm_role2picklist.picklistvalueid = aicrm_$tableName.picklist_valueid where roleid in(". generateQuestionMarks($roleids) .") order by sortid";
...
{
	global $adb;
	/*$sql="select id from aicrm_picklistvalues_seq";
	$picklistvalue_id = $adb->query_result($adb->pquery($sql, array()),0,'id');

	$qry = "update aicrm_picklistvalues_seq set id =?";
	$adb->pquery($qry, array(++$picklistvalue_id));*/
	return $adb->getUniqueID('aicrm_picklistvalues');
}
