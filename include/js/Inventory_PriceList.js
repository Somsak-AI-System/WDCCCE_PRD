...
jQuery(document).ready(function() {
	jQuery("#proTab").on("input",".selling_price",function() {
		var rowno = jQuery(this).data("rowno")	;
			cal_listprice_inc(rowno);
	});

	function cal_listprice_inc(rowno)
	{
		var listPrice = jQuery('#selling_price'+rowno).val();

			var vat = ((listPrice*107)/100);
			vat = isNaN(vat = parseFloat(vat)) ? 0:parseFloat(vat);
			jQuery('#selling_price_inc'+rowno).val(vat.toFixed(2));
...
function pricelist_startdate(dateText,selectedDate){
	...
	if(stop_date != '' ){
		...
		if(start_date_f < start_date_s){
			jQuery.messager.alert('Message','Start date more than End date','info');
		}else{
			jQuery('input[name="pricelist_startdate"]').val(checkInDate)
		}
...
//Stop Date 
function pricelist_enddate(dateText,selectedDate){
	
	var checkInDate =selectedDate;
	var Asplit_E = checkInDate.split('-');
	var start_date_f = new Date(Asplit_E[2],Asplit_E[1],Asplit_E[0]); 

	var start_date = jQuery('input[name="pricelist_startdate"]').val();
	...
	if(start_date != '' ){
		...
		if(start_date_s > start_date_f){
			...
			jQuery('input[name="pricelist_enddate"]').val(checkInDate)
		}
...
function productPickList(currObj,module, row_no) {
    ...
    if(record_id != '')
    	window.open("index.php?module=Products&action=Popup&html=Popup_picker&select=enable&form=HelpDeskEditView&popuptype="+popuptype+"&curr_row="+rowId+"&relmod_id="+record_id+"&parent_module=Accounts&return_module="+module+"&currencyid="+currencyid,"productWin","width=640,height=620,resizable=0,scrollbars=0,status=1,top=150,left=200");
    else
		window.open("index.php?module=Products&action=Popup&html=Popup_picker&select=enable&form=HelpDeskEditView&popuptype="+popuptype+"&curr_row="+rowId+"&return_module="+module+"&currencyid="+currencyid,"productWin","width=640,height=620,resizable=0,scrollbars=0,status=1,top=150,left=200");
}
...
function servicePickList(currObj,module, row_no) {
    ...
    if(record_id != '')
		...
		window.open("index.php?module=Service&action=Popup&html=Popup_picker&select=enable&form=HelpDeskEditView&popuptype="+popuptype+"&curr_row="+rowId+"&return_module="+module+"&currencyid="+currencyid,"productWin","width=640,height=620,resizable=0,scrollbars=0,status=1,top=150,left=200");
}
...
function sparepartPickList(currObj,module, row_no) {
    ...
    if(record_id != '')
		...
		window.open("index.php?module=Sparepart&action=Popup&html=Popup_picker&select=enable&form=HelpDeskEditView&popuptype="+popuptype+"&curr_row="+rowId+"&return_module="+module+"&currencyid="+currencyid,"productWin","width=640,height=620,resizable=0,scrollbars=0,status=1,top=150,left=200");
}
...
function calcTotal_inc(rowno) {
	var listPrice = document.getElementById('selling_price' + rowno).value;
	var vat = (listPrice * 107) / 100;
	    vat = isNaN(vat = parseFloat(vat)) ? 0 : parseFloat(vat);
	document.getElementById('selling_price_inc' + rowno).value = vat.toFixed(2);
...
function fnAddProductRow(module,image_path){
	...
	coltwo.innerHTML= '<table border="0" cellpadding="1" cellspacing="0" width="100%"><tr><td class="small"><input id="productName'+count+'" name="productName'+count+'" class="small" style="width: 70%;" value="" readonly="readonly" type="text">'+
						...
						'</td></tr><tr><td class="small" id="setComment'+count+'"><textarea id="comment'+count+'" name="comment'+count+'" class=detailedViewTextBox user-success style="width:70%;height:40px"></textarea><img src="themes/images/clear_field.gif" onClick="getObj(\'comment'+count+'\').value=\'\'"; style="cursor:pointer;" /></td></tr></tbody></table>';
	
	colthree.className = "crmTableRow small";
	colthree.style.textAlign='center';
	colthree.innerHTML='<input data-rowno="'+count+'" id="product_brand'+count+'" name="product_brand'+count+'" type="text" class="detailedViewTextBox product_brand" onFocus="this.className=\'detailedViewTextBoxOn product_brand\';" onBlur="this.className=\'detailedViewTextBox product_brand\'; "  value="" readonly="readonly" style="background-color:#CCC;border: 1px solid #999;"/>';

	colfour.className = "crmTableRow small";
	colfour.style.textAlign='center';
	colfour.innerHTML='<input data-rowno="'+count+'" id="product_weight_per_box'+count+'" name="product_weight_per_box'+count+'" type="text" class="detailedViewTextBox product_weight_per_box" onFocus="this.className=\'detailedViewTextBoxOn product_weight_per_box\';" onBlur="this.className=\'detailedViewTextBox product_weight_per_box\'; "  value="" readonly="readonly" style="background-color:#CCC;border: 1px solid #999;"/>';

	colfive.className = "crmTableRow small";
	colfive.style.textAlign='center';
	colfive.innerHTML='<input data-rowno="'+count+'" id="productstatus'+count+'" name="productstatus'+count+'" type="text" class="detailedViewTextBox productstatus" onFocus="this.className=\'detailedViewTextBoxOn productstatus\';" onBlur="this.className=\'detailedViewTextBox productstatus\'; "  value="" readonly="readonly" style="background-color:#CCC;border: 1px solid #999;"/>';

	colsix.className = "crmTableRow small";
	colsix.style.textAlign='center';
	colsix.innerHTML='<input data-rowno="'+count+'" id="pricelist_showroom'+count+'" name="pricelist_showroom'+count+'" type="text" class="detailedViewTextBox pricelist_showroom" onFocus="this.className=\'detailedViewTextBoxOn pricelist_showroom\';" onBlur="this.className=\'detailedViewTextBox pricelist_showroom\'; "  value="" />';

	colseven.className = "crmTableRow small";
	colseven.style.textAlign='center';
	colseven.innerHTML='<input data-rowno="'+count+'" id="listprice_project'+count+'" name="listprice_project'+count+'" type="text" class="detailedViewTextBox listprice_project" onFocus="this.className=\'detailedViewTextBoxOn listprice_project\';" onBlur="this.className=\'detailedViewTextBox listprice_project\'; "  value="" />';

	coleight.className = "crmTableRow small";
	coleight.style.textAlign='center';
	coleight.innerHTML='<input data-rowno="'+count+'" id="pricelist_nomal'+count+'" name="pricelist_nomal'+count+'" type="text" class="detailedViewTextBox pricelist_nomal" onFocus="this.className=\'detailedViewTextBoxOn pricelist_nomal\';" onBlur="this.className=\'detailedViewTextBox pricelist_nomal\'; "  value="" />';

	colnine.className = "crmTableRow small";
	colnine.style.textAlign='center';
	colnine.innerHTML='<input data-rowno="'+count+'" id="pricelist_first_tier'+count+'" name="pricelist_first_tier'+count+'" type="text" class="detailedViewTextBox pricelist_first_tier" onFocus="this.className=\'detailedViewTextBoxOn pricelist_first_tier\';" onBlur="this.className=\'detailedViewTextBox pricelist_first_tier\'; "  value="" />';

	colten.className = "crmTableRow small";
	colten.style.textAlign='center';
	colten.innerHTML='<input data-rowno="'+count+'" id="pricelist_second_tier'+count+'" name="pricelist_second_tier'+count+'" type="text" class="detailedViewTextBox pricelist_second_tier" onFocus="this.className=\'detailedViewTextBoxOn pricelist_second_tier\';" onBlur="this.className=\'detailedViewTextBox pricelist_second_tier\'; "  value="" />';

	coleleven.className = "crmTableRow small";
	coleleven.style.textAlign='center';
	coleleven.innerHTML='<input data-rowno="'+count+'" id="pricelist_third_tier'+count+'" name="pricelist_third_tier'+count+'" type="text" class="detailedViewTextBox pricelist_third_tier" onFocus="this.className=\'detailedViewTextBoxOn pricelist_third_tier\';" onBlur="this.className=\'detailedViewTextBox pricelist_third_tier\'; "  value="" />';

	// coltwelve.className = "crmTableRow small";
	// coltwelve.style.textAlign='center';
	// coltwelve.innerHTML='<input data-rowno="'+count+'" id="selling_price'+count+'" name="selling_price'+count+'" type="text" class="detailedViewTextBox selling_price" onFocus="this.className=\'detailedViewTextBoxOn selling_price\';" onBlur="this.className=\'detailedViewTextBox selling_price\'; calcTotal();"  value="0" />';

	// colthirteen.className = "crmTableRow small";
	// colthirteen.style.textAlign='center';
	// colthirteen.innerHTML='<input data-rowno="'+count+'" id="selling_price_inc'+count+'" name="selling_price_inc'+count+'" type="text" class="detailedViewTextBox selling_price_inc" onFocus="this.className=\'detailedViewTextBoxOn selling_price_inc\';" readonly="readonly" style="background-color:#CCC;border: 1px solid #999;" onBlur="this.className=\'detailedViewTextBox selling_price_inc\'; "  value="0" />';
...
{
        var stock=getObj("qty_instock"+curr_row).innerHTML;
...
{
	var aFieldIds = Array('hidtax_row_no','productName','subproduct_ids','hdnProductId','comment','qty','listPrice','discount_type','discount_percentage','discount_amount','tax1_percentage','hidden_tax1_percentage','popup_tax_row','tax2_percentage','hidden_tax2_percentage','lineItemType','uom','qty_act','listprice_total','qty_ship','qty_remain','status_dtl','StandardPrice','standard_price_inc','listprice_inc','productstatus','productcategory','unit','monetary','productsize','product_brand','product_weight_per_box','productstatus','pricelist_showroom','listprice_project','pricelist_nomal','selling_price','selling_price_inc');
	...
	for(iCt=0;iCt<iMaxElement;iCt++)
	{
		...
		{
			var select1 = document.getElementById(sId);
			if(select1.tagName === 'SELECT') {
				
				var select2 = document.getElementById(sSwapId);
				var value1 = select1.options[select1.selectedIndex].value;
				var value2 = select2.options[select2.selectedIndex].value;
				
				//document.getElementById(sId).selectedIndex = value2;
				//document.getElementById(sSwapId).selectedIndex = value1;
				$("#"+sId).val(value2).trigger('change');
				$("#"+sSwapId).val(value1).trigger('change');

				/*jQuery("#"+sId+" option[value='"+value2+"']").attr("selected", "selected");
				jQuery("#"+sSwapId+" option[value='"+value1+"']").attr("selected", "selected");*/
	...
	for(iCt=0;iCt<iMaxElement;iCt++)
	{
		...
		if(document.getElementById(sId) && document.getElementById(sSwapId))
		{
			...
			//document.getElementById(sSwapId).value = document.getElementById(sSwapId).value;
		}
	}
    ...
    if(document.selectall.selected_id != undefined)
    {
        ...
        if ( x == undefined) {
            if (document.selectall.selected_id.checked) {
				...
				var pack_size = prod_array['pack_size'];
				var test_box = prod_array['test_box'];
				var uom = prod_array['uom'];
				var qty = prod_array['qty'];
				var pro_type = prod_array['pro_type'];
				var promotion_id = prod_array['promotion_id'];
				var return_module = prod_array['return_module'];
				var relmod_id = prod_array['relmod_id'];
				var parent_module = prod_array['parent_module'];
				var productcode = prod_array['productcode'];
				...
				var products_businessplusno = prod_array['products_businessplusno'];
				var mat_gp1_desciption = prod_array['mat_gp1_desciption'];
				var mat_gp3_desciption = prod_array['mat_gp3_desciption'];
				var mat_gp4_desciption = prod_array['mat_gp4_desciption'];
				var piece_per_carton = prod_array['piece_per_carton'];
				var squaremeters_per_carton = prod_array['squaremeters_per_carton'];
				var price_per_piece = prod_array['price_per_piece'];
				var price_per_squaremeter = prod_array['price_per_squaremeter'];
				var productsubcategory = prod_array['productsubcategory'];
					
				var module_id = '';
				var productcode_view = '';
				var pricetype = '';
				var business_code = '';

                if(mod!='PurchaseOrder') {
                    var qtyinstk = prod_array['qtyinstk'];
                    set_return_inventory(prod_id,prod_name,unit_price,qtyinstk,taxstring,parseInt(row_id),desc,subprod_ids,uom,'','','');
                } else {
                    set_return_inventory_po(prod_id,prod_name,unit_price,taxstring,parseInt(row_id),desc,subprod_ids,uom);
                }
                y=1;
            } else {
            ...
            y=0;
            for(i = 0; i < x ; i++) {
                if(document.selectall.selected_id[i].checked) {//alert(i);
                    ...
                    //alert(prod_id,prod_name,unit_price,qtyinstk,taxstring,parseInt(row_id),desc,subprod_ids,uom);
                    if(mod!='PurchaseOrder') {
                        var qtyinstk = prod_array['qtyinstk'];//
                    	...
                    	set_return_inventory(prod_id,prod_code,prod_name,unit_price,qtyinstk,taxstring,parseInt(row_id),desc,subprod_ids,pro_type,qty,uom,promotion_id,module_id,relmod_id,return_module,productcode,parent_module,productcode_view,listprice_inc,pricetype,business_code,unit,sellingprice,productcategory,productsubcategory,mat_gp3_desciption,productstatus);
