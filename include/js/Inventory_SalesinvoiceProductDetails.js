...
function deleteRow(module,i,image_path)
{
    ...
    var iPrevCount = iPrevRowIndex ?? 1;
...
/**
 * Sync hidden selling_price from selling_price_product + selling_price_delivery (Quotes Item Details)
 * Exposed on window for inline handlers (onkeyup/onBlur in template).
 */
function syncSellingPriceFromProductDelivery(rowId) {
    var productElem = document.getElementById("selling_price_product" + rowId);
    var deliveryElem = document.getElementById("selling_price_delivery" + rowId);
    var sellingPriceElem = document.getElementById("selling_price" + rowId);
    if (!sellingPriceElem) return;
    var product = parseFloat(productElem ? productElem.value : 0) || 0;
    var delivery = parseFloat(deliveryElem ? deliveryElem.value : 0) || 0;
    sellingPriceElem.value = roundValue((product + delivery).toString());
}
if (typeof window !== 'undefined') {
    window.syncSellingPriceFromProductDelivery = syncSellingPriceFromProductDelivery;
}
...
function calcProductTotal(rowId) {
    ...
    if(document.getElementById('deleted'+rowId) && document.getElementById('deleted'+rowId).value == 0)
    {
        ...
        group_tax_percentage = document.getElementById("group_tax_percentage1")?.value || 0;
        ...
        if(pricelist_type == "ราคาต่อตร.ม." && sales_unit == "Box"){
            ...
            // ปัดเศษก่อนคูณ box_quantity เพื่อให้ตรงกับ PHP (เหมือนกับ round() ใน PHP)
            // PHP: round(($selling_price*$package_size_sqm_per_box), 2) แล้วคูณ $box_quantity โดยไม่ปัดเศษอีก
            // ใช้วิธีเดียวกับ PHP round() ที่ปัดเศษแบบ "round half up"
            var selling_price_val = parseFloat(selling_price);
            var package_size_val = parseFloat(package_size_sqm_per_box);
            var multiply_result = selling_price_val * package_size_val;
            // แก้ปัญหา floating point precision โดยเพิ่มค่าเล็กน้อยก่อนปัดเศษ
            // เพื่อให้ได้ผลลัพธ์เหมือน PHP round() ที่ปัดเศษแบบ "round half up"
            var net_price = Math.round((multiply_result + 0.0000001) * 100) / 100;
            netprice = parseFloat(net_price) * parseFloat(getObj("box_quantity"+rowId).value);
            console.log('net_price===',selling_price_val,'*',package_size_val,'=',multiply_result,'=> rounded:',net_price)
            if(pricetype=="Include Vat"){
                // Total in row (Total after discount+Discount)
                // Total in row เอาราคาขายคูณจำนวนตรม
                hdn_cal_total = (roundValue(eval((getObj("regular_price"+rowId).value * getObj("package_size_sqm_per_box"+rowId).value))) *getObj("box_quantity"+rowId).value);
                // Discount in row
                hdn_cal_discount = (roundValue(eval((getObj("product_discount"+rowId).value * getObj("package_size_sqm_per_box"+rowId).value))) *getObj("box_quantity"+rowId).value);
            }else{
                // Total in row
                hdn_cal_total = (roundValue(eval((getObj("regular_price"+rowId).value * getObj("package_size_sqm_per_box"+rowId).value))) *getObj("box_quantity"+rowId).value);
                // Discount in row
            ...
            netprice = (roundValue(eval((getObj("selling_price"+rowId).value * getObj("package_size_sheet_per_box"+rowId).value))) *getObj("box_quantity"+rowId).value);

            if(pricetype=="Include Vat"){
                // Total in row (Total after discount+Discount)

                // Total in row เอาราคาขายคูณจำนวนแผ่น
                hdn_cal_total = (roundValue(eval((getObj("regular_price"+rowId).value * getObj("package_size_sheet_per_box"+rowId).value))) *getObj("box_quantity"+rowId).value);
                // Discount in row
                hdn_cal_discount = (roundValue(eval((getObj("product_discount"+rowId).value * getObj("package_size_sheet_per_box"+rowId).value))) *getObj("box_quantity"+rowId).value);
            }else{
                // Total in row
                hdn_cal_total = (roundValue(eval((getObj("regular_price"+rowId).value * getObj("package_size_sheet_per_box"+rowId).value))) *getObj("box_quantity"+rowId).value);
                // Discount in row
            ...
            netprice = (roundValue((roundValue(eval((getObj("selling_price"+rowId).value * getObj("package_size_sqm_per_box"+rowId).value)))) / getObj("package_size_sheet_per_box"+rowId).value)) * getObj("box_quantity"+rowId).value;

            if(pricetype=="Include Vat"){
                // Total in row (Total after discount+Discount)
                // Total in row เอาราคาขายคูณจำนวนตรม
                hdn_cal_total = (roundValue((roundValue(eval((getObj("regular_price"+rowId).value * getObj("package_size_sqm_per_box"+rowId).value)))) / getObj("package_size_sheet_per_box"+rowId).value)) * getObj("box_quantity"+rowId).value;
                // Discount in row
                hdn_cal_discount = (roundValue((roundValue(eval((getObj("product_discount"+rowId).value * getObj("package_size_sqm_per_box"+rowId).value)))) / getObj("package_size_sheet_per_box"+rowId).value)) * getObj("box_quantity"+rowId).value;
            }else{
                // Total in row
                hdn_cal_total = (roundValue((roundValue(eval((getObj("regular_price"+rowId).value * getObj("package_size_sqm_per_box"+rowId).value)))) / getObj("package_size_sheet_per_box"+rowId).value)) * getObj("box_quantity"+rowId).value;
                // Discount in row
        ...
        }else if(pricelist_type == "ราคาต่อตร.ม." && (sales_unit == "Sq.m." || sales_unit == "SQ.M." || sales_unit == "SQ.M")){ //รอสรุป
            ...
            if(pricetype=="Include Vat"){
                ...
                if((pricelist_type == "ราคาต่อกล่อง" || pricelist_type == "ราคาต่อหน่วย") && sales_unit == "Box"){
                    ...
                    hdn_cal_total = (roundValue(eval(getObj("regular_price"+rowId).value * getObj("box_quantity"+rowId).value)));
                }else{
                    hdn_cal_total = (roundValue(eval(getObj("regular_price"+rowId).value * getObj("box_quantity"+rowId).value)));
                }
        ...
        document.getElementById("hdn_cal_total"+rowId).value = roundValue(hdn_cal_total);
        document.getElementById("hdn_cal_discount"+rowId).value = roundValue(hdn_cal_discount);
...
function popupPricelist(currObj,curr_row) {
    ...
    var pricetype = document.getElementById("pricetype").value;
    var assigneduserid = document.querySelector('select[name="assigned_user_id"]').value;
    
    if(productid != ''){
        var w = 880, h = 650;
        var left = (typeof screen !== 'undefined' && screen.width) ? Math.max(0, (screen.width - w) / 2) : 100;
        var top = (typeof screen !== 'undefined' && screen.height) ? Math.max(0, (screen.height - h) / 2) : 50;
        window.open('search_pop_pricelist.php?curr_row='+curr_row+'&productid='+productid+'&pricetype='+pricetype+'&assigned_user_id='+assigneduserid,'Price List','resizable=yes,left='+left+',top='+top+',width='+w+',height='+h+',toolbar=no,scrollbars=no,menubar=no,location=no');
    }else{
...
var remove_Comma = (num) => {
	if (num === undefined || num === null || num === '') return 0
	return parseFloat(num.toString().replace(/,/g, ''))
}

var currency_Format = (num, decimal) => {
	decimal = decimal ?? 0
	num = num ?? 0
...
function calcGrandTotal() {
    var netTotal = 0.0,
        hdn_cal_discount = 0.0,
        total_after_discount = 0.0,
        bill_discount = 0.0,
        total_after_bill_discount = 0.0,
        tax_final = 0.0,
        total_after_vat = 0.0
        discount_coupons = 0.0
        grandTotal = 0.0;
	...
	/*
        ...
        netTotal += value;
	*/
            var row = element.closest("tr")?.closest("td")?.closest("tr"); // หา row ที่ element อยู่
            var deleted = row ? row.querySelector('[id^="deleted"]') : null; // หา element ที่มี id ขึ้นต้นด้วย "deleted"

            if (deleted && deleted.value === "0") { // ตรวจสอบว่ามีค่า deleted และเป็น "0"
	        var value = parseFloat(element.value) || 0;
        	netTotal += value;
            }

    });
    netTotal = roundValue(netTotal);
    // console.log("ราคารวม : ", netTotal);

    var nTotal = remove_Comma(netTotal)
    var activeModule = jQuery('#active_module').val()
    var tmp_total_after_discount = 0
    var oTax = remove_Comma(jQuery('#q_tax').val());
    var oDeposit = remove_Comma(jQuery('#deposit').val());
    var oSubTotal = remove_Comma(jQuery('#sub_total').val());
    var oSubDiscount = remove_Comma(jQuery('#sub_discount').val());
	...
	*/
    ...
    sum_hdn_cal_discount = roundValue(sum_hdn_cal_discount);
    // console.log("Discount : ", sum_hdn_cal_discount);

    /**** Total After Discount ****/
    var netPriceLineItem = document.querySelectorAll('[id^="netPrice"]');
    var sum_netPrice = 0;

        netPriceLineItem.forEach(function (element) {
        /*
            var value = parseFloat(element.textContent) || 0;
            // console.log("sum_netPrice : ", value);
            sum_netPrice += value;
        */
            var row = element.closest("tr"); // หา row ที่ element อยู่
            var deleted = row ? row.querySelector('[id^="deleted"]') : null; // หา element ที่มี id ขึ้นต้นด้วย "deleted"
    ...
    // total_after_discount = netTotal - sum_hdn_cal_discount;
    total_after_discount = sum_netPrice;
    total_after_discount = roundValue(parseFloat(total_after_discount));
    // console.log("Total After Discount : ", total_after_discount);


    /**** ส่วนลด Discount ****/
    sum_hdn_cal_discount = parseFloat(netTotal) - parseFloat(total_after_discount);
    sum_hdn_cal_discount = roundValue(sum_hdn_cal_discount);
    /**** ส่วนลดท้ายบิล ****/
    
    // bill_discount = document.getElementById("bill_discount").innerHTML;
    var hdnBillDiscountEl = document.getElementById("hdn_bill_discount");
    bill_discount = (hdnBillDiscountEl && hdnBillDiscountEl.value !== undefined) ? hdnBillDiscountEl.value : '';
    bill_discount = bill_discount === '' ? 0.00 : roundValue(parseFloat(bill_discount));
    // console.log("ส่วนลดท้ายบิล : ", bill_discount);

    /**** Total หลังลดท้ายบิล ****/
    total_after_bill_discount = total_after_discount - bill_discount;
    // total_after_bill_discount = document.getElementById("hdn_total_after_bill_discount").value;
    total_after_bill_discount = total_after_bill_discount === '' ? 0.00 : roundValue(parseFloat(total_after_bill_discount));
    // console.log("Total หลังลดท้ายบิล : ", total_after_bill_discount);

    /**** Vat ****/
    // ใช้ Math.floor() เพื่อให้ตรงกับ PHP ที่ใช้ค่าจากฐานข้อมูล
    // PHP: ใช้ค่าจากฐานข้อมูล $rs_quotes['tax_final'] ซึ่งคำนวณจาก round(total_after_bill_discount * 0.07, 2)
    // แต่ค่าจากฐานข้อมูลเป็น 364.12 ซึ่งหมายความว่าใช้วิธีปัดเศษแบบ truncate
    var vat_calc = total_after_bill_discount * 0.07;
    // ใช้ Math.floor() เพื่อปัดเศษแบบ truncate ให้ได้ 364.12 แทน 364.13
    tax_final = Math.floor(vat_calc * 100) / 100;
    // console.log("Vat : ", tax_final);

    /**** Total Net Amount including VAT ****/
    total_after_vat = parseFloat(total_after_bill_discount) + parseFloat(tax_final);
    // ปัดเศษ Total Net Amount including VAT เพื่อให้ตรงกับ PHP
    total_after_vat = roundValue(total_after_vat);
    // console.log("Total Net Amount including VAT : ", total_after_vat);

    /**** คูปองส่วนลด (บาท)  ****/
    var discount_coupons_el = document.getElementById("discount_coupons");
    discount_coupons = (discount_coupons_el && discount_coupons_el.value) ? discount_coupons_el.value : '';
    discount_coupons = discount_coupons === '' ? 0.00 : roundValue(parseFloat(discount_coupons));
    // console.log("คูปองส่วนลด (บาท) : ", discount_coupons);

    /**** Grand Total ****/
    grandTotal = parseFloat(total_after_vat) - parseFloat(discount_coupons);
    grandTotal = roundValue(grandTotal);
    // console.log("Grand Total : ", grandTotal);

    var pricetypeEl = document.getElementById("pricetype");
    var pricetype = pricetypeEl ? pricetypeEl.value : '';

    if(pricetype=="Include Vat"){
        var sum_hdn_cal_discount_inc = 0.0,
            total_after_discount_inc = 0.0,
            netTotal_inc = 0.0,
            total_after_bill_discount_inc = 0.0,
            tax_final_inc = 0.0,
            total_after_vat_inc = 0.0,
            grandTotal_inc = 0.0,
            hdn_total_after_bill_discount = 0.0;

        
        total_after_discount_inc = roundValue(total_after_discount/1.07); // Total After Discount
        sum_hdn_cal_discount_inc = roundValue(sum_hdn_cal_discount/1.07); // Discount

        
        netTotal_inc = roundValue(parseFloat(total_after_discount_inc) + parseFloat(sum_hdn_cal_discount_inc)); // Grand Total
        total_after_bill_discount_inc = roundValue(parseFloat(total_after_discount_inc) - parseFloat(bill_discount)); // Total หลังลดท้ายบิล
        // total_after_bill_discount_inc = roundValue(parseFloat(total_after_bill_discount) - parseFloat(total_after_bill_discount_inc)); // Total หลังลดท้ายบิล
        // console.log("Total หลังลดท้ายบิล EXC ",total_after_bill_discount);
        // console.log("Total หลังลดท้ายบิล Inc ",total_after_bill_discount_inc);
        
        nTotal = netTotal_inc

        var netTotalEl = document.getElementById("netTotal");
        if (netTotalEl) netTotalEl.innerHTML = netTotal_inc.toString(); // TOTAL PRICE
        var subtotalEl = document.getElementById("subtotal");
        if (subtotalEl) subtotalEl.value = netTotal_inc; // TOTAL PRICE
        var discountTotalFinalEl = document.getElementById("discountTotal_final");
        if (discountTotalFinalEl) discountTotalFinalEl.value = sum_hdn_cal_discount_inc; // Discount
        var afterdisFinalEl = document.getElementById("afterdis_final");
        if (afterdisFinalEl) afterdisFinalEl.innerHTML = total_after_discount_inc.toString(); // Total After Discount  
        tmp_total_after_discount = total_after_discount_inc
        // document.getElementById("bill_discount").innerHTML = bill_discount.toString(); // ส่วนลดท้ายบิล

        setDiscount(this,'_final');
        var billDiscountEl = document.getElementById("bill_discount");
        bill_discount = billDiscountEl ? billDiscountEl.innerHTML : 0;
        // alert(total_after_discount_inc); alert(parseFloat(bill_discount));
        // console.log("total_after_discount_inc ",total_after_discount_inc);
        // console.log("bill_discount ",bill_discount);
        total_after_bill_discount_inc = roundValue(parseFloat(total_after_discount_inc) - parseFloat(bill_discount));

        // console.log("total_after_bill_discount ",total_after_bill_discount);
        // console.log("total_after_bill_discount_inc ",total_after_bill_discount_inc);
        tax_final_inc = roundValue(parseFloat(total_after_bill_discount) - parseFloat(total_after_bill_discount_inc)); // Vat

        var totalAfterBillDiscEl = document.getElementById("total_after_bill_discount");
        if (totalAfterBillDiscEl) totalAfterBillDiscEl.innerHTML = total_after_bill_discount_inc.toString(); // Total หลังลดท้ายบิล
        var hdnTotalAfterBillDiscEl = document.getElementById("hdn_total_after_bill_discount");
        if (hdnTotalAfterBillDiscEl) hdnTotalAfterBillDiscEl.value = total_after_bill_discount_inc; // Total หลังลดท้ายบิล

        
        total_after_vat_inc = roundValue(parseFloat(total_after_bill_discount_inc) + parseFloat(tax_final_inc)); // Total Net Amount including VAT
        grandTotal_inc = roundValue(parseFloat(total_after_vat_inc) - parseFloat(discount_coupons));
        
        var taxFinalEl = document.getElementById("tax_final");
        if (taxFinalEl) taxFinalEl.value = tax_final_inc; // Vat

        var totalAfterVatEl = document.getElementById("total_after_vat");
        if (totalAfterVatEl) totalAfterVatEl.innerHTML = total_after_vat_inc.toString(); // Total Net Amount including VAT
        var totalWithoutVatEl = document.getElementById("total_without_vat");
        if (totalWithoutVatEl) totalWithoutVatEl.value = total_after_vat_inc; // Total Net Amount including VAT

        var modeEl = document.getElementById("mode");
        var mode = modeEl ? modeEl.value : '';
        var totalDeliveredEl = document.getElementById("hdn_total_delivered_value");
        var total_delivered_value = totalDeliveredEl ? totalDeliveredEl.value : '';
        if(mode == 'edit'){
            if(!isNaN(total_delivered_value) && total_delivered_value > 0){
                if(grandTotal_inc < total_delivered_value){
                    if (!window._lastGrandTotalAlertTime || (Date.now() - window._lastGrandTotalAlertTime > 3000)) {
                        window._lastGrandTotalAlertTime = Date.now();
                        alert("ยอด Grand Total น้อยกว่ายอดที่ส่งมอบ กรุณาตรวจสอบอีกครั้ง");
                    }
                }
            }
        }
        ...
        if (grandTotalEl) grandTotalEl.innerHTML = grandTotal_inc.toString(); // Grand Total 
        var totalEl = document.getElementById("total");
        if (totalEl) totalEl.value = grandTotal_inc.toString(); // Grand Total
    }else{
        nTotal = netTotal

        var netTotalEl2 = document.getElementById("netTotal");
        if (netTotalEl2) netTotalEl2.innerHTML = netTotal.toString(); // TOTAL PRICE
        var subtotalEl2 = document.getElementById("subtotal");
        if (subtotalEl2) subtotalEl2.value = netTotal; // TOTAL PRICE

        var discountTotalFinalEl2 = document.getElementById("discountTotal_final");
        if (discountTotalFinalEl2) discountTotalFinalEl2.value = sum_hdn_cal_discount; // Discount
        var afterdisFinalEl2 = document.getElementById("afterdis_final");
        if (afterdisFinalEl2) afterdisFinalEl2.innerHTML = total_after_discount.toString(); // Total After Discount  
        tmp_total_after_discount = total_after_discount
        var billDiscountEl2 = document.getElementById("bill_discount");
        if (billDiscountEl2) billDiscountEl2.innerHTML = bill_discount.toString(); // ส่วนลดท้ายบิล

        var totalAfterBillDiscEl2 = document.getElementById("total_after_bill_discount");
        if (totalAfterBillDiscEl2) totalAfterBillDiscEl2.innerHTML = total_after_bill_discount.toString(); // Total หลังลดท้ายบิล
        var hdnTotalAfterBillDiscEl2 = document.getElementById("hdn_total_after_bill_discount");
        if (hdnTotalAfterBillDiscEl2) hdnTotalAfterBillDiscEl2.value = total_after_bill_discount; // Total หลังลดท้ายบิล

        var taxFinalEl2 = document.getElementById("tax_final");
        if (taxFinalEl2) taxFinalEl2.value = tax_final; // Vat

        var totalAfterVatEl2 = document.getElementById("total_after_vat");
        if (totalAfterVatEl2) totalAfterVatEl2.innerHTML = total_after_vat.toString(); // Total Net Amount including VAT
        var totalWithoutVatEl2 = document.getElementById("total_without_vat");
        if (totalWithoutVatEl2) totalWithoutVatEl2.value = total_after_vat; // Total Net Amount including VAT

        
        var modeEl2 = document.getElementById("mode");
        var mode2 = modeEl2 ? modeEl2.value : '';
        var totalDeliveredEl2 = document.getElementById("hdn_total_delivered_value");
        var total_delivered_value2 = totalDeliveredEl2 ? totalDeliveredEl2.value : '';
        if(mode2 == 'edit'){
            if(!isNaN(total_delivered_value2) && total_delivered_value2 > 0){
                if(grandTotal < total_delivered_value2){
                    if (!window._lastGrandTotalAlertTime || (Date.now() - window._lastGrandTotalAlertTime > 3000)) {
                        window._lastGrandTotalAlertTime = Date.now();
                        alert("ยอด Grand Total น้อยกว่ายอดที่ส่งมอบ กรุณาตรวจสอบอีกครั้ง");
                    }
        ...
        var grandTotalEl2 = document.getElementById("grandTotal");
        if (grandTotalEl2) grandTotalEl2.innerHTML = grandTotal.toString(); // Grand Total 
        var totalEl2 = document.getElementById("total");
        if (totalEl2) totalEl2.value = grandTotal.toString(); // Grand Total
    }

    if(activeModule == 'Salesinvoice'){
        // nTotal = 40000
        // oSubTotal = 100000
        // oSubDiscount = 5000
        
        console.log(tmp_total_after_discount)
        var discount_coupons = remove_Comma(jQuery('#discount_coupons').val())
        var discount = remove_Comma(currency_Format((nTotal / oSubTotal) * oSubDiscount, 2))
        var after_discount = remove_Comma(currency_Format(tmp_total_after_discount - discount, 2))
        var new_deposit = remove_Comma(currency_Format((oDeposit * after_discount) / 100, 2))
        var after_deposit = remove_Comma(currency_Format(after_discount - new_deposit, 2))
        var new_vat = remove_Comma(currency_Format(after_deposit * (oTax / 100), 2))
        var final_total = remove_Comma(currency_Format(after_deposit + new_vat, 2))

        // console.log(discount, after_discount, new_deposit, after_deposit, new_vat, final_total, discount_coupons, (final_total - discount_coupons))

        jQuery('#bill_discount').html(discount)
        jQuery('#hdn_bill_discount').val(discount)

        jQuery('#total_after_bill_discount').html(after_discount)
        jQuery('#hdn_total_after_bill_discount').val(after_discount)

        jQuery('#n_deposit').val(new_deposit)
        jQuery('#n_deposit_display').html(new_deposit)

        jQuery('#n_after_deposit').val(after_deposit)
        jQuery('#n_after_deposit_display').html(after_deposit)

        jQuery('#tax_final').val(new_vat)
        jQuery('#total_without_vat').val(final_total)
        jQuery('#total_after_vat').html(final_total)

        jQuery('#grandTotal').html(final_total - discount_coupons)
        jQuery('#total').val(final_total - discount_coupons)
    }
...
function validateInventory(module)
{
    ...
    if(module == 'Quotes'){ 
        var payment_terms = jQuery("select[name='payment_terms']").val();
        if(payment_terms === '' || payment_terms == null){
            alert('ระยะเวลาชำระเงิน ต้องมีข้อมูล เป็นค่าว่างไม่ได้');
            jQuery("select[name='payment_terms']").focus();
            return false;
        }  
        ...
        // }
            ...
            jQuery('[id^="product_price_type"]').each(function(i, e) {
                ...
                if((selectedValue == '' || selectedValue == '0') && hdnProductId != ''){                
                    flag = 1;
                    // เก็บ element แรกที่พบปัญหาไว้
                    if (problemElement === null) {
                        problemElement = jQuery(e);
                    }
                }
            });

            if(flag == 1){
                alert('Select Product Price Type');
                // Focus และ scroll ไปที่ dropdown ที่มีปัญหา
                if (problemElement !== null) {
            ...
            var flag_pricelistid = 0;
            jQuery('[id^="pricelist"]:not([id*="_type"])').each(function(i, e) {
                // 1. สร้าง ID ของ element 'delete' ที่สอดคล้องกับแถวนั้น
                //    เนื่องจาก 'e' คือ element 'pricelist' ในแถวที่ 'i'
                //    เราจึงคาดว่า element 'delete' ที่สอดคล้องจะมี ID เป็น 'delete' + i
                var deleteId = 'deleted' + (i+1); 
                var deleteValue = jQuery('#' + deleteId).val();

                // 2. ตรวจสอบค่าของ deleteValue ก่อน
                //    ถ้านำมาเป็นค่า 1 (หรือ '1' ซึ่ง .val() อาจคืนค่าเป็น string) 
                //    ให้ข้ามการตรวจสอบ pricelist ไป (ใช้ continue)
                if (deleteValue == 1 || deleteValue == '1') {
                    return true; // ใน jQuery.each() การใช้ 'return true' จะเป็นการ 'continue' ไปยังรอบถัดไป
                }
                
                // 3. ตรวจสอบ product_dummyN 
                //    ถ้าเป็น "YES" ให้ข้ามการตรวจสอบ pricelist
                //    ถ้าเป็น "NO" ต้องตรวจสอบ pricelist
                var productDummyId = 'product_dummy' + (i+1);
                var productDummyValue = jQuery('#' + productDummyId).val();
                if (productDummyValue == 'YES' || productDummyValue == 'yes') {
                    return true; // skip การตรวจสอบ pricelist
                }
                
                // 4. ถ้า product_dummy เป็น "NO" หรือค่าอื่น จึงทำการตรวจสอบ pricelist
                console.log('productDummyValue',productDummyValue);
                var selectedValue = jQuery(e).val();
                // alert(selectedValue);
            ...
            if(flag_pricelistid == 1){
                alert('โปรดระบุ Product Price List');
                return false;
            }
            ...
            // Validate: Sum delivered qty (delivery table) must not exceed product qty (main table)
            // This prevents the case where user edits main product qty after filling delivery qty.
            if (!validateDeliveryQtyBeforeSave()) {
                return false;
            }
...
/**
 * Validate that for each product, the sum of delivered quantities (delivery table)
 * does not exceed the product quantity in the main Quote line items.
 *
 * Delivery qty inputs are identified by `.sub-qty-input[data-productid]`
 * (rendered by `modules/Quotes/DeliveryDate/list.php`).
 *
 * Returns true if OK, otherwise alerts and returns false to block save.
 */
function validateDeliveryQtyBeforeSave() {
    // If delivery UI is not present on this page, skip validation.
    var deliveryInputs = document.querySelectorAll('.sub-qty-input[data-productid]');
    if (!deliveryInputs || deliveryInputs.length === 0) return true;
    ...
    // Map main product qty by productId (skip deleted rows).
    var maxRowCount = 0;
    var proTab = document.getElementById('proTab');
    if (proTab && proTab.rows && proTab.rows.length) {
        maxRowCount = proTab.rows.length - 2; // two header rows
    }

    var mainQtyByProductId = {};
    var mainNameByProductId = {};
    for (var i = 1; i <= maxRowCount; i++) {
        var delEl = document.getElementById('deleted' + i);
        if (delEl && String(delEl.value) === '1') continue;

        var pidEl = document.getElementById('hdnProductId' + i);
        var pid = pidEl ? String(pidEl.value || '').trim() : '';
        if (!pid) continue;

        var qtyEl = document.getElementById('box_quantity' + i);
        var qty = qtyEl ? parseNum(qtyEl.value) : 0;
        mainQtyByProductId[pid] = qty;

        var nameEl = document.getElementById('productName' + i);
        var name = nameEl ? String(nameEl.value || '').trim() : '';
        if (name) mainNameByProductId[pid] = name;
    }

    // Sum delivered qty per productId.
    var deliveredSumByProductId = {};
    for (var d = 0; d < deliveryInputs.length; d++) {
        var input = deliveryInputs[d];
        var pid2 = input.getAttribute('data-productid');
        if (!pid2) continue;
        pid2 = String(pid2).trim();
        if (!pid2) continue;

        var v2 = parseNum(input.value);
        deliveredSumByProductId[pid2] = (deliveredSumByProductId[pid2] || 0) + v2;

        // Fallback product name from delivery table if main name missing
        if (!mainNameByProductId[pid2]) {
            // Best-effort: find readonly product name input in delivery UI
            if (typeof jQuery !== 'undefined') {
                var $hidden = jQuery('input.sub-product-id[value="' + pid2 + '"]');
                if ($hidden.length) {
                    var $prodRow = $hidden.first().closest('.product-entry-row');
                    var $visible = $prodRow.find('input.form-control[readonly]').first();
                    if ($visible.length && $visible.val()) {
                        mainNameByProductId[pid2] = String($visible.val()).trim();
                    }
                }
            }
        }
    ...
    // Compare like `checkProductQtyLimit`: alert + reset offending input + focus, then block save.
    for (var pidKey in deliveredSumByProductId) {
        if (!Object.prototype.hasOwnProperty.call(deliveredSumByProductId, pidKey)) continue;
        var delivered = deliveredSumByProductId[pidKey] || 0;
        var limit = (typeof mainQtyByProductId[pidKey] !== 'undefined') ? mainQtyByProductId[pidKey] : 0;

        // If product was removed from main table, treat as invalid when delivered > 0 (limit will be 0).
        if (delivered > limit + 1e-9) {
            // Find an input for this productId (use the last one to mimic editing the latest row)
            var offendingInput = null;
            for (var x = deliveryInputs.length - 1; x >= 0; x--) {
                if (String(deliveryInputs[x].getAttribute('data-productid') || '').trim() === String(pidKey)) {
                    offendingInput = deliveryInputs[x];
                    break;
                }
            }

            alert("จำนวนรวมสิ้นค้าที่ส่ง (" + delivered.toFixed(2) + ") เกินจำนวนสินค้าที่มี (" + limit.toFixed(2) + ")");

            if (offendingInput) {
                offendingInput.value = '0';
                if (typeof jQuery !== 'undefined' && typeof jQuery.fn !== 'undefined' && typeof jQuery.fn.autoNumeric !== 'undefined') {
                    try { jQuery(offendingInput).autoNumeric('set', 0); } catch (e) { /* ignore */ }
                }
                try { offendingInput.focus(); } catch (e2) { /* ignore */ }
            }
            return false;
        }
    }
...
function fnAddProductRowInventory(module,image_path){
    ...
    coltwo.innerHTML= '<table border="0" cellpadding="1" cellspacing="0" width="100%"><tr><td class="small">' +
            ...
            '<input type="hidden" id="product_dummy'+count+'" name="product_dummy'+count+'" value="" />'+
    ...
    colnine.innerHTML='<td class="crmTableRow small lineOnTop" style="white-space: nowrap;" align="center"><input type="hidden" id="pricelist'+count+'" name="pricelist'+count+'" /><input type="hidden" id="tier_type'+count+'" name="tier_type'+count+'" /><input data-rowno="'+count+'" id="regular_price'+count+'" name="regular_price'+count+'" type="text" class="detailedViewTextBox small regular_price" onfocus="this.className=\'regular_price detailedViewTextBoxOn\'" onBlur="this.className=\'regular_price detailedViewTextBox\';setDiscount(this,'+count+'); callTaxCalc('+count+');calcTotal();" value="" readonly="readonly" style="display: inline-block; vertical-align: middle; width:80px; background-color: #CCC;"/><a href="javascript:void(0);" onclick="popupPricelist(this,'+count+');" style="display: inline-block; vertical-align: middle; padding-left: 10px;"><img src="themes/softed/images/search.gif" border="0" title="" alt="" style="cursor: pointer;"></a><br><br><select class="small" id="pricelist_type'+count+'" name="pricelist_type'+count+'" style="margin-bottom:5px;">'+getHTMLpricelist_type("pricelist_type"+count)+'</select></td>';

    colten.className = "crmTableRow small";
    colten.style.textAlign='center';
    colten.innerHTML='<td class="crmTableRow small lineOnTop" align="center"><input type="hidden" id="selling_price'+count+'" name="selling_price'+count+'" value=""/><div class="small" style="margin-bottom:4px;"><label class="small">ราคาขายไม่รวมค่าขนส่ง</label><br/><input data-rowno="'+count+'" id="selling_price_product'+count+'" name="selling_price_product'+count+'" type="text" class="small selling_price_product" style="width:70px" onfocus="this.className=\'selling_price_product detailedViewTextBoxOn\'" onBlur="this.className=\'selling_price_product detailedViewTextBox\';syncSellingPriceFromProductDelivery(\''+count+'\');setDiscount(this,'+count+'); callTaxCalc('+count+');calcTotal();" onkeyup="syncSellingPriceFromProductDelivery(\''+count+'\');" value=""/></div><div class="small"><label class="small">ราคาค่าขนส่ง</label><br/><input data-rowno="'+count+'" id="selling_price_delivery'+count+'" name="selling_price_delivery'+count+'" type="text" class="small selling_price_delivery" style="width:70px" onfocus="this.className=\'selling_price_delivery detailedViewTextBoxOn\'" onBlur="this.className=\'selling_price_delivery detailedViewTextBox\';syncSellingPriceFromProductDelivery(\''+count+'\');setDiscount(this,'+count+'); callTaxCalc('+count+');calcTotal();" onkeyup="syncSellingPriceFromProductDelivery(\''+count+'\');" value="0"/></div></td>';
...
function setDiscount(currObj,curr_row)
{
    ...
    // ถ้า product_dummy เป็น YES ให้คัดลอก selling_price ไปยัง regular_price
    var productDummyValue = document.getElementById("product_dummy"+curr_row)?.value;
    if (productDummyValue == 'YES' || productDummyValue == 'yes') {
        var sellingPrice = document.getElementById("selling_price"+curr_row)?.value;
        if (sellingPrice && sellingPrice !== '') {
            document.getElementById("regular_price"+curr_row).value = sellingPrice;
        }
    }
    ...
    var bill_discount = 0.00,hdn_bill_discount = 0.00, netTotal = 0.00, afterdis_final = 0.00, total_after_discount = 0.00;
    var afterdis_final = document.getElementById("afterdis_final").innerHTML;
   ...
   */
	...
	*/
    ...
    total_after_discount = parseFloat(netTotal)-parseFloat(sum_hdn_cal_discount);
    // document.getElementById("hdn_total_after_bill_discount").value =  total_after_discount; // Total หลังลดท้ายบิล
    ...
    if(discount_checks[1]!=undefined && discount_checks[1].checked == true)
    {
        ...
        if(curr_row == '_final')
        {
            ...
            // hdn_bill_discount = eval(total_after_discount)*eval(discount_percentage_final_value)/eval(100);
        }
        else//This is to calculate the product discount
        {
            var discount_percentage_value = document.getElementById("discount_percentage"+curr_row).value;
            ...
            // hdn_bill_discount = eval(document.getElementById("productTotal"+curr_row).innerHTML)*eval(discount_percentage_value)/eval(100);
        }

        var discount_percentage_final = document.getElementById("discount_percentage"+curr_row).value;
            if(discount_percentage_final == '') discount_percentage_final = 0.00;

        // console.log("total_after_discount ",total_after_discount);   
        // console.log("discount_percentage_final ",discount_percentage_final);   
        hdn_bill_discount = eval(total_after_discount)*eval(discount_percentage_final)/eval(100);

        // console.log("Total After Discount Exc", total_after_discount);
        // console.log("Total After Discount Inc", afterdis_final);
        // console.log("ส่วนลดท้ายบิล", bill_discount);
        // console.log("ส่วนลดท้ายบิล hdn", hdn_bill_discount);
        ...
        document.getElementById("hdn_bill_discount").value = roundValue(hdn_bill_discount);
    }
    if(discount_checks[2]!=undefined && discount_checks[2].checked == true)
    {
        document.getElementById("discount_type"+curr_row).value = 'amount';
...
function calcGroupTax()
{
    ...
    {
        var group_tax_percentage = document.getElementById("group_tax_percentage"+i)?.value || 0;
...
function moveUpDown(sType,oModule,iIndex)
{
    ...
    var aFieldIds = Array('hidtax_row_no','productCodeView','productcode','productName','subproduct_ids','hdnProductId','comment','product_finish','product_size_mm','product_thinkness','competitor_brand','competitor_price','compet_brand_in_proj','compet_brand_in_proj_price','qty','product_unit','product_cost_avg','selling_price','selling_price_product','selling_price_delivery','product_price_type','package_size_sheet_per_box','package_size_sqm_per_box','box_quantity','sheet_quantity','sqm_quantity','regular_price','product_discount','pricelist_type','sales_unit','hdn_cal_total','hdn_cal_discount','product_dummy');
        ...
        if (x == undefined) {
            ...
            for (i = 0; i < x; i++) {
                if (document.selectall.selected_id[i].checked) {
                    ...
                    var product_dummy = prod_array['product_dummy'];
                    ...
                    if(return_module == 'Quotes'){
                        var qtyinstk = prod_array['qtyinstk'];
                        set_return_inventory_quotation(prod_id, prod_code, prod_name, unit_price, qtyinstk, taxstring, parseInt(row_id), desc, subprod_ids, uom, '', '', '', return_module, '', '', '', listprice_inc,product_dummy);
                    }else if(return_module == 'Salesorder'){
...
function selectDeal(check,type,frmName,return_module='',fldname)
{	
	var record = document.getElementsByName("record")[0].value;
  	
	projectsid = frmName.projectsid.value;
	if(projectsid == 0){
		projectsid ='';
	}

	record_id = projectsid;
	module_string = "&parent_module=Projects";
	
	if(record_id != ''){
    	window.open("index.php?module=Deal&action=Popup&html=Popup_picker&popuptype=specific&form=EditView"+module_string+"&relmod_id="+record_id+"&return_module="+return_module+"&field="+fldname,"test","width=1000,height=602,resizable=0,scrollbars=0");
    }else{
        alert("กรุณาเลือก Project ก่อน เพื่อแสดงรายการ Deal ที่เกี่ยวข้อง");
    }
