...
function premiumPickList(currObj,module, row_no) {
	var trObj=currObj.parentNode.parentNode
    ...
    if(document.getElementsByName("account_id").length != 0)
    	record_id= document.EditView.account_id.value;
    if(record_id != '')
    	window.open("index.php?module=Premium&action=Popup&html=Popup_picker&select=enable&form=HelpDeskEditView&popuptype="+popuptype+"&curr_row="+rowId+"&relmod_id="+record_id+"&parent_module=Accounts&return_module="+module+"&currencyid=","productWin","width=640,height=600,resizable=0,scrollbars=0,status=1,top=150,left=200");
    else
		window.open("index.php?module=Premium&action=Popup&html=Popup_picker&select=enable&form=HelpDeskEditView&popuptype="+popuptype+"&curr_row="+rowId+"&return_module="+module+"&currencyid=","productWin","width=640,height=600,resizable=0,scrollbars=0,status=1,top=150,left=200");
		//alert(popuptype);
}
...
function validateInventory(module) 
{
	...
	if(module == 'Premium' || module == 'Products' || module == 'Vendors' || module == 'PriceBooks' || module == 'Services')
	{
		return true;
	}
...
function fnAddProductRow(module,image_path){
	...
	coltwo.innerHTML= '<table border="0" cellpadding="1" cellspacing="0" width="100%"><tr><td class="small"><input id="productName'+count+'" name="productName'+count+'" class="small" style="width: 70%;" value="" readonly="readonly" type="text">'+
						...
						'&nbsp;&nbsp;&nbsp;&nbsp;<img id="searchIcon'+count+'" title="Premium" src="themes/images/products.gif" style="cursor: pointer;" onclick="premiumPickList(this,\''+module+'\','+count+')" align="absmiddle">'+
	...
	colthree.innerHTML='<input id="premium_code'+count+'" name="premium_code'+count+'" type="text" class=\'detailedViewTextBox\'" onBlur="this.className=\'detailedViewTextBox\'" onFocus="this.className=\'detailedViewTextBoxOn\'"  value="" style="width:100px"/>';

	colfour.className = "crmTableRow small"
	colfour.innerHTML='<input id="score'+count+'" name="score'+count+'" type="text" class=\'detailedViewTextBox\'" onBlur="this.className=\'detailedViewTextBox\'" onFocus="this.className=\'detailedViewTextBoxOn\'"  value="" style="width:100px"/>';
...
{//alert("44");
	var aFieldIds = Array('premium_code','score','hidtax_row_no','productName','subproduct_ids','hdnProductId','comment','qty','listPrice','discount_type','discount_percentage','discount_amount','tax1_percentage','hidden_tax1_percentage','popup_tax_row','tax2_percentage','hidden_tax2_percentage','lineItemType');
