...
//    document.getElementById('discountTotal_final').value = roundValue(sum);
   ...
   if(pricetype=="Include Vat"){
        // Discount
        sum_hdn_cal_discount = eval(sum_hdn_cal_discount)/1.07;
        document.getElementById("discountTotal_final").value = roundValue(sum_hdn_cal_discount);
        // alert(sum_netPrice);

        // Total After Discount
        var afterdis_final = 0;
        afterdis_final = eval(sum_netPrice)/1.07;
        console.log("Total After Discount = ",sum_netPrice,"/1.07 = ",afterdis_final);
        document.getElementById("afterdis_final").innerHTML = roundValue(afterdis_final);
        

         // TOTAL PRICE
         var netTotal = 0;
             netTotal = eval(sum_hdn_cal_discount) + eval(afterdis_final);
         document.getElementById("netTotal").innerHTML = roundValue(netTotal);
         document.getElementById("subtotal").value = roundValue(netTotal);
   }else{
        // TOTAL PRICE
...
function populateSelect(id,data) {
    ...
    data.forEach(option => {
        ...
        if (id.startsWith('pricelist_type')) {
            // Set "ราคาต่อหน่วย" as the selected option
            if (option.name === "ราคาต่อหน่วย") {
