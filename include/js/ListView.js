...
function mass_sms(module) {
   ...
   if(count > 1) {
      ...
      //alert(idstring);
      var url = 'smssend.php?crmid='+idstring+'&module='+module;
      var fs = window.open(url,'SMS','resizable=no,left=200,top=100,width=520,height=300,toolbar=no,scrollbars=yes,menubar=no,location=no');
   fs.focus();
   return true;
...
function showDefaultCustomView(selectView,module,parenttab)
{
	...
	new Ajax.Request(
                ...
                {queue: {position: 'end', scope: 'command'},
                       	method: 'post',
                        postBody:"module="+module+"&action="+module+"Ajax&file=ListView&ajax=true&start=1&viewname="+viewName+"&parenttab="+parenttab+padding_url,
                        onComplete: function(response) {
