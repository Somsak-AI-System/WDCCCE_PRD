...
{
	if(fieldLabel === 'รหัสลูกค้า SAP') return false;
      var mouseArea="";
...
function dtlViewAjaxSave(fieldLabel,module,uitype,tableName,fieldName,crmId)
{
	...
	{
		if(fieldName == 'termno'){
			tagValue = document.getElementById(txtBox).value;
		}else{
	...
	new Ajax.Request(
		'index.php',
                {queue: {position: 'end', scope: 'command'},
					...
					}else if(response.responseText.indexOf(":#:MAXTAG") > -1){
						$("vtbusy_info").style.display="none";
						
						$("tagCloudDisplay").style.border='1px solid #ff7488';
						$("tagCloudDisplay").style.borderRadius ='5px';
						$("error-tag").style.display='block';
