...
class freetag {
	...
	/**
	 * @access private
	 * @param string The prefix of freetag database aicrm_tables.
	 */
	var $_table_prefix = 'aicrm_';
	/**
	 * @access private
	...
	/**
	 * @access private
	 * @param string Whether to prevent multiple aicrm_users from tagging the same object. By default, set to block (ala Upcoming.org)
	 */
	 ...
	 * - aicrm_table_prefix: If you wish to create multiple Freetag databases on the same database, you can put a prefix in front of the aicrm_table names and pass separate prefixes to the constructor. [default: '']
	 ...
	 * - block_multiuser_tag_on_object: Set to 0 in order to allow individual aicrm_users to all tag the same object with the same tag. Default is 1 to only allow one occurence of a tag per object. [default: 1]
...
/*
	 ...
	 * tagArray parameter. Use this to provide tag combo services to your aicrm_users.
	 *
	...
	function get_tags_on_object($object_id, $offset = 0, $limit = 10, $tagger_id = NULL) {
		...
		$sql = "SELECT DISTINCT tag, raw_tag, tagger_id, id
			...
			$limit_sql
			";
		//echo ' <br><br>get_tags_on_object sql is ' .$sql; print_r($params);
		$rs = $adb->pquery($sql, $params) or die("Error: $sql");
		$retarr = array();
		while(!$rs->EOF) {
			$retarr[] = array(
	...
	function safe_tag($tagger_id, $object_id, $tag, $module) {
		...
		// Dynamically switch between allowing duplication between aicrm_users on the constructor param 'block_multiuser_tag_on_object'.
		...
		if(!$rs->EOF) {
			...
			// Add new tag! 
			$tag_id = $adb->getUniqueId('aicrm_freetags');
	 ...
	 * @param string The raw string form of the tag to delete. See above for aicrm_notes.
	 *
	 * @return string Returns the tag in normalized form.
	 */ 
	 ...
	 * @return string Returns the tag in normalized form.
	 */
	function tag_object($tagger_id, $object_id, $tag_string, $module,$list_max_tags, $skip_updates = 1) {
		if($tag_string == '') {
		...
		$sum_tag = (count($oldTags)+count($tagArray));
		//echo $sum_tag;
		
		if($sum_tag > $list_max_tags){
			return false;	
		}
		$preserveTags = array();
	...
	function _parse_tags($tag_string) {
		...
		foreach ($words as $key => $word)
		{
			...
			if (($delim % 2 == 1) && $words[$key - 1] == '"') {
				...
				$newwords = array_merge($newwords, preg_split('/,/', $word, -1, PREG_SPLIT_NO_EMPTY));
			}
		}
		return $newwords;
	}
	...
	function get_tag_cloud_html($module="",$tagger_id = NULL,$obj_id= NULL,$num_tags = 100, $min_font_size = 12, $max_font_size = 20, $font_units = 'px', $span_class = '', $tag_page_url = '/tag/') {
		global $theme;
		$theme_path="themes/".$theme."/";
		$image_path=$theme_path."images/";	
		$tag_list = $this->get_tag_cloud_tags($num_tags, $tagger_id,$module,$obj_id);
		//echo "<pre>"; print_r($tag_list); echo "</pre>";
		...
		{	
			foreach($tag_list[0] as $tag => $qty) {
				...
				$cloud_span[] = '<span id="tag_'.$tag_list[1][$tag].'" style="background-color: #a0a0a024;border-radius: 50px;height: 30px;vertical-align: middle;display: inline-flex;margin-bottom:5px;" class="' . $span_class . '" onMouseOver=$("tagspan_'.$tag_list[1][$tag].'").style.display="inline"; onMouseOut=$("tagspan_'.$tag_list[1][$tag].'").style.display="none";><a class="tagit" href="index.php?module=Home&action=UnifiedSearch&search_module='.$module.'&query_string='. urlencode($tag) . '" style="color: #000;padding: 0.6rem 1rem 0.6rem 1rem;font-size: '. $size . $font_units . '">' . htmlspecialchars(stripslashes($tag)) . '</a><span class="'. $span_class .'" id="tagspan_'.$tag_list[1][$tag].'" style="display:none;cursor:pointer;margin-top:11px;margin-right: 5px;" onClick="DeleteTag('.$tag_list[1][$tag].','.$obj_id.');"><img src="' . aicrm_imageurl('del_tag.gif', $theme) . '"></span></span>';

				/*$cloud_span[] = '<span id="tag_'.$tag_list[1][$tag].'" style="background-color: #a0a0a024;border-radius: 50px;height: 30px;vertical-align: middle;display: inline-flex;" class="' . $span_class . '" onMouseOver=$("tagspan_'.$tag_list[1][$tag].'").style.display="inline"; onMouseOut=$("tagspan_'.$tag_list[1][$tag].'").style.display="none";><a class="tagit" href="index.php?module=Home&action=UnifiedSearch&search_module='.$module.'&query_string='. urlencode($tag) . '" style="color: #000;padding: 0.6rem 1rem 0.6rem 1rem;font-weight: 600;font-size: '. $size . $font_units . '">' . htmlspecialchars(stripslashes($tag)) . '</a><span class="'. $span_class .'" id="tagspan_'.$tag_list[1][$tag].'" style="display:none;cursor:pointer;" onClick="DeleteTag('.$tag_list[1][$tag].','.$obj_id.');"><img src="' . aicrm_imageurl('del_tag.gif', $theme) . '"></span></span>';*/
			}
		}else
		{
			foreach($tag_list[0] as $tag => $qty) {
				//echo $qty; echo "<br>";
				//$size = $min_font_size + ($qty - $min_qty) * 3;
				$size = $min_font_size + (1 - $min_qty) * 3;
				$cloud_span[] = '<span class="' . $span_class . '" style="background-color: #a0a0a024;border-radius: 50px;height: 30px;vertical-align: middle;display: inline-flex;margin-bottom: 5px;"><a class="tagit" href="index.php?module=Home&action=UnifiedSearch&search_module='.$module.'&query_string='. urlencode($tag) . '" style="color: #000;padding: 0.5rem 1rem 0.5rem 1rem;font-size: '. $size . $font_units . '">' . htmlspecialchars(stripslashes($tag)) . '</a></span>';
			}
	 ...
	 * Confusing? Think of it like e-commerce's "Other aicrm_users who bought this also bought," 
