...
/**
 * Function to get aicrm_field typename
 * @param $uitype :: uitype -- Type integer
 * returns the aicrm_field type name -- Type string
 */
function getCustomFieldTypeName($uitype)
{
	...
	elseif($uitype == 58 || $uitype == 942)
	{
		$fldname = $mod_strings['LBL_SPECIAL'];//special
	}
	elseif($uitype == 85)
	{
...
//ekk========================================================================================	
	elseif($uitype == 901)
	{
		$fldname = $mod_strings['PickList_product'];
	}
	elseif($uitype == 902)
	{
		$fldname = $mod_strings['PickList_Account'];
	}
	elseif($uitype == 903)
	{
		$fldname = $mod_strings['PickList_User'];
	}
//ekk========================================================================================	
...
/**
 * Function to get custom aicrm_fields
 * @param $module :: aicrm_table name -- Type string
...
function getCustomFieldArray($module)
{
	...
	global $adb;
	$custquery = "select tablename,fieldname from aicrm_field where tablename=? and aicrm_field.presence in (0,2) order by tablename";
	$custresult = $adb->pquery($custquery, array('aicrm_'.strtolower($module).'cf'));
	$custFldArray = Array();
...
/**
 * Function to get columnname and aicrm_fieldlabel from aicrm_field aicrm_table
 * @param $module :: module name -- Type string
 * @param $trans_array :: translated column aicrm_fields -- Type array
...
function getCustomFieldTrans($module, $trans_array)
{
	...
	$custquery = "select columnname,fieldlabel from aicrm_field where generatedtype=2 and aicrm_field.presence in (0,2) and tabid=?";
...
/**
 * Function to get customfield record from aicrm_field aicrm_table
 * @param $tab :: Tab ID -- Type integer
 * @param $datatype :: aicrm_field name -- Type string
 * @param $id :: aicrm_field Id -- Type integer
...
function getCustomFieldData($tab,$id,$datatype)
{
	...
	global $adb;
	$query = "select * from aicrm_field where tabid=? and fieldid=? and aicrm_field.presence in (0,2)";
 ...
 * @param $label :: aicrm_field typename -- Type string
 * @param $typeofdata :: datatype -- Type string
 * returns the aicrm_field type,length,decimal
...
//ekk========================================================================================	
	elseif($label == $mod_strings['PickList_product'])
	{
		$fieldtype = '13';
	}
	elseif($label == $mod_strings['PickList_Account'])
	{
		$fieldtype = '14';
	}
	elseif($label == $mod_strings['PickList_User'])
	{
		$fieldtype = '15';
	}
//ekk========================================================================================	
...
function getCalendarCustomFields($tabid,$mode='edit',$col_fields='') {
	...
	$custparams = array('aicrm_activitycf', $tabid);
	
	if($is_admin == true || $profileGlobalPermission[1] == 0 || $profileGlobalPermission[2] == 0) {
		$custquery = "select * from aicrm_field where tablename=? AND aicrm_field.tabid=? ORDER BY fieldid";		
	} else {
		$profileList = getCurrentUserProfileList();
 		$custquery = "SELECT aicrm_field.* FROM aicrm_field" .
 				...
 				" WHERE aicrm_field.tablename=? AND aicrm_field.tabid=? AND aicrm_profile2field.visible=0" .
 				" AND aicrm_def_org_field.visible=0 AND aicrm_profile2field.profileid IN (". generateQuestionMarks($profileList) .")" .
 				" ORDER BY aicrm_field.fieldid";
