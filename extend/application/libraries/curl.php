...
class Curl {
    ...
    public function simple_post($url, $params = array(), $options = array(),$format=null)
    {
		...
		if(!$response){
			...
			//$response->Message = "Error  ".$this->error_code ." :: ". $this->error_string;
			$a_response["IsError"] = false;
			$a_response["Message"] =  "Error  ".$this->error_code ." :: ". $this->error_string;
			/*
			if($this->error_code=="28"){
				$a_response["Message"] =  "Error  ".$this->error_code ." :: ".  "Timeout";
			}else 	if($this->error_code=="7"){
				$a_response["Message"] =  "Error  ".$this->error_code ." :: ". "Connect Failed";
			}else {
				$a_response["Message"] = "Error  ".$this->error_code ." :: ". $this->error_string;
			}*/
    ...
    public function execute()
    {
        ...
        if(!isset($this->options[CURLOPT_TIMEOUT]))           $this->options[CURLOPT_TIMEOUT] = 120;
        ...
        if(!isset($this->options[CURLOPT_SSL_VERIFYHOST]))       $this->options[CURLOPT_SSL_VERIFYHOST] = FALSE;
