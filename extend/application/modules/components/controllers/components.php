...
class components extends MX_Controller {
	...
	public function GetSaleTeam(){
		...
		if($is_admin == "on"){
			$sql = "SELECT roleid AS id,rolename FROM aicrm_role ORDER BY parentrole ASC";
		}else{
			$sql = "SELECT roleid AS id,rolename FROM aicrm_role WHERE (parentrole like '".$parentrole."::%' OR roleid = '".$roleid."') ORDER BY parentrole ASC";
		}
		$query = $this->db->query($sql);
		$a_response =  $query->result_array();
		echo json_encode($a_response);
	...
	public function QuoteNo()
	{
		...
		if($this->input->get_post('quote_no_from')!=''){
			$s_quote_no = $this->input->get_post('quote_no_from');
		}elseif($this->input->get_post('quote_no_to')!=''){
			$s_quote_no = $this->input->get_post('quote_no_to');
		}else{
	...
	{
		$sql = "SELECT quotation_typeid, quotation_type FROM aicrm_quotation_type WHERE presence=1 and quotation_type != '--None--' ORDER BY CONVERT(`aicrm_quotation_type`.`quotation_type` USING tis620) ";
		$query = $this->db->query($sql);
		$a_data =  $query->result_array();
		echo json_encode($a_data);
	}
