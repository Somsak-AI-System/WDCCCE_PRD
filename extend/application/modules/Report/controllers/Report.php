       ...
       /* $query = $this->report_model->getReportsQuery($data);
        ...
        if($report['reportcharttype'] == 'line-basic'){
            ...
            if($report['reportid'] == '42'){
                ...
                //ifnull(aicrm_leaddetails.leadsource,'ไม่ระบุ') as leadsource , 
                ...
                $leadsource = array();
                $this->load->model("Report_model");
                $a_leadsource = $this->Report_model->get_leadsourcepicklist(); //alert($a_leadsource); exit;
                
                foreach ($a_leadsource as $key => $value)
                {
                    array_push($leadsource, $value['leadsource']);
                }
                //alert($leadsource); exit;
                array_push($leadsource, '');

                
                //$leadsource = ['เฟสบุ๊ค','ไลน์ออฟฟิเชียล','ไลน์ส่วนตัว','เฟสบุ๊คส่วนตัว','สาขา','อื่นๆ','ไม่ระบุ',''];
                /*เฟสบุ๊ค
                ไลน์ออฟฟิเชียล
                ไลน์ส่วนตัว
                เฟสบุ๊คส่วนตัว
                ...
                $leadstatus = array();
                $this->load->model("Report_model");
                $a_leadstatus = $this->Report_model->get_leadstatuspicklist(); //alert($a_leadsource); exit;
                foreach ($a_leadstatus as $key => $value)
                {
                    array_push($leadstatus, $value['leadstatus']);
                }
                array_push($leadstatus, '');
                ...
                foreach ($set_temp as $k_temp => $v_temp) {
                    ...
                    foreach ($v_temp as $k_month => $v_month) {
                        ...
                        foreach ($leadstatus as $k_cat) {
                            /*if($k_cat == ''){
                                $temp[$k_temp][$k_month]['ไม่ระบุ'] = ['record'=> 0, 'leadstatus'=> 'ไม่ระบุ', 'month'=> $k_month, 'year'=> $k_temp ,'name'=>$month[$k_month]." ".$k_temp];
                            }else{
