...
<div id="smartwizard" class="sw-main sw-theme-arrows">
    <ul class="sw_arraw">
        ...
        <li><a href="#step-1">2. Select Columns</a></li>
        <li><a href="#step-2">3. Filters</a></li>
    </ul>
        ...
        <div id="step-1">
            <div class="step_box">
                <form id="form-step-2" data-role="validator">
                    <div class="row form-group">
                        <div class="col-sm-12">
                            <h3>Select Columns (MAX 25)</h3>
                            <select class="form-control" id="select_columns" name="select_columns" style="width:100%">
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="row form-group">
                        <div class="col-sm-5">
                            <h3>Group By</h3>
                            <input class="form-control select_group_by" id="select_group_by1" style="width: 100%;">
                        </div>
                        <div class="col-sm-5">
                            <h3>Set Order</h3>
                            <input type="radio" name="order1" id="order1_asc"> <label for="order1_asc">Ascending</label>
                            <input type="radio" name="order1" id="order1_desc"> <label for="order1_desc">Descending</label>
                        </div>
                    </div>

                    <div class="row form-group">
                        <div class="col-sm-5">
                            <input class="form-control select_group_by" id="select_group_by2" style="width: 100%;">
                        </div>
                        <div class="col-sm-5">
                            <input type="radio" name="order2" id="order2_asc"> <label for="order2_asc">Ascending</label>
                            <input type="radio" name="order2" id="order2_desc"> <label for="order2_desc">Descending</label>
                        </div>
                    </div>

                    <div class="row form-group">
                        <div class="col-sm-5">
                            <input class="form-control select_group_by" id="select_group_by3" style="width: 100%;">
                        </div>
                        <div class="col-sm-5">
                            <input type="radio" name="order3" id="order3_asc"> <label for="order3_asc">Ascending</label>
                            <input type="radio" name="order3" id="order3_desc"> <label for="order3_desc">Descending</label>
                        </div>
                    </div>
                </form>
            </div>
...
<script>
    $(function(){
        ...
        locaction_href = locaction_href.split('#');
        if((locaction_href[1]!='step-0' && locaction_href[1]!=undefined) && $.isEmptyObject(input_values)){
            window.location.href = '<?php echo site_url('report/detailreport'); ?>';
        }
        ...
        $("#select_columns").kendoMultiSelect({
        ...
        var btnFinish = $('<button></button>',{ id:'btn_finish'}).text('Finish')
            ...
            .on('click', function(){
                ...
                input_values['report_type'] = 'detailreport';
        ...
        $("#smartwizard").on("leaveStep", function(e, anchorObject, stepNumber, stepDirection) {
            ...
            if(stepDirection === 'forward' && elm_form){
                ...
                if(stepNumber == 0){
                    ...
                    $.post(site_url('report/getBlocksFields'), selected_module ,function(rs){
                        var select_columns = $('#select_columns').data("kendoMultiSelect");
                        var dataSource = new kendo.data.DataSource({
                            data: rs,
                            group: { field:"blocklabel" }
                        });
                        select_columns.setDataSource(dataSource);
                    },'json');
                    ...
                    $.post('<?php echo site_url('report/getGroupbyFields'); ?>', selected_module, function(rs){
                        ...
                        $('.select_group_by').each(function(i,e){
                            var div_id = $(e).attr('id');
                            if(div_id != undefined){
                                var select_group = $('#'+div_id).data("kendoComboBox");
