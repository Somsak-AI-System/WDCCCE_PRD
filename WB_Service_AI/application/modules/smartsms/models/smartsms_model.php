<?php
class Smartsms_model extends CI_Model
{
  var $ci;
   ...
   function get_total($a_condition=array())
  {
  	try {
  		if (!empty($a_condition)) {
  			...
  			$a_condition["aicrm_crmentity.setype"] = "SmartSms";
  			$a_condition["aicrm_crmentity.deleted"] = "0";
  		}

  		$this->db->select("count(DISTINCT aicrm_smartsms.smartsmsid) as total");
  		$this->db->join('aicrm_smartsmscf', 'aicrm_smartsmscf.smartsmsid = aicrm_smartsms.smartsmsid',"inner");
  		$this->db->join('aicrm_crmentity', 'aicrm_crmentity.crmid = aicrm_smartsms.smartsmsid',"inner");
  		

      $query = $this->db->get('aicrm_smartsms');
  		if(!$query){
