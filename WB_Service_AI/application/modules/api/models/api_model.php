<?php
class api_model extends CI_Model
{
  var $ci;

  /**
   */
  function __construct()
  {
    parent::__construct();
    $this->ci = & get_instance();
    $this->_limit = 100;
  }

   function get_total($a_condition=array())
  {
  	try {
  		if (!empty($a_condition)) {
  			$this->db->where($a_condition);
  		}else{
  			$a_condition["aicrm_crmentity.setype"] = "Contacts";
  			$a_condition["aicrm_crmentity.deleted"] = "0";
  		}

  		$this->db->select("count(DISTINCT aicrm_contactdetails.contactid) as total");
  		$this->db->join('aicrm_contactscf', 'aicrm_contactscf.contactid = aicrm_contactdetails.contactid');
		$this->db->join('aicrm_contactsubdetails', 'aicrm_contactsubdetails.contactsubscriptionid = aicrm_contactdetails.contactid',"left");
		$this->db->join('aicrm_contactaddress', 'aicrm_contactaddress.contactaddressid = aicrm_contactdetails.contactid' ,"left");
  		$this->db->join('aicrm_crmentity', 'aicrm_crmentity.crmid = aicrm_contactdetails.contactid');
  		$query = $this->db->get('aicrm_contactdetails');
  		if(!$query){
  			$a_return["status"] = false;
  			$a_return["error"] = $this->db->_error_message();
  			$a_return["result"] = "";
  		}else{
  			$a_result  = $query->result_array() ;

  			if (!empty($a_result)) {
  				$a_return["status"] = true;
  				$a_return["error"] =  "";
  				$a_return["result"] = $a_result[0];
  			}else{
  				$a_return["status"] = false;
  				$a_return["error"] =  "No Data";
  				$a_return["result"] = "";
  			}
  		}
  	}catch (Exception $e) {
  		$a_return["status"] = false;
  		$a_return["error"] =  $e->getMessage();
  		$a_return["result"] = "";
  	}
  	return $a_return;
  }


  function get_list($a_condition=array(),$a_order=array(),$a_limit=array())
  {
  	try {
  		$a_condition["aicrm_crmentity.setype"] = "Contacts";
  		$a_condition["aicrm_crmentity.deleted"] = "0";
		
		$a_select = $this->crmentity->Get_Field_Show("4",'');//tabid Contacts 4
  		
		//print_r($a_select); exit;
		$a_field = $this->crmentity->Get_Select_Field($a_select);
  		//print_r($a_field); exit;
  		if($a_field!=""){
  			$a_field_check = "aicrm_contactdetails.contactid," .$a_field;
  			$this->db->select($a_field_check);
  		}
		
  		$this->db->join('aicrm_contactscf', 'aicrm_contactscf.contactid = aicrm_contactdetails.contactid');
		$this->db->join('aicrm_contactsubdetails', 'aicrm_contactsubdetails.contactsubscriptionid = aicrm_contactdetails.contactid',"left");
		$this->db->join('aicrm_contactaddress', 'aicrm_contactaddress.contactaddressid = aicrm_contactdetails.contactid' ,"left");
  		$this->db->join('aicrm_crmentity', 'aicrm_crmentity.crmid = aicrm_contactdetails.contactid');
		$this->db->join('aicrm_account', 'aicrm_account.accountid = aicrm_contactdetails.accountid',"left");

   		if (!empty($a_condition)) {
  			$this->db->where($a_condition);
  		}
  		if (!empty($a_order)) {
  			for($i=0;$i<count($a_order);$i++){
  				$this->db->order_by($a_order[$i]["field"], $a_order[$i]["order"]);
  			}
  		}
  		if (empty($a_limit)) {
  			$a_limit["limit"] = $this->_limit;
  			$a_limit["offset"] = 0;
  			$this->db->limit($a_limit["limit"],$a_limit["offset"]);
  		}else if($a_limit["limit"]==0){

  		}else{
  			$this->db->limit($a_limit["limit"],$a_limit["offset"]);
  		}
  		$query = $this->db->get('aicrm_contactdetails');
		
		/*echo $this->db->last_query();
	    exit;*/


  		if(!$query){
  			$a_return["status"] = false;
  			$a_return["error"] = $this->db->_error_message();
  			$a_return["result"] = "";
  		}else{
  			$a_result  = $query->result_array() ;

  			$a_total = $this->get_total($a_condition) ;
  			$a_data["offset"] = $a_limit["offset"];
  			$a_data["limit"] = $a_limit["limit"];
  			$a_data["total"] = ($a_total["status"]) ? $a_total["result"]["total"] : 0;
  			$a_data["data"] = $a_result;
  			if (!empty($a_result)) {
  				$a_return["status"] = true;
  				$a_return["error"] =  "";
  				$a_return["result"] = $a_data;
  			}else{
  				$a_return["status"] = false;
  				$a_return["error"] =  "No Data";
  				$a_return["result"] = "";
  			}
  		}
  		}catch (Exception $e) {
  			$a_return["status"] = false;
  			$a_return["error"] =  $e->getMessage();
  			$a_return["result"] = "";
  		}
  		return $a_return;
  }


  function get_total_template($a_condition=array())
  {
    try {
      if (!empty($a_condition)) {
        $this->db->where($a_condition);
      }else{
        $a_condition["aicrm_crmentity.setype"] = "questionnairetemplate";
        $a_condition["aicrm_crmentity.deleted"] = "0";
      }

      $this->db->select("count(DISTINCT aicrm_questionnairetemplate.questionnairetemplateid) as total");
      $this->db->join('aicrm_questionnairetemplatecf', 'aicrm_questionnairetemplatecf.questionnairetemplateid = aicrm_questionnairetemplate.questionnairetemplateid');
      $this->db->join('aicrm_crmentity', 'aicrm_crmentity.crmid = aicrm_questionnairetemplate.questionnairetemplateid');
      $query = $this->db->get('aicrm_questionnairetemplate');
      
      if(!$query){
        $a_return["status"] = false;
        $a_return["error"] = $this->db->_error_message();
        $a_return["result"] = "";
      }else{
        $a_result  = $query->result_array() ;

        if (!empty($a_result)) {
          $a_return["status"] = true;
          $a_return["error"] =  "";
          $a_return["result"] = $a_result[0];
        }else{
          $a_return["status"] = false;
          $a_return["error"] =  "No Data";
          $a_return["result"] = "";
        }
      }
    }catch (Exception $e) {
      $a_return["status"] = false;
      $a_return["error"] =  $e->getMessage();
      $a_return["result"] = "";
    }
    return $a_return;
  }


  function get_template($a_condition=array(),$a_order=array(),$a_limit=array())
  {

    try {
      $a_condition["aicrm_crmentity.setype"] = "questionnairetemplate";
      $a_condition["aicrm_crmentity.deleted"] = "0";
    
    
      $this->db->select("aicrm_questionnairetemplate.questionnairetemplateid as templateid, aicrm_questionnairetemplate.questionnairetemplate_name as name,aicrm_questionnairetemplate.scoring");
      $this->db->join('aicrm_questionnairetemplatecf', 'aicrm_questionnairetemplatecf.questionnairetemplateid = aicrm_questionnairetemplate.questionnairetemplateid');
      $this->db->join('aicrm_crmentity', 'aicrm_crmentity.crmid = aicrm_questionnairetemplate.questionnairetemplateid');

      if (!empty($a_condition)) {
        $this->db->where($a_condition);
      }
      if (!empty($a_order)) {
        for($i=0;$i<count($a_order);$i++){
          $this->db->order_by($a_order[$i]["field"], $a_order[$i]["order"]);
        }
      }

      $this->db->order_by("aicrm_questionnairetemplate.questionnairetemplateid", "desc");

      if (empty($a_limit)) {
        $a_limit["limit"] = $this->_limit;
        $a_limit["offset"] = 0;
        $this->db->limit($a_limit["limit"],$a_limit["offset"]);
      }else if($a_limit["limit"]==0){

      }else{
        $this->db->limit($a_limit["limit"],$a_limit["offset"]);
      }

      $query = $this->db->get('aicrm_questionnairetemplate');
    
      /*echo $this->db->last_query();
      exit;*/


      if(!$query){
        $a_return["status"] = false;
        $a_return["error"] = $this->db->_error_message();
        $a_return["result"] = "";
      }else{
        $a_result  = $query->result_array() ;

        $a_total = $this->get_total_template($a_condition) ;
        $a_data["offset"] = $a_limit["offset"];
        $a_data["limit"] = $a_limit["limit"];
        $a_data["total"] = ($a_total["status"]) ? $a_total["result"]["total"] : 0;
        $a_data["data"] = $a_result;
        if (!empty($a_result)) {
          $a_return["status"] = true;
          $a_return["error"] =  "";
          $a_return["result"] = $a_data;
        }else{
          $a_return["status"] = false;
          $a_return["error"] =  "No Data";
          $a_return["result"] = "";
        }
      }
      }catch (Exception $e) {
        $a_return["status"] = false;
        $a_return["error"] =  $e->getMessage();
        $a_return["result"] = "";
      }
      return $a_return;
  }

}