...
class Indexselect_model extends CI_Model
{
   ...
   function get_total($search_module,$where_user="")
  {
  	try {
		...
		$select=$this->crmentity->Get_QuerySelect($search_module);
	...
	try {
		...
		list($total,$data)  = $this->get_query($module,$search_module,$like,$id,$section,$relate_field,$where_user,$limit);
    ...
    foreach ($relate_field as $key => $value) {
	  ...
	  if ($relate_field['projectid']!="") {
		$projectid = $relate_field['projectid'];
	  }

	  if ($relate_field['dealid']!="") {
     ...
     if($search_module=="Folder"){
		...
		Case "Deal":
				if($like!=""){
					$like ="AND `aicrm_deals`.`deal_name` like '".$like."'";
					$id="";
				}else if($id!=""){
					$id = "AND  `aicrm_deals`.`dealid`= '".$id."'";
					$like ="";
				}else{
					$like ="";
					$id="";
		...
		ORDER BY `aicrm_deals`.`deal_name` asc ";
