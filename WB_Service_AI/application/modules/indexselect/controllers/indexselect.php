...
class Indexselect extends REST_Controller
{
  ...
  function __construct()
  {
	...
	private function set_param($a_param=array())
	{
		...
		if (isset($a_param["selectfield"]) && $a_param["selectfield"]!="" && isset($a_param["fieldname"]) && $a_param["fieldname"]!="") {
			$a_condition["selectfield"][$a_param["selectfield"]] =  "%".$a_param["fieldname"]."%" ;
		}
	...
	private function get_cache($a_params=array())
	{
		...
		$relatedcrmid = @$a_params['relatedcrmid'];
		...
		if($a_data===false)
		{
			$a_list=$this->Indexselect_model->get_list($a_condition,$a_order,$a_limit,$module,$search_module,$section,$relate_field,$userid,$a_params,$relatedcrmid);
	...
	public function list_select_post()
	{
		...
		if($search_module=="Case"){
			...
			$search_module = "Projectorder";
		}elseif ($search_module=="Project Order") {
			$search_module = "Projects";
		}
