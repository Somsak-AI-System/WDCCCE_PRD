...
class Knowledge_model extends CI_Model
{
   ...
   function get_total($a_condition=array())
  {
  	try {
  		if (!empty($a_condition)) {
  			...
  			$a_condition["aicrm_crmentity.setype"] = "KnowledgeBase";
  			$a_condition["aicrm_crmentity.deleted"] = "0";
  		}

  		$this->db->select("count(DISTINCT aicrm_knowledgebase.knowledgebaseid) as total");
  		$this->db->join('aicrm_knowledgebasecf', 'aicrm_knowledgebasecf.knowledgebaseid = aicrm_knowledgebase.knowledgebaseid',"inner");
  		$this->db->join('aicrm_crmentity', 'aicrm_crmentity.crmid = aicrm_knowledgebase.knowledgebaseid',"inner");
  		

      $query = $this->db->get('aicrm_knowledgebase');
  		if(!$query){
  ...
  function get_list($a_condition=array(),$a_order=array(),$a_limit=array(),$userid = 0)
  {
      ...
      $this->db->select("aicrm_knowledgebase.*, tbt_favorite.flag" );
      $this->db->join('aicrm_knowledgebasecf', 'aicrm_knowledgebasecf.knowledgebaseid = aicrm_knowledgebase.knowledgebaseid', "inner");
      $this->db->join('aicrm_crmentity', 'aicrm_crmentity.crmid = aicrm_knowledgebase.knowledgebaseid',"inner");
      $this->db->join('tbt_favorite', 'tbt_favorite.knowledgebaseid = aicrm_knowledgebase.knowledgebaseid and tbt_favorite.userid ='.$userid, "left");
