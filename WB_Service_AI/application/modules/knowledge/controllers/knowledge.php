...
class Knowledge extends REST_Controller
{
  ...
  private $crmid;
  private $tab_name = array('aicrm_crmentity','aicrm_knowledgebase','aicrm_knowledgebasecf','tbt_favorite');
  private $tab_name_index = array('aicrm_crmentity'=>'crmid','aicrm_knowledgebase'=>'knowledgebaseid','aicrm_knowledgebasecf'=>'knowledgebaseid','tbt_favorite'=>'knowledgebaseid');
	...
	private function set_param($a_param=array())
	{
		...
		if (isset($a_param["crmid"]) && $a_param["crmid"]!="") {
			$a_condition["aicrm_knowledgebase.knowledgebaseid"] =  $a_param["crmid"] ;
		}

		if (isset($a_param["knowledgebasename"]) && $a_param["knowledgebasename"]!="") {
			$a_condition["aicrm_knowledgebase.knowledgebase_name like"] = "%".$a_param["knowledgebasename"]."%";
		}

		// if (isset($a_param["userid"]) && $a_param["userid"]!="") {
		// 	$a_condition["aicrm_knowledgebase.userid like"] = "%".$a_param["userid"]."%";
		// }
	...
	{	
		...
		//echo 666;
		$userid = isset($a_params['userid']) && @$a_params['userid'] != '' ? $a_params["userid"] : 0;
		//$userid = isset(@$a_params['userid'] && @$a_params['userid'] != '') ? $a_params["userid"] : 0;
		//$userid = isset(@$a_params['userid']) ? $a_params["userid"] : 0;
		//isset($a_data["data"]["total"]) ? $a_data["data"]["total"]:0;
		//echo $userid ; exit;
		...
		if($a_data===false)
		{
			$a_list=$this->knowledge_model->get_list($a_condition,$a_order,$a_limit,$userid);
			//alert($a_list); exit();
	...
	public function insert_content_post(){
		...
		$response_data = $this->get_insert_data($a_request);
	  	...
	  	$this->common->_filename= "Insert_KnowledgeBase";
	...
	private function get_insert_data($a_request){
	  	...
	  	$data=isset($a_request['data']) ? $a_request['data'] : "";
	  		...
	  		if(count($data[0])>0 and $module=="KnowledgeBase"){
	...
	public function get_updeatcountkm($a_request=""){

		$crmid=isset($a_request['crmid']) ? $a_request['crmid'] : "";

		$sql ="UPDATE aicrm_knowledgebase SET knowledge_base_count_view = (knowledge_base_count_view+1)
			WHERE knowledgebaseid = ".$crmid ." ";
	...
	public function update_lead_post(){
		...
		$request_body = file_get_contents('php://input');
	  	...
	  	$response_data = null;
	  	$a_request =$dataJson; 
		$response_data = $this->get_updeatcountkm($a_request);
	...
	public function add_favoritekm($a_request=""){

		// alert($a_request); exit();

		$crmid=isset($a_request['crmid']) ? $a_request['crmid'] : "";
		$data = isset($a_request['data'][0]) ? $a_request['data'][0] : "";
		...
		if ($flag == 'true') {
			...
			$sql = "INSERT INTO tbt_favorite (userid, knowledgebaseid) VALUES (".$userid .",".$crmid .")" ;
			//echo $sql; exit;
		} else {

			$sql = "DELETE FROM tbt_favorite WHERE tbt_favorite.userid = ".$userid ." AND tbt_favorite.knowledgebaseid = ".$crmid ." ";
			//echo $sql; exit;
		}
	...
	public function favorite_post() {
		...
		$response_data = $this->add_favoritekm($a_request);
