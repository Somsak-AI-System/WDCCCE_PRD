...
class Scan extends REST_Controller
{
	...
	private function set_param($a_param=array())
	{
		...
		if (isset($a_param["code"]) && $a_param["code"]!="") {
			$a_condition["aicrm_voucher.voucher_no"] =  $a_param["code"] ;
		}
		return $a_condition;
	}
	...
	{	
		...
		if($a_data===false)
		{	
			$a_list = $this->scan_model->get_list($a_condition,$a_order,$a_limit,$a_params);
			$a_data = $this->get_data_url($a_list);
	...
	private function get_data_url($a_data)
	{	
		$url_web = $this->config->item('url_web');

		//alert($a_data);// exit;
		if($a_data['result'] != ''){
			//$config['url_web']
			//alert($a_data['result']['data'][0]['voucherid']); exit;
			$voucherid = $a_data['result']['data'][0]['voucherid'];
			$title = $a_data['result']['data'][0]['voucher_name'];
			$a_data['result']['data'][0] = array();
			$a_data['result']['data'][0]['title'] = $title;
			$a_data['result']['data'][0]['url'] = $url_web."?crmid=".$voucherid;
		}
		return $a_data;
	}
