...
class Announcement_model extends CI_Model
{
  ...
  function get_noti_list($a_condition=array(),$a_order=array(),$a_limit=array(),$a_param=array())
  {
    ...
    try {
      ...
      $this->db->select("aicrm_announcement.announcementid , ,aicrm_announcement.announcement_name, ai_notification.crmid , ai_notification.module , ai_notification.userid ,ai_notification.message ,ai_notification.flagread ,ai_notification.flagaccept , ai_notification.datesend as senddate , concat(ai_notification.timesend,':00') as sendtime  ,DATE_FORMAT(ai_notification.acceptdate, '%Y-%m-%d') as acceptdate, DATE_FORMAT(ai_notification.acceptdate, '%H:%i:%s') as accepttime ",false);
  ...
  function get_detail($a_condition=array(),$a_order=array(),$a_limit=array(),$a_param=array())
  {
    $this->load->library('crmentity');
    try {
      $a_condition["aicrm_crmentity.setype"] = "Announcement";
      ...
      $this->db->select("aicrm_announcement.announcementid,aicrm_announcement.announcement_no,aicrm_announcement.announcement_name,aicrm_announcement.detail, ai_notification.datesend as senddate , concat(ai_notification.timesend,':00') as sendtime ,DATE_FORMAT(ai_notification.readdate, '%Y-%m-%d') as readdate, DATE_FORMAT(ai_notification.readdate, '%H:%i:%s') as readtime ",false);
        //aicrm_announcement.senddate,aicrm_announcement.sendtime 
      $this->db->join('aicrm_announcementcf', 'aicrm_announcementcf.announcementid = aicrm_announcement.announcementid', "inner");
      $this->db->join('aicrm_crmentity', 'aicrm_crmentity.crmid = aicrm_announcement.announcementid',"inner");
      $this->db->join('aicrm_groups', 'aicrm_groups.groupid = aicrm_crmentity.smownerid',"left");
      $this->db->join('aicrm_users', 'aicrm_users.id = aicrm_crmentity.smownerid',"left");
