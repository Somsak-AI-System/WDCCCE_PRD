  ...
  //private $tab_name_index = array('aicrm_crmentity'=>'crmid','aicrm_account'=>'accountid','aicrm_accountbillads'=>'accountaddressid','aicrm_agenthipads'=>'accountaddressid','aicrm_agentcf'=>'accountid');
	...
	private function get_social($a_params=array())
    {
        ...
        if ($a_data === false) {
            ...
            $a_list=$this->agent_model->get_list($a_condition,$a_order,$a_limit,$a_params);
            //alert($a_list); exit;
    ...
    private function get_socialdetail($a_params=array())
    {
        ...
        if ($a_data === false) {
            ...
            if(isset($a_params['customerid']) && $a_params['customerid'] != ''){
            	...
            	$this->lib_socket->emit_socket($a_params['customerid'],'Agent');
            }
			$a_data = $this->get_data($a_list);
			$a_data["status"] = $a_list["status"];
			$a_data["error"] = $a_list["error"]  ;
    ...
    /*public function emit_socket($customerid = '',$displayName=''){
		...
		$data = $this->agent_model->get_list_socket($customerid); //alert($data['result']['data']); exit;
		//$data_message=$this->agent_model->get_list_detail_socket($customerid);//alert($data_message['result']['data']); exit;
	...
	public function searchmessage_post(){
		$this->common->_filename= "Agent_getsocial";
		$request_body = file_get_contents('php://input');
		$dataJson     = json_decode($request_body,true);
        $a_request = $dataJson;
        $url = "http://$_SERVER[HTTP_HOST]$_SERVER[REQUEST_URI]";
        $this->common->set_log('Get Search Message ==>',$url,$a_request);

        $a_data = $this->get_searchmessage($a_request);
        
        $this->return_data($a_data);
    }
	
	private function get_searchmessage($a_params=array())
    {
        if(!$this->oauth_server->verifyResourceRequest(OAuth2\Request::createFromGlobals())){
			$a_data['status'] = false;
            $a_data['error'] = 'Access Token not found';
        ...
        if ($a_data === false) {
            ...
            $a_list=$this->agent_model->get_list_search($a_condition,$a_order,$a_limit,$a_params);
			$a_data = $this->get_data($a_list);
			$a_data["status"] = $a_list["status"];
			$a_data["error"] = $a_list["error"]  ;
			$a_data["data"] = $a_list["result"];
	...
	private function set_param($a_param=array())
	{
		...
		if (isset($a_param["socialname"]) && $a_param["socialname"]!="") {
			...
			OR message_chathistory3.message LIKE '%".$a_param["socialname"]."%' 
			OR CONCAT (aicrm_users.first_name,aicrm_users.last_name) LIKE '%".$a_param["socialname"]."%' OR aicrm_freetags.tag LIKE '%".$a_param["socialname"]."%')"
			] = NULL ;
		}
		//aicrm_users
		...
		if (isset($a_param["socialid"]) && $a_param["socialid"]!="" && $a_param["socialid"]!="0") {
			$a_condition["aicrm_social_config.id"] = $a_param["socialid"] ;
		}


		if (isset($a_param["chatno"]) && $a_param["chatno"]!="") {
			$min = ($a_param["chatno"] -7);
			$max = ($a_param["chatno"] + 7);
			$this->db->where('message_chathistory.chatno >=', $min);
			$this->db->where('message_chathistory.chatno <=', $max);
		}
	...
	public function deleted_post(){
		...
		$response_data = $this->deleted_chat($a_request);
	  	
	  	$url = "http://$_SERVER[HTTP_HOST]$_SERVER[REQUEST_URI]";
	  	$this->common->_filename= "Deleted_Chat";
	...
	private function deleted_chat($a_request){
		...
		if(!$this->oauth_server->verifyResourceRequest(OAuth2\Request::createFromGlobals())){
	  	...
	  	$response_data = null;
	  	$module=isset($a_request['module']) ? $a_request['module'] : "";
	  	$customerid=isset($a_request['customerid']) ? $a_request['customerid'] : "";
	  	$action=isset($a_request['action']) ? $a_request['action'] : "";
	  	
	  	//if($module== "Agent" && $customerid != ''){
	  	if($customerid != ''){		
  				if($action=="deleted" && $customerid != ''){
	  				$sql = 'update message_customer set deleted = 1 where customerid = "'.$customerid.'"; ';
	  				$this->db->query($sql);
	  				$a_return["Message"] = "Deleted Complete";
	  				$a_return["data"] =array(
	...
	public function reply_post(){
		//echo false; exit;
	  	...
	  	$a_request =$dataJson; 
	  	//alert($a_request);exit;
		$response_data = $this->reply_chat($a_request);
	  	
	  	$url = "http://$_SERVER[HTTP_HOST]$_SERVER[REQUEST_URI]";
	  	$this->common->_filename= "Reply_Chat";
	...
	private function reply_chat($a_request){
		...
		if(!$this->oauth_server->verifyResourceRequest(OAuth2\Request::createFromGlobals())){
	  	...
	  	$module=isset($a_request['module']) ? $a_request['module'] : "";
	  	$customerid=isset($a_request['customerid']) ? $a_request['customerid'] : "";
	  	$reply=isset($a_request['reply']) ? $a_request['reply'] : "0";
	  	$action=isset($a_request['action']) ? $a_request['action'] : "";
	  		...
	  		if($module== "Agent" && $customerid != ''){
	  				...
	  				if($action=="edit" && $customerid != ''){
		  				$sql = 'update message_customer set auto_reply = "'.$reply.'" where customerid = "'.$customerid.'"; ';
		  				//echo $sql; exit;
		  				$this->db->query($sql);
	...
	public function get_assignto_post(){
		$this->common->_filename= "Agent_get_assignto";
		$request_body = file_get_contents('php://input');
		$dataJson     = json_decode($request_body,true);
        $a_request = $dataJson;
        $url = "http://$_SERVER[HTTP_HOST]$_SERVER[REQUEST_URI]";
        $this->common->set_log('Get Assignto==>',$url,$a_request);

        $a_data = $this->get_assignto($a_request);
        
        $this->return_data($a_data);
    }

    private function get_assignto($a_params=array())
    {
        if(!$this->oauth_server->verifyResourceRequest(OAuth2\Request::createFromGlobals())){
			$a_data['status'] = false;
            $a_data['error'] = 'Access Token not found';
        ...
        $p_condition = $this->set_param($a_params);
        $a_condition = array();
        ...
        if ($a_data === false) {
            ...
            $a_list=$this->agent_model->get_list_assignto($a_condition,$a_order,$a_limit,$a_params);
            
            $ass_list=$this->agent_model->get_assignto($a_condition,$a_order,$a_limit,$a_params);

			$a_data = $this->get_data_assignto($a_list,$ass_list['result']['data']);
    ...
    private function get_data_assignto($a_data,$ass_list)
	{	
		$data = array();
		$u_data = array();
		if($a_data['result'] != ''){
			$u_data = $a_data['result']['data'];

			$a_data['result']['data']= array();

			$a_data['result']['data']['assignto'] = $ass_list;
			$a_data['result']['data']['user'] = $u_data;	
		}
		
		return $a_data;
	}

	public function update_assignto_post(){
		//echo false; exit;
	  	$request_body = file_get_contents('php://input');
	  	$dataJson     = json_decode($request_body,true);
	  	...
	  	$a_request =$dataJson; 
	  	//alert($a_request);exit;
		$response_data = $this->update_assignto($a_request);
	  	
	  	$url = "http://$_SERVER[HTTP_HOST]$_SERVER[REQUEST_URI]";
	  	$this->common->_filename= "Update_Assignto";
	...
	private function update_assignto($a_request){
		...
		if(!$this->oauth_server->verifyResourceRequest(OAuth2\Request::createFromGlobals())){
	  	...
	  	$module=isset($a_request['module']) ? $a_request['module'] : "";
	  	$customerid=isset($a_request['customerid']) ? $a_request['customerid'] : "";
	  	$socialid=isset($a_request['socialid']) ? $a_request['socialid'] : "0";
	  	$userid=isset($a_request['userid']) ? $a_request['userid'] : "0";
	  	$assignto=isset($a_request['assignto']) ? $a_request['assignto'] : "0";
	  	$action=isset($a_request['action']) ? $a_request['action'] : "";
	  	$chat_status = isset($a_request['chat_status']) ? $a_request['chat_status'] : "";
	  		
	  		if($module== "Agent" && $customerid != ''){
	  				
	  				if($action=="edit" && $customerid != ''){
		  				$sql = 'update message_customer set smownerid = "'.$assignto.'" where customerid = "'.$customerid.'"; ';
		  				$this->db->query($sql);
