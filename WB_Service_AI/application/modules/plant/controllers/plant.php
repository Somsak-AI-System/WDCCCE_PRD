...
class Plant extends REST_Controller
{
  ...
  private $crmid;
  private $tab_name = array('aicrm_crmentity','aicrm_plant','aicrm_plantcf');
  private $tab_name_index = array('aicrm_crmentity'=>'crmid','aicrm_plant'=>'plantid','aicrm_plantcf'=>'plantid');
	...
	private function set_param($a_param=array())
	{
		...
		if (isset($a_param["crmid"]) && $a_param["crmid"]!="") {
			$a_condition["aicrm_plant.plantid"] =  $a_param["crmid"] ;
		}

		if (isset($a_param["plant_id"]) && $a_param["plant_id"]!="") {
			$a_condition["aicrm_troubletickets.plant_id like"] = "%".$a_param["plant_id"]."%";
		}

		if (isset($a_param["cityplant"]) && $a_param["cityplant"]!="") {
			$a_condition["aicrm_plant.province_plant"] =  $a_param["cityplant"];
		}
		
		if (isset($a_param["trucksizeplant"]) && $a_param["trucksizeplant"]!="") {
			$a_condition["aicrm_pricelists.truck_size"] =  $a_param["trucksizeplant"];
		}

		if (isset($a_param["mattypeplant"]) && $a_param["mattypeplant"]!="") {
			$a_condition["aicrm_pricelists.mat_type"] =  $a_param["mattypeplant"];
		}

		if (isset($a_param["strengthplant"]) && $a_param["strengthplant"]!="") {
			
			$strength = explode(",", $a_param["strengthplant"]);
			$this->db->where_in('aicrm_pricelists.strength', $strength);
		}
		//strength
		//truck_size
		return $a_condition;
	...
	private function get_cache($a_params=array())
	{
		...
		if($a_data===false)
		{
			$a_list=$this->plant_model->get_list($a_condition,$a_order,$a_limit);
	...
	private function get_cacheplant($a_params=array())
	{
		$this->load->library('managecached_library');
		$this->load->model("plant_model");
		...
		if($a_data===false)
		{
			$a_list=$this->plant_model->get_listplant($a_condition,$a_order,$a_limit);
			// alert($a_list); exit();
			$a_data = $this->get_data($a_list);
			$a_data["data"] = $a_data["result"];
			$a_data["limit"] = $a_limit["limit"]  ;
	...
	public function list_content_plantpricelist_get()
	{
		$a_param =  $this->input->get();
		$a_data =$this->get_cacheplant($a_param);
		$this->return_data($a_data);
	}
	
	public function list_content_plantpricelist_post()
	{
		// echo "hi"; exit();
		$request_body = file_get_contents('php://input');
		$a_param     = json_decode($request_body,true);
	...
	public function list_plant_post()
	{
		$request_body = file_get_contents('php://input');
		$a_param     = json_decode($request_body,true);
		
		$a_data =$this->get_plant($a_param);
		
		$this->return_data($a_data);
	}

	private function get_plant($a_params=array())
	{
		$this->load->library('managecached_library');
		$this->load->model("plant_model");
		...
		if($a_data===false)
		{
			$a_list=$this->plant_model->get_plant($a_condition,$a_order,$a_limit);
			$a_data = $this->get_data_plant($a_list);
			$a_data["data"] = $a_data["result"];
			$a_data["limit"] = $a_limit["limit"]  ;
			$a_data["offset"] = $a_limit["offset"]  ;
			$a_data["time"] = date("Y-m-d H:i:s");
	...
	private function get_data_plant($a_data)
	{	//alert($a_data); exit;
		if($a_data['result'] != ''){
			/*foreach($a_data['result']['data'] as $key => $val ){
				$sub_index = iconv_substr(@$val['accountname'],0,1,"UTF-8");
