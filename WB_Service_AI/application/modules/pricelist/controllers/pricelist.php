...
class Pricelist extends REST_Controller
{
  ...
  private $crmid;
  private $tab_name = array('aicrm_crmentity','aicrm_pricelists','aicrm_pricelistscf');
  private $tab_name_index = array('aicrm_crmentity'=>'crmid','aicrm_pricelists'=>'pricelistid','aicrm_pricelistscf'=>'pricelistid');
	...
	private function set_param($a_param=array())
	{
		...
		if (isset($a_param["crmid"]) && $a_param["crmid"]!="") {
			$a_condition["aicrm_pricelists.pricelistid"] =  $a_param["crmid"] ;
		}
	...
	private function get_cache($a_params=array())
	{
		...
		if($a_data===false)
		{
			$a_list=$this->pricelist_model->get_list($a_condition,$a_order,$a_limit);
			// alert($a_list); exit();
