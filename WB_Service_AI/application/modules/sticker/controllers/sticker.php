...
class Sticker extends REST_Controller
{
  private $crmid;
	...
	public function stickerpackage_post(){
		//$this->common->_filename= "Stickerpackage";
		$request_body = file_get_contents('php://input');
		$dataJson     = json_decode($request_body,true);
        $a_request = $dataJson;
        //$url = "http://$_SERVER[HTTP_HOST]$_SERVER[REQUEST_URI]";
        //$this->common->set_log('Get Sticker Package==>',$url,$a_request);

        $a_data = $this->get_package($a_request);
        
        $this->return_data($a_data);
    }
	

	private function get_package($a_params=array())
    {
        ...
        if ($a_data === false) {
            ...
            $a_list=$this->sticker_model->get_list_packageline($a_condition,$a_order,$a_limit);
    ...
    public function stickerlist_post(){
		//$this->common->_filename= "Stickerpackage";
		$request_body = file_get_contents('php://input');
		$dataJson     = json_decode($request_body,true);
        $a_request = $dataJson;
        ...
        $a_data = $this->get_stickerlist($a_request);
        
        $this->return_data($a_data);
    }
	

	private function get_stickerlist($a_params=array())
    {
        if(!$this->oauth_server->verifyResourceRequest(OAuth2\Request::createFromGlobals())){
			$a_data['status'] = false;
            $a_data['error'] = 'Access Token not found';
        ...
        if ($a_data === false) {
            ...
            $a_list=$this->sticker_model->get_list_sticker($a_condition,$a_order,$a_limit);
	...
	private function set_param($a_param=array())
	{
		...
		//packageid
		if (isset($a_param["packageid"]) && $a_param["packageid"]!="") {
			$a_condition["stickerlist_line.packageid"] = $a_param["packageid"] ;
		}
		return $a_condition;
	}
