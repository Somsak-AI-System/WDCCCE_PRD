...
class Relatedmodule extends REST_Controller
{
 ...
 if(count($data)>0){
	...
	public function delete_relate_post()
	{
		$request_body = file_get_contents('php://input');
		 ...
		 $a_request =$dataJson;
	 	 ...
	 	 $a_data=$this->relatedmodule_model->deleted_data($a_request);

	 	if($a_data['status']==true){
			$a_return["Type"] = "S";
			$a_return["Message"] = $a_data['error'];
		}else{
			$a_return["Type"] = "E";
			$a_return["Message"] = !empty($a_data['error']) ? $a_data['error'] : "Fail" ;

		}

		 $url = "http://$_SERVER[HTTP_HOST]$_SERVER[REQUEST_URI]";
		 $this->common->_filename= "Delete_Releted";
		 $this->common->set_log($url,$a_request,$a_return);
		 if ($a_return) {
			 $this->response($a_return, 200); // 200 being the HTTP response code
		 } else {
			 $this->response(array(
