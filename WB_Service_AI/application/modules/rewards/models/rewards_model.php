...
class rewards_model extends CI_Model
{
  ...
  function get_total($a_condition=array())
  {
  	try {
  		...
  		$this->db->select("count(DISTINCT aicrm_rewards.rewardsid) as total");
  		$this->db->join('aicrm_rewardscf', 'aicrm_rewardscf.rewardsid = aicrm_rewards.rewardsid');
  		$this->db->join('aicrm_crmentity', 'aicrm_crmentity.crmid = aicrm_rewards.rewardsid');
  		$query = $this->db->get('aicrm_rewards');
  		if(!$query){
