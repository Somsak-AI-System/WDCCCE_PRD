...
class Serial extends REST_Controller
{
  ...
  private $crmid;
  private $tab_name = array('aicrm_crmentity','aicrm_serial','aicrm_serialcf');
  private $tab_name_index =  array('aicrm_crmentity'=>'crmid','aicrm_serial'=>'serialid','aicrm_serialcf'=>'serialid');
	...
	public function insert_content_post(){
	  	...
	  	$this->common->_filename= "Insert_Serial";
		...
		$dataJson     = json_decode($request_body,true);
	  	...
	  	$this->common->set_log('Before Insert Serial ==>',$url,$a_request);
		$response_data = $this->get_insert_data($a_request);	
	  	$this->common->set_log('After Insert Serial ==>',$a_request,$response_data);
	...
	private function get_insert_data($a_request){
		...
		$DocNo=isset($a_request['crmid']) ? $a_request['crmid'] : "";
	  	...
	  	$userid=isset($a_request['userid']) ? $a_request['userid'] : "1";
	  		...
	  		if(count($data[0])>0 and $module=="Serial"){
	...
	public function get_serial_post(){

		$this->common->_filename= "Detail_Serial";
	  	header('Content-Type:application/json; charset=UTF-8');
	  	$request_body = file_get_contents('php://input');
		$dataJson     = json_decode($request_body,true);
	  	$response_data = null;
	  	...
	  	$this->common->set_log('Before Detail Serial ==>',$url,$a_request);

		$response_data = $this->serial_model->get_serial($a_request);
		// alert($response_data);exit;

	  	$this->common->set_log('After Detail Serial ==>',$a_request,$response_data);
