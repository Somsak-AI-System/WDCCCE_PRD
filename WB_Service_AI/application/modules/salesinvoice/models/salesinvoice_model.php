...
class Salesinvoice_model extends CI_Model
{
  ...
  function get_total($a_condition=array())
  {
  	try {
  		if (!empty($a_condition)) {
  			...
  			$a_condition["aicrm_crmentity.setype"] = "Salesinvoice";
  			$a_condition["aicrm_crmentity.deleted"] = "0";
  		}

  		$this->db->select("count(DISTINCT aicrm_salesinvoice.salesinvoiceid) as total");
  		$this->db->join('aicrm_salesinvoicecf', 'aicrm_salesinvoicecf.salesinvoiceid = aicrm_salesinvoice.salesinvoiceid',"inner");
  		$this->db->join('aicrm_crmentity', 'aicrm_crmentity.crmid = aicrm_salesinvoice.salesinvoiceid',"inner");
      $this->db->join('aicrm_account', 'aicrm_account.accountid = aicrm_salesinvoice.accountid',"inner");
  	 
      $query = $this->db->get('aicrm_salesinvoice');
  ...
  function get_list($a_condition=array(),$a_order=array(),$a_limit=array())
  {
      ...
      $this->db->select("aicrm_salesinvoice.invoice_date , aicrm_salesinvoice.total as sales_amount ");
      $this->db->join('aicrm_salesinvoicecf', 'aicrm_salesinvoicecf.salesinvoiceid = aicrm_salesinvoice.salesinvoiceid', "inner");
      $this->db->join('aicrm_crmentity', 'aicrm_crmentity.crmid = aicrm_salesinvoice.salesinvoiceid',"inner");
      $this->db->join('aicrm_account', 'aicrm_account.accountid = aicrm_salesinvoice.accountid',"inner");
