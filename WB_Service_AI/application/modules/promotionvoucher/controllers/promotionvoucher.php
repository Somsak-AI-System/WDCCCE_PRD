...
class Promotionvoucher extends REST_Controller
{

  private $crmid;
  private $tab_name = array('aicrm_crmentity','aicrm_promotionvoucher','aicrm_promotionvouchercf');
  private $tab_name_index = array('aicrm_crmentity'=>'crmid','aicrm_promotionvoucher'=>'promotionvoucherid','aicrm_promotionvouchercf'=>'promotionvoucherid');
	...
	function __construct()
	{
		...
		$this->_module = 'Promotionvoucher';
		$this->load->model("promotionvoucher_model");
		$this->_return = array(
			'Type' => "S",
	...
	private function set_param($a_param=array())
	{
		...
		if (isset($a_param["crmid"]) && $a_param["crmid"]!="") {
			$a_condition["aicrm_promotionvoucher.promotionvoucherid"] =  $a_param["crmid"] ;
		}

		if (isset($a_param["accountid"]) && $a_param["accountid"]!="") {
			$a_condition["aicrm_promotionvoucher.accountid"] = $a_param["accountid"];
		}
		if (isset($a_param["contactid"]) && $a_param["contactid"]!="") {
			$a_condition["aicrm_promotionvoucher.contactid"] = $a_param["contactid"];
		}
	...
	private function get_cache($a_params=array())
	{
		...
		if($a_data===false)
		{
			$a_list=$this->promotionvoucher_model->get_list($a_condition,$a_order,$a_limit);
			//alert($a_list); exit();
	...
	public function insert_content_post(){
		...
		$response_data = $this->get_insert_data($a_request);
	  	...
	  	$this->common->_filename= "Insert_Promotionvoucher";
	...
	private function get_insert_data($a_request){
	  	...
	  	$data=isset($a_request['data']) ? $a_request['data'] : "";
	  		...
	  		if(count($data[0])>0 and $module== "Promotionvoucher"){
	...
	private function get_dataVoucher($a_request){
		...
		if (isset($keyword) && $keyword!="") {
			...
			$select = "SELECT aicrm_promotionvoucher.promotionvoucherid,
				aicrm_promotionvoucher.promotionvoucher_vkeyword,
				aicrm_promotionvoucher.promotionvoucher_vwording,
				aicrm_promotionvoucher.enddate
		...
		WHERE
			...
			AND aicrm_promotionvoucher.promotionvoucher_vkeyword <> ''
			AND aicrm_promotionvoucher.promotionvoucher_vkeyword = '".$keyword."' ";
			
			$query = $this->db->query($select);
			
			$a_data =  $query->result_array();
			$promotionvoucherid = $a_data[0]['promotionvoucherid'];
			$wording = $a_data[0]['promotionvoucher_vwording'];
			$enddate = $a_data[0]['enddate'];

			if(count($a_data) > 0){

				//Check promotionvoucher expiry date
				...
				AND aicrm_voucher.promotionvoucherid ='".$promotionvoucherid."'
