<?php
    include_once("mydbi_ext.inc");
	class generate extends mydb_ext{
		var $total_columns;
		var $total_rows;
		var $sql;
		var $data;
		...
		function generate($dbconfig, $dbname){
			//print_r($dbconfig);
			 $this->connect( $dbconfig["db_server"], $dbconfig["db_username"], $dbconfig["db_password"],$dbconfig["db_name"]);
	...
	//	echo 	$this->results ;
			...
			$this->total_columns = $this->num_fields(); 
				$numFields = mysqli_num_fields($this->results);
				$index = 0;
				$j = 0;
				while ($j < $numFields) {
					$column = mysqli_fetch_field($this->results);
