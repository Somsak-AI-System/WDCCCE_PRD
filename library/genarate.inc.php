	...
	class genarate extends mydb_ext{
	 ...
	 function unsetVariables2(){
		...
		function preparedata(){//generate all data
			...
			$row=0; //row = 0 for header
			$column=0;
			//echo count($this->fetch_array());
			while($rowdata = $this->fetch_array()){
				for($column=0;$column<$this->total_columns;$column++){
					$this->data[$row][$column] = $rowdata[$column];
				}
				$row++;
			}
			
			if(count($this->data) > 0){
				if(($this->sorts != "")&&($this->key_sort !="")){
					//$this->data = $this->mu_sort($this->data,$this->sorts,$this->key_sort);
					//echo 1;
				}
			}
			//$this->data = $this->mu_sort($this->data,$this->sorts,$this->key_sort);
