...
class CRMEntity
{
    ...
    function uploadAndSaveFile($id, $module, $file_details)
    {
        ...
        if ($save_file == 'true' && $upload_status == 'true') {
            ...
            if ($module == 'Contacts' || $module == 'Products' || $module == 'KnowledgeBase') {
                $sql1 = "insert into aicrm_crmentity (crmid,smcreatorid,smownerid,setype,description,createdtime,modifiedtime) values(?, ?, ?, ?, ?, ?, ?)";
...
//#################### AUTO RUN ###################
        if ($insert_flag == "1") {
            if ($_REQUEST['module'] == "Accounts" && $table_name == "aicrm_account") {
                ...
                $cd = get_autorun("ACC" . substr((date("Y") + 543), -2), "Accounts", "4");
                $this->column_fields['account_no'] = $cd;
                $sqlupdate = " update  aicrm_account set account_no = '" . $cd . "' where  accountid= '" . $this->id . "' ";
                $adb->pquery($sqlupdate, array());

            } else if ($_REQUEST['module'] == "Products" && $table_name == "aicrm_products") {
                include_once("include/myFunction.php");
                $cd = get_autorun("PRO" . substr((date("Y") + 543), -2), "Products", "4");
            ...
            /*} else if ($_REQUEST['module'] == "Contacts" && $table_name == "aicrm_contactdetails") {
                include_once("include/myFunction.php");
                $cd = get_autorun("CON" . substr((date("Y") + 543), -2), "Contacts", "4");
            ...
            } else if ($_REQUEST['module'] == "Quotes" && $table_name == "aicrm_quotescf") {
                ...
                if (isset($_REQUEST["revise_from"]) && $_REQUEST["revise_from"] != "") {
                    ...
                    include_once("include/myFunction.php");
                    $cd = get_autorun_quotes("QBIO-" .substr((date("Y") + 543), -2), date("m"), date("d"), "Quotes", "5");
                ...
                include_once("include/myFunction.php");
                $cd = get_autorun("PRJ" . substr((date("Y") + 543), -2), "Projects", "4");
                ...
                include_once("include/myFunction.php");
                $cd = get_autorun("LEA" . substr((date("Y") + 543), -2), "Leads", "4");
                ...
                include_once("include/myFunction.php");
                $cd = get_autorun("ACT" . substr((date("Y") + 543), -2), "Activitys", "4");
                ...
                include_once("include/myFunction.php");
                $cd = get_autorun("CAM" . substr((date("Y") + 543), -2), "Campaigns", "4");
                ...
                include_once("include/myFunction.php");
                $cd = get_autorun("SMS" . substr((date("Y") + 543), -2), "SmartSms", "4");
                ...
                include_once("include/myFunction.php");
                $cd = get_autorun("SME" . substr((date("Y") + 543), -2), "Smartemail", "4");
                ...
                include_once("include/myFunction.php");
                $cd = get_autorun("PR" . substr((date("Y") + 543), -2), "PriceList", "4");
                ...
                include_once("include/myFunction.php");
                $cd = get_autorun("JOB" . substr((date("Y") + 543), -2), "Job", "4");
                ...
                include_once("include/myFunction.php");
                $cd = get_autorun("CPT" . substr((date("Y") + 543), -2), "Competitor", "4");
                ...
                $sql_update = " update aicrm_order set purchase_no = '',tax_no = '' where orderid = '" . $this->id . "' ";
                $adb->pquery($sql_update, array());
                /*$auto = str_replace("QU","",$cd);
                $purchase_no = 'PO'.$auto;
                $tax_no = 'IN'.$auto;
                $sql_update = " update aicrm_order set purchase_no = '".$purchase_no."' ,tax_no = '" . $tax_no . "' where orderid = '" . $this->id . "' ";
                $adb->pquery($sql_update, array());*/
...
//#################### AUTO RUN ###################
    ...
    function whomToSendMail($module, $insertion_mode, $assigntype)
    {
        ...
        global $old_user_id1;
        $HELPDESK_SUPPORT_EMAIL_ID = 'support@aisyst.com';
...
// Code included by Jaguar - Ends
    ...
    function save($module_name, $fileid = '')
    {
        ...
        //Event triggering code
        if ($module_name != "Serial" && $module_name != "Errors" && $module_name != "Errorslist" && $module_name != "Sparepart" && $module_name != "Sparepartlist" && $module_name != "Calendar" && $module_name != "Quotes" && $module_name != "Projects" && $module_name != "Opportunity" && $module_name != "Activitys" && $module_name != "KnowledgeBase" && $module_name != "Quotation" && $module_name != "PriceList" && $module_name != "Job" && $module_name != "SmartSms" && $module_name != "Smartemail" && $module_name != "Competitor" && $module_name != "Campaigns" && $module_name != "Faq"&& $module_name != "Plant"&& $module_name != "Order" ) {
