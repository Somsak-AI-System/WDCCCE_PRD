...
class Template
{
	...
	public function build($view, $data = array(), $return = FALSE)
	{
		...
		unset($data);
			...
			{
				$this->_screen = $this->_guess_screen();
			}
		...
		$template['screen']	= $this->_screen;
		$template['modulename'] = $this->_modulename;
		...
		public function screen()
	{
		...
		if (func_num_args() >= 1)
		{
			$screen_segments = func_get_args();
			$this->_screen = implode($this->_screen_separator, $screen_segments);
		}
		...
		public function modulename()
	{
		...
		{
			$screen_segments = func_get_args();
			$this->_modulename = implode($this->_modulename_separator, $screen_segments);
		}
...
private function _guess_screen()
	{
		$this->_ci->load->helper('inflector');
		...
		if ($this->_method != 'index')
		{
			$screen_parts[] = $this->_method;
		}

		// Make sure controller name is not the same as the method name
		if ( ! in_array($this->_controller, $screen_parts))
		{
			$screen_parts[] = $this->_controller;
		}

		// Is there a module? Make sure it is not named the same as the method or controller
		if ( ! empty($this->_module) AND ! in_array($this->_module, $screen_parts))
		{
			$screen_parts[] = $this->_module;
		}

		// Glue the title pieces together using the title separator setting
		$screen = humanize(implode($this->_screen_separator, $screen_parts));
