...
class ExportDataExcel extends ExportData {
	...
	function generateHeader() {
		...
		$output .= "<Style ss:ID=\"sWD\"><Alignment ss:Horizontal=\"Center\"  ss:Vertical=\"Top\" ss:WrapText=\"1\"/></Style>\n";
		$output .= "</Styles>\n";
	...
	if(preg_match("/^-?\d+(?:[.,]\d+)?$/",$item) && (strlen($item) < 20)) {
			$type = 'String';
		}
		...
		else 	if(preg_match("/^(\d{1,2}|\d{4})[\/\-]\d{1,2}[\/\-](\d{1,2}|\d{4})([^\d].+)?$/",$item) &&
