			...
			if ($validate === true) {
                ...
                if (!$data['hasWarnings']) {
                    foreach($data['files'] as $key => $file) {
                        if (move_uploaded_file($file['tmp_name'], iconv("UTF-8", "TIS-620",$file['file']))) {
...
//				return $this->codeToMessage('invalid_folder_path');
			...
			if (!file_exists($this->options['uploadDir']))
				return 'Upload folder not exist';
			if (!is_writable($this->options['uploadDir']))
				return 'Upload folder not writable';
