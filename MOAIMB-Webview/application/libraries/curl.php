...
class Curl {
    ...
    function __construct($url = '')
    {
        ...
        $this->log = $this->CI->config->item('log');
    ...
    public function simple_post($url, $params = array(), $options = array(),$format=null)
    {
    	if ($this->log["status"]) {
    		$this->CI->load->library("common");
    		$this->CI->common->set_log_begin($url,$params);
    		$this->CI->common->_filename = $this->_filename;
    	}

    		$this->create($url);
    		$this->post($params, $options,$format);
    		$response = $this->execute();

    		if(!$response){
    			$lang = $this->CI->config->item('lang');
    			$this->CI->lang->load('ai',$lang);
    			$a_response["IsError"] = true;
    			if($this->error_code=="28"){
    				$a_response["ErrorMessage"] = lang("msg_error_time_out");
    			}else 	if($this->error_code=="7"){
    					$a_response["ErrorMessage"] = lang("msg_error_connect_failed");
    			}else {
    				$a_response["ErrorMessage"] = "Error  ".$this->error_code ." :: ". $this->error_string;
    			}
    			$a_response["ResultList"] = "";
    			$a_response["RowCount"] =0;
    			$response = json_encode($a_response);
    		}

		if ($this->log["status"]) {
			$this->CI->common->set_log($url,$params,$response);
			//$this->CI->common->set_log_error_criticals($url,$params,$response);
		}
		$return = json_decode($response,true);
		//var_dump( $return["IsError"]);
		if(isset($return["IsError"]) && $return["IsError"]===true)
		{
			$this->CI->load->library("common");
			$this->CI->common->set_log_error($url,$params,$response);
    ...
    public function post($params = array(), $options = array(),$format=null) {
	//alert($params);
    ...
    public function execute()
    {
        ...
        if(!isset($this->options[CURLOPT_TIMEOUT]))           $this->options[CURLOPT_TIMEOUT] =600;
