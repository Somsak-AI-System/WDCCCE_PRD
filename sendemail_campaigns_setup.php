...
//get email====================================================================================
	...
	echo "<pre>"; print_r($campaign); echo "</pre>"; exit;

	for($i=0;$i<count($campaign);$i++){
		//Set ตัวแฟร
		$new_table="tbt_email_log_smartemailid_".$campaign[$i][0];
		$date_start=$campaign[$i][3]." ".$campaign[$i][13];
		...
		//Check Path Folder
		$folder_name=date('Ymd',strtotime($campaign[$i][3]))."_".$campaign[$i][0];
		if (!file_exists($path."/EDM/".$folder_name)){
			if (!mkdir($path."/EDM/".$folder_name, 0777, true)){}
		}
			//Set ตัวแปร Body

			$email_body=$campaign[$i][6];
			//Check ตัวแปรที่ส่งแบบ Presonal ($-------$) ครอบหน้าหลัง
			$str_arr = getInbetweenStrings($systax, $systax, $campaign[$i][6]);
...
/*--/////////////////เพิ่ม data table แบ่ง group-personal //////////////////////////////--*/
		 ...
		 $massage=$campaign[$i][6];                 
		 if(strstr($massage,$systax))
		 {
		 	//personal
		  	$sql="update aicrm_smartemail set email_type = 'personal' where smartemailid=".$crmid."";
			$genarate->query($sql,"all");
		 }else{
		 	//group
		   	$sql="update aicrm_smartemail set email_type = 'group' where smartemailid=".$crmid."";
		 	$genarate->query($sql,"all");              
		}
		//print_r($sql);exit();
/*--////////////////////////////////////////////////////////////////////////////////////--*/
...
//			echo $sql;exit;
			for($k=0;$k<count($dataacc);$k++){
				...
				foreach($replace_arr as $id => $a){
					//echo $id.' => '. $a['module'];
						...
						$new_string = str_ireplace($find, $replace, $campaign[$i][6]);
								...
								for($k=0;$k<count($dataacc);$k++){
									...
									for($w=0;$w<count($email);$w++){
										if($email[$w]!=""){
											$sql="
											insert into ".$new_table."(campaignid,email_marketingid,emailtargetlistid,from_id,to_name,to_email,from_module,status,message,unsubscribe)
											values('".$campaign[$i][0]."','0','0','".$dataacc[$k][0]."','".$dataacc[$k][1]."','".trim(str_replace("\n", '', $email[$w]))."','".$dataacc[$k][3]."','0','".$new_string."','".$dataacc[$k][4]."')";
											echo $sql; 
											$genarate->query($sql,"all");
										}/*else{
											$sql="
	...
	$folder_name=date('Ymd',strtotime($date_start))."_".$campaignid;
			...
			// $url_click_view.='212link2121212click_type212CLICKTYPE212url212';
...
//			';
			...
			// $unsubscribe_url = 'http://appsrv2.git.or.th:8080/crm/edm/unsubscribe/'.$from_module.'/'.$from_id.'/'.$campaignid;
			$unsubscribe_url = $url_path.'/unsubscribe.php?accountid='.$from_id.'&campaignid='.$campaignid;
//			$unsubscribe_link = 'หากท่านไม่ต้องการรับข่าวสารจากทางเราอีก <a href="'.$unsubscribe_url.'">กรุณายกเลิกการรับข่าวสารที่นี่</a>';
