...
function LogintoVtigerCRM($user_name,$password,$version)
{
	...
	if($version != $aicrm_current_version)
	{
...
function create_account($username,$sessionid,$accountname,$email,$phone,$primary_address_street,$primary_address_city,$primary_address_state,$primary_address_postalcode,$primary_address_country)
{
	...
	if(isPermitted("Accounts","EditView") == "yes")
	{
		$query = "SELECT accountname FROM aicrm_account,aicrm_crmentity WHERE accountname =? and aicrm_account.accountid = aicrm_crmentity.crmid and aicrm_crmentity.deleted != 1";
...
function GetPicklistValues($username,$sessionid,$tablename)
{
	...
	{
		$query = "select " . $adb->sql_escape_string($tablename) . " from aicrm_". $adb->sql_escape_string($tablename);		
	...
	{
		$query = "select " .$adb->sql_escape_string($tablename) . " from aicrm_". $adb->sql_escape_string($tablename) ." inner join aicrm_role2picklist on aicrm_role2picklist.picklistvalueid = aicrm_". $adb->sql_escape_string($tablename) .".picklist_valueid where roleid=? and picklistid in (select picklistid from aicrm_". $adb->sql_escape_string($tablename)." ) order by sortid";	
...
function unsetServerSessionId($id)
{
	...
	$adb->query("delete from aicrm_soapservice where type='FireFox' and id=$id");
...
function getServerSessionId($id)
{
	...
	$query = "select * from aicrm_soapservice where type='FireFox' and id={$id}";
