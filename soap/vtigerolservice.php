...
function AddContacts($username,$session,$cntdtls)
{
	...
	if($is_admin == true || $profileGlobalPermission[1] == 0 || $profileGlobalPermission[2] == 0) {
    	$sql1 = "select fieldname,columnname from aicrm_field where tabid=4 and aicrm_field.presence in (0,2)";
		$params1 = array();
  	} else {
...
function AddTasks($username,$session,$taskdtls)
{
	...
	if($is_admin == true || $profileGlobalPermission[1] == 0 || $profileGlobalPermission[2] == 0) {
    	$sql1 = "select fieldname,columnname from aicrm_field where tabid=9 and aicrm_field.presence in (0,2)";
		$params1 = array();
  	} else {
    	$profileList = getCurrentUserProfileList();
    	$sql1 = "select fieldname,columnname from aicrm_field inner join aicrm_profile2field on aicrm_profile2field.fieldid=aicrm_field.fieldid inner join aicrm_def_org_field on aicrm_def_org_field.fieldid=aicrm_field.fieldid where aicrm_field.tabid=9 and aicrm_field.displaytype in (1,2,4,3) and aicrm_profile2field.visible=0 and aicrm_def_org_field.visible=0 and aicrm_field.presence in (0,2)";
  		$params1 = array();
		if (count($profileList) > 0) {
			$sql1 .= " and aicrm_profile2field.profileid in (". generateQuestionMarks($profileList) .")";
			array_push($params1, $profileList);
...
function AddClndr($username,$session,$clndrdtls)
{
	...
	if($is_admin == true || $profileGlobalPermission[1] == 0 || $profileGlobalPermission[2] == 0) {
    	$sql1 = "select fieldname,columnname from aicrm_field where tabid=16 and aicrm_field.presence in (0,2)";
		$params1 = array();
  	} else {
    	$profileList = getCurrentUserProfileList();
    	$sql1 = "select fieldname,columnname from aicrm_field inner join aicrm_profile2field on aicrm_profile2field.fieldid=aicrm_field.fieldid inner join aicrm_def_org_field on aicrm_def_org_field.fieldid=aicrm_field.fieldid where aicrm_field.tabid=16 and aicrm_field.displaytype in (1,2,4,3) and aicrm_profile2field.visible=0 and aicrm_def_org_field.visible=0 and aicrm_field.presence in (0,2)";
  		$params1 = array();
		if (count($profileList) > 0) {
			$sql1 .= " and aicrm_profile2field.profileid in (". generateQuestionMarks($profileList) .")";
			array_push($params1, $profileList);
...
function unsetServerSessionId($id)
{
	...
	$adb->query("delete from aicrm_soapservice where type='Outlook' and id=$id");
...
function getServerSessionId($id)
{
	...
	$query = "select * from aicrm_soapservice where type='Outlook' and id={$id}";
