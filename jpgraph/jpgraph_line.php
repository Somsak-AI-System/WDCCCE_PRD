...
// PUBLIC METHODS	
	...
	if($_SESSION["graph_no"]<>"" and $_SESSION["graph_no"]=="1"){
		$this->value->Stroke($img,number_format($this->coords[0][$startpoint],0)." / ".number_format(($this->coords1[0][$startpoint]),0)." ",$xt,$yt);
	}else{
		$this->value->Stroke($img,number_format($this->coords[0][$startpoint],0),$xt,$yt);
	}
	$img->SetColor($this->color);
	$img->SetLineWeight($this->weight);
	...
	while( $pnts < $numpoints ) {
	    ...
	    $y_old = $y;
		if($_SESSION["graph_no"]<>"" and $_SESSION["graph_no"]=="1"){
			$this->StrokeDataValue($img,number_format($this->coords[0][$pnts],0)." / ".number_format(($this->coords1[0][$pnts]),0)." ",$xt,$yt);
		}else{
			$this->StrokeDataValue($img,number_format($this->coords[0][$pnts],0),$xt,$yt);
		}
	...
	for( $pnts=0; $pnts<$numpoints; ++$pnts) {
		...
		if($_SESSION["graph_no"]<>"" and $_SESSION["graph_no"]=="1"){
			$lite=number_format(($this->coords1[0][$pnts]/100),0);
			$this->StrokeDataValue($img,number_format($this->coords[0][$pnts],0)." / ".number_format(($this->coords1[0][$pnts]),0)." ",$xt,$yt);
		}else{
			$this->StrokeDataValue($img,number_format($this->coords[0][$pnts],0),$xt,$yt);
		}
