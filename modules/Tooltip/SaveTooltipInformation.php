...
function SaveTooltipInformation($fieldid, $sequence, $view=1){
	...
	if(empty($fieldid)){
		...
		if(!empty($_REQUEST['checkedFields'])){
			...
			//add to aicrm_quickview table
			foreach($checkedFields as $checkedField){
				$query = "insert into aicrm_quickview (fieldid,related_fieldid,sequence,currentview) values (?,?,?,?)";
...
function deleteOldInfo($fieldid, $view=1){
	...
	$query = "delete from aicrm_quickview where fieldid = ? and currentview = ?";
...
function getFieldLabels($checkedFields){
	...
	$sql = "select * from aicrm_field where fieldid in (".generateQuestionMarks($checkedFields).") and aicrm_field.presence in (0,2)";
