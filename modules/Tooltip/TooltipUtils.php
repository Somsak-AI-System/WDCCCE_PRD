...
function getFieldList($module_name, $field_name = "") {
	...
	$query.= " and columnname not like 'imagename' and uitype not in (61, 122) and aicrm_field.presence in (0,2)";
...
function moduleList() {
	global $adb;
	$sql = "select distinct aicrm_field.tabid,name from aicrm_field inner join aicrm_tab on aicrm_field.tabid=aicrm_tab.tabid where name not in ('Calendar','Events','Users')";
...
function tooltip_exists($fieldid, $related_fieldid) {
	global $adb;
	$query = "select * from aicrm_quickview where fieldid=? and related_fieldid=?";
...
function getToolTipText($view,$fieldname,$module,$value){
	...
	$quickview = 'select fieldname,fieldlabel from aicrm_quickview inner join aicrm_field on aicrm_quickview.related_fieldid=aicrm_field.fieldid where aicrm_quickview.fieldid = ? and currentview= ? and aicrm_field.presence in (0,2) order by aicrm_quickview.sequence';
...
function ToolTipExists($fieldname,$tabid){
	if(empty($fieldname) || empty($tabid)){
		...
		global $adb;
		$sql = "select fieldid from aicrm_field where tabid = ? and fieldname = ? and aicrm_field.presence in (0,2)";
		$result = $adb->pquery($sql,array($tabid,$fieldname));
		$count = $adb->num_rows($result);
		if($count > 0){
			$fieldid = $adb->query_result($result,0,'fieldid');
		
			$sql = "select * from aicrm_quickview where fieldid = ?";
...
function QuickViewFieldList($module){
	...
	$query = "select * from aicrm_field where tabid = ? and columnname not like 'imagename' and uitype not in (61, 122) and aicrm_field.presence in (0,2)";
