...
class Template extends CRMEntity {
	var $log;
	var $db;
	var $table_name = "aicrm_template";
	var $table_index= 'templateid';
	var $_images_path= 'upload/template/';
	var $table_comment = "aicrm_templatecomments";

	var $tab_name = Array('aicrm_crmentity','aicrm_template','aicrm_templatecf');
	var $tab_name_index = Array('aicrm_crmentity'=>'crmid','aicrm_template'=>'templateid','aicrm_templatecf'=>'templateid','aicrm_templatecomments'=>'templateid');
	/**
	 * Mandatory table for supporting custom fields.
	 */
	var $customFieldTable = Array('aicrm_templatecf', 'templateid');
	var $column_fields = Array();

	var $sortby_fields = Array('templateid','template_name','smownerid');

	var $list_fields = Array(
		'Template No'=>Array('Template'=>'template_no'),
		'Template Name'=>Array('Template'=>'template_name'),
	...
	var $search_fields = Array(
		'Template No'=>Array('aicrm_template'=>'template_no'),
		'Template Name'=>Array('aicrm_template'=>'template_name'),
		'Assigned To' => Array('crmentity'=>'smownerid')
	);
	...
	var $mandatory_fields = Array('assigned_user_id','createdtime' ,'modifiedtime','templateid');

	function Template()
	{
		$this->log =LoggerManager::getLogger('Template');
		$this->db = PearDatabase::getInstance();
		$this->column_fields = getColumnFields('Template');
	}
	function save_module()
	{
		global $adb;
		$this->insertIntoAttachment($this->id,'Template');
		$this->insertIntoCommentTable("aicrm_templatecomments",'templateid');
	...
	function getCommentInformation($crmid)
	{
		...
		$sql = "select * from aicrm_templatecomments where templateid=? order by createdtime desc";
	...
	function getSortOrder()
	{
		...
		if(isset($_REQUEST['sorder']))
			...
			$sorder = (($_SESSION['TEMPLATE_SORT_ORDER'] != '')?($_SESSION['TEMPLATE_SORT_ORDER']):($this->default_sort_order));
	...
	function getOrderBy()
	{
		...
		if (isset($_REQUEST['order_by']))
			...
			$order_by = (($_SESSION['TEMPLATE_ORDER_BY'] != '')?($_SESSION['TEMPLATE_ORDER_BY']):($use_default_order_by));
	...
	function generateReportsSecQuery($module,$secmodule){
		$query = $this->getRelationQuery($module,$secmodule,"aicrm_template","templateid");
		$query .=" left join aicrm_crmentity as aicrm_crmentityEmailtarget on aicrm_crmentityEmailtarget.crmid=aicrm_template.templateid and aicrm_crmentityEmailtarget.deleted=0
				left join aicrm_templatecf on aicrm_templatecf.templateid = aicrm_crmentityEmailtarget.crmid
	...
	function create_export_query($where)
	{
		...
		$sql = getPermittedFieldsQuery("Template", "detail_view");
		...
		$query = "SELECT $fields_list,case when (aicrm_users.user_name not like '') then aicrm_users.user_name else aicrm_groups.groupname end as user_name
				...
				INNER JOIN aicrm_template ON aicrm_template.templateid = aicrm_crmentity.crmid
				INNER JOIN aicrm_templatecf ON aicrm_templatecf.templateid = aicrm_template.templateid
		...
		if($is_admin==false && $profileGlobalPermission[1] == 1 && $profileGlobalPermission[2] == 1 && $defaultOrgSharingPermission[6] == 3)
		{
			...
			$query = $query." ".getListViewSecurityParameter("Template");
		}
	...
	function setRelationTables($secmodule){
		$rel_tables = array (
			"Documents" => array("aicrm_senotesrel"=>array("crmid","notesid"),"aicrm_template"=>"templateid"),
		);
	...
	function insertIntoAttachment($id,$module){
		...
		if($module == 'Template' && $_REQUEST['del_file_list'] != '')
		{
	...
	function get_activities($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		$query = "SELECT aicrm_activity.*, aicrm_activitycf.*,
			...
			LEFT JOIN aicrm_groups ON aicrm_groups.groupid = aicrm_crmentity.smownerid
			INNER JOIN aicrm_template ON aicrm_template.templateid = aicrm_activity.parentid
			WHERE aicrm_template.templateid = ".$id."
