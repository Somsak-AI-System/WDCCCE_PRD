...
$log->debug("DEBUG In add2db.php");
	...
	if(move_uploaded_file($_FILES["filename"]["tmp_name"],$upload_filepath.$current_id."_".$_FILES["filename"]["name"])) 
	{
		...
		if($filesize != 0)	
		{
			...
			{
				$crmid = $_REQUEST['return_id'];
				$query = 'select accountid from aicrm_contactdetails where contactid=?';
				...
				if($adb->num_rows($result) != 0)
				{
					...
					$query = "select attachmentsid, name, path from aicrm_attachments where name=?";
			...
			$sql = "insert into aicrm_attachments(attachmentsid, name, description, type,path,subject) values(?,?,?,?,?,?)";
			...
			{
				$log->debug("DEBUG: inserting into aicrm_seattachmentsrel from add2db 2");
				$sql1 = "insert into aicrm_seattachmentsrel values(?,?)";
