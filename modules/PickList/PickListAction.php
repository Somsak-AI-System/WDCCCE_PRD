...
if($mode == 'add'){
	...
	for($i=0; $i<$count;$i++){
		...
		if(!empty($val)){
			...
			if($tableName == 'stage'){
				$sql = "insert into aicrm_$tableName (stageid,stage,presence,picklist_valueid,percentage) values (?,?,?,?,?)";
				$adb->pquery($sql, array($id, $val, 1, $picklist_valueid,''));
			}elseif($tableName == 'quotation_type'){
				$sql = "insert into aicrm_$tableName values (?,?,?,?,?,?,?,?,?)";
				$adb->pquery($sql, array($id, $val, 1, $picklist_valueid,'','','','',''));	
			}else{
				$sql = "insert into aicrm_$tableName values (?,?,?,?)";
				$adb->pquery($sql, array($id, $val, 1, $picklist_valueid));	
	...
	$qry="select tablename,columnname from aicrm_field where fieldname=? and presence in (0,2)";
	...
	for($i=0; $i<count($newValues);$i++){
		...
		if($newVal != $oldVal){
			$sql = "UPDATE aicrm_$tableName SET $tableName=? WHERE $tableName=?";
	...
	if(!empty($replaceVal)){
		$sql = "select * from aicrm_$tableName where $tableName=?";
	...
	for($i=0;$i<count($values);$i++){
		...
		if(!empty($replaceVal)){
			$sql = "select * from aicrm_role2picklist where picklistvalueid=?";
			$result = $adb->pquery($sql, array($replacePicklistID));
			$count = $adb->num_rows($result);
			
			if($count == 0){
				$sql = "update aicrm_role2picklist set picklistvalueid=? where picklistvalueid=?";
				$adb->pquery($sql, array($replacePicklistID, $origPicklistID));
			}
		}
		
		$sql = "delete from aicrm_$tableName where $tableName=?";
		$adb->pquery($sql, array($values[$i]));
		$sql = "delete from aicrm_role2picklist where picklistvalueid=?";
