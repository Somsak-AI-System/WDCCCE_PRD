...
$values = htmlspecialchars_decode(vtlib_purify($_REQUEST['values']));
...
$sql = 'SELECT * FROM aicrm_picklist WHERE name = ?';
$result = $adb->pquery($sql, array($tableName));
if($adb->num_rows($result) > 0){
	$picklistid = $adb->query_result($result, 0, "picklistid");
	//delete all role
	//$sql = "DELETE FROM aicrm_role2picklist WHERE picklistid='".$picklistid."'";
...
function assignValues($picklistid, $roleid, $values, $tableName){
	global $adb;
	$count = count($values);
	$sql = "DELETE FROM aicrm_role2picklist WHERE roleid='".$roleid."' AND picklistid='".$picklistid."'";
	$adb->pquery($sql, array());
	for($i=0;$i<$count;$i++){
		$pickVal = $values[$i];
