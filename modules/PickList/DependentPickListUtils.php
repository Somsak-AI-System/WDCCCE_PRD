...
class Vtiger_DependencyPicklist {
	...
	static function getDependentPicklistFields($module='') {
		...
		if(empty($module)) {
			$result = $adb->pquery('SELECT DISTINCT sourcefield, targetfield, tabid FROM aicrm_picklist_dependency', array());
		} else {
			$tabId = getTabid($module);
			$result = $adb->pquery('SELECT DISTINCT sourcefield, targetfield, tabid FROM aicrm_picklist_dependency WHERE tabid=?', array($tabId));
		}
	    ...
	    if($noofrows > 0){
	    	...
	    	for($i=0; $i<$noofrows; ++$i){
	    		...
	    		$fieldResult = $adb->pquery('SELECT fieldlabel FROM aicrm_field WHERE fieldname = ?', array($sourceField));
	    		$sourceFieldLabel = $adb->query_result($fieldResult,0,'fieldlabel');
	    		
	    		$fieldResult = $adb->pquery('SELECT fieldlabel FROM aicrm_field WHERE fieldname = ?', array($targetField));
	...
	static function getAvailablePicklists($module){
		...
		$query="select aicrm_field.fieldlabel,aicrm_field.fieldname" .
				" FROM aicrm_field inner join aicrm_picklist on aicrm_field.fieldname = aicrm_picklist.name" .
				" where displaytype=1 and aicrm_field.tabid=? and aicrm_field.uitype in ('15','16') " .
	...
	static function savePickListDependencies($module, $dependencyMap) {
		...
		for($i=0; $i<count($valueMapping); ++$i) {
			...
			$checkForExistenceResult = $adb->pquery("SELECT id FROM aicrm_picklist_dependency WHERE tabid=? AND sourcefield=? AND targetfield=? AND sourcevalue=?",
			...
			if($adb->num_rows($checkForExistenceResult) > 0) {
				...
				$adb->pquery("UPDATE aicrm_picklist_dependency SET targetvalues=?, criteria=? WHERE id=?", 
										array($serializedTargetValues, $serializedCriteria, $dependencyId));
			} else {
				$adb->pquery("INSERT INTO aicrm_picklist_dependency (id, tabid, sourcefield, targetfield, sourcevalue, targetvalues, criteria)
								VALUES (?,?,?,?,?,?,?)",
							array($adb->getUniqueID('aicrm_picklist_dependency'), $tabId, $sourceField, $targetField, $sourceValue, 
	...
	static function deletePickListDependencies($module, $sourceField, $targetField) {
		...
		$adb->pquery("DELETE FROM aicrm_picklist_dependency WHERE tabid=? AND sourcefield=? AND targetfield=?", 
	...
	static function getPickListDependency($module, $sourceField, $targetField) {
		...
		$result = $adb->pquery('SELECT * FROM aicrm_picklist_dependency WHERE tabid=? AND sourcefield=? AND targetfield=?', 
									array($tabId,$sourceField,$targetField));
	...
	static function getPicklistDependencyDatasource($module) {
		...
		$result = $adb->pquery('SELECT * FROM aicrm_picklist_dependency WHERE tabid=?', array($tabId));
		$noofrows = $adb->num_rows($result);
    	...
    	for($i=0; $i<$noofrows; ++$i){
    		...
    		$targetValues = $adb->query_result($result, $i, 'targetvalues');
    		$unserializedTargetValues = Zend_Json::decode(html_entity_decode($targetValues));    		
    		$criteria = $adb->query_result($result, $i, 'criteria');
    		...
    		if(empty($picklistDependencyDatasource[$sourceField]['__DEFAULT__'][$targetField])) {
    			$picklistDependencyDatasource[$sourceField]['__DEFAULT__'][$targetField] = getAllPicklistValues($targetField);
    		}
    	}
	...
	static function checkCyclicDependency($module, $sourceField, $targetField) {
		...
		$result = $adb->pquery('SELECT 1 FROM aicrm_picklist_dependency
	...
	static function getDependentPickListModules(){
		...
		$query = 'SELECT distinct aicrm_field.tabid, aicrm_tab.tablabel, aicrm_tab.name as tabname FROM aicrm_field
						INNER JOIN aicrm_tab ON aicrm_tab.tabid = aicrm_field.tabid
						INNER JOIN aicrm_picklist ON aicrm_picklist.name = aicrm_field.fieldname
					WHERE uitype IN (15,16)
						AND aicrm_field.tabid != 29
						AND aicrm_field.displaytype = 1
						AND aicrm_field.presence in (0,2)
					GROUP BY aicrm_field.tabid HAVING count(*) > 1';
		// END
