...
//for setting aicrm_accountid=0 for the contacts which are deleted
$ct_query = "select crmid from aicrm_crmentity where setype='Contacts' and deleted=1";
...
{
	$update_query = "update aicrm_contactdetails set accountid = 0 where contactid in (". generateQuestionMarks($deleted_id) .")";
	$result = $adb->pquery($update_query, array($deleted_id));
}
//End setting aicrm_accountid=0 for the contacts which are deleted
...
{
	$query1="select aicrm_tab.name,aicrm_field.tablename,aicrm_field.columnname,aicrm_field.fieldlabel from aicrm_field inner join aicrm_tab on aicrm_tab.tabid = aicrm_field.tabid where aicrm_field.tabid in (4,6) and aicrm_field.block <> 75 and aicrm_field.presence in (0,2) order by aicrm_field.tablename";
	$params1 = array();
}
else
{
	$profileList = getCurrentUserProfileList();
	$query1="select aicrm_tab.name,aicrm_field.tablename,aicrm_field.columnname,aicrm_field.fieldlabel from aicrm_field inner join aicrm_tab on aicrm_tab.tabid = aicrm_field.tabid INNER JOIN aicrm_profile2field ON aicrm_profile2field.fieldid=aicrm_field.fieldid INNER JOIN aicrm_def_org_field ON aicrm_def_org_field.fieldid=aicrm_field.fieldid where aicrm_field.tabid in (4,6) and aicrm_field.block <> 75 AND aicrm_profile2field.visible=0 AND aicrm_def_org_field.visible=0 AND aicrm_profile2field.profileid IN (". generateQuestionMarks($profileList) .") and aicrm_field.presence in (0,2) GROUP BY aicrm_field.fieldid order by aicrm_field.tablename";
...
{ 
	...
	if($tablename == "crmentity")
	{
		...
		{
			$tablename = "aicrm_crmentityContacts";
		}
	}
	...
	{
		$querycolumns[$x] = "aicrm_accountAccount.accountname";
	}
	if($columnname == "accountid")
	{
		$querycolumns[$x] = "aicrm_accountContacts.accountname";
	}
	if($columnname == "reportsto")
	{
		$querycolumns[$x] = "aicrm_contactdetailsContacts.lastname";
	}
...
if(count($querycolumns) > 0)
{
	...
	$query = "select  $selectcolumns from aicrm_account 
				...
				left join aicrm_account as aicrm_accountAccount on aicrm_accountAccount.accountid = aicrm_account.parentid
				...
				left join aicrm_crmentity as aicrm_crmentityContacts on aicrm_crmentityContacts.crmid = aicrm_contactdetails.contactid 
				...
				left join aicrm_users as usersContacts on usersContacts.id = aicrm_crmentityContacts.smownerid
				LEFT JOIN aicrm_groups as groupsContacts
					ON groupsContacts.groupid = aicrm_crmentity.smownerid
				where aicrm_crmentity.deleted=0 and (aicrm_crmentityContacts.deleted=0 || aicrm_crmentityContacts.deleted is null) and aicrm_account.accountid in(". generateQuestionMarks($mass_merge) .")";
