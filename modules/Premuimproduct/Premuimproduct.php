...
class Premuimproduct extends CRMEntity
{
// Account is used to store aicrm_account information.
    var $log;
    var $db;
    var $table_name = "aicrm_premuimproduct";
    var $table_index = 'premuimproductid';
    var $table_comment = "aicrm_premuimproductcomments";

    var $tab_name = Array('aicrm_crmentity', 'aicrm_premuimproduct', 'aicrm_premuimproductcf');
    var $tab_name_index = Array('aicrm_crmentity' => 'crmid', 'aicrm_premuimproduct' => 'premuimproductid', 'aicrm_premuimproductcf' => 'premuimproductid','aicrm_premuimproductcomments' => 'premuimproductid');
    /**
     * Mandatory table for supporting custom fields.
     */
    var $customFieldTable = Array('aicrm_premuimproductcf', 'premuimproductid');
    var $column_fields = Array();

    var $sortby_fields = Array('premuimproductid', 'premuimproduct_name', 'smownerid','accountname');

    var $list_fields = Array(
        'หมายเลขสินค้าพรีเมี่ยม' => Array('premuimproduct' => 'premuimproduct_no'),
        'รหัสสินค้าพรีเมี่ยม' => Array('premuimproduct' => 'premium_product_code'),
        'ชื่อสินค้าพรีเมี่ยม (EN)' => Array('premuimproduct' => 'premuimproduct_name'),
        'รูปแบบสินค้าพรีเมี่ยม' => Array('premuimproduct' => 'premium_product_type'),
        'ราคาสินค้าพรีเมี่ยม' => Array('premuimproduct' => 'premium_price'),
    );

    var $list_fields_name = Array(
        'หมายเลขสินค้าพรีเมี่ยม' => 'premuimproduct_no',
        'รหัสสินค้าพรีเมี่ยม' => 'premium_product_code',
        'ชื่อสินค้าพรีเมี่ยม (EN)' => 'premuimproduct_name',
        'รูปแบบสินค้าพรีเมี่ยม' => 'premium_product_type',
        'ราคาสินค้าพรีเมี่ยม' => 'premium_price',
    );

    var $list_link_field = 'premuimproduct_no';
    ...
    var $search_fields = Array(
        'หมายเลขสินค้าพรีเมี่ยม' => Array('aicrm_premuimproduct' => 'premuimproduct_no'),
        'รหัสสินค้าพรีเมี่ยม' => Array('premuimproduct' => 'premium_product_code'),
        'ชื่อสินค้าพรีเมี่ยม (EN)' => Array('aicrm_premuimproduct' => 'premuimproduct_name'),
        'รูปแบบสินค้าพรีเมี่ยม' => Array('premuimproduct' => 'premium_product_type'),
        'ราคาสินค้าพรีเมี่ยม' => Array('premuimproduct' => 'premium_price'),
    );
    ...
    var $mandatory_fields = Array('assigned_user_id', 'createdtime', 'modifiedtime', 'premuimproductid');

    function Premuimproduct()
    {
        $this->log = LoggerManager::getLogger('Premuimproduct');
        $this->db = PearDatabase::getInstance();
        $this->column_fields = getColumnFields('Premuimproduct');
    }
    ...
    function save_module($module)
    {
        ...
        if ($_REQUEST['action'] != 'PremuimproductAjax' && $_REQUEST['ajxaction'] != 'DETAILVIEW' && $_REQUEST['action'] != 'MassEditSave') {
            //Based on the total Number of rows we will save the product relationship with this entity
            //saveInventoryProductDetails($this, 'Premuimproduct');
        }
        $this->insertIntoCommentTable("aicrm_premuimproductcomments",'premuimproductid');
    ...
    function getCommentInformation($crmid)
    {
        ...
        $sql = "select * from aicrm_premuimproductcomments where premuimproductid=? order by createdtime desc";
    ...
    function insertIntoAttachment($id,$module){
        ...
        if($module == 'Premuimproduct' && $_REQUEST['del_file_list'] != '')
        {
    ...
    {
        $query = $this->getRelationQuery($module, $secmodule, "aicrm_premuimproduct", "premuimproductid");
        $query .= " LEFT JOIN aicrm_crmentity AS aicrm_crmentityPremuimproduct ON aicrm_crmentityPremuimproduct.crmid = aicrm_premuimproduct.premuimproductid
                    AND aicrm_crmentityPremuimproduct.deleted = 0
                    LEFT JOIN aicrm_premuimproductcf ON aicrm_premuimproductcf.premuimproductid = aicrm_crmentityPremuimproduct.crmid
                    LEFT JOIN aicrm_account as aicrm_accountPremuimproduct ON aicrm_accountPremuimproduct.accountid = aicrm_premuimproduct.account_id
                    LEFT JOIN aicrm_groups AS aicrm_groupsPremuimproduct ON aicrm_groupsPremuimproduct.groupid = aicrm_crmentityPremuimproduct.smownerid
                    LEFT JOIN aicrm_users AS aicrm_usersPremuimproduct ON aicrm_usersPremuimproduct.id = aicrm_crmentityPremuimproduct.smownerid
                    left join aicrm_users as aicrm_usersModifiedPremuimproduct on aicrm_crmentity.smcreatorid=aicrm_usersModifiedPremuimproduct.id
                    left join aicrm_users as aicrm_usersCreatorPremuimproduct on aicrm_crmentity.smcreatorid=aicrm_usersModifiedPremuimproduct.id";
        //echo $query;exit;
        return $query;
    }
    ...
    function create_export_query($where)
    {
        ...
        $sql = getPermittedFieldsQuery("Premuimproduct", "detail_view");
        ...
        $query = "SELECT $fields_list,case when (aicrm_users.user_name not like '') then aicrm_users.user_name else aicrm_groups.groupname end as user_name 
				...
				INNER JOIN aicrm_premuimproduct ON aicrm_premuimproduct.premuimproductid = aicrm_crmentity.crmid
        ...
        if ($is_admin == false && $profileGlobalPermission[1] == 1 && $profileGlobalPermission[2] == 1 && $defaultOrgSharingPermission[6] == 3) {
            ...
            $query = $query . " " . getListViewSecurityParameter("Premuimproduct");
        }
    ...
    {
        $rel_tables = array(
            "Documents" => array("aicrm_senotesrel" => array("crmid", "notesid"), "aicrm_premuimproduct" => "premuimproductid"),
            "Accounts" => array("aicrm_account"=>array("accountid","accountid"),"aicrm_premuimproduct"=>"account_id"),
        );
