...
$_SESSION['JOB_ORDER_BY'] = $order_by;
$_SESSION['JOB_SORT_ORDER'] = $sorder;
//<<<<<<<<<<<<<<<<<<< sorting - stored in session >>>>>>>>>>>>>>>>>>>>
...
if(isPermitted('Job','Delete','') == 'yes')
{
	$other_text['del'] = $app_strings[LBL_MASS_DELETE];
}
if(isPermitted('Job','EditView','') == 'yes')
{
...
if($viewid != "0")
{
	...
	$query = $oCustomView->getModifiedCvListQuery($viewid,$listquery,"Job");
}else
{
	$query = getListQuery("Job");
}
//<<<<<<<<customview>>>>>>>>>
...
if(isset($order_by) && $order_by != '')
{
	if($order_by == 'smownerid')
        ...
        {
		$tablename = getTableNameForField('Job',$order_by);
...
$listview_header = getListViewHeader($focus,"Job",$url_string,$sorder,$order_by,"",$oCustomView);
$smarty->assign("LISTHEADER", $listview_header);

$listview_header_search = getSearchListHeaderValues($focus,"Job",$url_string,$sorder,$order_by,"",$oCustomView);
$smarty->assign("SEARCHLISTHEADER",$listview_header_search);

$listview_entries = getListViewEntries($focus,"Job",$list_result,$navigation_array,"","","EditView","Delete",$oCustomView);
...
$navigationOutput = getTableHeaderSimpleNavigation($navigation_array, $url_string,"Job","index",$viewid);
