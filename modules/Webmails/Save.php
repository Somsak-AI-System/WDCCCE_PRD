...
//Save the To field information in aicrm_emaildetails
$all_to_ids = $email->to;
...
//to save the email details in aicrm_emaildetails aicrm_tables
$fieldid = $adb->query_result($adb->pquery('select fieldid from aicrm_field where tablename="aicrm_contactdetails" and fieldname="email" and columnname="email" and aicrm_field.presence in (0,2)', array()),0,'fieldid');
...
function add_attachment_to_contact($cid,$email,$emailid) {
	// add aicrm_attachments to contact
	...
	for($j=0;$j<2;$j++) {
	    ...
	    for($i=0,$num_files=count($attachments);$i<$num_files;$i++)
	    {
            ...
            $query = "insert into aicrm_crmentity (crmid,smcreatorid,smownerid,setype,description,createdtime,modifiedtime) values(?,?,?,?,?,?,?)";
            ...
            $sql = "insert into aicrm_attachments (attachmentsid,name,description,type,path) values(?,?,?,?,?)";
            ...
            $result = $adb->pquery($sql, $params);
                ...
                if(!empty($result)){
	                ...
	                $sql1 = "insert into aicrm_senotesrel values(?,?)";
	                $params1 = array($cid, $document->id);
	                $result = $adb->pquery($sql1, $params1);
	                
	                $sql1 = "insert into aicrm_seattachmentsrel values(?,?)";
