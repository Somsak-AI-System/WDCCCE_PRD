...
if($email->relationship != 0 && $email->relationship["type"] == "Leads") {
	$q = "SELECT aicrm_leaddetails.firstname, aicrm_leaddetails.lastname, aicrm_leaddetails.email, aicrm_leaddetails.company, aicrm_crmentity.smownerid from aicrm_leaddetails left join aicrm_crmentity on aicrm_crmentity.crmid=aicrm_leaddetails.leadid WHERE aicrm_leaddetails.leadid=?";
...
if($email->relationship != 0 && $email->relationship["type"] == "Contacts") {
	$q = "SELECT aicrm_contactdetails.firstname, aicrm_contactdetails.lastname, aicrm_contactdetails.email, aicrm_contactdetails.title, aicrm_crmentity.smownerid from aicrm_contactdetails left join aicrm_crmentity on aicrm_crmentity.crmid=aicrm_contactdetails.contactid WHERE aicrm_contactdetails.contactid=?";
	$rs = $adb->pquery($q, array($email->relationship["id"]));
...
if($email->relationship != 0 && $email->relationship["type"] == "Accounts") {
	$q = "SELECT acccount.accountname, aicrm_account.email1, aicrm_account.website, aicrm_account.industry, aicrm_crmentity.smownerid from aicrm_account left join aicrm_crmentity on aicrm_crmentity.crmid=aicrm_account.accountid WHERE aicrm_account.accountid=?";
	$rs = $adb->pquery($q, array($email->relationship["id"]));
