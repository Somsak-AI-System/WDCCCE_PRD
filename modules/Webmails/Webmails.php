...
class Webmails extends CRMEntity {
	...
	// leads search
	$sql = "SELECT * from aicrm_leaddetails left join aicrm_crmentity on aicrm_crmentity.crmid=aicrm_leaddetails.leadid where aicrm_leaddetails.email = ? AND aicrm_crmentity.deleted='0'";
	...
	// contacts search
	$sql = "SELECT * from aicrm_contactdetails left join aicrm_crmentity on aicrm_crmentity.crmid=aicrm_contactdetails.contactid where aicrm_contactdetails.email = ?  AND aicrm_crmentity.deleted='0'";
	$res = $this->db->pquery($sql,array(trim($this->from)),true,"Error: "."<BR>$query");
	$numRows = $this->db->num_rows($res);
	if($numRows > 0)
	...
	// aicrm_accounts search
	$sql = "SELECT * from aicrm_account left join aicrm_crmentity on aicrm_crmentity.crmid=aicrm_account.accountid where aicrm_account.email1 = ? OR aicrm_account.email1=?  AND aicrm_crmentity.deleted='0'";
