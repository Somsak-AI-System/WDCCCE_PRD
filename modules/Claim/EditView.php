...
global $app_strings,$mod_strings,$currentModule,$current_user,$theme;
...
if(isset($_REQUEST['record']) && $_REQUEST['record'] !='') 
{
	...
	$focus->mode = 'edit'; 	
	$focus->retrieve_entity_info($_REQUEST['record'],"Claim");
...
if(empty($_REQUEST['record']) && $focus->mode != 'edit'){
	...
	$focus->column_fields['cf_3321']='Active';
}
...
if($focus->mode == ''){
        $focus->column_fields['claim_date'] = date('Y-m-d');
        $focus->column_fields['claim_time'] = date('H:i');

        $query = "SELECT * FROM aicrm_role WHERE roleid=?";
        $rs = $adb->pquery($query, [$current_user->roleid]);
        $role = $adb->fetch_array($rs);

        $focus->column_fields['claim_from'] = $current_user->first_name.' '.$current_user->last_name;
        $focus->column_fields['sale_channel'] = $role['rolename'];
}
