...
$log = LoggerManager::getLogger('Claim_list');
...
$_SESSION['CLAIM_ORDER_BY'] = $order_by;
$_SESSION['CLAIM_SORT_ORDER'] = $sorder;
//<<<<<<<<<<<<<<<<<<< sorting - stored in session >>>>>>>>>>>>>>>>>>>>
...
$oCustomView = new CustomView("Claim");
...
if(isPermitted('Claim','Delete','') == 'yes')
{
	$other_text['del'] = $app_strings[LBL_MASS_DELETE];
}
if(isPermitted('Claim','EditView','') == 'yes')
{
...
if($viewid != "0")
{
	$listquery = getListQuery("Claim");
	$query = $oCustomView->getModifiedCvListQuery($viewid,$listquery,"Claim");
}else
{
	$query = getListQuery("Claim");
}
//<<<<<<<<customview>>>>>>>>>
...
if(isset($order_by) && $order_by != '')
{
	if($order_by == 'smownerid')
        ...
        {
		$tablename = getTableNameForField('Claim',$order_by);
...
$listview_header = getListViewHeader($focus,"Claim",$url_string,$sorder,$order_by,"",$oCustomView);
$smarty->assign("LISTHEADER", $listview_header);

$listview_header_search = getSearchListHeaderValues($focus,"Claim",$url_string,$sorder,$order_by,"",$oCustomView);
$smarty->assign("SEARCHLISTHEADER",$listview_header_search);

$listview_entries = getListViewEntries($focus,"Claim",$list_result,$navigation_array,"","","EditView","Delete",$oCustomView);
...
$navigationOutput = getTableHeaderSimpleNavigation($navigation_array, $url_string,"Claim","index",$viewid);
