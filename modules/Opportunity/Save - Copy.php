...
//new SLA==================================================================================================================
			...
			order by aicrm_opportunity.opportunityid DESC   limit 1
			";
			//echo $sql;exit;
			$data_start = $adb->pquery($sql, array());
			$rows1 = $adb->num_rows($data_start);
			...
			if($M>1){
				for($i=0;$i<$M;$i++){
					if($i==0){
						...
						where 1
						and aicrm_crmentityJOB1.deleted =0
						and aicrm_opportunitycf.cf_1328='".$case_date1."'
						and aicrm_opportunity.parent_id='".$ticketid."'
						and cf_1224!='Assistant'
						and cf_1212 in(10,12,13)/**/
						order by aicrm_opportunity.opportunityid desc    limit 1
						...
						where 1
						and aicrm_crmentityJOB1.deleted =0
						and aicrm_opportunitycf.cf_1463='".$chk_case_date."'
						and aicrm_opportunity.parent_id='".$ticketid."'
						and cf_1224!='Assistant'
						...
						*/
						order by aicrm_opportunity.opportunityid asc   limit 1
						";
						//echo $sql."<br>";exit;
						$data_start = $adb->pquery($sql, array());
						$rows1 = $adb->num_rows($data_start);
						...
						where 1
						and aicrm_crmentityJOB1.deleted =0
						and aicrm_opportunitycf.cf_1211='".$chk_case_date."'
						and aicrm_opportunity.parent_id='".$ticketid."'
						and cf_1224!='Assistant'
						and cf_1212 in(10,12,13)/**/
						order by aicrm_opportunity.opportunityid asc   limit 1
...
//new SLA 28-5-2014=====================================
			...
			order by aicrm_opportunitycf.cf_1211 DESC   limit 1
			";
			//echo $sql;exit;
			$data_start = $adb->pquery($sql, array());
			$rows1 = $adb->num_rows($data_start);
			...
			$sql="
			select
			aicrm_opportunity.opportunityid
			...
			where 1
			and aicrm_crmentityJOB1.deleted =0
			and aicrm_opportunitycf.cf_1463!=''
			and aicrm_opportunity.parent_id='".$ticketid."'
			and cf_1212 in(10,12,13)

			order by aicrm_opportunity.opportunityid
			";
			//echo $sql;exit;
			$data_start = $adb->pquery($sql, array());
			$rows1 = $adb->num_rows($data_start);
...
//ส่วน update===========================================================================================================================
	...
	$sql="update aicrm_opportunitycf set cf_1258='".$OT1."', cf_1259='".$OT2."', cf_1260='".$OT3."', cf_1261='".$OT4."' 
	,cf_1222='".$TotalExpense."'
	where opportunityid='".$focus->id."' ";
	//echo $sql;exit;
	$adb->pquery($sql, array());
...
//คำนวณ Respons Time 
	...
	GROUP BY cf_1211   
	order by aicrm_opportunity.opportunityid asc
	limit 1
	";
	//echo $sql;exit;
	$data_start = $adb->pquery($sql, array());
