...
if(trim($subject) != "")
{
	if(isset($storearray) && $camodule != "")
	{
		foreach($storearray as $id)
		{
			...
			{
				$sql="select * from aicrm_contactdetails inner join aicrm_crmentity on aicrm_crmentity.crmid = aicrm_contactdetails.contactid where aicrm_crmentity.deleted =0 and aicrm_contactdetails.contactid=?";
			...
			{
				$sql="select * from aicrm_leaddetails inner join aicrm_crmentity on aicrm_crmentity.crmid = aicrm_leaddetails.leadid where aicrm_crmentity.deleted =0 and aicrm_leaddetails.leadid=?";
				$result = $adb->pquery($sql, array($id));
				$camodulerow = $adb->fetch_array($result);
				if(isset($camodulerow))
				{
			...
			{
				$sql="select * from aicrm_account inner join aicrm_crmentity on aicrm_crmentity.crmid = aicrm_account.accountid where aicrm_crmentity.deleted =0 and aicrm_account.accountid=?";
				$result = $adb->pquery($sql, array($id));
				$camodulerow = $adb->fetch_array($result);
				if(isset($camodulerow))
				{
...
function SendMailtoCustomView($module,$id,$to,$current_user_id,$subject,$contents)
{
	...
	{
		global $adb;
		$sql = "select * from aicrm_users where id= ?";
