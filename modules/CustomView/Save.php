...
if($cvmodule != "") {
	...
	//this is  will cause only the chosen fields to be added to the aicrm_cvcolumnlist table 
	...
	for ($i=0;$i<count($allKeys);$i++) {
	   ...
	   if($string == "column") {
		   //the contusion, will cause only the chosen fields to be added to the aicrm_cvcolumnlist table 
	...
	if(!$cvid) {
		...
		if($genCVid != "") {
			$customviewsql = "INSERT INTO aicrm_customview(cvid, viewname, setdefault, setmetrics, entitytype, status, userid) 
				VALUES (?,?,?,?,?,?,?)";
			...
			$log->info("CustomView :: Save :: aicrm_customview created successfully");
						
			if($setdefault == 1) {
				$sql_result = $adb->pquery("SELECT * FROM aicrm_user_module_preferences WHERE userid = ? and tabid = ?",array($current_user->id, $cv_tabid));
				if($adb->num_rows($sql_result) > 0) {
					$updatedefaultsql = "UPDATE aicrm_user_module_preferences SET default_cvid = ? WHERE userid = ? and tabid = ?";
					$updatedefaultresult = $adb->pquery($updatedefaultsql, array($genCVid, $current_user->id, $cv_tabid));
				} else {
					$insertdefaultsql = "INSERT INTO aicrm_user_module_preferences(userid, tabid, default_cvid) values (?,?,?)";
					$insertdefaultresult = $adb->pquery($insertdefaultsql, array($userid, $cv_tabid, $genCVid));
				}
			} else {
				$sql_result = $adb->pquery("SELECT * FROM aicrm_user_module_preferences WHERE userid = ? and tabid = ?",array($current_user->id, $cv_tabid));
				if($adb->num_rows($sql_result) > 0) {
					$deletedefaultsql = "DELETE FROM aicrm_user_module_preferences WHERE userid = ? and tabid = ?";
			...
			if($customviewresult) {
				if(isset($columnslist)) {
					...
					$log->info("CustomView :: Save :: aicrm_cvcolumnlist created successfully");
					if($std_filter_list["columnname"] !="") {
						...
						$log->info("CustomView :: Save :: aicrm_cvstdfilter created successfully");
					}
					...
					$log->info("CustomView :: Save :: aicrm_cvadvfilter created successfully");
				}
			}
			$cvid = $genCVid;
		}
		...
		if($is_admin == true || $current_user->id) {
			$updatecvsql = "UPDATE aicrm_customview
			...
			$log->info("CustomView :: Save :: aicrm_customview upated successfully".$cvid);
			...
			$deletesql = "DELETE FROM aicrm_cvcolumnlist WHERE cvid = ?";
			$deleteresult = $adb->pquery($deletesql, array($cvid));
	
			$deletesql = "DELETE FROM aicrm_cvstdfilter WHERE cvid = ?";
			$deleteresult = $adb->pquery($deletesql, array($cvid));
	
			$deletesql = "DELETE FROM aicrm_cvadvfilter WHERE cvid = ?";
			$deleteresult = $adb->pquery($deletesql, array($cvid));
			$log->info("CustomView :: Save :: aicrm_cvcolumnlist,cvstdfilter,cvadvfilter deleted successfully before update".$genCVid);
			...
			if($updatecvresult) {
				if(isset($columnslist)) {
					...
					$log->info("CustomView :: Save :: aicrm_cvcolumnlist update successfully".$genCVid);
					if($std_filter_list["columnname"] !="") {
						$stdfiltersql = "INSERT INTO aicrm_cvstdfilter (cvid,columnname,stdfilter,startdate,enddate) VALUES (?,?,?,?,?)";
						$stdfilterparams = array($genCVid, $std_filter_list["columnname"], $std_filter_list["stdfilter"], $adb->formatDate($std_filter_list["startdate"], true), $adb->formatDate($std_filter_list["enddate"], true));
						$stdfilterresult = $adb->pquery($stdfiltersql, $stdfilterparams);
						$log->info("CustomView :: Save :: aicrm_cvstdfilter update successfully".$genCVid);
					}
					for($i=0;$i<count($adv_filter_col);$i++) {
						$col = explode(":",$adv_filter_col[$i]);
						$temp_val = explode(",",$adv_filter_value[$i]);
					...
					$log->info("CustomView :: Save :: aicrm_cvadvfilter update successfully".$cvid);
				}
			}
		}
	}
