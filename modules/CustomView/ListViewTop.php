...
function getKeyMetrics($maxval,$calCnt)
{
	...
	$metriclists = getMetricList($maxval);
	...
	if(isset($metriclists))
	{
		foreach ($metriclists as $key => $metriclist)
		{
			...
			if($metriclist['module'] == 'Calendar')
				$listquery .= " AND aicrm_activity.activitytype != 'Emails' ";
			...
			if($metriclist['module'] == "Calendar" and !$adb->isPostgres())
				$metricsql.=" group by aicrm_activity.activityid ";
...
function getMetricList($maxval)
{
	...
	$ssql .= " where aicrm_customview.setmetrics = 1 ";
	$sparams = array();
	...
	$ssql .= " order by aicrm_customview.entitytype limit ".$maxval."";
