	...
	foreach($storearray as $id)
	{
		if(isPermitted($return_module,'EditView',$id) == 'yes')
		{
			...
			if($return_module == "Calendar"){
				$del_act = "delete from aicrm_salesmanactivityrel where smid=(select smownerid from aicrm_crmentity where crmid=?) and activityid=?";
				$adb->pquery($del_act,array($id, $id));
				if($_REQUEST['owner_type'] == 'User') {
					$count_r = $adb->pquery("select * from aicrm_salesmanactivityrel where smid=? and activityid=?",array($idval, $id));
					if($adb->num_rows($count_r) == 0) {
						$insert = "insert into aicrm_salesmanactivityrel values(?,?)";
						$result = $adb->pquery($insert, array($idval, $id));
					}
				}
			}	
			//Now we have to update the smownerid
			$sql = "update aicrm_crmentity set modifiedby=?, smownerid=?, modifiedtime=? where crmid=?";
...
elseif(isset($_REQUEST['leadval']) && $_REQUEST['leadval']!='')
{
	...
	foreach($storearray as $id)
	{
		...
		{
			if($id != '') {
				$sql = "update aicrm_leaddetails set leadstatus=? where leadid=?";
				$result = $adb->pquery($sql, array($leadstatusval, $id));
				$query = "update aicrm_crmentity set modifiedby=?, modifiedtime=? where crmid=?";
