 ...
 ********************************************************************************/
/*********************************************************************************
 * $Header: /advent/projects/wesat/aicrm_crm/sugarcrm/modules/Users/Save.php,v 1.14 2005/03/17 06:37:39 rank Exp $
...
{
        $user_query = "SELECT user_name FROM aicrm_users WHERE user_name =?";
        $user_result = $adb->pquery($user_query, array($user_name));
        $group_query = "SELECT groupname FROM aicrm_groups WHERE groupname =?";
...
if(! $_REQUEST['changepassword'] == 'true')
{
	...
	if(isset($_REQUEST['pm_flag']) && $_REQUEST['pm_flag'] == 'on')
		$focus->column_fields['pm_flag'] = 1;
	else
		$focus->column_fields['pm_flag'] = 0;

	if(isset($_REQUEST['wecare_flag']) && $_REQUEST['wecare_flag'] == 'on')
		$focus->column_fields['wecare_flag'] = 1;
	else
		$focus->column_fields['wecare_flag'] = 0;
	...
	$query_prev_interval = $adb->pquery("SELECT reminder_interval from aicrm_users where id=?",array($focus->id));
...
if($_REQUEST['mode'] == 'create') {
	global $app_strings, $mod_strings, $default_charset, $site_URL;
