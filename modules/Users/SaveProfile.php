...
$profile_id = $adb->getUniqueID("aicrm_profile");
//Inserting values into Profile Table
$sql1 = "insert into aicrm_profile(profileid, profilename, description) values(?,?,?)";
$adb->pquery($sql1, array($profile_id,$profilename, $description));
	...
	$prof_query="select profileid from aicrm_profile order by profileid ASC";
	$prof_result = $adb->pquery($prof_query, array());
	$first_prof_id = $adb->query_result($prof_result,0,'profileid');

//Add --30-05-2023 -Bas
$sql_insert="insert into aicrm_profile2tab values(?,?,?)";
$adb->pquery($sql_insert, array($profileid, 3, 0));

$tab_perr_result = $adb->pquery("select * from aicrm_profile2tab where profileid=?", array($first_prof_id));
$act_perr_result = $adb->pquery("select * from aicrm_profile2standardpermissions where profileid=?", array($first_prof_id));
$act_utility_result = $adb->pquery("select * from aicrm_profile2utility where profileid=?", array($first_prof_id));
