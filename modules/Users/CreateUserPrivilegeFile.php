...
function createUserSharingPrivilegesfile($userid)
{
	...
	if($handle)
	{
		...
		else
		{
			...
			$relModSharArr=Array();	
			$query ="select * from aicrm_datashare_relatedmodules";
			...
			$custom_modules = getSharingModuleList(
				Array('Leads', 'Accounts', 'Contacts', 'Potentials', 'HelpDesk', 'Emails', 'Campaigns','Quotes', 'PurchaseOrder', 'SalesOrder', 'Invoice'));
...
function getUserModuleSharingObjects($module,$userid,$def_org_share,$current_user_roles,$parent_roles,$current_user_groups)
{
	...
	if($def_org_share[$mod_tabid] == 3 || $def_org_share[$mod_tabid] == 0)
	{
		...
		$grp_read_per=Array();
		$grp_write_per=Array();
		//Retreiving from aicrm_role to aicrm_role
		$query="select aicrm_datashare_role2role.* from aicrm_datashare_role2role inner join aicrm_datashare_module_rel on aicrm_datashare_module_rel.shareid=aicrm_datashare_role2role.shareid where aicrm_datashare_module_rel.tabid=? and aicrm_datashare_role2role.to_roleid=?";
		...
		$query="select aicrm_datashare_role2rs.* from aicrm_datashare_role2rs inner join aicrm_datashare_module_rel on aicrm_datashare_module_rel.shareid=aicrm_datashare_role2rs.shareid where aicrm_datashare_module_rel.tabid=? and aicrm_datashare_role2rs.to_roleandsubid in (". generateQuestionMarks($parRoleList) .")";
		...
		$query="select aicrm_datashare_role2group.* from aicrm_datashare_role2group inner join aicrm_datashare_module_rel on aicrm_datashare_module_rel.shareid=aicrm_datashare_role2group.shareid where aicrm_datashare_module_rel.tabid=?";
		$qparams = array($mod_tabid);
		if (count($groupList) > 0) {
			$query .= " and aicrm_datashare_role2group.to_groupid in (". generateQuestionMarks($groupList) .")";
		...
		//Retreiving from rs to aicrm_role
		$query="select aicrm_datashare_rs2role.* from aicrm_datashare_rs2role inner join aicrm_datashare_module_rel on aicrm_datashare_module_rel.shareid=aicrm_datashare_rs2role.shareid where aicrm_datashare_module_rel.tabid=? and aicrm_datashare_rs2role.to_roleid=?";
		$result=$adb->pquery($query, array($mod_tabid, $current_user_roles));
		$num_rows=$adb->num_rows($result);
		for($i=0;$i<$num_rows;$i++)
		{
		...
		array_push($parRoleList, $current_user_roles);
		$query="select aicrm_datashare_rs2rs.* from aicrm_datashare_rs2rs inner join aicrm_datashare_module_rel on aicrm_datashare_module_rel.shareid=aicrm_datashare_rs2rs.shareid where aicrm_datashare_module_rel.tabid=? and aicrm_datashare_rs2rs.to_roleandsubid in (". generateQuestionMarks($parRoleList) .")";
		$result=$adb->pquery($query, array($mod_tabid, $parRoleList));
		$num_rows=$adb->num_rows($result);
		for($i=0;$i<$num_rows;$i++)
		{
		...
		$query="select aicrm_datashare_rs2grp.* from aicrm_datashare_rs2grp inner join aicrm_datashare_module_rel on aicrm_datashare_module_rel.shareid=aicrm_datashare_rs2grp.shareid where aicrm_datashare_module_rel.tabid=?";
		$qparams = array($mod_tabid);
		if (count($groupList) > 0) {
			$query .= " and aicrm_datashare_rs2grp.to_groupid in (". generateQuestionMarks($groupList) .")";
			array_push($qparams, $groupList);
		}
		$result=$adb->pquery($query, $qparams);
		$num_rows=$adb->num_rows($result);
		...
		$query="select aicrm_datashare_grp2role.* from aicrm_datashare_grp2role inner join aicrm_datashare_module_rel on aicrm_datashare_module_rel.shareid=aicrm_datashare_grp2role.shareid where aicrm_datashare_module_rel.tabid=? and aicrm_datashare_grp2role.to_roleid=?";
		$result=$adb->pquery($query, array($mod_tabid, $current_user_roles));
		$num_rows=$adb->num_rows($result);
		for($i=0;$i<$num_rows;$i++)
		{
		...
		$query="select aicrm_datashare_grp2rs.* from aicrm_datashare_grp2rs inner join aicrm_datashare_module_rel on aicrm_datashare_module_rel.shareid=aicrm_datashare_grp2rs.shareid where aicrm_datashare_module_rel.tabid=? and aicrm_datashare_grp2rs.to_roleandsubid in (". generateQuestionMarks($parRoleList) .")";
		$result=$adb->pquery($query, array($mod_tabid, $parRoleList));
		$num_rows=$adb->num_rows($result);
		for($i=0;$i<$num_rows;$i++)
		{
		...
		$query="select aicrm_datashare_grp2grp.* from aicrm_datashare_grp2grp inner join aicrm_datashare_module_rel on aicrm_datashare_module_rel.shareid=aicrm_datashare_grp2grp.shareid where aicrm_datashare_module_rel.tabid=?";
		$qparams = array($mod_tabid);
		if (count($groupList) > 0) {
			$query .= " and aicrm_datashare_grp2grp.to_groupid in (". generateQuestionMarks($groupList) .")";
			array_push($qparams, $groupList);
		}
		$result=$adb->pquery($query, $qparams);
		$num_rows=$adb->num_rows($result);
...
function getRelatedModuleSharingArray($par_mod,$share_mod,$mod_sharingrule_members,$mod_share_read_per,$mod_share_write_per,$def_org_share)
{
	...
	if($def_org_share[$share_mod_id] == 3 || $def_org_share[$share_mod_id] == 0)
	{
		...
		{
			$query = "select aicrm_datashare_relatedmodule_permission.* from aicrm_datashare_relatedmodule_permission inner join aicrm_datashare_relatedmodules on aicrm_datashare_relatedmodules.datashare_relatedmodule_id=aicrm_datashare_relatedmodule_permission.datashare_relatedmodule_id where aicrm_datashare_relatedmodule_permission.shareid=? and aicrm_datashare_relatedmodules.tabid=? and aicrm_datashare_relatedmodules.relatedto_tabid=?";
...
function populateSharingtmptables($userid)
{
	...
	//Deleting from the existing aicrm_tables
	$table_arr=Array('aicrm_tmp_read_user_sharing_per', 'aicrm_tmp_write_user_sharing_per','aicrm_tmp_read_group_sharing_per','aicrm_tmp_write_group_sharing_per','aicrm_tmp_read_user_rel_sharing_per','aicrm_tmp_write_user_rel_sharing_per','aicrm_tmp_read_group_rel_sharing_per','aicrm_tmp_write_group_rel_sharing_per');
...
function populateSharingPrivileges($enttype,$userid,$module,$pertype, $var_name_arr=false)
{
	...
	if($enttype=='USER')
	{
		if($pertype =='read')
		{
			$table_name='aicrm_tmp_read_user_sharing_per';
			$var_name=$module.'_share_read_permission';
		}
		elseif($pertype == 'write')
		{
			$table_name='aicrm_tmp_write_user_sharing_per';
	...
	elseif($enttype=='GROUP')
	{
		if($pertype =='read')
		{
			$table_name='aicrm_tmp_read_group_sharing_per';
			$var_name=$module.'_share_read_permission';
		}
		elseif($pertype == 'write')
		{
			$table_name='aicrm_tmp_write_group_sharing_per';
			$var_name=$module.'_share_write_permission';
		}
		// Lookup for the variable if not set through function argument
		if(!$var_name_arr) $var_name_arr=$$var_name;
...
function populateRelatedSharingPrivileges($enttype,$userid,$module,$relmodule,$pertype, $var_name_arr=false)
{
	...
	if($enttype=='USER')
	{
		if($pertype =='read')
		{
			$table_name='aicrm_tmp_read_user_rel_sharing_per';
			$var_name=$module.'_'.$relmodule.'_share_read_permission';
		}
		elseif($pertype == 'write')
		{
			$table_name='aicrm_tmp_write_user_rel_sharing_per';
	...
	elseif($enttype=='GROUP')
	{
		if($pertype =='read')
		{
			$table_name='aicrm_tmp_read_group_rel_sharing_per';
			$var_name=$module.'_'.$relmodule.'_share_read_permission';
		}
		elseif($pertype == 'write')
		{
			$table_name='aicrm_tmp_write_group_rel_sharing_per';
			$var_name=$module.'_'.$relmodule.'_share_write_permission';
		}
		// Lookup for the variable if not set through function argument
		if(!$var_name_arr) $var_name_arr=$$var_name;
