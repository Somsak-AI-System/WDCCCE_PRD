...
$query = $adb->pquery("SELECT tablename,entityidfield, fieldname from aicrm_entityname WHERE modulename = ?",array($sModule));
...
foreach ($fieldname as $fieldName) {
	...
	// Handling case where fieldname in aicrm_entityname mismatches fieldname in aicrm_field
	...
	// END
	//$sModule = (vtlib_purify($_REQUEST['CurModule']) == 'Calendar') ? 'Events' : vtlib_purify($_REQUEST['CurModule']);
...
$output = '<table width="100%" border="0" cellpadding="5" cellspacing="0" class="layerHeadingULine">
			...
			<td width="5%" align="right"><a href="javascript:fninvsh(\'lstRecordLayout\');"><img src="'. aicrm_imageurl('close.gif', $theme).'" border="0"  align="absmiddle" /></a></td>
			</tr>
