 ...
 ********************************************************************************/
/*********************************************************************************
 * $Header: /advent/projects/wesat/aicrm_crm/sugarcrm/modules/Users/Logout.php,v 1.8 2005/03/21 04:51:21 ray Exp $
...
if($enable_ftp_backup == 'true' && is_admin($current_user) == true)
{
	...
	$ftpuser = '';
	$ftppassword = '';
	$query = "select * from aicrm_systems where server_type=?";
...
if($enable_local_backup == 'true' && is_admin($current_user) == true)
{
		...
		$path_query = $adb->pquery("SELECT * FROM aicrm_systems WHERE server_type = ?",array('local_backup'));
...
//echo $_SESSION["user_start_time"];
//echo $_SESSION["user_end_time"];
//exit;
$result = $adb->query("SELECT count(*) as count FROM ai_check_user_login where user_id ='".$_SESSION["user_id"]."' and start_time='".$_SESSION["user_start_time"]."' and end_time='".$_SESSION["user_end_time"]."'");
$dbversion = $adb->query_result($result, 0, 'count');
$chk=$dbversion;

$query = "update ai_check_user_login set end_time=NOW(),status=? where user_id=? and id=?";
$params = array('1',$_SESSION["user_id"],$_SESSION["login_id"]);				
