 ...
 ********************************************************************************/
/*********************************************************************************
 * $Header: /advent/projects/wesat/aicrm_crm/sugarcrm/modules/Users/Forms.php,v 1.3 2004/11/08 13:48:29 jack Exp $
 * Description:  Contains a variety of utility functions used to display UI
 * components such as form aicrm_headers and footers.  Intended to be modified on a per
 * theme basis.
 * Portions created by SugarCRM are Copyright (C) SugarCRM, Inc.
 * All Rights Reserved.
 * Contributor(s): ______________________________________..
...
function checkAsteriskDetails(){
	...
	$sql = "select * from aicrm_asterisk";
...
function getAsteriskExtensions(){
	...
	$sql = "select * from aicrm_asteriskextensions where userid != ".$current_user->id;
...
$lbl_first_name = $mod_strings['LBL_LIST_FIRSTT_NAME'];
...
function verify_data(form) {
	...
	if(typeof form.asterisk_extension !== "undefined"){
	...
	} //first_name_th, last_name, first_name, section
	...
	if (trim(form.first_name_th.value) == "") {
		isError = true;
		errorMessage += "\\nชื่อ";
		oField_miss =form.first_name_th;
	}
	if (trim(form.last_name_th.value) == "") {
		isError = true;
		errorMessage += "\\nนามสกุล";
		oField_miss =form.last_name_th;
	}
	if (trim(form.section.value) == "--None--") {
		isError = true;
		errorMessage += "\\nSection";
		oField_miss =form.section;
	}
	...
	if(typeof form.yahoo_id !== "undefined"){
	...
	if(! upload_filter("imagename", "png|PNG") )
