...
$sql2 = "select * from aicrm_def_org_share where editstatus=0";
...
for($i=0; $i<$num_rows; $i++) {
	...
	$sql7="update aicrm_def_org_share set permission=? where tabid=? and ruleid=?";
	$adb->pquery($sql7, array($permission, $tabid, $ruleid));

	if($tabid == 6) {
		$sql8="update aicrm_def_org_share set permission=? where tabid=4";
		$adb->pquery($sql8, array($permission));
	}

	if($tabid == 9) {
		$sql8="update aicrm_def_org_share set permission=? where tabid=16";
		$adb->pquery($sql8, array($permission));		
	}
}
