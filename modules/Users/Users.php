...
/*********************************************************************************
 * $Header: /advent/projects/wesat/aicrm_crm/sugarcrm/modules/Users/Users.php,v 1.10 2005/04/19 14:40:48 ray Exp $
...
class Users extends CRMEntity {
	...
	var $tab_name = Array('aicrm_users','aicrm_attachments','aicrm_user2role','aicrm_asteriskextensions');
	var $tab_name_index = Array('aicrm_users'=>'id','aicrm_attachments'=>'attachmentsid','aicrm_user2role'=>'userid','aicrm_asteriskextensions'=>'userid');
	var $customFieldTable = Array('aicrm_users', 'id');
	...
	'section'=>'',
	'sales_org'=>'',
	'approve_expense_level1' =>'',
	'approve_expense_level2' =>'',
	'approve_expense_level3'=>'',
	'approve_expense_level4'=>'',
	'approve_sample_level1' =>'',
	'approve_sample_level2' =>'',
	'approve_sample_level3'=>'',
	'approve_sample_level4'=>'',
	'moaiwb'=>'',
	'moaimb'=>'',
	'passed_inspection'=>'',
);
	var $table_name = "aicrm_users";
	var $table_index= 'id';
	...
	var $search_fields = Array(
		'ชื่อ'=>Array('aicrm_users'=>'first_name'),
		'นามสกุล'=>Array('aicrm_users'=>'last_name'),
		'อีเมล'=>Array('aicrm_users'=>'email1')
	);
	...
	var $sortby_fields = Array('status','email1','phone_work','is_admin','user_name','first_name','last_name','roleid','phone_mobile','section','position');

	// This is the list of aicrm_fields that are in the lists.
	var $list_fields = Array(
		'ชื่อ'=>Array('aicrm_users'=>'first_name'),
		'นามสกุล'=>Array('aicrm_users'=>'last_name'),
		'Role Name'=>Array('aicrm_user2role'=>'roleid'),
		'User Name'=>Array('aicrm_users'=>'user_name'),
		'สถานะ'=>Array('aicrm_users'=>'status'),
		'อีเมล'=>Array('aicrm_users'=>'email1'),
		'เบอร์มือถือ'=>Array('aicrm_users'=>'phone_mobile'),
		'ตำแหน่ง'=>Array('aicrm_users'=>'position'),
		'แผนก'=>Array('aicrm_users'=>'section'),
		'Admin'=>Array('aicrm_users'=>'is_admin'),
	...
	function encrypt_password($user_password, $crypt_type='')
	{
		...
		//echo "login->".$user_password."->".$salt."<br>";
	...
	function doLogin($user_password) {
		global $AUTHCFG;
		$usr_name = $this->column_fields["user_name"];
		//echo strtoupper($AUTHCFG['authType']);
		switch (strtoupper($AUTHCFG['authType'])) {
			...
			default:
				...
				//echo "SELECT * from $this->table_name where user_name='$usr_name' AND user_password='$encrypted_password'";
	...
	function get_user_crypt_type() {
		...
		global $adb;
		$table_cols = $adb->getColumnNames("aicrm_users");
	...
	{
		global $adb;
		$query = "SELECT id from aicrm_users where user_name=? AND deleted=0";
	...
	function verify_data()
	{
		...
		$query = "SELECT user_name from aicrm_users where user_name=? AND id<>? AND deleted=0";
		$result =$this->db->pquery($query, array($usr_name, $this->id), true, "Error selecting possible duplicate users: ");
		$dup_users = $this->db->fetchByAssoc($result);

		$query = "SELECT user_name from aicrm_users where is_admin = 'on' AND deleted=0";
		$result =$this->db->pquery($query, array(), true, "Error selecting possible duplicate aicrm_users: ");
	...
	{
		$query = "SELECT u1.first_name, u1.last_name from aicrm_users u1, aicrm_users u2 where u1.id = u2.reports_to_id AND u2.id = ? and u1.deleted=0";
		$result =$this->db->pquery($query, array($this->id), true, "Error filling in additional detail aicrm_fields") ;
	...
	function saveentity($module)
	{
		...
		{
			//echo $table_name;
			if($table_name == 'aicrm_attachments')
			{
	...
	function insertIntoEntityTable($table_name, $module)
	{
		...
		if($insertion_mode == 'edit')
		{
			...
			$sql = "select * from aicrm_field where tabid=? and tablename=? and displaytype in (1,3) and aicrm_field.presence in (0,2)";
			$params = array($tabid, $table_name);
		}
		else
		{
			$column = $this->tab_name_index[$table_name];
			if($column == 'id' && $table_name == 'aicrm_users')
			{
		...
		for($i=0; $i<$noofrows; $i++)
		{
			...
			if($fldvalue=='') {
				...
				//$fldvalue =null;
				//echo $fieldname." ".$fldvalue."<br><br>";
				//echo "<pre>";print_r($_REQUEST);echo "</pre>";
				if($columname=="branchid"){
					//print_r($_REQUEST[$columname]);exit;
					if($_REQUEST[$columname] != '')
					{
						$fldvalue = $_REQUEST[$columname];
					}
					else
					{
						$fldvalue = 0;
				...
				fwrite($FileHandle,$columname."==>".$_REQUEST[$columname]."==>".$this->column_fields[$columname]."\r\n");
				fclose($FileHandle);*/
				if($columname=="branchid"){
					//print_r($_REQUEST[$columname]);exit;
					if($_REQUEST[$columname] != '')
					...
					else if($this->column_fields[$columname] != '')
					{
						$fldvalue = $this->column_fields[$columname];
					}
					else
					{
						$fldvalue = 0;
					...
					if(is_array($fldvalue)){
					  $field_list = implode(' |##| ',$fldvalue);
					}else{
					  $field_list = $fldvalue;
					}
					$fldvalue=$field_list;
					//print_r( $fldvalue);exit;

				}
				if($columname=="cf_1739" ){
					if($_REQUEST[$columname] === 'on' || $_REQUEST[$columname] == 1)
					{
						$fldvalue = 1;
					}
					else
				...
				if($_REQUEST[$columname] != '' && $fldvalue==""){
					$fldvalue = $_REQUEST[$columname];
				}
		...
		if($insertion_mode == 'edit')
		{
			...
			if(trim($update) != '')
			{
				...
				//echo $sql1;
				//echo "<pre>";print_r($update_params);echo "</pre>";
	...
	function retrieve_entity_info($record, $module)
	{
		...
		$tabid = getTabid($module);
		$sql1 =  "select * from aicrm_field where tabid=? and aicrm_field.presence in (0,2)";
		...
		$currency_query = "select * from aicrm_currency_info where id=? and currency_status='Active' and deleted=0";
		$currency_result = $adb->pquery($currency_query, array($this->column_fields["currency_id"]));
		if($adb->num_rows($currency_result) == 0)
		{
			$currency_query = "select * from aicrm_currency_info where id =1";
	...
	function uploadAndSaveFile($id,$module,$file_details)
	{
		...
		$binFile = iconv('UTF-8', 'windows-874', $binFile);

		$filetype= $file_details['type'];
		$filesize = $file_details['size'];
		$filetmp_name = $file_details['tmp_name'];

		$current_id = $this->db->getUniqueID("aicrm_crmentity");
		...
		if($save_file == 'true')
		{
			$sql1 = "insert into aicrm_crmentity (crmid,smcreatorid,smownerid,setype,description,createdtime,modifiedtime) values(?,?,?,?,?,?,?)";
 			$params1 = array($current_id, $current_user->id, $ownerid, $module." Attachment", $this->column_fields['description'], $this->db->formatString("aicrm_crmentity","createdtime",$date_var), $this->db->formatDate($date_var, true));
			...
			if($id != '')
			{
				$delquery = 'delete from aicrm_salesmanattachmentsrel where smid = ?';
				$this->db->pquery($delquery, array($id));
			}

			$sql3='insert into aicrm_salesmanattachmentsrel values(?,?)';
			$this->db->pquery($sql3, array($id, $current_id));

			//we should update the imagename in the users table
			$this->db->pquery("update aicrm_users set imagename=? where id=?", array($filename, $id));

			$_SESSION['imageuser'] =$site_URL.$upload_file_path.$current_id.'_'.$filename;
		}
		else
		{
	...
	function getHomeStuffOrder($id){
		...
		//$this->homeorder_array = array('HDB','CVLVT','LTFAQ','OLTSO','HLT','ALVT');
		$this->homeorder_array = array('CVLVT');
		$return_array = Array();
		$homeorder=Array();
		if($id != ''){
			$qry=" select distinct(aicrm_homedefault.hometype) from aicrm_homedefault inner join aicrm_homestuff  on aicrm_homestuff.stuffid=aicrm_homedefault.stuffid where aicrm_homestuff.visible=0 and aicrm_homestuff.userid=?";
	...
	function insertUserdetails($inVal)
	{
		...
		$sql="insert into aicrm_homestuff values(?,?,?,?,?,?)";
		...
		$sql="insert into aicrm_homestuff values(?,?,?,?,?,?)";
		$res=$adb->pquery($sql, array($s6,6,'Default',$uid,$visibility,'Top Case'));
		...
		$visibility=0;
		$sql="insert into aicrm_homestuff values($tc, 15, 'Tag Cloud', $uid, $visibility, 'Tags')";
		$adb->query($sql);
	...
	function unlinkRelationship($id, $return_module, $return_id) {
		...
		if($return_module == 'Accounts' ) {
			...
			$this->db->pquery($relation_query, array($id));
		} elseif($return_module == 'EmailTargetList') {//echo "555";exit;
			$relation_query = 'DELETE FROM aicrm_emailtargetlist_usersrel  WHERE id  =? AND  emailtargetlistid=?';
			$this->db->pquery($relation_query, array($id, $return_id));
		} elseif($return_module == 'Announcement') {
			$relation_query = 'DELETE FROM aicrm_announcement_usersrel  WHERE id  =? AND  announcementid =?';
			$this->db->pquery($relation_query, array($id, $return_id));
			...
			$relation_query = 'DELETE FROM aicrm_smartemail_usersrel  WHERE id  =? AND  smartemailid =?';
			$this->db->pquery($relation_query, array($id, $return_id));
	...
	/** function to save the order in which the modules have to be displayed in the home page for the specified user id
	 ...
	 function saveHomeStuffOrder($id)
	 {
		 ...
		 {
			 for($i = 0;$i < count($this->homeorder_array);$i++)
			 {//echo $this->homeorder_array[$i]."<br>";
				 if($_REQUEST[$this->homeorder_array[$i]] != '')
				 {
					 $save_array[] = $this->homeorder_array[$i];
					 $qry=" update aicrm_homestuff,aicrm_homedefault set aicrm_homestuff.visible=0 where aicrm_homestuff.stuffid=aicrm_homedefault.stuffid and aicrm_homestuff.userid=".$id." and aicrm_homedefault.hometype='".$this->homeorder_array[$i]."'";//To show the default Homestuff on the the Home Page
					 //echo $qry."<br>";
					 $result=$adb->query($qry);
				 }
				 else
				 {
					 $qry="update aicrm_homestuff,aicrm_homedefault set aicrm_homestuff.visible=1 where aicrm_homestuff.stuffid=aicrm_homedefault.stuffid and aicrm_homestuff.userid=".$id." and aicrm_homedefault.hometype='".$this->homeorder_array[$i]."'";//To hide the default Homestuff on the the Home Page
					 $result=$adb->query($qry);
				 }
			 }
			 if($save_array !="")
	...
	*/
	function resetReminderInterval($prev_reminder_interval)
	{
		...
		if($prev_reminder_interval != $this->column_fields['reminder_interval'] ){
			...
			$adb->pquery("UPDATE aicrm_users SET reminder_next_time=?, date_entered=? WHERE id=?",array($set_reminder_next, $this->column_fields['date_entered'], $this->id));
		}
	}
	...
	function generateReportsSecQuery($module,$secmodule){
		...
		left join aicrm_emailtargetlist_usersrel as aicrm_emailtargetlist_usersreltmpUsers on aicrm_emailtargetlist_usersreltmpUsers.emailtargetlistid = aicrm_emailtargetlists.emailtargetlistid and aicrm_emailtargetlist_usersreltmpUsers.id IN (SELECT id FROM aicrm_users  WHERE aicrm_users.deleted=0 AND aicrm_users.status='Active')

		left join aicrm_users on aicrm_users.id=aicrm_emailtargetlist_usersreltmpUsers.id
		left join aicrm_branchs ON aicrm_branchs.branchid = aicrm_users.branchid
		";
		//echo $query;exit;
		/*$query .=" left join aicrm_crmentity as aicrm_crmentityUsers on aicrm_crmentityUsers.crmid=aicrm_emailtargetlists.emailtargetlistid and aicrm_crmentityEmailTargetLists.deleted=0
				left join aicrm_emailtargetlistscf on aicrm_emailtargetlistscf.emailtargetlistid = aicrm_crmentityemailtargetlists.crmid
				left join aicrm_groups as aicrm_groupsEmailTargetLists on aicrm_groupsEmailTargetLists.groupid = aicrm_crmentityEmailTargetLists.smownerid
				left join aicrm_users as aicrm_usersEmailTargetLists on aicrm_usersemailtargetlists.id = aicrm_usersEmailTargetLists.smownerid"; */
		//echo $query;exit;
		return $query;
	}
}
