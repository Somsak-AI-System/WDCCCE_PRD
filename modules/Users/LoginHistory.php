...
class LoginHistory {
	...
	var $table_name = "aicrm_loginhistory";
	...
	var $list_fields = Array(
			'User Name'=>Array('aicrm_loginhistory'=>'user_name'), 
			'User IP'=>Array('aicrm_loginhistory'=>'user_ip'), 
			'Signin Time'=>Array('aicrm_loginhistory'=>'login_time'),
		        'Signout Time'=>Array('aicrm_loginhistory'=>'logout_time'), 
			'Status'=>Array('aicrm_loginhistory'=>'status'),
		);	
...
**/
	function getHistoryListViewEntries($username, $navigation_array, $sorder='', $orderby='')
	{
		...
		if($sorder != '' && $order_by != '')
	       		$list_query = "Select * from aicrm_loginhistory where user_name=? order by ".$order_by." ".$sorder;
		else
				$list_query = "Select * from aicrm_loginhistory where user_name=? order by ".$this->default_order_by." ".$this->default_sort_order;
	...
	*/
	function user_login(&$usname,&$usip,&$intime)
	{
		...
		$query = "Insert into aicrm_loginhistory (user_name, user_ip, logout_time, login_time, status) values (?,?,?,?,?)";
	...
	*/
	...
	{
		global $adb;
		$logid_qry = "SELECT max(login_id) AS login_id from aicrm_loginhistory where user_name=? and user_ip=?";
		...
		$query = "Update aicrm_loginhistory set logout_time =?, status=? where login_id = ?";
