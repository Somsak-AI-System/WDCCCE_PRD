...
//Retreiving the aicrm_tabs permission array
$tab_perr_result = $adb->pquery("select * from aicrm_profile2tab where profileid=?", array($profileid));
$act_perr_result = $adb->pquery("select * from aicrm_profile2standardpermissions where profileid=?", array($profileid));
$act_utility_result = $adb->pquery("select * from aicrm_profile2utility where profileid=?", array($profileid));
...
$num_act_util_per = $adb->num_rows($act_utility_result);
	...
	//Updating aicrm_profile2global permissons aicrm_table
	...
	$update_query = "update  aicrm_profile2globalpermissions set globalactionpermission=? where globalactionid=1 and profileid=?";
	$adb->pquery($update_query, array($view_all, $profileid));
	$update_query = "update  aicrm_profile2globalpermissions set globalactionpermission=? where globalactionid=2 and profileid=?";
	...
	for($i=0; $i<$num_tab_per; $i++)
	{
		...
		if($tab_id != 3 && $tab_id != 16)
		{
			...
			$update_query = "update aicrm_profile2tab set permissions=? where tabid=? and profileid=?";
			$adb->pquery($update_query, array($permission_value, $tab_id, $profileid));
			if($tab_id ==9)
			{
				$update_query = "update aicrm_profile2tab set permissions=? where tabid=16 and profileid=?";
	...
	for($i=0; $i<$num_act_per; $i++)
	{
		...
		if($tab_id != 16)
		{
			...
			$update_query = "update aicrm_profile2standardpermissions set permissions=? where tabid=? and Operation=? and profileid=?";
			$adb->pquery($update_query, array($permission_value, $tab_id, $action_id, $profileid));
			if($tab_id ==9)
			{
				$update_query = "update aicrm_profile2standardpermissions set permissions=? where tabid=16 and Operation=? and profileid=?";
	...
	for($i=0; $i<$num_act_util_per; $i++)
	{
		...
		$update_query = "update aicrm_profile2utility set permission=? where tabid=? and activityid=? and profileid=? ";
		//echo $update_query; 
		/*echo "<br>";
		echo $permission_value; 
		echo "<br>";
		echo $tab_id; 
		echo "<br>";
		echo $action_id; 
...
//exit;
	...
	foreach($modArr as $fld_module => $fld_label)
	{
		...
		for($i=0; $i<$noofrows; $i++)
		{
			...
			//Updating the Mandatory aicrm_fields
			...
			$update_query = "update aicrm_profile2field set visible=? where fieldid=? and profileid=? and tabid=?";
