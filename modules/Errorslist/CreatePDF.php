...
{
	$id = $_REQUEST['record'];
	$filepath='test/product/'.$id.'_Errorslist.pdf';
	$pdf->Output($filepath,'F'); //added file name to make it work in IE, also forces the download giving the user the option to save

}else {
	$pdf->Output('Errorslist.pdf','D'); //added file name to make it work in IE, also forces the download giving the user the option to save
	exit();
}
