...
if(isset($_REQUEST['record']) && $_REQUEST['record'] !='')
{
	...
	$focus->mode = 'edit'; 	
	$focus->retrieve_entity_info($_REQUEST['record'],"Errorslist");
	$focus->name=$focus->column_fields['errorslist_name'];		
}
...
if(isset($_REQUEST['parent_id']) && $_REQUEST['parent_id']!='' && $_REQUEST['record']=='')
{
    if($_REQUEST['return_module'] == 'Job') {
        require_once('modules/Job/Job.php');
        $focus->column_fields['jobid'] = $_REQUEST['parent_id'];
        $ins_focus = new Job();
        $ins_focus->retrieve_entity_info($_REQUEST['parent_id'], "Job");
        $log->debug("Inspection Id from the request is " . $_REQUEST['parent_id']);
    }else if ($_REQUEST['return_module'] == 'Errors'){
        require_once('modules/Errors/Errors.php');
        $ins_focus = new Errors();
        $ins_focus->retrieve_entity_info($_REQUEST['parent_id'], "Errors");

        $focus->column_fields['errorsid'] = $_REQUEST['parent_id'];
        $focus->column_fields['errorslist_name'] = $ins_focus->column_fields['errors_name'];
        $focus->column_fields['error_name'] = $ins_focus->column_fields['errors_name'];
...
$smarty->assign("SINGLE_MOD",'Errors List');
