...
class Promotion extends CRMEntity {
// Account is used to store aicrm_account information.
	var $log;
	var $db;
	var $table_name = "aicrm_promotion";
	var $table_index= 'promotionid';
	var $table_comment = "aicrm_promotioncomments";
	
	var $tab_name = Array('aicrm_crmentity','aicrm_promotion','aicrm_promotioncf');
	var $tab_name_index = Array('aicrm_crmentity'=>'crmid','aicrm_promotion'=>'promotionid','aicrm_promotioncf'=>'promotionid','aicrm_promotioncomments'=>'promotionid');
	/**
	 * Mandatory table for supporting custom fields.
	 */
	var $customFieldTable = Array('aicrm_promotioncf', 'promotionid');
	var $column_fields = Array();

	var $sortby_fields = Array('promotionid','promotion_name','smownerid');

	var $list_fields = Array(
		'หมายเลขโปรโมชั่น'=>Array('aicrm_promotion'=>'promotion_no'),
		'ชื่อโปรโมชั่น'=>Array('aicrm_promotion'=>'promotion_name'),
		'ผู้รับผิดชอบ' => Array('crmentity'=>'smownerid')
	);

	var $list_fields_name = Array(
		'หมายเลขโปรโมชั่น' => 'promotion_no',
		'ชื่อโปรโมชั่น'=> 'promotion_name',
		'ผู้รับผิดชอบ' => 'assigned_user_id'
	);	  			

	var $list_link_field= 'promotion_name';
	...
	var $mandatory_fields = Array('assigned_user_id','createdtime' ,'modifiedtime','promotionid');
	
	function Promotion() 
	{
		$this->log =LoggerManager::getLogger('Promotion');
		$this->db = PearDatabase::getInstance();
		$this->column_fields = getColumnFields('Promotion');
	}
	function save_module()
	{
		...
		if($_REQUEST['action'] != 'PromotionAjax' && $_REQUEST['ajxaction'] != 'DETAILVIEW' && $_REQUEST['action'] != 'MassEditSave')
		{
			//echo "ddddd".($_REQUEST["mySetup_Disc_chk"])."<br>";
			//Based on the total Number of rows we will save the product relationship with this entity
			//saveInventoryProductDetails($this, 'Promotion');
			/*Set Products - Promotion*/
			if($_REQUEST["mySetup_Disc_chk"]=="1"){
				save_pro_tab1($this, 'Promotion');	
			}else if($_REQUEST["mySetup_Disc_chk"]=="2"){
				save_pro_tab2($this, 'Promotion');
			}else if($_REQUEST["mySetup_Disc_chk"]=="3"){
				save_pro_tab3($this, 'Promotion');
			}
			/*Set Products - Promotion*/

		}
		$this->insertIntoCommentTable("aicrm_promotioncomments",'promotionid');
		$this->insertIntoAttachment($this->id,'Promotion');	
	...
	function getCommentInformation($crmid)
	{
		...
		$sql = "select * from aicrm_promotioncomments where promotionid=? order by createdtime desc";
	...
	function getSortOrder()
	{
		...
		if(isset($_REQUEST['sorder'])) 
			...
			$sorder = (($_SESSION['QUESTIONNARE_SORT_ORDER'] != '')?($_SESSION['QUESTIONNARE_SORT_ORDER']):($this->default_sort_order));
	...
	function getOrderBy()
	{
		...
		if (isset($_REQUEST['order_by'])) 
			...
			$order_by = (($_SESSION['QUESTIONNARE_ORDER_BY'] != '')?($_SESSION['QUESTIONNARE_ORDER_BY']):($use_default_order_by));
	...
	function generateReportsSecQuery($module,$secmodule){
		...
		$query = $this->getRelationQuery($module,$secmodule,"aicrm_promotion","promotionid");
		$query .=" left join aicrm_crmentity as aicrm_crmentityPromotion on aicrm_crmentityPromotion.crmid=aicrm_promotion.promotionid and aicrm_crmentityPromotion.deleted=0 
		left join aicrm_promotioncf on aicrm_promotioncf.promotionid = aicrm_crmentityPromotion.crmid 
		left join aicrm_groups as aicrm_groupsPromotion on aicrm_groupsPromotion.groupid = aicrm_crmentityPromotion.smownerid

		LEFT JOIN aicrm_users AS aicrm_usersCreatorPromotion ON aicrm_usersCreatorPromotion.id = aicrm_crmentityPromotion.smcreatorid
		LEFT JOIN aicrm_users AS aicrm_usersModifiedPromotion ON aicrm_usersModifiedPromotion.id = aicrm_crmentityPromotion.modifiedby
		left join aicrm_users as aicrm_usersPromotion on aicrm_usersPromotion.id = aicrm_crmentityPromotion.smownerid
		";

		if($module == "Campaigns" && $secmodule == "Promotion"){
			$query .=" LEFT JOIN aicrm_campaign AS aicrm_campaignPromotion ON aicrm_campaignPromotion.promotionid = aicrm_promotion.promotionid";
		}		
		return $query;
	}
	...
	function setRelationTables($secmodule){
		...
		$rel_tables = array (
			...
			"Documents" => array("aicrm_senotesrel"=>array("crmid","notesid"),"aicrm_promotion"=>"promotionid"),
	...
	function create_export_query($where)
	{
		...
		$sql = getPermittedFieldsQuery("Promotion", "detail_view");
		...
		INNER JOIN aicrm_promotion 	ON aicrm_promotion.promotionid = aicrm_crmentity.crmid
		...
		if($is_admin==false && $profileGlobalPermission[1] == 1 && $profileGlobalPermission[2] == 1 && $defaultOrgSharingPermission[6] == 3)
		{
			...
			$query = $query." ".getListViewSecurityParameter("Promotion");
		}
	...
	function insertIntoAttachment($id,$module){//echo "555";exit;
		//print_r($_REQUEST['del_file_list']);exit;
	global $log, $adb;
...
}else if($module == 'Promotion' && $_REQUEST['del_file_list'] != '')
{
	$del_file_list = explode("###",trim($_REQUEST['del_file_list'],"###"));
			//print_r($del_file_list);exit;
	foreach($del_file_list as $del_file_name)
	{
		$attach_res = $adb->pquery("select aicrm_attachments.attachmentsid from aicrm_attachments inner join aicrm_seattachmentsrel on aicrm_attachments.attachmentsid=aicrm_seattachmentsrel.attachmentsid where crmid='".$id."' and name='".str_replace("&","&amp;",$del_file_name)."';", array());
		$attachments_id = $adb->query_result($attach_res,0,'attachmentsid');
		$del_res1 = $adb->pquery("delete from aicrm_attachments where attachmentsid=?", array($attachments_id));
		$del_res2 = $adb->pquery("delete from aicrm_seattachmentsrel where attachmentsid=?", array($attachments_id));
	}
