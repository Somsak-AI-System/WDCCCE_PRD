...
$focus->column_fields['email_start_date'] = $focus->column_fields['email_start_date'] != '' && $focus->column_fields['email_start_date'] != '0000-00-00' ? date('Y-m-d', strtotime($focus->column_fields['email_start_date'])):'';
$focus->column_fields['prepare_date'] = $focus->column_fields['prepare_date'] != '' && $focus->column_fields['prepare_date'] != '0000-00-00' ? date('Y-m-d', strtotime($focus->column_fields['prepare_date'])):'';
$focus->column_fields['send_date'] = $focus->column_fields['send_date'] != '' && $focus->column_fields['send_date'] != '0000-00-00' ? date('Y-m-d', strtotime($focus->column_fields['send_date'])):'';

$focus->save("Smartquestionnaire");

$return_id = $focus->id;

$parenttab = getParentTab();
if(isset($_REQUEST['return_module']) && $_REQUEST['return_module'] != "") $return_module = vtlib_purify($_REQUEST['return_module']);
else $return_module = "Smartquestionnaire";
...
if(isset($_REQUEST['return_module']) && $_REQUEST['return_module'] == "Campaigns")
{
	if(isset($_REQUEST['return_id']) && $_REQUEST['return_id'] != "")
	{
		 ...
		 $sql = "delete from aicrm_campaignmaillistrel where smartquestionnaireid = ?";
		 $adb->pquery($sql, array($focus->id));
		 $sql = "insert into aicrm_campaignmaillistrel values (?,?)";
