...
$_SESSION['TOOLS_ORDER_BY'] = $order_by;
$_SESSION['TOOLS_SORT_ORDER'] = $sorder;
//<<<<<<<<<<<<<<<<<<< sorting - stored in session >>>>>>>>>>>>>>>>>>>>
...
if(isPermitted('Tools','Delete','') == 'yes')
{
	$other_text['del'] = $app_strings[LBL_MASS_DELETE];
}
if(isPermitted('Tools','EditView','') == 'yes')
{
...
if($viewid != "0")
{
	$listquery = getListQuery("Tools");
	$query = $oCustomView->getModifiedCvListQuery($viewid,$listquery,"Tools");
}else
{
	$query = getListQuery("Tools");
}
//<<<<<<<<customview>>>>>>>>>
if(isset($where) && $where != '')
{
...
if(isset($order_by) && $order_by != '')
{
	if($order_by == 'smownerid')
        ...
        {
		$tablename = getTableNameForField('Tools',$order_by);
...
$listview_header = getListViewHeader($focus,"Tools",$url_string,$sorder,$order_by,"",$oCustomView);
$smarty->assign("LISTHEADER", $listview_header);

$listview_header_search = getSearchListHeaderValues($focus,"Tools",$url_string,$sorder,$order_by,"",$oCustomView);
$smarty->assign("SEARCHLISTHEADER",$listview_header_search);

$listview_entries = getListViewEntries($focus,"Tools",$list_result,$navigation_array,"","","EditView","Delete",$oCustomView);
...
$navigationOutput = getTableHeaderSimpleNavigation($navigation_array, $url_string,"Tools","index",$viewid);
