...
?>
	...
	<div class="calAddEvent layerPopup" style="display:none;width:1000px;left:200px;" id="addEvent" align=center>
	...
	<input type="hidden" name="dateformat" value="<?php echo $date_format ?>">
		...
		<tr style="cursor:move;">
			<td class="layerPopupHeading" align = "left" id="moveEvent"><?php echo $mod_strings['LBL_ADD_EVENT']?></b></td>
				<td align=right><a href="javascript:ghide('addEvent');"><img src="<?php echo  aicrm_imageurl('close.gif', $theme)  ?>" border="0"  align="absmiddle" /></a></td>
		</tr>
		</table>
		...
		<td class=small >
			...
			<td width=80% align="left">
				<table>
					<tr><td>
							<?php echo getActFieldCombo('activitytype','aicrm_activitytype'); ?>
					</td></tr>
				</table>
			</td>
			</tr>
			<tr>
				...
				<td align="left">
                	<table width="20%" border="0" cellspacing="0" cellpadding="0">
                      <tr>
                        <td width="20%"><input name="subject" id="subject" type="text" value="">
                       <!-- <textarea value=""   id="projects_name" name="projects_name" tabindex="" class=detailedViewTextBox onFocus="this.className='detailedViewTextBoxOn'" onBlur="this.className='detailedViewTextBox'" readonly="readonly" style="width:290px; height:35px" ></textarea></td>
                        <td width="5%"><img  src="themes/softed/images/select.gif" alt="<?=$APP.LBL_SELECT?>" title="<?=$APP.LBL_SELECT?>" LANGUAGE=javascript onclick='return window.open("index.php?module=Projects&action=Popup&html=Popup_picker&form=HelpDeskEditView&popuptype=specific&fromlink=Xpop_calendar","test","width=640,height=602,resizable=0,scrollbars=0,top=150,left=200");' align="absmiddle" style='cursor:hand;cursor:pointer'></td>
                        <td width="5%"><input type="image" src="themes/softed/images/clear_field.gif" alt="<?=$APP.LBL_CLEAR?>" title="<?=$APP.LBL_CLEAR?>" LANGUAGE=javascript onClick="this.form.subject.value=''; this.form.projects_name.value=''; return false;" align="absmiddle" style='cursor:hand;cursor:pointer'></td>-->
                      </tr>
                    </table>
                
				
	
               <!-- <input name="subject1" type="text" class="textbox" value="" style="width:50%">&nbsp;&nbsp;&nbsp; -->
                <input name="visibility" type="hidden" class="textbox" value="">
            ...
            <input name="site_location" type="hidden" class="textbox" value="" style="width:50%">
			<tr>
				<td nowrap align="right"><b><?php echo $mod_strings['Location']?></b></td>
				<td align="left"><input name="site_location" type="text" class="textbox" value="" style="width:50%"></td>
			</tr>
		        <?php } ?>
			...
			<tr>
				<td colspan=2 width=80% align="center">
					...
					<tr>
						<?php if(getFieldVisibilityPermission('Events',$current_user->id,'eventstatus') == '0') { ?>
						<td valign=top><?php echo getActFieldCombo('eventstatus','aicrm_eventstatus'); ?></td>
						<?php } ?>	
						<td valign=top rowspan=2>
							...
							<?php } ?>
							<input name="sendnotification" type="hidden" class="textbox" value="">
					...
					<tr>
						<td valign=top><b><?php echo $mod_strings['Priority'] ; ?></b>
							<br><?php echo getActFieldCombo('taskpriority','aicrm_taskpriority'); ?>
						</td>
					</tr>
				        <?php } ?>
					</table>
	...
	<table width="100%" cellpadding="0" cellspacing="0" border="0" bgcolor="#FFFFFF">
		<tr>
			<td>
				...
				?>
				<?php if(getFieldVisibilityPermission('Events',$current_user->id,'eventstatus') == '0') { ?>
                    <a href="javascript:;" id="complete" onClick="fninvsh('eventcalAction')" class="calMnu">- <?php echo $mod_strings['LBL_AI_COMPLETE']?></a>
                    <a href="javascript:;" id="pending" onClick="fninvsh('eventcalAction')" class="calMnu">- <?php echo $mod_strings['LBL_AI_CANCEL']?></a>
                <?php }?>		
...
<div id="addEventDropDown" style="width: 270px; background-color: rgb(255, 255, 255); left: 69px; top: 282px; display: none;" onmouseover="fnShowEvent()" onmouseout="fnRemoveEvent()">
...
<?php
	...
	else
	{
		...
		if (count($roleids) > 1) {
			...
			$Res=$adb->pquery("select distinct activitytype from aicrm_activitytype inner join aicrm_role2picklist on aicrm_role2picklist.picklistvalueid = aicrm_activitytype.picklist_valueid where roleid = ? and picklistid in (select picklistid from aicrm_activitytype) order by sortid asc",array($role_id));
		}
	}
	$eventlist='';
	for($i=0; $i<$adb->num_rows($Res);$i++)
	{
		...
		//print_r($eventlist);
?>		
	<tr><td><a href='' id="add<?php echo $eventlist;?>" class='drop_down'><?php echo getTranslatedString($eventlist,'Calendar')?></a></td></tr>
  ...
  <input type="hidden" name="maintab" value="Calendar">
	<table border=0 cellspacing=0 cellpadding=5 width=100% class="layerHeadingULine">
		<tr style="cursor:move;">
			...
			<td align=right><a href="javascript:ghide('createTodo');"><img src="<?php echo  aicrm_imageurl('close.gif', $theme)?>" border="0"  align="absmiddle" /></a></td>
		</tr>
        </table>
				...
				<table border="0" cellpadding="3" cellspacing="0" width="80%">
					...
					<tr>
						<td align="left" valign="top">
							<?php if(getFieldVisibilityPermission('Calendar',$current_user->id,'taskstatus') == '0') { ?>
							<?php echo getActFieldCombo('taskstatus','aicrm_taskstatus'); ?>
							<?php } ?>	
						</td>
						<td align="left" valign="top">
							<?php if(getFieldVisibilityPermission('Calendar',$current_user->id,'taskpriority') == '0') { ?>
							<?php echo getActFieldCombo('taskpriority','aicrm_taskpriority'); ?>
							<?php } ?>
						</td>
						<td align="left" valign="top">
							<?php if(getFieldVisibilityPermission('Calendar',$current_user->id,'assigned_user_id') == '0') { ?>
...
<div id="act_changeowner" class="statechange" style="left:250px;top:200px;z-index:5000">
	...
	<table width="100%" border="0" cellpadding="3" cellspacing="0" >
		...
		<td class="genHeaderSmall" align="left" style="border-bottom:1px solid #CCCCCC;" width="60%"><?php echo $app_strings['LBL_CHANGE_OWNER']; ?></td>
			...
			<td align="right" style="border-bottom:1px solid #CCCCCC;" width="40%"><a href="javascript:fninvsh('act_changeowner')"><img src="<?php echo aicrm_imageurl('close.gif', $theme) ?>" align="absmiddle" border="0"></a></td>
		</tr>
		<tr>
		        <td colspan="3">&nbsp;</td>
	</tr>
...
<script language="JavaScript" type="text/javascript" src="asset/DataTables/js/jquery.dataTables.min.js"></script>
...
<style type="text/css">
.display.nowrap.dataTable.no-footer{
	width: 100% !important;
}
...
<div class="modal fade bd-example-modal-lg" id="myModalbirthday" tabindex="-1" role="dialog" aria-labelledby="basicModal" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
          <div class="modal-header">
            <h4 class="modal-title" id="myModalLabel" style="display: contents !important;">Birthday</h4>
