...
function getDayViewLayout(& $cal)
{
	...
	if(!empty($show_complete_view)) {
		...
		$dayview_layout .= '<tr><td width=12% class="lvtCol" bgcolor="blue" valign=top><img onClick="document.EventViewOption.complete_view.value=1;fnRedirect();" src="'.aicrm_imageurl('inactivate.gif', $theme).'" border="0"></td><td class="dvtCellInfo">&nbsp;</td><td class="dvtCellInfo">&nbsp;</td></tr>';
	}
	for($j=0;$j<24;$j++)
	{
...
function getWeekViewLayout(& $cal)
{
	...
	for($i=$day_start_hour;$i<=$day_end_hour;$i++)
	{
		...
		for ($column=0;$column<=6;$column++)
		{
			...
			if(isPermitted("Calendar","EditView") == "yes")
		                        $weekview_layout .='<img onClick="fnvshobj(this,\'addEvent\'); gshow(\'addEvent\',\'Call\',\''.$sttemp_date.'\',\''.$endtemp_date.'\',\''.$time_arr['starthour'].'\',\''.$time_arr['startmin'].'\',\''.$time_arr['startfmt'].'\',\''.$time_arr['endhour'].'\',\''.$time_arr['endmin'].'\',\''.$time_arr['endfmt'].'\',\'hourview\',\'event\')" src="'.aicrm_imageurl('cal_add.gif', $theme).'" border="0">';
	...
	for ($i = 0; $i < $rows; $i ++)
	{
		...
		for ($j = 0; $j < 7; $j ++)
			...
			if ($cal['slice']->start_time->getMonth() == $cal['calendar']->date_time->getMonth())
			{
				...
				if(isPermitted("Calendar","EditView") == "yes")
                                $monthview_layout .='<a onClick="fnvshobj(this,\'addEvent\'); gshow(\'addEvent\',\'Call\',\''.$temp_date.'\',\''.$endtemp_date.'\',\''.$time_arr['starthour'].'\',\''.$time_arr['startmin'].'\',\''.$time_arr['startfmt'].'\',\''.$time_arr['endhour'].'\',\''.$time_arr['endmin'].'\',\''.$time_arr['endfmt'].'\',\'hourview\',\'event\')" href="javascript:void(0)"><img src="' . aicrm_imageurl('cal_add.gif', $theme). '" border="0"></a>';
	...
	if(!empty($act))
	{
		for($i=0;$i<count($act);$i++)
		{
			...
			if(($current_user->column_fields['is_admin']!='on' && $adb->num_rows($role_list)==0 && (($adb->num_rows($is_shared)==0 && ($visibility=='Public' || $visibility=='Private')) || $visibility=='Private')) && $current_user->column_fields['user_name']!=$user)
			{
				...
				<tr><td align="center">';
				if($act[$i]->shared)
					$eventlayer .= '<img src="' . aicrm_imageurl('cal12x12Shared.gif', $theme). '" align="middle" border="0">';
				else
					$eventlayer .= '&nbsp;';
...
function getEventList(& $calendar,$start_date,$end_date,$info='')
{
	...
	$and = "AND ((((aicrm_activity.date_start between ? AND ?)
		OR (aicrm_activity.date_start < ? AND aicrm_activity.due_date > ?)
		OR (aicrm_activity.due_date between ? AND ?))
		AND (aicrm_recurringevents.activityid is NULL))
		OR (aicrm_recurringevents.recurringdate BETWEEN ? AND ?))";
	...
	$count_qry = "SELECT count(*) as count FROM aicrm_activity
		...
		LEFT OUTER JOIN aicrm_recurringevents
		ON aicrm_recurringevents.activityid = aicrm_activity.activityid
		WHERE aicrm_crmentity.deleted = 0
		AND (aicrm_activity.activitytype not in ('Emails','Task')) $and ";
		
	$query = "SELECT aicrm_groups.groupname, aicrm_users.user_name,aicrm_crmentity.smownerid, aicrm_crmentity.crmid,
	...
	if($info != '')
	{
		...
		$pending_query = $query." AND (aicrm_activity.eventstatus = 'Planned')".$com_q;
		$total_q =  $query."".$com_q;
