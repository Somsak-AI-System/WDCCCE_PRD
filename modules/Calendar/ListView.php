 ...
 ********************************************************************************/
/*********************************************************************************
 * $Header: /advent/projects/wesat/aicrm_crm/sugarcrm/modules/Activities/ListView.php,v 1.14 2005/03/26 09:45:00 samk Exp $
...
if($viewid == 0 )
{
	...
	echo "<div style='border: 3px solid rgb(153, 153, 153); background-color: rgb(255, 255, 255); width: 55%; position: relative; z-index: 10000000;'>
		...
		<tbody><tr>
		<td rowspan='2' width='11%'><img src='<?php echo aicrm_imageurl('close.gif', $theme) ?>'></td>
...
if(isset($where) && $where != '')
{
	if(isset($_REQUEST['from_homepagedb']) && $_REQUEST['from_homepagedb'] == 'true')
		$list_query .= " and ((aicrm_activity.status!='Completed' and aicrm_activity.status!='Deferred') or aicrm_activity.status is null) and ((aicrm_activity.eventstatus!='Held' and aicrm_activity.eventstatus!='Not Held') or aicrm_activity.eventstatus is null) AND ".$where;
	else
...
if (isset($_REQUEST['from_homepage'])) {
	...
	if ($_REQUEST['from_homepage'] == 'upcoming_activities')
		$list_query .= " AND (aicrm_activity.status is NULL OR aicrm_activity.status not in ('Completed','Deferred')) and (aicrm_activity.eventstatus is NULL OR  aicrm_activity.eventstatus not in ('Held','Not Held')) AND (date_start >= '$today' OR aicrm_recurringevents.recurringdate >= '$today')";
	elseif ($_REQUEST['from_homepage'] == 'pending_activities') 
		$list_query .= " AND (aicrm_activity.status is NULL OR aicrm_activity.status not in ('Completed','Deferred')) and (aicrm_activity.eventstatus is NULL OR  aicrm_activity.eventstatus not in ('Held','Not Held')) AND (due_date <= '$today' OR aicrm_recurringevents.recurringdate <= '$today')";
}
...
if(isset($order_by) && $order_by != '') {
	if($order_by == 'smownerid') {
		...
		if($order_by == 'lastname'){
         	$list_query .= ' ORDER BY aicrm_contactdetails.lastname '.$sorder;
		}else{
...
$smarty->assign("SINGLE_MOD",'Calendar');
...
//Cambiado code to add close button in custom aicrm_field
if (($viewid!=0)&&($viewid!="")){
  if (!isset($oCustomView->list_fields['Close'])) $oCustomView->list_fields['Close']=array ( 'aicrm_activity' => 'eventstatus' );
...
$alphabetical = AlphabeticalSearch($currentModule,'ListView','activitytype','true','basic',"","","","",$viewid);
...
$listquery1 =  getListQuery("Calendar");
$listviewquery1 = substr($listquery1, strpos($listquery1,'FROM'),strlen($listquery1));

$pquery1 = "select aicrm_activity.flag_send_report,aicrm_activity.activityid ";
...
if($rowCount>0){
	for($i=0;$i<=$rowCount;$i++){
		$a_data=array();
		$activityid=$adb->query_result($cf_res, $i, "activityid");
		$flag_send_report = $adb->query_result($cf_res,$i, "flag_send_report");
		$a_response[$activityid]["flag_send_report"] = $flag_send_report;
	}
}
