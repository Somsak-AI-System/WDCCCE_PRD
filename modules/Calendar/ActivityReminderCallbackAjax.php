...
$active = $adb->pquery("select * from aicrm_users where id=?",array($current_user->id));
...
if($active_res!='None'){
	...
	if($reminder_next_time <= $cur_time)
	{
		...
		$callback_query = 
		"SELECT * FROM aicrm_activity_reminder_popup inner join aicrm_crmentity where " .
		" aicrm_activity_reminder_popup.status = 0 and " .
		" aicrm_activity_reminder_popup.recordid = aicrm_crmentity.crmid " .
		" and aicrm_crmentity.smownerid = ".$current_user->id." and aicrm_crmentity.deleted = 0 " .
		" and ((DATE_FORMAT(aicrm_activity_reminder_popup.date_start,'%Y-%m-%d') < '" . $cbdate1 . "')" . 
		" OR ((DATE_FORMAT(aicrm_activity_reminder_popup.date_start,'%Y-%m-%d') = '" . $cbdate1 . "')" . 
		" AND (TIME_FORMAT(aicrm_activity_reminder_popup.time_start,'%H:%i') <= '" . $cbtime1 . "')))";
		...
		if($cbrows > 0) {
			for($index = 0; $index < $cbrows; ++$index) {
				...
				$mark_reminder_as_read = "UPDATE aicrm_activity_reminder_popup set status = 1 where reminderid = ?";
		...
		$adb->pquery("UPDATE aicrm_users set reminder_next_time=?, date_entered=? where id=?",array($reminder_next, $lastlogin_time, $current_user->id));
		...
		echo "<script type='text/javascript' id='_aicrm_activityreminder_callback_interval_'>$reminder_interval_reset</script>";
	}
}
}
