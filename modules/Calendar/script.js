...
//document.write("<script type='text/javascript' src='https://unpkg.com/popper.js'></"+"script>");
//document.write("<script type='text/javascript' src='https://cdnjs.cloudflare.com/ajax/libs/twitter-bootstrap/4.0.0-beta/js/bootstrap.min.js'></"+"script>");
...
function gshow(argg1,type,startdate,enddate,starthr,startmin,startfmt,endhr,endmin,endfmt,viewOption,subtab)
{
	...
	var y=document.getElementById(argg1).style;
		...
		if(type != 'todo' && type!='')
		{
			for(var i=0; i<document.EditView.activitytype.length-1; i++){
...
//		else 
        ...
        {
                alert("ประเภทงาน");
                document.EditView.cf_1998.focus()
                return false;
        }
		/*else if(trim(document.EditView.account_id.value) == "")
        {
                alert("Account Name cannot be empty");
                document.EditView.account_name.focus()
                return false;
        }
		else if(trim(document.EditView.cf_2005.value) == "")
        {
                alert("เบอร์ผู้ติดต่อ cannot be empty");
                document.EditView.cf_2005.focus()
                return false;
        }
		else if(trim(document.EditView.cf_2006.value) == "")
        {
                alert("อีเมล์ผู้ติดต่อ cannot be empty");
                document.EditView.cf_2006.focus()
                return false;
        }*/
        else
        {
...
function fnRedirect() {
    var e = document.getElementById("view_Option");
    var OptionData = e.value;
...
function fnRedirectBirthday() {
    var b = document.getElementById("viewBirthday");
    var OptionDataBirthday = b.value;
    //var OptionDataBirthday = $('viewBirthday').options[$('viewBirthday').selectedIndex].value;
    var e = document.getElementById("view_Option");
    var OptionData = e.value;
    //var OptionData = $('view_Option').options[$('view_Option').selectedIndex].value;
...
function fnAddEvent(obj,CurrObj,start_date,end_date,start_hr,start_min,start_fmt,end_hr,end_min,end_fmt,viewOption,subtab,eventlist){
	...
	for(var i=0;i<(eventlist.length-1);i++){
		//even_name="todo";
		//document.getElementById("add"+eventlist[i]).href="javascript:gshow('addEvent','"+eventlist[i]+"','"+start_date+"','"+end_date+"','"+start_hr+"','"+start_min+"','"+start_fmt+"','"+end_hr+"','"+end_min+"','"+end_fmt+"','"+viewOption+"','"+subtab+"');fnRemoveEvent();";
		document.getElementById("add"+eventlist[i]).href="javascript:window.location.assign('index.php?module=Calendar&action=EditView&return_action=DetailView&parenttab=Marketing&activitytype="+eventlist[i]+"')";
...
function getcalAction(obj,Lay,id,view,hour,dateVal,type){
    ...
    if(type == 'event')
    {
	var heldstatus = "eventstatus=เสร็จสิ้น";
	var notheldstatus = "eventstatus=ยกเลิก";
...
function fnAssignTo(){
        ...
        var option_Box = document.getElementById("parent_type");
        var option_select = option_Box.value;
...
function modal_birthday(day,month,view,module,viewtype){
    //$('#myModalbirthday').modal()
    //$('#myModalbirthday').modal('show');
    //alert(day+" "+month+" "+view);
    /*$('#myModalbirthday').on('show.bs.modal', function (event) {
    ...
    var url ='plugin/Calendar/get_birthday_calendar.php';
    jQuery.ajax(url, {
        type: 'POST',
        data: data,
        success: function (data){
            ...
            jQuery("#myModalbirthday").modal("toggle");
        },
        error: function (data){
            console.log(data);
        }
...
function reload_datatable(data){
    //console.log(data);
    var data_table = jQuery('#datatable').DataTable({
        "data" : data,
        "processing": true,
        "destroy" :true,
        "scrollX": false,
        ...
        "searching": false,
        "columns": [
            { "data": "number" ,className: "dt-body-center"  },
            { "data": "name" ,className: "dt-body-left" },
            { "data": "ageinyears",className: "dt-body-center" },
            { "data": "birthday",className: "dt-body-center" },
            { "data": "mobile" },
            { "data": "email" },
            { "data": "module" ,className: "dt-body-center" },
            { "data": null,  className: "dt-body-center" ,
                render: function ( data, type, row ) {
                    var linkdomain = jQuery(location).attr('href');
                    
                    if(data.module == 'Accounts'){
                        return '<a href="index.php?module=Accounts&action=DetailView&record='+data.crmid+'&parenttab=Sales" target="_blank">Open</a>'
                    }else if(data.module == 'Leads'){
                        return '<a href="index.php?module=Leads&action=DetailView&record='+data.crmid+'&parenttab=Sales" target="_blank">Open</a>'
                    }else{
                        return '<a href="index.php?module=Contacts&action=DetailView&record='+data.crmid+'&parenttab=Sales" target="_blank">Open</a>'
                    }

                    //return '<a href="user_project_edit.php?project='+data.crmid+'" target="_blank">Open</a>'
                } 
            }
        ],
        "columnDefs": [ {
