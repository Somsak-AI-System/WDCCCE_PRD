...
// functions added for group calendar	-Jaguar
	...
	function status_availability($owner,$userid,$activity_id,$avail_date,$activity_start_time,$activity_end_time)	
	{
		...
		if( $owner != $userid)
		{
			...
			$usr_query="select activityid,aicrm_activity.date_start,aicrm_activity.due_date, aicrm_activity.time_start,aicrm_activity.duration_hours,aicrm_activity.duration_minutes,aicrm_crmentity.smownerid from aicrm_activity,aicrm_crmentity where aicrm_crmentity.crmid=aicrm_activity.activityid and ('".$avail_date."' like date_start) and aicrm_crmentity.smownerid=? and aicrm_activity.activityid !=?  and aicrm_crmentity.deleted=0";
		}
		else
		{
			$usr_query="select activityid,aicrm_activity.date_start,aicrm_activity.due_date, aicrm_activity.time_start,aicrm_activity.duration_hours,aicrm_activity.duration_minutes,aicrm_crmentity.smownerid from aicrm_activity,aicrm_crmentity where aicrm_crmentity.crmid=aicrm_activity.activityid and ('".$avail_date."' like date_start) and aicrm_crmentity.smownerid=? and aicrm_activity.activityid !=? and aicrm_crmentity.deleted=0";
		...
		if($avail_flag!="true")
		{
			$recur_query="SELECT aicrm_activity.activityid, aicrm_activity.time_start, aicrm_activity.duration_hours, aicrm_activity.duration_minutes , aicrm_crmentity.smownerid, aicrm_recurringevents.recurringid, aicrm_recurringevents.recurringdate as date_start from aicrm_activity inner join aicrm_crmentity on aicrm_activity.activityid = aicrm_crmentity.crmid inner join aicrm_recurringevents on aicrm_activity.activityid=aicrm_recurringevents.activityid where ('".$avail_date."' like aicrm_recurringevents.recurringdate) and aicrm_crmentity.smownerid=? and aicrm_activity.activityid !=? and aicrm_crmentity.deleted=0";
	 	...
	 	if($avail_flag == "true")
                {
                        $availability=' <IMG SRC="' . aicrm_imageurl('busy.gif', $theme). '">';
                }
                else
                {
                        $availability=' <IMG SRC="' . aicrm_imageurl('free.gif', $theme). '">';
                }
