...
class Appointment
{
	...
	function readAppointment($userid, &$from_datetime, &$to_datetime, $view)
	{
		...
		$and = "
		AND ((aicrm_activity.date_start between '".$from_datetime->get_formatted_date()."' AND '".$to_datetime->get_formatted_date()."')
		OR (aicrm_activity.date_start < '".$from_datetime->get_formatted_date()."' AND aicrm_activity.due_date > '".$from_datetime->get_formatted_date()."')
		OR (aicrm_activity.due_date between '".$from_datetime->get_formatted_date()."' AND '".$to_datetime->get_formatted_date()."'))
		";
		
        	$q= "
			select aicrm_activity.*, aicrm_crmentity.*, case when (aicrm_users.user_name not like '') then aicrm_users.user_name else aicrm_groups.groupname end as user_name 
			FROM aicrm_activity 
			inner join aicrm_crmentity on aicrm_activity.activityid = aicrm_crmentity.crmid 
			/*left join aicrm_recurringevents on aicrm_activity.activityid=aicrm_recurringevents.activityid */
			left join aicrm_groups on aicrm_groups.groupid = aicrm_crmentity.smownerid 
			LEFT JOIN aicrm_users ON aicrm_users.id = aicrm_crmentity.smownerid 
			WHERE aicrm_crmentity.deleted = 0 
			and aicrm_activity.activitytype not in ('Emails','Task') 
			".$and ;
        ...
        //$q .= " AND aicrm_recurringevents.activityid is NULL ";
        $q .= " group by aicrm_activity.activityid ORDER by aicrm_activity.date_start,aicrm_activity.time_start";
		//echo $q;
		...
		$q = "SELECT aicrm_activity.*, aicrm_crmentity.*, case when (aicrm_users.user_name not like '') then aicrm_users.user_name else aicrm_groups.groupname end as user_name , aicrm_recurringevents.recurringid, aicrm_recurringevents.recurringdate as date_start ,aicrm_recurringevents.recurringtype,aicrm_groups.groupname from aicrm_activity inner join aicrm_crmentity on aicrm_activity.activityid = aicrm_crmentity.crmid inner join aicrm_recurringevents on aicrm_activity.activityid=aicrm_recurringevents.activityid left join aicrm_groups on aicrm_groups.groupid = aicrm_crmentity.smownerid LEFT JOIN aicrm_users ON aicrm_users.id = aicrm_crmentity.smownerid ";
        $q.=" where aicrm_crmentity.deleted = 0 and aicrm_activity.activitytype not in ('Emails','Task') AND (recurringdate between ? and ?) ";
        ...
        $q .= " ORDER by aicrm_recurringevents.recurringid";
		$r = $adb->pquery($q, $params);
        $n = $adb->getRowCount($r);
        $a = 0;
		while ( $a < $n )
	...
	/** To read and set the events value in Appointment Obj
          * @param $act_array -- The aicrm_activity array :: Type Array
          * @param $view -- The calendar view :: Type String
         */
...
/** To two array values
  * @param $a -- The aicrm_activity array :: Type Array
  * @param $b -- The aicrm_activity array :: Type Array
		...
		if($is_Admin == 'off' && $fldname != '')
			{
				...
				$sql="select * from aicrm_$fldname where $fldname=?";
