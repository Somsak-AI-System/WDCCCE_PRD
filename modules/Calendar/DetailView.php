 ...
 ********************************************************************************/
/*********************************************************************************
 * $Header: /advent/projects/wesat/aicrm_crm/sugarcrm/modules/Activities/DetailView.php,v 1.12 2005/03/17 11:26:49 ray Exp $
...
global $mod_strings, $currentModule,$adb, $current_user,$singlepane_view;
...
{
	$query = "select activitytype from aicrm_activity where activityid=?";
...
elseif($activity_mode == 'Events')
{
	...
	$query = 'select aicrm_recurringevents.recurringfreq,aicrm_recurringevents.recurringinfo from aicrm_recurringevents where aicrm_recurringevents.activityid = ?';
	...
	$sql = 'select aicrm_users.user_name,aicrm_invitees.* from aicrm_invitees left join aicrm_users on aicrm_invitees.inviteeid=aicrm_users.id where activityid=?';
...
if(isPermitted("Calendar","EditView",$_REQUEST['record']) == 'yes' || $focus->column_fields['eventstatus'] == "Complete")
