...
$questionnaire_answer = json_decode($_REQUEST['data_template'], TRUE);

$qustionnairetemplate = array();
$qustionnairetemplate = json_decode($_REQUEST['qustionnairetemplate'], TRUE); 
...
$focus->save("Questionnaireanswer");
$return_id = $focus->id;

$questionnairetemplateid = $_REQUEST['questionnairetemplateid'];

require_once("library/Questionnaire.php");
include("config.inc.php");
$Questionnaireanswer = new libquestionnaire($dbconfig);
$Questionnaireanswer->save_questionnaire($qustionnairetemplate,$focus->id,$_REQUEST['mode'],$questionnaire_answer,$questionnairetemplateid);

$parenttab = getParentTab();
if(isset($_REQUEST['return_module']) && $_REQUEST['return_module'] != "") $return_module = vtlib_purify($_REQUEST['return_module']);
else $return_module = "Questionnaireanswer";
...
if(isset($_REQUEST['return_module']) && $_REQUEST['return_module'] == "Accounts")
{
	if(isset($_REQUEST['return_id']) && $_REQUEST['return_id'] != "")
	{
		...
		$adb->pquery($sql, array($_REQUEST['return_id'],$_REQUEST['return_module'],$focus->id,"Questionnaireanswer"));
	}
}
