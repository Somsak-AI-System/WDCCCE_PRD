...
if(isset($_REQUEST['record']) && $_REQUEST['record'] !='') 
{
    ...
    $focus->name=$focus->column_fields['Questionnaireanswer_name'];       
}
...
if(isset($_REQUEST['isDuplicate']) && $_REQUEST['isDuplicate'] == 'true') 
{
    ...
    WHERE aicrm_questionnaireanswer.questionnaireanswerid = '".$_REQUEST['record']."' ";
    $a_data = $myLibrary_mysqli->select($pquery);

    $data_template = array();
    $d_awnser = array();

    $data_template['title'] = $a_data[0]['title_questionnaireanswer'];
    $pageid = '';
    $i=-1;$c=0;
...
if(empty($_REQUEST['record']) && $focus->mode != 'edit'){ 
    //$focus->column_fields['questionnaireanswer_date'] = date('Y-m-d');
    //$focus->column_fields['questionnaireanswer_time'] = date('H:i');
    setObjectValuesFromRequest($focus);
}

if(isset($_REQUEST['type']) && $_REQUEST['type'] !='') 
{
    $focus->column_fields['questionnaireanswer_type'] = $_REQUEST['type'];   
}
...
if($focus->mode == 'edit') {
    ...
    left join aicrm_questionnaire_choice on aicrm_questionnaire_choice.pageid = aicrm_questionnaire_page.pageid
    left join aicrm_questionnaire_choicedetail on aicrm_questionnaire_choicedetail.choiceid = aicrm_questionnaire_choice.choiceid
    WHERE aicrm_questionnaireanswer.questionnaireanswerid = '".$focus->id."' ";
    $a_data = $myLibrary_mysqli->select($pquery);
...
if($focus->mode == 'edit') {
    $smarty->assign("DATA_TEMPLATE",$data_template);
    $smarty->assign("DATA_ANSWER",$d_awnser);
    $smarty->display('Questionnaireanswer/InventoryEditView.tpl');
} else {
    $smarty->display('Questionnaireanswer/InventoryCreateView.tpl');
}
