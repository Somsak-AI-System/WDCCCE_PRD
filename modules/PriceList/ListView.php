...
$focus->initSortbyField('PriceList');
// END
...
$_SESSION['PRICELIST_ORDER_BY'] = $order_by;
$_SESSION['PRICELIST_SORT_ORDER'] = $sorder;
//<<<<<<<<<<<<<<<<<<< sorting - stored in session >>>>>>>>>>>>>>>>>>>>
/*Padding 20 -200*/
...
if(isPermitted('PriceList','Delete','') == 'yes')
{
	$other_text['del'] = $app_strings[LBL_MASS_DELETE];
}
if(isPermitted('PriceList','EditView','') == 'yes')
{
...
if($viewid != "0")
{
	$listquery = getListQuery("PriceList");
	$query = $oCustomView->getModifiedCvListQuery($viewid,$listquery,"PriceList");
}else
{
	$query = getListQuery("PriceList");
}
//<<<<<<<<customview>>>>>>>>>
//echo $query; exit;
if(isset($where) && $where != '')
...
if(isset($order_by) && $order_by != '')
{
	if($order_by == 'smownerid')
        ...
        {
		$tablename = getTableNameForField('Errors',$order_by);
...
$listview_header = getListViewHeader($focus,"PriceList",$url_string,$sorder,$order_by,"",$oCustomView);
$smarty->assign("LISTHEADER", $listview_header);

$listview_header_search = getSearchListHeaderValues($focus,"PriceList",$url_string,$sorder,$order_by,"",$oCustomView);
$smarty->assign("SEARCHLISTHEADER",$listview_header_search);

$listview_entries = getListViewEntries($focus,"PriceList",$list_result,$navigation_array,"","","EditView","Delete",$oCustomView);
...
$navigationOutput = getTableHeaderSimpleNavigation($navigation_array, $url_string,"PriceList","index",$viewid);
