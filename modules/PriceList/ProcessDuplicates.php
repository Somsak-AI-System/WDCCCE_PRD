...
if($mode == 'mergesave') {
	...
	$result =  $adb->pquery("SELECT count(*) AS count FROM aicrm_crmentity WHERE crmid=? and deleted=0", array($merge_id));
...
<?php
	...
	$sql="select bean_id from aicrm_users_last_import where bean_type=? and deleted=0";
