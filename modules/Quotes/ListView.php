...
// Get quotes Status
$listquery1 =  getListQuery("Quotes");
$listviewquery1 = substr($listquery1, strpos($listquery1,'FROM'),strlen($listquery1));

$pquery1 = "select aicrm_quotes.quotation_status,aicrm_quotes.quoteid ";
...
if($rowCount>0){
	for($i=0;$i<=$rowCount;$i++){
		$a_data=array();
		$quoteid=$adb->query_result($cf_res, $i, "quoteid");
		$quotesstt = $adb->query_result($cf_res,$i, "quotation_status");
		
		$a_response[$quoteid]["quotation_status"] = $quotesstt;
	}
}
//########## Quostatus;
