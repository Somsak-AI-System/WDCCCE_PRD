...
class Competitor extends CRMEntity
{
// Account is used to store aicrm_account information.
    var $log;
    var $db;
    var $table_name = "aicrm_competitor";
    var $table_index = 'competitorid';
    var $table_comment = "aicrm_competitorcomments";

    var $tab_name = Array('aicrm_crmentity', 'aicrm_competitor', 'aicrm_competitorcf');
    var $tab_name_index = Array('aicrm_crmentity' => 'crmid', 'aicrm_competitor' => 'competitorid', 'aicrm_competitorcf' => 'competitorid','aicrm_competitorcomments'=>'competitorid');
    /**
     * Mandatory table for supporting custom fields.
     */
    var $customFieldTable = Array('aicrm_competitorcf', 'competitorid');
    var $column_fields = Array();

    var $sortby_fields = Array('competitorid', 'competitor_name', 'smownerid','accountname');

    var $list_fields = Array(
        'หมายเลขคู่แข่ง' => Array('Competitor' => 'competitor_no'),
        'ชื่อคู่แข่ง' => Array('Competitor' => 'competitor_name'),
        'ประเภทธุรกิจของคู่แข่ง' => Array('Competitor' => 'competitor_business_type'),
        'ประเภทคู่แข่ง' => Array('Competitor' => 'competitor_type'),
    );

    var $list_fields_name = Array(
        'หมายเลขคู่แข่ง' => 'competitor_no',
        'ชื่อคู่แข่ง' => 'competitor_name',
        'ประเภทธุรกิจของคู่แข่ง' => 'competitor_business_type',
        'ประเภทคู่แข่ง' => 'competitor_type',
    );

    var $list_link_field = 'competitor_no';
    ...
    var $search_fields = Array(
        'หมายเลขคู่แข่ง' => Array('aicrm_competitor' => 'competitor_no'),
        'ชื่อคู่แข่ง' => Array('aicrm_competitor' => 'competitor_name'),
        'ประเภทธุรกิจของคู่แข่ง' => Array('aicrm_competitor' => 'competitor_business_type'),
        'ประเภทคู่แข่ง' => Array('aicrm_competitor' => 'competitor_type'),
    ...
    var $mandatory_fields = Array('assigned_user_id', 'createdtime', 'modifiedtime', 'competitorid');

    function Competitor()
    {
        $this->log = LoggerManager::getLogger('Competitor');
        $this->db = PearDatabase::getInstance();
        $this->column_fields = getColumnFields('Competitor');
    }
    ...
    function save_module()
    {
        ...
        if ($_REQUEST['action'] != 'CompetitorAjax' && $_REQUEST['ajxaction'] != 'DETAILVIEW' && $_REQUEST['action'] != 'MassEditSave') {
            //Based on the total Number of rows we will save the product relationship with this entity
            //saveInventoryProductDetails($this, 'Competitor');
            $this->insertIntoCommentTable("aicrm_competitorcomments",'competitorid');
        }
    ...
    function getCommentInformation($crmid)
    {
        ...
        $sql = "select * from aicrm_competitorcomments where competitorid=? order by createdtime desc";
    ...
    {
        $query = $this->getRelationQuery($module, $secmodule, "aicrm_competitor", "competitorid");
        $query .= " LEFT JOIN aicrm_crmentity AS aicrm_crmentityCompetitor ON aicrm_crmentityCompetitor.crmid = aicrm_competitor.competitorid
                    AND aicrm_crmentityCompetitor.deleted = 0
                    LEFT JOIN aicrm_competitorcf ON aicrm_competitorcf.competitorid = aicrm_crmentityCompetitor.crmid
                    LEFT JOIN aicrm_account as aicrm_accountCompetitor ON aicrm_accountCompetitor.accountid = aicrm_competitor.account_id
                    LEFT JOIN aicrm_groups AS aicrm_groupsCompetitor ON aicrm_groupsCompetitor.groupid = aicrm_crmentityCompetitor.smownerid
                    LEFT JOIN aicrm_users AS aicrm_usersCompetitor ON aicrm_usersCompetitor.id = aicrm_crmentityCompetitor.smownerid
                    left join aicrm_users as aicrm_usersModifiedCompetitor on aicrm_crmentity.smcreatorid=aicrm_usersModifiedCompetitor.id
                    left join aicrm_users as aicrm_usersCreatorCompetitor on aicrm_crmentity.smcreatorid=aicrm_usersModifiedCompetitor.id";
        //echo $query;exit;
        return $query;
    }
    ...
    function create_export_query($where)
    {
        ...
        $sql = getPermittedFieldsQuery("Competitor", "detail_view");
        ...
        $query = "SELECT $fields_list,case when (aicrm_users.user_name not like '') then aicrm_users.user_name else aicrm_groups.groupname end as user_name 
				...
				INNER JOIN aicrm_competitor ON aicrm_competitor.competitorid = aicrm_crmentity.crmid
        ...
        if ($is_admin == false && $profileGlobalPermission[1] == 1 && $profileGlobalPermission[2] == 1 && $defaultOrgSharingPermission[6] == 3) {
            ...
            $query = $query . " " . getListViewSecurityParameter("Competitor");
        }
    ...
    function setRelationTables($secmodule)
    {
        //echo $secmodule; exit;
        $rel_tables = array(
            "Documents" => array("aicrm_senotesrel" => array("crmid", "notesid"), "aicrm_competitor" => "competitorid"),
            "Accounts" => array("aicrm_account"=>array("accountid","accountid"),"aicrm_competitor"=>"account_id"),
            "Competitorproduct" => array("aicrm_competitorproduct"=>array("competitorid","competitorproductid"),"aicrm_competitor"=>"competitorid"),
        );
    ...
    function get_competitorproduct($id, $cur_tab_id, $rel_tab_id, $actions=false) {
        
        global $log, $singlepane_view,$currentModule,$current_user;
        $log->debug("Entering get_competitorproduct(".$id.") method ...");
        ...
        $query = "SELECT distinct(aicrm_competitorproduct.competitorproductid), case when (aicrm_users.user_name not like '') then aicrm_users.user_name else aicrm_groups.groupname end as user_name , aicrm_crmentity.*, aicrm_competitorproduct.*, aicrm_competitorproductcf.*
                ...
                WHERE aicrm_crmentity.deleted = 0 AND aicrm_competitorproduct.competitorid = '".$id."' ";
        $return_value = GetRelatedList($this_module, $related_module, $other, $query, $button, $returnset);

        if($return_value == null) $return_value = Array();
        $return_value['CUSTOM_BUTTON'] = $button;

        $log->debug("Exiting get_competitorproduct method ...");
        return $return_value;
    ...
    function get_activities($id, $cur_tab_id, $rel_tab_id, $actions=false) {
        ...
        $query = "SELECT aicrm_activity.*, aicrm_activitycf.*,
            ...
            LEFT JOIN aicrm_groups ON aicrm_groups.groupid = aicrm_crmentity.smownerid
            WHERE aicrm_activity.competitorid = ".$id."
    ...
    function get_deals($id, $cur_tab_id, $rel_tab_id, $actions=false) {
        ...
        $query = "SELECT distinct(aicrm_deal.dealid), case when (aicrm_users.user_name not like '') then aicrm_users.user_name else aicrm_groups.groupname end as user_name , aicrm_crmentity.*, aicrm_deal.*, aicrm_dealcf.*
                ...
                LEFT JOIN aicrm_users as create_by ON aicrm_crmentity.smcreatorid = create_by.id
                LEFT JOIN aicrm_users as modified_by ON aicrm_crmentity.modifiedby = modified_by.id
                WHERE aicrm_crmentity.deleted = 0 AND aicrm_deal.competitorid = '".$id."' ";
        $return_value = GetRelatedList($this_module, $related_module, $other, $query, $button, $returnset);
