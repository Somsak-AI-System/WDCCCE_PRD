...
require_once('include/utils/utils.php');require_once('modules/Competitor/Competitor.php');
...
if(isset($_REQUEST['record']) && $_REQUEST['record'] !='') 
{
	...
	$focus->mode = 'edit'; 	
	$focus->retrieve_entity_info($_REQUEST['record'],"Competitor");
	$focus->name=$focus->column_fields['competitor_name'];		
}
...
$sql = "SELECT aicrm_competitor.competitor_seq_no
	...
	GROUP BY aicrm_competitor.competitor_seq_no
	ORDER BY aicrm_competitor.competitor_seq_no ASC;";
$a_data = $myLibrary_mysqli->select($sql);

foreach ($a_data as $key => $value) {
	if($value['competitor_seq_no'] == 0){
		$seq0 = true;
	}elseif($value['competitor_seq_no'] == 1){
		$seq1 = true;
	}elseif($value['competitor_seq_no'] == 2){
		$seq2 = true;
	}elseif($value['competitor_seq_no'] == 3){
		$seq3 = true;
	}
}
$smarty->assign("seq_no0",$seq0);
$smarty->assign("seq_no1",$seq1);
$smarty->assign("seq_no2",$seq2);
$smarty->assign("seq_no3",$seq3);
