...
$focus->initSortbyField('Redemption');
// END
...
$_SESSION['REDEMPTION_ORDER_BY'] = $order_by;
$_SESSION['REDEMPTION_SORT_ORDER'] = $sorder;
//<<<<<<<<<<<<<<<<<<< sorting - stored in session >>>>>>>>>>>>>>>>>>>>
...
if(isPermitted('Redemption','Delete','') == 'yes')
{
	$other_text['del'] = $app_strings[LBL_MASS_DELETE];
}
if(isPermitted('Redemption','EditView','') == 'yes')
{
...
if($viewid != "0")
{
	$listquery = getListQuery("Redemption");
	$query = $oCustomView->getModifiedCvListQuery($viewid,$listquery,"Redemption");
}else
{
	$query = getListQuery("Redemption");
}
//<<<<<<<<customview>>>>>>>>>
...
if(isset($order_by) && $order_by != '')
{
	if($order_by == 'smownerid')
        ...
        {
		$tablename = getTableNameForField('Redemption',$order_by);
...
$listview_header = getListViewHeader($focus,"Redemption",$url_string,$sorder,$order_by,"",$oCustomView);
$smarty->assign("LISTHEADER", $listview_header);

$listview_header_search = getSearchListHeaderValues($focus,"Redemption",$url_string,$sorder,$order_by,"",$oCustomView);
$smarty->assign("SEARCHLISTHEADER",$listview_header_search);

$listview_entries = getListViewEntries($focus,"Redemption",$list_result,$navigation_array,"","","EditView","Delete",$oCustomView);
...
$navigationOutput = getTableHeaderSimpleNavigation($navigation_array, $url_string,"Redemption","index",$viewid);
