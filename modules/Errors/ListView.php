...
$_SESSION['ERRORS_ORDER_BY'] = $order_by;
$_SESSION['ERRORS_SORT_ORDER'] = $sorder;
//<<<<<<<<<<<<<<<<<<< sorting - stored in session >>>>>>>>>>>>>>>>>>>>
...
if(isPermitted('Errors','Delete','') == 'yes')
{
	$other_text['del'] = $app_strings[LBL_MASS_DELETE];
}
if(isPermitted('Errors','EditView','') == 'yes')
{
...
if($viewid != "0")
{
	$listquery = getListQuery("Errors");
	$query = $oCustomView->getModifiedCvListQuery($viewid,$listquery,"Errors");
}else
{
	$query = getListQuery("Errors");
}
//<<<<<<<<customview>>>>>>>>>
if(isset($where) && $where != '')
{
...
$listview_header = getListViewHeader($focus,"Errors",$url_string,$sorder,$order_by,"",$oCustomView);
$smarty->assign("LISTHEADER", $listview_header);

$listview_header_search = getSearchListHeaderValues($focus,"Errors",$url_string,$sorder,$order_by,"",$oCustomView);
$smarty->assign("SEARCHLISTHEADER",$listview_header_search);

$listview_entries = getListViewEntries($focus,"Errors",$list_result,$navigation_array,"","","EditView","Delete",$oCustomView);
...
$navigationOutput = getTableHeaderSimpleNavigation($navigation_array, $url_string,"Errors","index",$viewid);
