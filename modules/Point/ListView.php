...
$_SESSION['POINT_ORDER_BY'] = $order_by;
$_SESSION['POINT_SORT_ORDER'] = $sorder;
//<<<<<<<<<<<<<<<<<<< sorting - stored in session >>>>>>>>>>>>>>>>>>>>
...
if(isPermitted('Point','Delete','') == 'yes')
{
	$other_text['del'] = $app_strings[LBL_MASS_DELETE];
}
if(isPermitted('Point','EditView','') == 'yes')
{
...
if($viewid != "0")
{
	$listquery = getListQuery("Point");
	$query = $oCustomView->getModifiedCvListQuery($viewid,$listquery,"Point");
}else
{
	$query = getListQuery("Point");
}
//<<<<<<<<customview>>>>>>>>>
...
if(isset($order_by) && $order_by != '')
{
	if($order_by == 'smownerid')
        ...
        {
		$tablename = getTableNameForField('Point',$order_by);
...
$listview_header = getListViewHeader($focus,"Point",$url_string,$sorder,$order_by,"",$oCustomView);
$smarty->assign("LISTHEADER", $listview_header);

$listview_header_search = getSearchListHeaderValues($focus,"Point",$url_string,$sorder,$order_by,"",$oCustomView);
$smarty->assign("SEARCHLISTHEADER",$listview_header_search);

$listview_entries = getListViewEntries($focus,"Point",$list_result,$navigation_array,"","","EditView","Delete",$oCustomView);
...
$navigationOutput = getTableHeaderSimpleNavigation($navigation_array, $url_string,"Point","index",$viewid);
