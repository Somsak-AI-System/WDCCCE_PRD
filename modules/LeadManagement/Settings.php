...
$menu_array['LayoutEditor']['image_src'] = aicrm_imageurl('orgshar.gif', $theme);
...
if(vtlib_isModuleActive('FieldFormulas')) {
	$sql_result = $adb->pquery("select * from aicrm_settings_field where name = ? and active=0",array('LBL_FIELDFORMULAS'));
...
if(vtlib_isModuleActive('Tooltip')){
	$sql_result = $adb->pquery("select * from aicrm_settings_field where name = ? and active=0",array('LBL_TOOLTIP_MANAGEMENT'));
	if($adb->num_rows($sql_result) > 0) {
		$menu_array['Tooltip']['location'] = $adb->query_result($sql_result, 0, 'linkto').'&formodule='.$module;
		$menu_array['Tooltip']['image_src'] = aicrm_imageurl($adb->query_result($sql_result, 0, 'iconpath'), $theme);
...
$menu_array['LeadsMapping']['image_src'] = aicrm_imageurl('custom.gif', $theme);
...
$sql_result = $adb->pquery("SELECT * FROM aicrm_settings_field WHERE name = ? AND active=0",array('LBL_WORKFLOW_LIST'));
if($adb->num_rows($sql_result) > 0) {
	$menu_array['Workflow']['location'] = $adb->query_result($sql_result, 0, 'linkto').'&list_module='.$module;
	$menu_array['Workflow']['image_src'] = aicrm_imageurl($adb->query_result($sql_result, 0, 'iconpath'), $theme);
	$menu_array['Workflow']['desc'] = getTranslatedString($adb->query_result($sql_result, 0, 'description'),'com_aicrm_workflow');
	$menu_array['Workflow']['label'] = getTranslatedString($adb->query_result($sql_result, 0, 'name'),'com_aicrm_workflow');
}
