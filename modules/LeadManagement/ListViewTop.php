...
function getNewLeads($maxval,$calCnt)
{
	...
	global $current_user;
	$current_module_strings = return_module_language($current_language, 'LeadManagement');

	$theme_path="themes/".$theme."/";
	$image_path=$theme_path."images/";
	if($_REQUEST['lead_view']=='')
	{	
		$query = "select lead_view from aicrm_users where id =?";
	...
	$list_query = 'select aicrm_leadmanage.firstname, aicrm_leadmanage.lastname, aicrm_leadmanage.leadid, aicrm_leadmanage.company 
		from aicrm_leadmanage inner join aicrm_crmentity on aicrm_leadmanage.leadid = aicrm_crmentity.crmid 
		where aicrm_crmentity.deleted =0 AND aicrm_leadmanage.converted =0 AND 
		aicrm_leadmanage.leadstatus not in ("Lost Lead", "Junk Lead","'.$current_module_strings['Lost Lead'].'","'.$current_module_strings['Junk Lead'].'") 
		AND aicrm_crmentity.createdtime >=? AND aicrm_crmentity.smownerid = ?';
    ...
    foreach($open_lead_list as $lead)
	{
		...
		$value[]= '<a href="index.php?action=DetailView&module=LeadManagement&record='.$lead_fields['LEAD_ID'].'">'.$Top_Leads.'</a>';
		$value[]=$lead_fields['COMPANY'];

		$entries[$lead_fields['LEAD_ID']]=$value;
	}
	
	$search_qry = "&query=true&Fields0=aicrm_leadmanage.leadstatus&Condition0=isn&Srch_value0=Lost+Lead&Fields1=aicrm_leadmanage.leadstatus&Condition1=isn&Srch_value1=Junk+Lead&Fields2=aicrm_crmentity.smownerid&Condition2=is&Srch_value2=".$current_user->column_fields['user_name']."&Fields3=aicrm_crmentity.createdtime&Condition3=grteq&Srch_value3=".$start_date."&searchtype=advance&search_cnt=4&matchtype=all";

	$values=Array('ModuleName'=>'LeadManagement','Title'=>$title,'Header'=>$header,'Entries'=>$entries,'search_qry'=>$search_qry);
	$log->debug("Exiting getNewLeadManagement method ...");
