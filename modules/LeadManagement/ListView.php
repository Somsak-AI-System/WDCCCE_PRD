...
$focus = new LeadManagement();
// Initialize sort by fields
$focus->initSortbyField('LeadManagement');
// END
...
//<<<<cutomview>>>>>>>
$oCustomView = new CustomView("LeadManagement");
...
//Modified by Raju
//raju
if(isPermitted('LeadManagement','Delete','') == 'yes')
{
...
if(isPermitted('LeadManagement','EditView','') == 'yes')
{
    $other_text['mass_edit'] = $app_strings[LBL_MASS_EDIT];
	$other_text['c_owner'] = $app_strings[LBL_CHANGE_OWNER];
}

$fieldpermission = getFieldVisibilityPermission('LeadManagement',$current_user->id,'leadstatus');
...
if($viewid != "0")
{
	$listquery = getListQuery("LeadManagement");
	$query = $oCustomView->getModifiedCvListQuery($viewid,$listquery,"LeadManagement");
}else
{
	$query = getListQuery("LeadManagement");
}
//<<<<<<<<customview>>>>>>>>>
...
{
	$tablename = getTableNameForField('LeadManagement',$order_by);
...
if(isPermitted("LeadManagement","Merge") == 'yes') 
{
	$wordTemplateResult = fetchWordTemplateList("LeadManagement");
	...
	if($tempCount > 0)
	{
		...
		$smarty->assign("MERGEBUTTON","<td><input title=\"$app_strings[LBL_MERGE_BUTTON_TITLE]\" accessKey=\"$app_strings[LBL_MERGE_BUTTON_KEY]\" class=\"crmbutton small create\" onclick=\"return massMerge('LeadManagement')\" type=\"submit\" name=\"Merge\" value=\" $app_strings[LBL_MERGE_BUTTON_LABEL]\"></td>");
	}
	else
        {
...
$listview_header = getListViewHeader($focus,"LeadManagement",$url_string,$sorder,$order_by,"",$oCustomView);
$smarty->assign("LISTHEADER", $listview_header);

$listview_header_search=getSearchListHeaderValues($focus,"LeadManagement",$url_string,$sorder,$order_by,"",$oCustomView);
$smarty->assign("SEARCHLISTHEADER", $listview_header_search);

$listview_entries = getListViewEntries($focus,"LeadManagement",$list_result,$navigation_array,"","","EditView","Delete",$oCustomView);
...
$navigationOutput = getTableHeaderSimpleNavigation($navigation_array, $url_string,"LeadManagement","index",$viewid);
