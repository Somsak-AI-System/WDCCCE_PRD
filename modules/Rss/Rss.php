...
class vtigerRSS extends CRMEntity
{
	...
	function saveRSSUrl($url,$makestarred=0)
	{
		...
		if ($url != "")
		{
			...
			$genRssId = $adb->getUniqueID("aicrm_rss");
			$sSQL = "insert into aicrm_rss (RSSID,RSSURL,RSSTITLE,RSSTYPE,STARRED) values (?,?,?,?,?)";
	...
	function getCRMRssFeeds()
	{
		...
		$sSQL = "select * from aicrm_rss where rsstype=1";
		...
		while($allrssrow = $adb->fetch_array($result))
		{
			...
			{
				$shtml .= "<td width=\"15\">
					<img src=\"". aicrm_imageurl('onstar.gif', $theme) ."\" align=\"absmiddle\" onMouseOver=\"this.style.cursor='pointer'\" id=\"star-$allrssrow[rssid]\"></td>";
			}else
			{
				$shtml .= "<td width=\"15\">
					<img src=\"". aicrm_imageurl('offstar.gif', $theme) ."\" align=\"absmiddle\" onMouseOver=\"this.style.cursor='pointer'\" id=\"star-$allrssrow[rssid]\" onClick=\"makedefaultRss($allrssrow[rssid])\"></td>";
			}
	...
	function getRssUrlfromId($rssid)
	{
		...
		if($rssid != "")
		{
			$sSQL = "select * from aicrm_rss where rssid=?";
	...
	function getRSSHeadings($rssid)
	{
		...
		if($rssid != "")
		{
			...
			if(count($rssrow) > 0)
			{
				...
				if($rssrow[starred] == 1)
				{
					$shtml .= "<img src=\"". aicrm_imageurl('starred.gif', $theme) ."\" align=\"absmiddle\">";
				}else
				{
					$shtml .= "<img src=\"". aicrm_imageurl('unstarred.gif', $theme) ."\" align=\"absmiddle\">";
				}		
	...
	function getTopStarredRSSFeeds()
	{
		...
		$sSQL = "select * from aicrm_rss where starred=1";
		$result = $adb->pquery($sSQL, array());
		$shtml .= "<img src=\"". aicrm_imageurl('rss.gif', $theme) ."\" border=\"0\" align=\"absmiddle\" hspace=\"2\"><a href=\"#\" onclick='window.open(\"index.php?module=Rss&action=Popup\",\"new\",\"width=500,height=300,resizable=1,scrollbars=1\");'>Add New Rss</a>";

		while($allrssrow = $adb->fetch_array($result))
		{
			$shtml .= "<img src=\"". aicrm_imageurl('rss.gif', $theme) ."\" border=\"0\" align=\"absmiddle\" hspace=\"2\">"; 
	...
	function getRssFeedsbyCategory()
	{
		...
		while($allrssrow = $adb->fetch_array($result))
		{
			...
			if($allrssrow["starred"] == 1)
			{
				 	   $shtml .= "<img src=\"". aicrm_imageurl('onstar.gif', $theme) ."\" align=\"absmiddle\" onMouseOver=\"this.style.cursor='pointer'\" id=\"star-$allrssrow[rssid]\">";
			}else
			{
				 	   $shtml .= "<img src=\"". aicrm_imageurl('offstar.gif', $theme) ."\" align=\"absmiddle\" onMouseOver=\"this.style.cursor='pointer'\" id=\"star-$allrssrow[rssid]\"  onClick=\"makedefaultRss($allrssrow[rssid])\">";
			}
					   $shtml .= "</td>";
	...
	function trash($module, $id) {
		...
		$del_query = 'DELETE FROM aicrm_rss WHERE rssid=?';
...
function gerRssTitle($id='')
{
	global $adb;
	if($id == '') {
		$query = 'select * from aicrm_rss where starred=1';
		$params = array();	 
	} else {		
		$query = 'select * from aicrm_rss where rssid =?';	 
		$params = array($id);
	}
