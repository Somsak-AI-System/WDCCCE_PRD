...
$log->debug("Inside Purchasesorder EditView");
...
if (isset ($_REQUEST['record']) && $_REQUEST['record'] != '') {
	if (isset ($_REQUEST['convertmode']) && $_REQUEST['convertmode'] == 'quotetopro') {
		...
		$quote_focus = new Purchasesorder();
		$quote_focus->id = $quoteid;
		$quote_focus->retrieve_entity_info($quoteid, "Purchasesorder");
		...
		//Added to display the Purchasesorder's associated aicrm_products -- when we create SO from Purchasesorder DetailView
		$associated_prod = getAssociatedProducts("Purchasesorder", $quote_focus);
		...
		$focus->id = $_REQUEST['record'];
		$focus->mode = 'edit';
		$focus->retrieve_entity_info($_REQUEST['record'], "Purchasesorder");
		$focus->name = $focus->column_fields['purchasesorder_name'];
	}
} else {
	if (isset ($_REQUEST['convertmode']) && $_REQUEST['convertmode'] == 'update_quote_val') {
		...
		//Added to display the Purchasesorder's associated aicrm_products -- when we select Quote in New SO page
		if (isset ($_REQUEST['quote_id']) && $_REQUEST['quote_id'] != '') {
			$associated_prod = getAssociatedProducts("Purchasesorder", $quote_focus, $focus->column_fields['quote_id']);
		}
...
if(isset($_REQUEST['isDuplicate']) && $_REQUEST['isDuplicate'] == 'true') {
	...
	$associated_prod = getAssociatedProductsPurchasesorder("Purchasesorder",$focus);
	$log->debug("Mode is Duplicate. Purchasesorderid to be duplicated is ".$focus->id);
	$focus->id = "";
    $focus->mode = ''; 	
}
if(empty($_REQUEST['record']) && $focus->mode != 'edit'){
	...
	$focus->column_fields['po_date'] = date("Y-m-d");
	$focus->column_fields['po_status'] = "Open";
	$focus->column_fields['term_condition'] = "1. Please confirm ETA and ETD schedule for every Purchase Order asap\r\n2. Enter this order in accordance with the prices, terms, delivery method,and specifications listed above.\r\n3. Please notify us immediately if you are unable to ship as specified.\r\n4. Send all correspondece to:\r\nMs.Sutussanee Techamanakul\r\nPlaner Manager, Marketing\r\nGreenlam Asia Pacific (Thailand) Co.,Ltd.\r\nEmail: sutussanee.th@greenlam.com\r\nMobile: +66 81 651 2690";
...
if(isset($_REQUEST['potential_id']) && $_REQUEST['potential_id'] !=''){
	...
	$log->debug("Purchasesorder EditView: Potential Id from the request is ".$_REQUEST['potential_id']);
	$associated_prod = getAssociatedProductsPurchasesorder("Potentials",$focus,$focus->column_fields['potential_id']);
}

if(isset($_REQUEST['product_id']) && $_REQUEST['product_id'] !=''){
    $focus->column_fields['product_id'] = $_REQUEST['product_id'];
    $log->debug("Product Id from the request is ".$_REQUEST['product_id']);
    $associated_prod = getAssociatedProductsPurchasesorder("Products",$focus,$focus->column_fields['product_id']);
...
if(!empty($_REQUEST['parent_id']) && !empty($_REQUEST['return_module'])){
    if ($_REQUEST['return_module'] == 'Services') {
	    ...
	    $associated_prod = getAssociatedProductsPurchasesorder("Services",$focus,$focus->column_fields['product_id']);
		for ($i=1; $i<=count($associated_prod);$i++) {
			$associated_prod_id = $associated_prod[$i]['hdnProductId'.$i];
...
else $smarty->assign("RETURN_MODULE","Purchasesorder");
...
$smarty->assign("a_gr_percentage", getGR_Percentage());
$smarty->assign("a_po_price_type", get_po_price_type());

if($focus->mode == 'edit'){
	$smarty->display("Purchasesorder/InventoryEditView.tpl");
}else{
	$smarty->display('Purchasesorder/InventoryCreateView.tpl');
}
