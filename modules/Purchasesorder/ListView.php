...
$focus->initSortbyField('Purchasesorder');
// END
...
$_SESSION['PURCHASESORDER_ORDER_BY'] = $order_by;
$_SESSION['PURCHASESORDER_SORT_ORDER'] = $sorder;
//<<<<<<<<<<<<<<<<<<< sorting - stored in session >>>>>>>>>>>>>>>>>>>>
...
if(isPermitted('Purchasesorder','Delete','') == 'yes')
{
	$other_text['del'] = $app_strings[LBL_MASS_DELETE];
}
if(isPermitted('Purchasesorder','EditView','') == 'yes')
{
...
if($viewid != "0")
{
	$listquery = getListQuery("Purchasesorder");
	$query = $oCustomView->getModifiedCvListQuery($viewid,$listquery,"Purchasesorder");
}else
{
	$query = getListQuery("Purchasesorder");
}
...
if(isset($order_by) && $order_by != '')
{
	if($order_by == 'smownerid'){
		...
		$tablename = getTableNameForField('Purchasesorder',$order_by);
...
$listview_header = getListViewHeader($focus,"Purchasesorder",$url_string,$sorder,$order_by,"",$oCustomView);
$smarty->assign("LISTHEADER", $listview_header);

$listview_header_search = getSearchListHeaderValues($focus,"Purchasesorder",$url_string,$sorder,$order_by,"",$oCustomView);
$smarty->assign("SEARCHLISTHEADER",$listview_header_search);

$listview_entries = getListViewEntries($focus,"Purchasesorder",$list_result,$navigation_array,"","","EditView","Delete",$oCustomView);
...
$navigationOutput = getTableHeaderSimpleNavigation($navigation_array, $url_string,"Purchasesorder","index",$viewid);
...
if($rowCount>0){
	for($i=0;$i<=$rowCount;$i++){
		$a_data=array();
		$purchasesorderid=$adb->query_result($list_result, $i, "crmid");
		$display= isPermitted('Purchasesorder',"EditView",$purchasesorderid);
		$a_response[$purchasesorderid]["display"] = $display;
	}
}
