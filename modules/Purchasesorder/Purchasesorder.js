...
function set_return_specific(product_id, product_name) {
        ...
        var fldName = getOpenerObj("purchasesorder_name");
...
function set_return_project_address(project_id,project_name, formName ,account_id,account_name,cf_3995,contact_id,contacts_name,con_department,con_position,email,mobile,phone) {
		...
		if(typeof(form.purchasesorder_name) != 'undefined')
            form.purchasesorder_name.value = project_name;
...
function set_return_inventory_purchasesorder(purchasesorderid,curr_row,purchasesorder_no,purchasesorder_name,po_date,po_status){

    if(po_status=="Completed"){
        alert("This Item Detail already Completed.");
        return false;
    }else{
        window.opener.document.EditView.elements["poName"+curr_row].value = purchasesorder_name;
        window.opener.document.EditView.elements["purchasesorder_no"+curr_row].value = purchasesorder_no;
        window.opener.document.EditView.elements["hdnPoId"+curr_row].value = purchasesorderid;
        window.opener.document.EditView.elements["purchase_order_date"+curr_row].value = po_date;
    }
}
