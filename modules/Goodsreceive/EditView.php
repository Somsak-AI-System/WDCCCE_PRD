...
$log->debug("Inside Goodsreceive EditView");
...
if (isset ($_REQUEST['record']) && $_REQUEST['record'] != '') {
	if (isset ($_REQUEST['convertmode']) && $_REQUEST['convertmode'] == 'quotetopro') {
		...
		$associated_prod = getAssociatedGoodsreceive("Goodsreceive", $quote_focus);
	...
	elseif (isset ($_REQUEST['convertmode']) && $_REQUEST['convertmode'] == 'update_quote_val') {
		...
		$focus->id = $_REQUEST['record'];
		$focus->mode = 'edit';
		$focus->retrieve_entity_info($_REQUEST['record'], "Goodsreceive");
		$focus->name = $focus->column_fields['goodsreceive_name'];
	}
} else {
	if (isset ($_REQUEST['convertmode']) && $_REQUEST['convertmode'] == 'update_quote_val') {
		...
		if (isset ($_REQUEST['quote_id']) && $_REQUEST['quote_id'] != '') {
			$associated_prod = getAssociatedGoodsreceive("Goodsreceive", $quote_focus, $focus->column_fields['quote_id']);
		}
...
if(isset($_REQUEST['isDuplicate']) && $_REQUEST['isDuplicate'] == 'true') {
	...
	$QUOTE_associated_prod = getAssociatedGoodsreceive("Goodsreceive",$focus);
	$log->debug("Mode is Duplicate. Goodsreceiveid to be duplicated is ".$focus->id);
	$focus->id = "";
    $focus->mode = ''; 	
}
...
if($focus->mode == 'edit')
{
	...
	$associated_prod = getAssociatedGoodsreceive("Goodsreceive",$focus);//getProductDetailsBlockInfo('edit','Quotes',$focus); 
	$smarty->assign("ASSOCIATEDPRODUCTS", $associated_prod);
...
else $smarty->assign("RETURN_MODULE","Goodsreceive");
...
// END
//echo "<pre>"; print_r(getAssociatedGoodsreceive("Goodsreceive",$focus)); echo "</pre>"; exit;
...
if($focus->mode == 'edit')
	$smarty->display("Goodsreceive/InventoryEditView.tpl");
else
	$smarty->display('Goodsreceive/InventoryCreateView.tpl');
