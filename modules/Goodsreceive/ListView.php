...
$focus->initSortbyField('Goodsreceive');
// END
...
$_SESSION['GOODSRECEIVE_ORDER_BY'] = $order_by;
$_SESSION['GOODSRECEIVE_SORT_ORDER'] = $sorder;
//<<<<<<<<<<<<<<<<<<< sorting - stored in session >>>>>>>>>>>>>>>>>>>>
...
if(isPermitted('Goodsreceive','Delete','') == 'yes')
{
	$other_text['del'] = $app_strings[LBL_MASS_DELETE];
}
if(isPermitted('Goodsreceive','EditView','') == 'yes')
{
...
if($viewid != "0")
{
	$listquery = getListQuery("Goodsreceive");
	$query = $oCustomView->getModifiedCvListQuery($viewid,$listquery,"Goodsreceive");
}else
{
	$query = getListQuery("Goodsreceive");
}
...
if(isset($order_by) && $order_by != '')
{
	if($order_by == 'smownerid'){
		...
		$tablename = getTableNameForField('Goodsreceive',$order_by);
...
$listview_header = getListViewHeader($focus,"Goodsreceive",$url_string,$sorder,$order_by,"",$oCustomView);
$smarty->assign("LISTHEADER", $listview_header);

$listview_header_search = getSearchListHeaderValues($focus,"Goodsreceive",$url_string,$sorder,$order_by,"",$oCustomView);
$smarty->assign("SEARCHLISTHEADER",$listview_header_search);

$listview_entries = getListViewEntries($focus,"Goodsreceive",$list_result,$navigation_array,"","","EditView","Delete",$oCustomView);
...
$navigationOutput = getTableHeaderSimpleNavigation($navigation_array, $url_string,"Goodsreceive","index",$viewid);
...
if($rowCount>0){
	for($i=0;$i<=$rowCount;$i++){
		$a_data=array();
		$goodsreceiveid=$adb->query_result($list_result, $i, "crmid");
		$display= isPermitted('Goodsreceive',"EditView",$goodsreceiveid);
		$a_response[$goodsreceiveid]["display"] = $display;
	}
}
