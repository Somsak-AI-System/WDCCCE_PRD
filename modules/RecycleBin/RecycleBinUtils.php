...
$rb_table_col_array=array('aicrm_account.accountname','aicrm_contactdetails.firstname,aicrm_contactdetails.lastname','aicrm_products.productname','aicrm_campaign.campaignname','aicrm_quotes.subject','aicrm_vendor.vendorname','aicrm_potential.potentialname','aicrm_salesorder.subject','aicrm_vendor.vendorname','aicrm_contactdetails.firstname,aicrm_contactdetails.lastname');
...
function getValuesforRBColumns($column_name,$search_string) {
	...
	$sql = "select concat(tablename,':',fieldname) as tablename from aicrm_entityname where entityidfield=? or entityidcolumn=?";
...
{
	//echo $module."/".$search_field."/".$search_string; exit;
	global $log;
	...
	{	
		...
		$qry="select aicrm_field.columnname,tablename from aicrm_field where tabid=? and (fieldname=? or columnname=?) and aicrm_field.presence in (0,2)";
		...
		if($noofrows!=0)
		{
			...
			if($table_name == "aicrm_crmentity" && $column_name == "smownerid")
			{
				$where = get_usersid($table_name,$column_name,$search_string);
			}
			elseif($table_name == "aicrm_activity" && $column_name == "status")
			{
				$where="$table_name.$column_name like '".formatForSqlLike($search_string)."' or aicrm_activity.eventstatus like '".formatForSqlLike($search_string)."'";
			}
			elseif($table_name == "aicrm_pricebook" && $column_name == "active")
			{
			...
			elseif($table_name == "aicrm_activity" && $column_name == "status")
			{
				$where="$table_name.$column_name like '%".$search_string."%' or aicrm_activity.eventstatus like '".formatForSqlLike($search_string)."'";
			}
			$sql = "select concat(tablename,':',fieldname) as tablename from aicrm_entityname where entityidfield='$column_name' or entityidcolumn='$column_name'"; 
			$no_of_rows = $adb->num_rows($adb->query($sql));
			if($no_of_rows >= 1)
			{
				$where = getValuesforRBColumns($column_name,$search_string);
			}
			else if(($column_name != "status" || $table_name !='aicrm_activity')  && ($table_name != 'aicrm_crmentity' || $column_name != 'smownerid' ) && ($table_name != 'aicrm_pricebook' || $column_name != 'active') )
			{
