...
$skip_modules = array('Webmails','Events','Users','CustomizeReport','dashboadbusupportteam','Reports');
$skip_tab_ids = array();
...
$sql = 'SELECT tabid, name FROM aicrm_tab WHERE presence=0 AND isentitytype=1 ';
...
if(count($module_name) > 0)
{
	...
	$list_query = preg_replace("/aicrm_crmentity.deleted\s*=\s*0/i", 'aicrm_crmentity.deleted = 1', $list_query);
	...
	//echo $start."/".$noofrows."/".$list_max_entries_per_page;
...
$query = "select fieldname from aicrm_entityname where modulename =?";
