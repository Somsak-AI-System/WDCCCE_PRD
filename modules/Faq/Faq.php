...
// Account is used to store aicrm_account information.
	var $log;
	var $db;
	var $table_name = "aicrm_faq";
	var $table_index= 'faqid';
	var $_images_path= 'upload/faq/';
	var $table_comment = "aicrm_faqcomments";

	var $tab_name = Array('aicrm_crmentity','aicrm_faq','aicrm_faqcf');
	var $tab_name_index = Array('aicrm_crmentity'=>'crmid','aicrm_faq'=>'faqid','aicrm_faqcf'=>'faqid','aicrm_faqcomments'=>'faqid');
	/**
	 * Mandatory table for supporting custom fields.
	 */
	var $customFieldTable = Array('aicrm_faqcf', 'faqid');
	var $column_fields = Array();

	var $sortby_fields = Array('faqid','faq_name','smownerid');

	var $list_fields = Array(
		'Faq No'=>Array('Faq'=>'faq_no'),
		'Faq Name'=>Array('Faq'=>'faq_name'),
	...
	var $search_fields = Array(
		'Faq No'=>Array('aicrm_faq'=>'faq_no'),
		'Faq Name'=>Array('aicrm_faq'=>'faq_name'),
		'Assigned To' => Array('crmentity'=>'smownerid')
	);
	...
	var $mandatory_fields = Array('assigned_user_id','createdtime' ,'modifiedtime','faqid');
	...
	{
		global $adb;
		$this->insertIntoAttachment($this->id,'Faq');
		$this->insertIntoCommentTable("aicrm_faqcomments",'faqid');
	...
	function getCommentInformation($crmid)
	{
		...
		$sql = "select * from aicrm_faqcomments where faqid=? order by createdtime desc";
	...
	function getSortOrder()
	{
		...
		if(isset($_REQUEST['sorder']))
			...
			$sorder = (($_SESSION['FAQ_SORT_ORDER'] != '')?($_SESSION['FAQ_SORT_ORDER']):($this->default_sort_order));
	...
	function getOrderBy()
	{
		...
		if (isset($_REQUEST['order_by']))
			...
			$order_by = (($_SESSION['FAQ_ORDER_BY'] != '')?($_SESSION['FAQ_ORDER_BY']):($use_default_order_by));
	...
	function generateReportsSecQuery($module,$secmodule){
		$query = $this->getRelationQuery($module,$secmodule,"aicrm_faq","faqid");
		$query .=" left join aicrm_crmentity as aicrm_crmentityEmailtarget on aicrm_crmentityEmailtarget.crmid=aicrm_faq.faqid and aicrm_crmentityEmailtarget.deleted=0
				left join aicrm_faqcf on aicrm_faqcf.faqid = aicrm_crmentityEmailtarget.crmid
	...
	function create_export_query($where)
	{
		...
		$sql = getPermittedFieldsQuery("Faq", "detail_view");
		...
		$query = "SELECT $fields_list,case when (aicrm_users.user_name not like '') then aicrm_users.user_name else aicrm_groups.groupname end as user_name
				...
				INNER JOIN aicrm_faq ON aicrm_faq.faqid = aicrm_crmentity.crmid
		...
		if($is_admin==false && $profileGlobalPermission[1] == 1 && $profileGlobalPermission[2] == 1 && $defaultOrgSharingPermission[6] == 3)
		{
			...
			$query = $query." ".getListViewSecurityParameter("Faq");
		}
	...
	function setRelationTables($secmodule){
		$rel_tables = array (
			"Documents" => array("aicrm_senotesrel"=>array("crmid","notesid"),"aicrm_faq"=>"faqid"),
		);
	...
	function insertIntoAttachment($id,$module){
		...
		if($module == 'Faq' && $_REQUEST['del_file_list'] != '')
		{
