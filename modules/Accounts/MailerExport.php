...
function getStdContactFlds(&$queryFields, $adb, $valueArray)
{
  ...
  $query = "SELECT fieldid, columnname, fieldlabel FROM aicrm_field WHERE tablename='aicrm_contactdetails' AND uitype='56' and aicrm_field.presence in (0,2)";
...
if ($step == "ask")
{
  ...
  $queryFields = Array();
	// get the Contacts CF fields
 	$cfquery = "SELECT columnname,fieldlabel,uitype FROM aicrm_field WHERE tablename='aicrm_contactscf' and aicrm_field.presence in (0,2)";
	...
	for ($tmp=0; $tmp < $adb->num_rows($result); $tmp++)
	{
	    ...
	    elseif ($cfTmp['uitype'] == 15)
	    {
        	...
        	$cfValues = "SELECT ".$cfColName.",".$cfColName."id FROM aicrm_".$cfColName;
...
else
{
  ...
  if (($export_type == "email") || ($export_type == "emailplus") )
  {
	...
	if(count($fields) > 0)
	 ...
	 $exquery[0] = "SELECT crmentity.crmid, contactdetails.contactid,
	   ...
	   contactdetails.lastname, contactdetails.email  FROM aicrm_account
	   INNER JOIN aicrm_crmentity crmentity on crmentity.crmid=aicrm_account.accountid
	   ...
	   INNER JOIN aicrm_contactdetails contactdetails ON aicrm_account.accountid = contactdetails.accountid
	   INNER JOIN aicrm_contactscf contactscf ON contactscf.contactid = contactdetails.contactid
    ...
    {     
		$exquery[1] = "SELECT crmentity.crmid, contactdetails.contactid,
		    contactdetails.salutation, contactdetails.firstname,
		    contactdetails.lastname, aicrm_account.email1  FROM aicrm_account
		    INNER JOIN aicrm_crmentity crmentity ON crmentity.crmid=aicrm_account.accountid
		    INNER JOIN aicrm_accountbillads ON aicrm_account.accountid=aicrm_accountbillads.accountaddressid
		    INNER JOIN aicrm_accountshipads ON aicrm_account.accountid=aicrm_accountshipads.accountaddressid
		    INNER JOIN aicrm_accountscf ON aicrm_account.accountid = aicrm_accountscf.accountid
		    ...
		    AND contactdetails.email = '' AND aicrm_account.email1 != '' ".$where;
  ...
  else if ($export_type == "full")
  {
	...
	if(count($fields) > 0)
	    ...
	    $exquery[0] = "select crmentity.crmid, contactdetails.contactid,
		    contactdetails.salutation, contactdetails.firstname,
		    contactdetails.lastname, contactdetails.email, aicrm_account.accountname,"
	      ." aicrm_account.phone, aicrm_account.website, aicrm_accountshipads.ship_street,"
	      ." aicrm_accountshipads.ship_code,"
	      ." aicrm_accountshipads.ship_city, aicrm_accountshipads.ship_state,"
	      ." aicrm_accountshipads.ship_country,"
	      ." aicrm_accountbillads.bill_street, aicrm_accountbillads.bill_code,"
	      ." aicrm_accountbillads.bill_city, aicrm_accountbillads.bill_state,"
	      ." aicrm_accountbillads.bill_country"
	      ." FROM aicrm_account INNER JOIN aicrm_crmentity crmentity ON crmentity.crmid=aicrm_account.accountid"
  ...
  for ($temp = 0; $temp < count($exquery); $temp++)
  {
	  ...
	  if ($temp == 0)  // We only need the headers for first query
	  {
	    ...
	    {
	      if (strstr ($fields_array[$arraywalk], "aicrm_cf_"))
	      {
	  ...
	  while($val = $adb->fetchByAssoc($result, -1, false))
		{
			...
			$line = "\"" .$line;
			$line .= "\"\r\n";
			$content .= iconv( 'UTF-8' ,'TIS-620',$line);
	  }
	}
