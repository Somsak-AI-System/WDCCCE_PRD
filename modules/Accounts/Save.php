 ...
 ********************************************************************************/
/*********************************************************************************
 * $Header: /advent/projects/wesat/aicrm_crm/sugarcrm/modules/Accounts/Save.php,v 1.7 2005/03/15 09:55:31 shaw Exp $
...
if(isset($_REQUEST['dup_check']) && $_REQUEST['dup_check'] != '')
{
	...
	//echo $id;
	if(isset($id) && $id !='') {
		$query .= " and aicrm_account.accountid != ?";
...
//Send notification mail to the assigned to owner about the aicrm_account creation

if($focus->column_fields['notify_owner'] == 1 || $focus->column_fields['notify_owner'] == 'on'){
	$status = sendNotificationToOwner('Accounts',$focus);
}

if(isset($_REQUEST['qcinform'])){
	
	echo json_encode($focus);
	exit;
...
function save_customfields($entity_id)
{
	...
	$custquery = "SELECT * FROM aicrm_accountcf WHERE aicrm_accountid = ?";
	...
	if($adb->num_rows($result) != 0)
	{
		...
		{
			//Update Block
            $query = "UPDATE aicrm_accountcf SET $update WHERE aicrm_accountid=?";
		...
		{
			//Insert Block
			$query = "INSERT INTO aicrm_accountcf ($columns) VALUES(" . generateQuestionMarks($ins_params) .")";
