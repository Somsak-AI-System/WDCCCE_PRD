...
if(!empty($_REQUEST['primecvid'])){
	...
	$queryprime="select cvid,columnname from aicrm_cvcolumnlist where columnname not like '%::%' and cvid=?";
	  ...
	  die;
		...
		for($i=0;$i<$adb->num_rows($result);$i++){
			...
			if($columnname != ''){
				...
				if($fld_permission == 0){
					$field_query = $adb->pquery("SELECT fieldlabel FROM aicrm_field WHERE fieldname = ? AND tablename = ? and aicrm_field.presence in (0,2)", array($fieldname,$prifldarr[0]));
...
if(!empty($_REQUEST['showmaxval']) && !empty($_REQUEST['sid'])){
	...
	global $adb;
	$query="select stufftype from aicrm_homestuff where stuffid=?";
	$res=$adb->pquery($query, array($sid));
	$stufftypename=$adb->query_result($res,0,"stufftype");
	if($stufftypename=="Module"){
		$qry="update aicrm_homemodule set maxentries=? where stuffid=?";
		$result=$adb->pquery($qry, array($maxval, $sid));
	}else if($stufftypename=="RSS"){
		$qry="update aicrm_homerss set maxentries=? where stuffid=?";
		$result=$adb->pquery($qry, array($maxval, $sid));
	}else if($stufftypename=="Default"){
		$qry="update aicrm_homedefault set maxentries=? where stuffid=?";
		$result=$adb->pquery($qry, array($maxval, $sid));
	}
...
if(!empty($_REQUEST['dashVal'])){
	$did=$_REQUEST['did'];
	global $adb;
	$qry="update aicrm_homedashbd set dashbdtype=? where stuffid=?";	
...
if(!empty($_REQUEST['homestuffid'])){
	...
	global $adb;
	$query="delete from aicrm_homestuff where stuffid=?";
...
if(!empty($_REQUEST['matrixsequence'])){
	...
	for($i=count($sequence)-1, $seq=0;$i>=0;$i--, $seq++){
		$query = 'update aicrm_homestuff set stuffsequence=? where stuffid=?';
...
if(isset($_REQUEST['act']) && $_REQUEST['act'] =="hide"){
	...
	$qry="update aicrm_homestuff set visible=1 where stuffid=?";
...
if(!empty($_REQUEST['layout'])){
	...
	$sql = "delete from aicrm_home_layout where userid=?";
	$result = $adb->pquery($sql, array($current_user->id));
	
	$sql = "insert into aicrm_home_layout values (?, ?)";
