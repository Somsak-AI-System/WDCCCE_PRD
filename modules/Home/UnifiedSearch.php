...
if(ereg('^[ก-์ ะ-ู เ-แ]+$', $query_string))
{   
    $string_thai = true;
}
...
if(isset($query_string) && $query_string != ''){
	...
	//echo $search_val;
	//echo "<pre>"; print_r($search_module); echo "</pre>"; exit;
	if($curModule=='Home') {
	...
	foreach($object_array as $module => $object_name){
		if ($curModule == 'Home' || ($curModule == $module && !empty($_REQUEST['ajax']))) {
			...
			if(isPermitted($module,"index") == "yes"){
				...
				$cv_res = $adb->pquery("select cvid from aicrm_customview where viewname='All' and entitytype=?", array($module));
				...
				if($search_module != ''){//This is for Tag search
					...
					$where = getTagWhereModule($search_val,$current_user->id);//Tags
					$where .= getUnifiedWhere($listquery,$module,$search_val,$string_thai);
 ...
 * @return string $where      -- where condition with the list of crmids, will like aicrm_crmentity.crmid in (1,3,4,etc.,)
 */
function getTagWhereModule($search_val,$current_user_id){
	require_once('include/freetag/freetag.class.php');

	$freetag_obj = new freetag();
	$crmid_array = $freetag_obj->get_objects_with_tag_all($search_val,$current_user_id);
	//echo "<pre>"; print_r($crmid_array); echo "</pre>"; exit;
	$where = " aicrm_crmentity.crmid IN (";
...
function getSearchModulesComboList($search_module){
	...
	?>
		...
		<td nowrap align="right"><?php echo $app_strings['LBL_SHOW_RESULTS'] ?>&nbsp;
		    <select id="global_search_module" name="global_search_module" onChange="displayModuleList(this);" class="small" style="width:20%; margin-top: 10px; ">
...
function getSearchModules($filter = array()){
	...
	//$sql = 'select distinct aicrm_field.tabid,name from aicrm_field inner join aicrm_tab on aicrm_tab.tabid=aicrm_field.tabid where aicrm_tab.tabid not in (16,29)';
	$sql = 'select distinct aicrm_field.tabid,name from aicrm_field inner join aicrm_tab on aicrm_tab.tabid=aicrm_field.tabid where aicrm_tab.tabid not in (16,29) and aicrm_tab.presence != 1 and aicrm_field.presence in (0,2)';
	// END
