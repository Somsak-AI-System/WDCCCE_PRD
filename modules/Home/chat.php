...
class Chat
{
  ...
  function Chat()
  {
    ...
    if(!isset($_SESSION["mlid"]))
      {
	$res = $adb->pquery("show table status like 'aicrm_chat_msg'", array());
	...
	$this->setUserNick();
      ...
      {
	$res = $adb->pquery("update aicrm_chat_users set ping=now() where session=?", array(session_id()));
  ...
  function setUserNick()
  {
    ...
    $res = $adb->pquery("select id from aicrm_chat_users where session=?", array(session_id()));
    ...
    $res = $adb->pquery("show table status like 'aicrm_chat_users'", array());
    $line = $adb->fetch_array($res);
    if(intval($line['Auto_increment']) == 0)
    ...
    $sql = "insert into aicrm_chat_users(nick,session,ping,ip) values (?,?, now(), ?)";
  ...
  function getUserList()
  {
    ...
    $tmp = '';
    $sql = "delete from aicrm_chat_users where ((unix_timestamp(now())-unix_timestamp(ping))>?)";
	$params = array($chat_conf['alive_time']);
	$res = $adb->pquery($sql, $params);
	
    $res = $adb->pquery("select id,nick from aicrm_chat_users", array());
    if($adb->num_rows($res)==0)
      {
	$this->json = '';
	return;
  ...
  function getAllPVChat()
  {
	...
	$sql ="select ms.id mid,ms.chat_from mfrom,ms.chat_to mto,pv.id id,us.nick `chat_from`,ms.msg msg from aicrm_chat_users us,aicrm_chat_pvchat pv,aicrm_chat_msg ms where pv.msg=ms.id and us.id=ms.chat_from and ms.id>? and ((ms.chat_from=? and ms.chat_to>0) or (ms.chat_to=? and ms.chat_from>0)) order by ms.born limit 0, " . $chat_conf['msg_limit'];
  ...
  function getPubChat()
  {
    ...
    $sql = "select ms.id mid,ms.chat_from mfrom,ms.chat_to mto,p.id id,us.nick `chat_from`,ms.msg msg from aicrm_chat_users us,aicrm_chat_pchat p,aicrm_chat_msg ms where p.msg=ms.id and us.id=ms.chat_from and ms.id>? and ms.chat_to=0 order by ms.born limit 0," . $chat_conf['msg_limit'];
	...
	switch($words[0])
	  ...
	  case '\nick':
	    if(isset($words[1]) && strlen($words[1]) > 3)
	      {
			$res = $adb->pquery("select nick from aicrm_chat_users where id=?", array($_SESSION['chat_user']));
			$line = $adb->fetch_array($res);
			$res = $adb->pquery("update aicrm_chat_users set nick=? where id=?", array($words[1], $_SESSION['chat_user']));
  ...
  function submit($msg, $to=0)
  {
	...
	//$sql = "insert into aicrm_chat_msg set chat_from=?, chat_to=?, born=now(), msg=?";
    $sql = "insert into aicrm_chat_msg(chat_from, chat_to, born, msg) values (?,?, now(), ?)";
    ...
    $res = $adb->pquery("insert into aicrm_chat_".$chat."chat set msg=LAST_INSERT_ID()", array());
  }
  ...
  {
	global $adb;
    $sql = "delete from aicrm_chat_msg where (`chat_from`=? and `chat_to`=?) or (`chat_from`=? and `chat_to`=?)";
