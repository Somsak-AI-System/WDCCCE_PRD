...
$query = "select aicrm_crmentity.setype,aicrm_crmentity.crmid,aicrm_crmentity.smcreatorid,aicrm_crmentity.modifiedtime from aicrm_crmentity inner join aicrm_ownernotify on aicrm_crmentity.crmid=aicrm_ownernotify.crmid";

$result = $adb->pquery($query, array());
for($i=0;$i<$adb->num_rows($result);$i++){
    $mod_notify[$i] = $adb->fetch_array($result);
	if($mod_notify[$i]['setype']=='Accounts'){
		$tempquery='select aicrm_accountname from aicrm_account where aicrm_accountid=?';
		...
		$tempquery='select aicrm_potentialname from aicrm_potential where aicrm_potentialid=?';
		$tempresult=$adb->pquery($tempquery, array($mod_notify[$i]['crmid']));
		...
		$tempquery='select subject from aicrm_quotes where quoteid=?';
		$tempresult=$adb->pquery($tempquery, array($mod_notify[$i]['crmid']));
		$quote_subject=$adb->fetch_array($tempresult);
		$notify_values[$i]=$quote_subject['subject'];
	}else if($mod_notify[$i]['setype']=='Invoice'){
		$tempquery='select subject from aicrm_invoice where aicrm_invoiceid=?';
		$tempresult=$adb->pquery($tempquery, array($mod_notify[$i]['crmid']));
