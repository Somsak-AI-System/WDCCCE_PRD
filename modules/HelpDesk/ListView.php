...
if(isset($where) && $where != '')
{
	if(isset($_REQUEST['from_homepagedb']) && $_REQUEST['from_homepagedb'] == 'true')
		$list_query .= " and (aicrm_troubletickets.status!='Closed' or aicrm_troubletickets.status is null) and ".$where;
...
if(isset($order_by) && $order_by != '')
{
	...
	{
		if( $adb->dbType == "pgsql")
 		    $list_query .= ' GROUP BY aicrm_users.user_name';
		$list_query .= " ORDER BY case when (aicrm_users.user_name not like '') then aicrm_users.user_name else aicrm_groups.groupname end ".$sorder;
	}
	else
	{
...
{
	if( $adb->dbType == "pgsql")
 	    $list_query .= ' GROUP BY aicrm_troubletickets.ticketid';
 	$list_query .= ' ORDER BY aicrm_troubletickets.ticketid DESC';	
}
//echo $list_query;
