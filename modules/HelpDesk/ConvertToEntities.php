...
$query='select * from aicrm_troubletickets';
...
for($i=0;$i<count($idlist);$i++)
{
	...
	{
		$resultset=$adb->pquery('select * from aicrm_users where user_name=?', array($_REQUEST['assigned_user_id']));
		$changevalue=$adb->query_result($resultset,0,"id");
		$changekey='assigned_user_id';
	}
	$sql="update aicrm_troubletickets set $changekey = ? where id = ?";
