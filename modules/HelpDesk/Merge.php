...
{
	$query1="select aicrm_tab.name,aicrm_field.tablename,aicrm_field.columnname,aicrm_field.fieldlabel from aicrm_field inner join aicrm_tab on aicrm_tab.tabid = aicrm_field.tabid where aicrm_field.tabid in (13,4,6) and aicrm_field.uitype <> 61 and block <> 75 and block <> 30 and aicrm_field.presence in (0,2) order by aicrm_field.tablename";
	$params1 = array();
}
else
{
	$profileList = getCurrentUserProfileList();
	$query1="select aicrm_tab.name,aicrm_field.tablename,aicrm_field.columnname,aicrm_field.fieldlabel from aicrm_field inner join aicrm_tab on aicrm_tab.tabid = aicrm_field.tabid INNER JOIN aicrm_profile2field ON aicrm_profile2field.fieldid=aicrm_field.fieldid INNER JOIN aicrm_def_org_field ON aicrm_def_org_field.fieldid=aicrm_field.fieldid where aicrm_field.tabid in (13,4,6) and aicrm_field.uitype <> 61 and block <> 75 and block <> 30 AND aicrm_profile2field.visible=0 AND aicrm_def_org_field.visible=0 AND aicrm_profile2field.profileid IN (". generateQuestionMarks($profileList) .") and aicrm_field.presence in (0,2) GROUP BY aicrm_field.fieldid order by aicrm_field.tablename";
...
if(count($querycolumns) > 0)
{
	...
	$query ="select ".$selectcolumns." from aicrm_troubletickets
			...
			left join aicrm_crmentity as crmentityRelHelpDesk on crmentityRelHelpDesk.crmid = aicrm_troubletickets.parent_id
			left join aicrm_account as accountRelHelpDesk on accountRelHelpDesk.accountid=crmentityRelHelpDesk.crmid
			left join aicrm_contactdetails as contactdetailsRelHelpDesk on contactdetailsRelHelpDesk.contactid= crmentityRelHelpDesk.crmid
			left join aicrm_products as productsRel on productsRel.productid = aicrm_troubletickets.product_id
			...
			left join aicrm_crmentity as crmentityAccounts on crmentityAccounts.crmid = aicrm_account.accountid 
			...
			left join aicrm_accountscf on aicrm_accountbillads.accountaddressid = aicrm_accountscf.accountid 
			left join aicrm_account as accountAccount on accountAccount.accountid = aicrm_troubletickets.parent_id
			left join aicrm_users as usersAccounts on usersAccounts.id = crmentityAccounts.smownerid
			LEFT JOIN aicrm_groups as groupsAccounts
				ON groupsAccounts.groupid = aicrm_crmentity.smownerid
			left join aicrm_contactdetails on aicrm_contactdetails.contactid = aicrm_troubletickets.parent_id               
			left join aicrm_crmentity as crmentityContacts on crmentityContacts.crmid = aicrm_contactdetails.contactid 
			...
			left join aicrm_customerdetails on aicrm_contactdetails.contactid = aicrm_customerdetails.customerid 
			left join aicrm_contactdetails as contactdetailsContacts on contactdetailsContacts.contactid = aicrm_contactdetails.reportsto
			left join aicrm_account as accountContacts on accountContacts.accountid = aicrm_contactdetails.accountid 
			left join aicrm_users as usersContacts on usersContacts.id = crmentityContacts.smownerid
			LEFT JOIN aicrm_groups as groupsContacts
				ON groupsContacts.groupid = aicrm_crmentity.smownerid
			where aicrm_crmentity.deleted=0 and ((crmentityContacts.deleted=0 || crmentityContacts.deleted is null)||(crmentityAccounts.deleted=0 || crmentityAccounts.deleted is null)) 
			and aicrm_troubletickets.ticketid in (". generateQuestionMarks($mass_merge) .")";
