...
function getMyTickets($maxval,$calCnt)
{
	...
	$search_query  = "SELECT aicrm_troubletickets.*, aicrm_crmentity.*
		...
		where aicrm_crmentity.smownerid = ? and aicrm_crmentity.deleted = 0 and aicrm_troubletickets.status <> 'Closed' ORDER BY createdtime DESC";
	...
	if($adb->num_rows($tktresult))
	{
		...
		$search_qry = "&query=true&Fields0=aicrm_troubletickets.status&Condition0=isn&Srch_value0=closed&Fields1=aicrm_crmentity.smownerid&Condition1=is&Srch_value1=".$current_user->column_fields['user_name']."&searchtype=advance&search_cnt=2&matchtype=all";
