...
foreach($storearray as $id)
{
	...
	if($id != '')
	{
		...
		elseif($dest_mod =='Leads' || $dest_mod =='Accounts' ||$dest_mod =='Contacts' ||$dest_mod =='Potentials' || $dest_mod=='Products'){
			$query = $adb->pquery("SELECT * from aicrm_seproductsrel WHERE crmid=? and productid=?",array($forCRMRecord,$id));
			
			if($adb->num_rows($query)==0){
				$adb->pquery("insert into aicrm_seproductsrel values (?,?,?)", array($id, $forCRMRecord, $dest_mod));
			}
		}
		else {
