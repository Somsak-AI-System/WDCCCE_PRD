...
if(isset($_REQUEST['pricebook_id']) && $_REQUEST['pricebook_id']!='')
{
	...
	foreach($storearray as $id)
	{
		if($id != '') {
			...
			//Updating the aicrm_pricebook product rel aicrm_table
			 $log->info("Products :: Inserting aicrm_products to price book");
			$query= "insert into aicrm_pricebookproductrel (pricebookid,productid,listprice,usedcurrency) values(?,?,?,?)";
