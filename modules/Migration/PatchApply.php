...
global $aicrm_current_version;
$res = $adb->pquery("select * from aicrm_version", array());
if($adb->num_rows($res))
{
	$res = $adb->pquery("update aicrm_version set old_version=?,current_version=?", array($versions[$source_version], $aicrm_current_version));
}
else
{
	$adb->pquery("insert into aicrm_version (id, old_version, current_version) values ('', ?, ?);", array($versions[$source_version], $aicrm_current_version));
}

//If currency name in config.inc.php file and currency name in aicrm_currency_info table is differ then we have to change in config.inc.php file
$mig_currency = $adb->query_result($adb->pquery("select currency_name from aicrm_currency_info", array()),0,'currency_name');
