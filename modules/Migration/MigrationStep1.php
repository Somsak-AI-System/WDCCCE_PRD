...
elseif(is_file($aicrm_home."/mysql/bin/mysqldump"))
{
	$installed_mysql_path = $aicrm_home."/mysql/bin/mysqldump";
	$getmysqlpath = 0;
}
elseif(substr($_ENV["OS"],0,3) == "Win")
{
	if(is_file($aicrm_home.'\mysql\bin\mysql.exe'))
	{
		$installed_mysql_path = $aicrm_home.'\mysql\bin\mysqldump.exe';
		@session_unregister('set_server_mysql_path');
		$_SESSION['set_server_mysql_path'] = $aicrm_home.'\mysql\bin';
		$getmysqlpath = 0;
	}
	else
	{
