...
// Get Lead Convert Status
$listquery1 =  getListQuery("Leads");
$listviewquery1 = substr($listquery1, strpos($listquery1,'FROM'),strlen($listquery1));

$pquery1 = "select aicrm_leaddetails.converted,aicrm_leaddetails.leadid ";
...
if($rowCount>0){
	for($i=0;$i<=$rowCount;$i++){
		$a_data=array();
		$leadid=$adb->query_result($cf_res, $i, "leadid");
		$converted = $adb->query_result($cf_res,$i, "converted");

		$a_response[$leadid]["lead_convert"] = $converted;
	}
}
