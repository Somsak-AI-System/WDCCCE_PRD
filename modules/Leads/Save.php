...
if(isset($_REQUEST['dup_check']) && $_REQUEST['dup_check'] != '')
{
	...
	$query = "SELECT firstname , lastname FROM aicrm_leaddetails ,aicrm_crmentity WHERE firstname =? and lastname =?  and aicrm_leaddetails.leadid = aicrm_crmentity.crmid and aicrm_crmentity.deleted != 1";
	$params = array($value);
...
if(isset($_REQUEST['return_module']) && $_REQUEST['return_module'] == "Campaigns")
{
	...
	{ 
		 $sql = "delete from aicrm_campaignleadrel where leadid = ?";
		 $adb->pquery($sql, array($focus->id));
		 $sql = "insert into aicrm_campaignleadrel values (?,?)";
...
if(isset($_REQUEST['return_module']) && $_REQUEST['return_module'] == "Smartemail")
{
	if(isset($_REQUEST['return_id']) && $_REQUEST['return_id'] != "")
	{
		 $sql = "insert into aicrm_smartemail_leadsrel values (?,?)";
		 $adb->pquery($sql, array($_REQUEST['return_id'], $focus->id));
	}
}
...
if(isset($_REQUEST['return_module']) && $_REQUEST['return_module'] == "EmailTargetList")
{
	if(isset($_REQUEST['return_id']) && $_REQUEST['return_id'] != "")
	{
		 $sql = "delete from aicrm_emailtargetlist_leadsrel    where leadid  = ?";
		 $adb->pquery($sql, array($focus->id));
		 $sql = "insert into aicrm_emailtargetlist_leadsrel    values (?,?)";
		 $adb->pquery($sql, array($_REQUEST['return_id'], $focus->id));
	}
}
//================================================================================================
global $old_user_id1,$new_user_id1;
if($_REQUEST['mode'] == 'edit')
	$reply = 'Re : ';
else
	$reply = '';
	
$query="select lead_no from aicrm_leaddetails where leadid=?";
$params = array($focus->id);
$data_tik = $adb->pquery($query, $params);
$lead_no=$adb->query_result($data_tik,0,'lead_no');
$_REQUEST['lead_no']=$lead_no;

$subject = $reply."[ Leads ]"." หมายเลข : ".$lead_no;
$data['sub']=$_REQUEST['lead_no'];
$data['mode']= $_REQUEST['mode'];
$email_body = getTicketDetails($focus->id,$data);
//get email========================================================================	
require_once('include/email_alert/email_alert.php');
$email_body =GetEmail("Leads",'header_alert.jpg',$focus->id,"leadid");
//get email========================================================================	
...
$user_email_create=getUserEmailId('id',$user_creatorid);
$user_emailid = getUserEmailId('id',$focus->column_fields['assigned_user_id']);
// if($user_emailid != ''){
// 	if($_REQUEST['mode'] != 'edit'){
// 		$mail_status = send_mail('Leads',$user_emailid,$HELPDESK_SUPPORT_NAME,$HELPDESK_SUPPORT_EMAIL_ID,$subject,$email_body);
// 		$mail_status = send_mail('Leads',$user_email_create,$HELPDESK_SUPPORT_NAME,$HELPDESK_SUPPORT_EMAIL_ID,$subject,$email_body);
// 	}else{
// 		if($old_user_id1!=$new_user_id1){
// 			$mail_status = send_mail('Leads',$user_emailid,$HELPDESK_SUPPORT_NAME,$HELPDESK_SUPPORT_EMAIL_ID,$subject,$email_body);
// 		}else{
// 			$mail_status = send_mail('Leads',$user_emailid,$HELPDESK_SUPPORT_NAME,$HELPDESK_SUPPORT_EMAIL_ID,$subject,$email_body);
// 		}
...
function save_customfields($entity_id)
{
	$log->debug("Entering save_customfields(".$entity_id.") method ...");
	 $log->debug("save custom aicrm_field invoked ".$entity_id);
	global $adb;
