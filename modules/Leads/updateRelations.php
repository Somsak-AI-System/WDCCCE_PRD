...
foreach($storearray as $id)
{
	if($id != '')
	{
		if($dest_mod == 'Products'){
			$adb->pquery("insert into aicrm_seproductsrel values (?,?,?)", array($forCRMRecord,$id,$currentModule));
		}elseif($dest_mod == 'Campaigns'){	
	    	$adb->pquery("insert into  aicrm_campaignleadrel values(?,?)", array($id,$forCRMRecord));
	    }elseif($dest_mod == 'Documents'){
	    	$adb->pquery("insert into aicrm_senotesrel values (?,?)", array($forCRMRecord,$id));
	    }elseif($dest_mod == 'Smartquestionnaire'){
            $adb->pquery("insert into  aicrm_smartquestionnaire_leadsrel values(?,?)", array($id,$forCRMRecord));
	    }else {
