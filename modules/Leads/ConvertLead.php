...
$sql = "SELECT firstname, lastname, company, idcardno, smownerid from aicrm_leaddetails inner join aicrm_crmentity on aicrm_crmentity.crmid=aicrm_leaddetails.leadid where aicrm_leaddetails.leadid =?";
...
if($sortid != '') {
	$sales_stage_query="select sales_stage from aicrm_sales_stage";
	$params = array();
}
elseif($is_admin)
{
	$sales_stage_query="select sales_stage from aicrm_sales_stage inner join aicrm_role2picklist on aicrm_role2picklist.picklistvalueid = aicrm_sales_stage.picklist_valueid where roleid != 'H1' order by sortid";
	$params = array();
}
else
{
	if (count($roleids) > 0) {
		$sales_stage_query="select sales_stage from aicrm_sales_stage inner join aicrm_role2picklist on aicrm_role2picklist.picklistvalueid = aicrm_sales_stage.picklist_valueid where roleid in (". generateQuestionMarks($roleids) .") and picklistid in (select picklistid from aicrm_sales_stage) order by sortid";
		$params = array($roleids);
	} else {
		$sales_stage_query="select sales_stage from aicrm_sales_stage inner join aicrm_role2picklist on aicrm_role2picklist.picklistvalueid = aicrm_sales_stage.picklist_valueid where picklistid in (select picklistid from aicrm_sales_stage) order by sortid";
		$params = array();
	}
}
...
function isActive($field,$mod){
	global $adb,$theme;
	$tabid = getTabid($mod);
	$query = 'select * from aicrm_field where fieldname = ?  and tabid = ? and presence in (0,2)';
...
$convertlead = '<form name="ConvertLead" method="POST" action="index.php" onsubmit="return request_data(ConvertLead); VtigerJS_DialogBox.block();">
	...
	<table width="100%" border="0" cellpadding="5" cellspacing="0" class="layerHeadingULine">
		<tr>
			<td class="genHeaderSmall" align="left"><img src="'. aicrm_imageurl('repeat.svg', $theme).'" style="width:25px; height:22px;"> '.$mod_strings['LBL_CONVERT_LEAD'].' '.$firstname.' '.$lastname.'</td>
			<td align="right"><a href="javascript:fninvsh(\'orgLay\');"><img src="'. aicrm_imageurl('close.gif', $theme).'" align="absmiddle" border="0"></a></td>
		</tr>
		</table>
	...
	<tr>
		<td class=small >
			<table border=0 celspacing=0 cellpadding=5 width=100% align=center bgcolor=white>
                ...
                <tr>
						<td align="right" class="dvtCellLabel" width="50%">'.$mod_strings['LBL_ASSIGNED_TO'].'</td>
						...
						<span id="assign_user" style="display:'.$userdisplay.'">
                       		<select name="assigned_user_id" class="small user-success">'.get_select_options_with_id(get_user_array(false,"Active", $userid,$private),$userid).'</select>
						</span>
						<span id="assign_team" style="display:'.$groupdisplay.'">
                       		<select name="assigned_group_id" class="small user-success">'.get_select_options_with_id(get_group_array(false,"Active", $userid,$private),$userid).'</select>
						</span>
						</td>
				</tr>';
			...
			if(vtlib_isModuleActive('Accounts') && (isPermitted('Accounts','EditView')== 'yes')){
				$convertlead .= '<tr>
					<td align="right" class="dvtCellLabel"><font color="red">* </font>'.$mod_strings['LBL_ACCOUNT_NAME'].'</td>
					<td class="dvtCellInfo"><input type="text" name="account_name" class="detailedViewTextBox" value="'.$company.'" style="width:97%"></td>
				</tr>';

				$convertlead .= '<tr>
					<td align="right" class="dvtCellLabel">เลขประจำตัวผู้เสียภาษี</td>
					<td class="dvtCellInfo"><input type="text" name="idcardno" class="detailedViewTextBox" value="'.$idcardno.'" style="width:97%"></td>
				</tr>';
			}
...
foreach($fields_list as $mod=>$fields){
	foreach($fields as $key=>$field) {
		if(getFieldVisibilityPermission($mod, $current_user->id, $field)=='0' && (isActive($field,$mod) == true)){
			...
			global $adb;
			$query = 'select typeofdata from aicrm_field where fieldname = ? and tabid = ?';
...
//Do not create a New Contact upon Conversion

//Contact
//onClick="fnSlide2(\'ch\',\'cc\')"
$convertlead .='
<tr>
	<td align="right" class="dvtCellLabel">'.$mod_strings['LBL_DO_NOT_CREATE_NEW_CONTACT'].'</td>
	<td class="dvtCellInfo"><input type="checkbox" name="createcontact" ></td>
</tr>
<tr>
	<td align="right" class="dvtCellLabel">'.$mod_strings['LBL_CONTACT_NAME'].'</td>
	<td class="dvtCellInfo"><input type="text" name="contact_name" class="detailedViewTextBox" value="'.$firstname." ".$lastname.'" readonly="readonly" style="width:97%"></td>
</tr>';

//Deal
/*<tr>
	<td align="right" class="dvtCellLabel">'.$mod_strings['LBL_DEAL_NAME'].'</td>
	<td class="dvtCellInfo"><input type="text" name="deal_name" class="detailedViewTextBox" value="'.$deal_name.'" readonly="readonly" style="width:97%"></td>
</tr>*/
...
//return request_data(ConvertLead);
//return verify_data(ConvertLead)
$convertlead .='</table>
			</td>
		</tr>
	</table>
	...
	<tr>
			<td align="center">
				...
				<button title='.$app_strings['LBL_SAVE_BUTTON_LABEL'].' class="crmbutton small save" onclick="this.form.action.value=\'LeadConvertToEntities\'; " type="submit" name="button" style="width:70px">
            		<img src="themes/softed/images/save_button_w.png" border="0" style="width: 15px; height: 17px; vertical-align: middle;"> '.$app_strings['LBL_SAVE_BUTTON_LABEL'].'
            	
            	</button>

            	&nbsp;
            	
            	<button title='.$app_strings['LBL_CANCEL_BUTTON_LABEL'].' accesskey="X" class="crmbutton small cancel" onClick="hide(\'orgLay\')" type="button" name="button" value="Cancel">
            		<img src="themes/softed/images/cancel_o.png" border="0" style="width: 17px;height: 17px; vertical-align: middle;"> '.$app_strings['LBL_CANCEL_BUTTON_LABEL'].'
            	</button>
