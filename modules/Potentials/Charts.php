 ...
 ********************************************************************************/
/*********************************************************************************
 * $Header: /advent/projects/wesat/aicrm_crm/sugarcrm/modules/Potentials/Charts.php,v 1.12 2005/04/20 20:23:34 ray Exp $
...
class jpgraph {
	/**
	 * Creates opportunity pipeline image as a horizontal accumlated bar graph for multiple aicrm_users.
	...
	function outcome_by_month($date_start='1971-10-15', $date_end='2071-10-15', $user_id=array('1'), $cache_file_name='a_file', $refresh=false,$width=900,$height=500){
		...
		if (!file_exists($cache_file_name) || !file_exists($cache_file_name.'.map') || $refresh == true) {
			...
			$opp = new Potentials();
			$opp_list = $opp->get_full_list("aicrm_potential.amount DESC, aicrm_potential.closingdate DESC", $where);
	...
	/**
	 * Creates lead_source_by_outcome pipeline image as a horizontal accumlated bar graph for multiple aicrm_users.
	 ...
	 * param $ids - list of assigned aicrm_users of opps to find
		...
		if (!file_exists($cache_file_name) || !file_exists($cache_file_name.'.map') || $refresh == true) {
			...
			foreach ($legends as $lead_source_key=>$lead_source_translation) {
				if (isset($sum[$lead_source_key]))
				{
					...
					{
						// put none in if the aicrm_field is blank.
