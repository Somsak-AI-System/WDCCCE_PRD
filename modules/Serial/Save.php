...
{
    //started
    $value = $_REQUEST['serial_name'];
    $query = "SELECT serial_name FROM aicrm_serial,aicrm_crmentity WHERE serial_name =? and aicrm_serial.serialid = aicrm_crmentity.crmid and aicrm_crmentity.deleted != 1";
    $params = array($value);
    $id = $_REQUEST['record'];
    if(isset($id) && $id !='') {
        $query .= " and aicrm_serial.serialid != ?";
    ...
    {
        echo $mod_strings['LBL_SERIAL_EXIST'];
...
if(isset($_REQUEST['return_module']) && $_REQUEST['return_module'] == "Accounts")
{
	if(isset($_REQUEST['return_id']) && $_REQUEST['return_id'] != "")
	{
		 ...
		 $adb->pquery($sql, array($_REQUEST['return_id'],$_REQUEST['return_module'],$focus->id,"Serial"));
	}
}
