...
$_SESSION['SERIAL_ORDER_BY'] = $order_by;
$_SESSION['SERIAL_SORT_ORDER'] = $sorder;
//<<<<<<<<<<<<<<<<<<< sorting - stored in session >>>>>>>>>>>>>>>>>>>>
...
if(isPermitted('Serial','Delete','') == 'yes')
{
	$other_text['del'] = $app_strings[LBL_MASS_DELETE];
}
if(isPermitted('Serial','EditView','') == 'yes')
{
...
if($viewid != "0")
{
	$listquery = getListQuery("Serial");
	$query = $oCustomView->getModifiedCvListQuery($viewid,$listquery,"Serial");
}else
{
	$query = getListQuery("Serial");
}
...
if(isset($order_by) && $order_by != '')
{
	if($order_by == 'smownerid')
        ...
        {
		$tablename = getTableNameForField('Serial',$order_by);
...
$listview_header = getListViewHeader($focus,"Serial",$url_string,$sorder,$order_by,"",$oCustomView);
$smarty->assign("LISTHEADER", $listview_header);

$listview_header_search = getSearchListHeaderValues($focus,"Serial",$url_string,$sorder,$order_by,"",$oCustomView);
$smarty->assign("SEARCHLISTHEADER",$listview_header_search);

$listview_entries = getListViewEntries($focus,"Serial",$list_result,$navigation_array,"","","EditView","Delete",$oCustomView);
...
$navigationOutput = getTableHeaderSimpleNavigation($navigation_array, $url_string,"Serial","index",$viewid);
