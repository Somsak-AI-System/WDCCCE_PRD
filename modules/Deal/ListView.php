...
$_SESSION['DEAL_ORDER_BY'] = $order_by;
$_SESSION['DEAL_SORT_ORDER'] = $sorder;
//<<<<<<<<<<<<<<<<<<< sorting - stored in session >>>>>>>>>>>>>>>>>>>>
...
if(isPermitted('Deal','Delete','') == 'yes')
{
	$other_text['del'] = $app_strings[LBL_MASS_DELETE];
}
if(isPermitted('Deal','EditView','') == 'yes')
{
...
if($viewid != "0")
{
	$listquery = getListQuery("Deal");
	$query = $oCustomView->getModifiedCvListQuery($viewid,$listquery,"Deal");
}else
{
	$query = getListQuery("Deal");
}
//<<<<<<<<customview>>>>>>>>>
...
if(isset($order_by) && $order_by != '')
{
	if($order_by == 'smownerid'){
		...
		$tablename = getTableNameForField('Deal',$order_by);
...
$listview_header = getListViewHeader($focus,"Deal",$url_string,$sorder,$order_by,"",$oCustomView);
$smarty->assign("LISTHEADER", $listview_header);

$listview_header_search = getSearchListHeaderValues($focus,"Deal",$url_string,$sorder,$order_by,"",$oCustomView);
$smarty->assign("SEARCHLISTHEADER",$listview_header_search);

$listview_entries = getListViewEntries($focus,"Deal",$list_result,$navigation_array,"","","EditView","Delete",$oCustomView);
...
$navigationOutput = getTableHeaderSimpleNavigation($navigation_array, $url_string,"Deal","index",$viewid);
