...
$log->debug("Inside Projects EditView");
...
if (isset ($_REQUEST['record']) && $_REQUEST['record'] != '') {
	if (isset ($_REQUEST['convertmode']) && $_REQUEST['convertmode'] == 'quotetopro') {
		...
		$associated_prod = getAssociatedProjects("Projects", $quote_focus);
	...
	elseif (isset ($_REQUEST['convertmode']) && $_REQUEST['convertmode'] == 'update_quote_val') {
		...
		$focus->id = $_REQUEST['record'];
		$focus->mode = 'edit';
		$focus->retrieve_entity_info($_REQUEST['record'], "Projects");
		$focus->name = $focus->column_fields['projects_name'];
	}
} else {
	if (isset ($_REQUEST['convertmode']) && $_REQUEST['convertmode'] == 'update_quote_val') {
		...
		if (isset ($_REQUEST['quote_id']) && $_REQUEST['quote_id'] != '') {
			$associated_prod = getAssociatedProjects("Projects", $quote_focus, $focus->column_fields['quote_id']);
		}
...
if(isset($_REQUEST['isDuplicate']) && $_REQUEST['isDuplicate'] == 'true') {
	...
	$QUOTE_associated_prod = getAssociatedProjects("Projects",$focus);
	$log->debug("Mode is Duplicate. Projectsid to be duplicated is ".$focus->id);
	$focus->id = "";
    $focus->mode = ''; 	
}
if(empty($_REQUEST['record']) && $focus->mode != 'edit'){
	$focus->column_fields['project_s_date'] = date('Y-m-d');
	$focus->column_fields['project_estimate_e_date'] = date('Y-m-d');
...
if(isset($_REQUEST['potential_id']) && $_REQUEST['potential_id'] !=''){
	...
	$associated_prod = getAssociatedProjects("Potentials",$focus,$focus->column_fields['potential_id']);
}

if(isset($_REQUEST['product_id']) && $_REQUEST['product_id'] !=''){
    $focus->column_fields['product_id'] = $_REQUEST['product_id'];
    $log->debug("Product Id from the request is ".$_REQUEST['product_id']);
    $associated_prod = getAssociatedProjects("Products",$focus,$focus->column_fields['product_id']);
...
if(!empty($_REQUEST['parent_id']) && !empty($_REQUEST['return_module'])){
    if ($_REQUEST['return_module'] == 'Services') {
	    ...
	    $associated_prod = getAssociatedProjects("Services",$focus,$focus->column_fields['product_id']);
		for ($i=1; $i<=count($associated_prod);$i++) {
			$associated_prod_id = $associated_prod[$i]['hdnProductId'.$i];
...
if(isset($_REQUEST['account_id']) && $_REQUEST['account_id']!='' && $_REQUEST['record']=='')
{
	...
	$focus->column_fields['addressline1']=$acct_focus->column_fields['addressline1'];
	$focus->column_fields['addressline2']=$acct_focus->column_fields['addressline2'];
	$focus->column_fields['accountvillage']=$acct_focus->column_fields['accountvillage'];
	$focus->column_fields['accountalley']=$acct_focus->column_fields['accountalley'];
	$focus->column_fields['accountroad']=$acct_focus->column_fields['accountroad'];
    $focus->column_fields['region']=$acct_focus->column_fields['region'];
...
if($focus->mode == 'edit')
{
	...
	$associated_prod = getAssociatedProjects("Projects",$focus);
	$smarty->assign("ASSOCIATEDPRODUCTS", $associated_prod);
	$smarty->assign("MODE", $focus->mode);

	$associated_comp = getAssociatedCompetitorproduct("Projects",$focus);
	$smarty->assign("ASSOCIATEDCOMPETITOR", $associated_comp);
...
else $smarty->assign("RETURN_MODULE","Projects");
...
if($focus->mode == 'edit')
	$smarty->display("Projects/InventoryEditView.tpl");
else
	$smarty->display('Projects/InventoryCreateView.tpl');
