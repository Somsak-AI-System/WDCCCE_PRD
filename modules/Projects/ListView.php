...
$focus->initSortbyField('Projects');
// END
...
$_SESSION['PROJECTS_ORDER_BY'] = $order_by;
$_SESSION['PROJECTS_SORT_ORDER'] = $sorder;
//<<<<<<<<<<<<<<<<<<< sorting - stored in session >>>>>>>>>>>>>>>>>>>>
...
if(isPermitted('Projects','Delete','') == 'yes')
{
	$other_text['del'] = $app_strings[LBL_MASS_DELETE];
}
if(isPermitted('Projects','EditView','') == 'yes')
{
...
if($viewid != "0")
{
	$listquery = getListQuery("Projects");
	$query = $oCustomView->getModifiedCvListQuery($viewid,$listquery,"Projects");
}else
{
	$query = getListQuery("Projects");
}
//<<<<<<<<customview>>>>>>>>>
...
if(isset($order_by) && $order_by != '')
{
	if($order_by == 'smownerid'){
		...
		$tablename = getTableNameForField('Projects',$order_by);
...
$listview_header = getListViewHeader($focus,"Projects",$url_string,$sorder,$order_by,"",$oCustomView);
$smarty->assign("LISTHEADER", $listview_header);

$listview_header_search = getSearchListHeaderValues($focus,"Projects",$url_string,$sorder,$order_by,"",$oCustomView);
$smarty->assign("SEARCHLISTHEADER",$listview_header_search);

$listview_entries = getListViewEntries($focus,"Projects",$list_result,$navigation_array,"","","EditView","Delete",$oCustomView);
...
$navigationOutput = getTableHeaderSimpleNavigation($navigation_array, $url_string,"Projects","index",$viewid);
...
if($rowCount>0){
	for($i=0;$i<=$rowCount;$i++){
		$a_data=array();
		$projectsid=$adb->query_result($list_result, $i, "crmid");
		$display= isPermitted('Projects',"EditView",$projectsid);
		$a_response[$projectsid]["display"] = $display;
	}
}
