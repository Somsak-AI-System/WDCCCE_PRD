...
//$data_template = $_REQUEST['data_template'];
//parse_str($_REQUEST['data_template'], $data_template);
// $template = $_REQUEST['data_template'];
$data_template = $_REQUEST;
// echo "<pre>"; print_r($template); echo "</pre>"; exit;
// parse_str($template, $data_template);
...
$focus->save("Inspectiontemplate");
$return_id = $focus->id;
//sleep(5);
if($_REQUEST['mode'] != 'edit'){
    require_once("library/Inspectiontemplate.php");
    include("config.inc.php");
    // echo "<pre>"; print_r($data_template); echo "</pre>"; exit;
    $inspectiontemplate = new libinspectiontemplate($dbconfig);
    $inspectiontemplate->save_template($data_template,$focus->id,$_REQUEST['mode']);
}


$parenttab = getParentTab();
if(isset($_REQUEST['return_module']) && $_REQUEST['return_module'] != "") $return_module = vtlib_purify($_REQUEST['return_module']);
else $return_module = "Inspectiontemplate";
...
if(isset($_REQUEST['return_module']) && $_REQUEST['return_module'] == "Campaigns")
{
    ...
    {
        $sql = "delete from aicrm_campaignmaillistrel where inspectiontemplateid = ?";
