...
if($_REQUEST["list_type"] == "EmailTarget"){//echo "55";exit;
		$reltable = "aicrm_inspectiontemplate_emailtargetrel";
		$relid = "emailtargetid";

}
elseif($_REQUEST["list_type"] == "Contacts"){
		$reltable = "aicrm_inspectiontemplaterel";
		$relid = "contactid";
		$type = "Contacts";
}
...
elseif($_REQUEST["list_type"] == "Opportunity"){
		$reltable = "aicrm_inspectiontemplate_opportunityrel";
		$relid = "opportunityid";
}
//echo "555";exit;
//print_r($rs);exit;
while($row=$adb->fetch_array($rs)) {
	...
	$sql = "delete from $reltable where $relid = ? and inspectiontemplateid = ?";
...
header("Location: index.php?module=Inspectiontemplate&action=InspectiontemplateAjax&file=CallRelatedList&ajax=true&record=".vtlib_purify($_REQUEST['return_id']));
/*echo "<script >window.location.href='index.php';</script>";*/
//header("Location: index.php?module=Inspectiontemplate&action=InspectiontemplateAjax&file=CallRelatedList&ajax=true&record=993788&parenttab=Marketing");
?>