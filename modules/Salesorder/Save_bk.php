...
//update=====================================
	$sql=" select parent_id from aicrm_salesorder where salesorderid='".$focus->id."'";
	...
	if($rows>0){
		for($i=0;$i<$rows;$i++){
			...
			left JOIN aicrm_crmentity as aicrm_crmentityJOB1  ON aicrm_crmentityJOB1.crmid = aicrm_salesorder.salesorderid
			...
			and aicrm_salesorder.parent_id='".$ticketid."'
	...
	where 1
	and aicrm_crmentityJOB1.deleted =0
	and aicrm_salesorder.salesorderid='".$focus->id."'
	order by cf_1211 ASC  limit 1
	";
	//echo $sql."<br>";
	$data_start = $adb->pquery($sql, array());
	...
	$sql="update aicrm_salesordercf set cf_1258='".$OT1."', cf_1259='".$OT2."', cf_1260='".$OT3."', cf_1261='".$OT4."' where salesorderid='".$focus->id."' ";
	$adb->pquery($sql, array());
