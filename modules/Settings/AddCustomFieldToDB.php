...
//checking if the user is trying to create a custom aicrm_field which already exists  
...
if($adb->num_rows($checkresult) > 0) {
	...
	if($fldmodule == 'HelpDesk') {
		$tableName='aicrm_ticketcf';
	} elseif($fldmodule == 'Products') {
		$tableName='aicrm_productcf';
	} elseif($fldmodule == 'Vendors') {
		$tableName='aicrm_vendorcf';
	} elseif($fldmodule == 'PriceBooks') {
		$tableName='aicrm_pricebookcf';
	} elseif($fldmodule == 'Calendar') {
		$tableName='aicrm_activitycf';
	}
//ekk add======================================================================================
	elseif($fldmodule == 'ServiceRequest') {
   	 $tableName='aicrm_servicerequestscf';
 	 }
	elseif($fldmodule == 'InternalTraining') {
   	 $tableName='aicrm_internaltrainingscf';
 	 }	
	elseif($fldmodule == 'Questionnaire') {
   	 	$tableName='aicrm_questionnairescf';
 	 }		
	 elseif($fldmodule == 'Member') {
   	 	$tableName='aicrm_memberscf';
 	 }
...
//ekk add======================================================================================	
	...
	// No Decimal Pleaces Handling
   	...
   	//1. add the customfield aicrm_table to the aicrm_field aicrm_table as Block4
	//2. fetch the contents of the custom aicrm_field and show in the UI
  
	$custfld_sequece=$adb->getUniqueID("aicrm_customfield_sequence");
	...
	$blockid ='';
		...
		{
			$query = "update aicrm_field set fieldlabel=?, typeofdata=? where fieldid=?";
                        $adb->pquery($query, array($fldlabel, $uichekdata, $_REQUEST['fieldid']));
		}
		else if($_REQUEST['fieldid'] == '')
		{
			$query = "insert into aicrm_field (tabid,fieldid,columnname,tablename,generatedtype,uitype,fieldname,fieldlabel,
