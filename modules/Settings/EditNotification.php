...
//Added to show the previous selected value when editing
	...
	$query='select aicrm_emailtemplates.templateid,aicrm_emailtemplates.templatename from aicrm_notificationscheduler inner join aicrm_emailtemplates on aicrm_emailtemplates.templateid=aicrm_notificationscheduler.notificationbody where schedulednotificationid=?';
	$result = $adb->pquery($query, array($id));
	$selected_item=$adb->query_result($result,0,'templateid');

	$sql="select * from aicrm_notificationscheduler where schedulednotificationid = ?";
...
//Get all the email template name and id and send it to tpl to generate the combo box
    $query="select templateid,templatename from aicrm_emailtemplates";
