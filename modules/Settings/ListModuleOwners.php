...
if($_REQUEST['list_module_mode'] != '' && $_REQUEST['list_module_mode'] == 'save')
{
	foreach($module_array as $val)
	{
		...
		if($tabid != '' && $userid != '')
		{
			$sql = 'update aicrm_moduleowners set user_id = ? where tabid = ?';
...
if($_REQUEST['file_mode'] != 'ajax')
	 ...
	 *  @ param $tabid
	 *  It gets the module owner for the given tabid from aicrm_moduleowners table
	 *  returns the userid of the module owner for the given tabid
	 */
function getModuleOwner($tabid)
{
	global $adb;
	$sql = "select * from aicrm_moduleowners where tabid=?";
...
function getModuleNameList1()
{
	...
	//$sql = "select aicrm_moduleowners.*, aicrm_tab.name from aicrm_moduleowners inner join aicrm_tab on aicrm_moduleowners.tabid = aicrm_tab.tabid order by aicrm_tab.tabsequence";
	$sql = "select aicrm_moduleowners.*, aicrm_tab.name from aicrm_moduleowners inner join aicrm_tab on aicrm_moduleowners.tabid = aicrm_tab.tabid WHERE aicrm_tab.presence != 1 order by aicrm_tab.tabsequence";
	// END
