...
global $log,$path_img_template;
//echo $path_img_template;exit;
...
$body = str_replace('/UserFiles/Image',$path_img_template,fck_from_html($_REQUEST["body"]));

if(isset($templateid) && $templateid !='')
{
	$log->info("the templateid is set");  
	$sql = "update aicrm_emailtemplates set foldername =?, templatename =?, subject =?, description =?, body =? where templateid =?";
...
{
	$templateid = $db->getUniqueID('aicrm_emailtemplates');
	$sql = "insert into aicrm_emailtemplates values (?,?,?,?,?,?,?)";
