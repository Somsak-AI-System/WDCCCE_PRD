...
$qry="select roleid,picklistvalueid from aicrm_role2picklist left join aicrm_$tableName on aicrm_$tableName.picklist_valueid=aicrm_role2picklist.picklistvalueid where roleid=? and picklistid=? and presence=1";
$res = $adb->pquery($qry, array($roleid, $picklistid));
$num_row = $adb->num_rows($res);
for($s=0;$s < $num_row; $s++)
{
	$valid = $adb->query_result($res,$s,'picklistvalueid');
	$sql="delete from aicrm_role2picklist where roleid=? and picklistvalueid=?";
...
$columnName = $tableName;
 for($i = 0; $i < $count; $i++)
 {
	 ...
	 if($pickArray[$i] != '')
	 {
	 	...
	 	$picklistcount =0;
		$sql = "select picklist_valueid from aicrm_$tableName where $tableName=?";
		$pick_valueid = $adb->query_result($adb->pquery($sql, array($pickArray[$i])),0,'picklist_valueid');
		 ...
		 $sql ="select max(sortid)+1 as sortid from aicrm_role2picklist left join aicrm_$tableName on aicrm_$tableName.picklist_valueid=aicrm_role2picklist.picklistvalueid where roleid=? and picklistid=?  and presence=0";
