<?
	...
	if($_REQUEST["action"]=="edit" and $_REQUEST["id"]!="" and $_REQUEST["myaction"]!="save"){
		$id=$_REQUEST["id"];
		$sql="select * from aicrm_weekly_plan where id='".$id."'";
		//echo $sql;
	...
	if($_REQUEST["myaction"]=="save"){
		...
		if($_REQUEST["date_send"]==""){
			...
			echo "<script>alert('กรุณาเลือก Send To');</script>";	
					...
					$sql="update aicrm_weekly_plan set 
...
<title>Weekly Plan Config</title>
...
<form name="warranty" method="post" enctype="multipart/form-data" action="weekly_config.php">
<input type="hidden" name="myaction" value="" />
<input type="hidden" name="action" value="<?=$_REQUEST["action"]?>" />
<input type="hidden" name="id" value="<?=$_REQUEST["id"]?>" />
<input type="hidden" name="action_type" value="weeklyplantemplates" />
<table width="100%" border="0" class="crmTable small" cellpadding="5" cellspacing="0">
        <tr>
          <td colspan="2" align="left" bgcolor="#999999" class="dvInnerHeader"><strong>Weekly Plan Config</strong><font color="#000000"><strong></strong></font></td>
        </tr>
        <tr>
