<?
	...
	if($_REQUEST["action"]=="edit" and $_REQUEST["cf_1598id"]!="" and $_REQUEST["myaction"]!="save"){
		$cf_1598id=$_REQUEST["cf_1598id"];
		$sql="select * from aicrm_cf_1598 where cf_1598id='".$cf_1598id."'";
		//echo $sql;
		$data = $generate->process($sql,"all");	
		if(count($data)>0){
			$_REQUEST["cf_1598"]=$data[0]['cf_1598'];
	...
	if($_REQUEST["myaction"]=="save"){
		if($_REQUEST["action"]=="edit"){
			$sql="update aicrm_cf_1598 set 
			chk_st='".$_REQUEST["chk_st"]."'
			,chk_en='".$_REQUEST["chk_en"]."'
			,chk_count_from='".$_REQUEST["chk_count_from"]."'
			,chk_count_to='".$_REQUEST["chk_count_to"]."'
			where cf_1598id='".$_REQUEST["cf_1598id"]."'
			";
			//echo $sql;exit;
			$generate->query($sql);
			echo "<script type='text/javascript'> alert('Edit Complete');window.close();window.opener.parent.location.replace('../../index.php?module=Settings&action=list_acc_grade_Y&parenttab=Settings');</script>";	
		}
	}
?>
...
<form name="warranty" method="post" enctype="multipart/form-data" action="edit_acc_grade_Y.php">
<input type="hidden" name="myaction" value="" />
<input type="hidden" name="action" value="<?=$_REQUEST["action"]?>" />
<input type="hidden" name="cf_1598id" value="<?=$_REQUEST["cf_1598id"]?>" />
<table width="100%" border="0" class="crmTable small" cellpadding="5" cellspacing="0">
        <tr>
          <td colspan="2" align="left" bgcolor="#999999" class="dvInnerHeader"><strong>Account Grade Per Year</strong><font color="#000000"><strong></strong></font></td>
        </tr>
        <tr>
          <td width="40%" align="left" class="dvtCellLabel">Account Grade</td>
          <td width="60%" align="left" class="dvtCellInfo"><input name="cf_1598" type="text" class="detailedViewTextBox" id ="cf_1598" tabindex="" onfocus="this.className='detailedViewTextBoxOn'" onblur="this.className='detailedViewTextBox'" value="<?=$_REQUEST["cf_1598"]?>" readonly="readonly" style="background-color:#CCC"   /></td>
        </tr>
        <tr>
