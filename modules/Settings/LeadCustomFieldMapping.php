...
function getAccountCustomValues($leadid,$accountid)
{
	global $adb;
	$accountcf=Array();
	$sql="select fieldid,fieldlabel,uitype,typeofdata from aicrm_field,aicrm_tab where aicrm_field.tabid=aicrm_tab.tabid and generatedtype=2 and aicrm_tab.name='Accounts' and aicrm_field.presence in (0,2)";
	//echo $sql; exit;
...
{	
	global $adb;	
	$contactcf=Array();
	$sql="select fieldid,fieldlabel,uitype,typeofdata from aicrm_field,aicrm_tab where aicrm_field.tabid=aicrm_tab.tabid and generatedtype=2 and aicrm_tab.name='Contacts'and aicrm_field.presence in (0,2)";
	$result = $adb->pquery($sql, array());
	$noofrows = $adb->num_rows($result);
	for($i=0; $i<$noofrows; $i++)
	{
...
function getPotentialCustomValues($leadid,$potentialid)
{
	global $adb;	
	$potentialcf=Array();
	$sql="select fieldid,fieldlabel,uitype,typeofdata from aicrm_field,aicrm_tab where aicrm_field.tabid=aicrm_tab.tabid and generatedtype=2 and aicrm_tab.name='Potentials' and aicrm_field.presence in (0,2)";
	$result = $adb->pquery($sql, array());
	$noofrows = $adb->num_rows($result);
	for($i=0; $i<$noofrows; $i++)
	{
...
function getDealCustomValues($leadid,$dealid)
{
	global $adb;	
	$potentialcf=Array();
	$sql="select fieldid,fieldlabel,uitype,typeofdata from aicrm_field,aicrm_tab where aicrm_field.tabid=aicrm_tab.tabid and generatedtype=2 and aicrm_tab.name='Deal' and aicrm_field.presence in (0,2)";
	//echo $sql; exit;
	$result = $adb->pquery($sql, array());
	$noofrows = $adb->num_rows($result);
	for($i=0; $i<$noofrows; $i++)
	{
		$potential_field['fieldid']=$adb->query_result($result,$i,"fieldid");
		$potential_field['fieldlabel']=getTranslatedString($adb->query_result($result,$i,"fieldlabel"),'Deal');
		$potential_field['typeofdata']=$adb->query_result($result,$i,"typeofdata");
		$potential_field['fieldtype']=getCustomFieldTypeName($adb->query_result($result,$i,"uitype"));

		if($potential_field['fieldid']==$dealid)
			 $potential_field['selected']="selected";
		else
             $potential_field['selected'] = "";
		$potential_cfelement[]=$potential_field;
	}
	$potentialcf[$leadid.'_deal']=$potential_cfelement;
    return $potentialcf;
}
/**
...
function customFieldMappings()
{
	...
	$convert_sql="select aicrm_convertleadmapping.*,uitype,fieldlabel,typeofdata from aicrm_convertleadmapping inner join aicrm_field on aicrm_field.fieldid = aicrm_convertleadmapping.leadfid and aicrm_field.presence in (0,2)";
	...
	for($j=0; $j<$no_rows; $j++)
	{
		...
		//$dealid=$adb->query_result($convert_result,$j,"dealfid");
		...
		//$lead_field['deal'] = getDealCustomValues($leadid,$dealid);
