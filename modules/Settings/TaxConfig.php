...
function updateTaxPercentages($new_percentages, $sh='')
{
	...
	foreach($new_percentages as $taxid => $new_val)
	{
		...
		{
			if($sh != '' && $sh == 'sh')
				$query = "update aicrm_shippingtaxinfo set percentage=? where taxid=?";
			else
				$query = "update aicrm_inventorytaxinfo set percentage =? where taxid=?";
...
function updateTaxLabels($new_labels, $sh='')
{
	...
	foreach($new_labels as $taxid => $new_val)
	{
		...
		{
			if($sh != '' && $sh == 'sh')
				$query = "update aicrm_shippingtaxinfo set taxlabel= ? where taxid=?";
			else
				$query = "update aicrm_inventorytaxinfo set taxlabel = ? where taxid=?";
			$adb->pquery($query, array($new_val, $taxid));
		}
	}
...
function addTaxType($taxlabel, $taxvalue, $sh='')
{
	...
	if($sh != '' && $sh == 'sh')
		$check_query = "select taxlabel from aicrm_shippingtaxinfo where taxlabel=?";
	else
		$check_query = "select taxlabel from aicrm_inventorytaxinfo where taxlabel=?";
	...
	if($sh != '' && $sh == 'sh')
	{
		$taxid = $adb->getUniqueID("aicrm_shippingtaxinfo");
		$taxname = "shtax".$taxid;
		$query = "alter table aicrm_inventoryshippingrel add column $taxname decimal(7,3) default NULL";
	}
	else
	{
		$taxid = $adb->getUniqueID("aicrm_inventorytaxinfo");
		$taxname = "tax".$taxid;
		$query = "alter table aicrm_inventoryproductrel add column $taxname decimal(7,3) default NULL";
	}
	...
	{
		if($sh != '' && $sh == 'sh') 
			$query1 = "insert into aicrm_shippingtaxinfo values(?,?,?,?,?)";
		else
			$query1 = "insert into aicrm_inventorytaxinfo values(?,?,?,?,?)";
 ...
 *	@param string $sh - sh or empty, if sh passed then the enable/disable will be done in shipping and handling tax table ie.,aicrm_shippingtaxinfo  else this enable/disable will be done in Product tax table ie., in aicrm_inventorytaxinfo
 *	@return void
 */
function changeDeleted($taxname, $deleted, $sh='')
{
	...
	if($sh == 'sh')
		$adb->pquery("update aicrm_shippingtaxinfo set deleted=? where taxname=?", array($deleted, $taxname));
	else
		$adb->pquery("update aicrm_inventorytaxinfo set deleted=? where taxname=?", array($deleted, $taxname));
