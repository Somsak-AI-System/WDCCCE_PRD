...
function getFieldListEntries($module)
{
	...
	$dbQuery = "select aicrm_blocks.*,aicrm_tab.presence as tabpresence  from aicrm_blocks" .
				" inner join aicrm_tab on aicrm_tab.tabid = aicrm_blocks.tabid" .
				" where aicrm_blocks.tabid=?  and aicrm_tab.presence = 0 order by sequence";
	...
	if($row!='')
	{
		do
		{
			...
			{
			  	$sql_field="select * from  aicrm_field where block=? and aicrm_field.displaytype IN (1,2,4) order by sequence"; 
			  	$sql_field_params = array($row["blockid"]);
			}else
			{
			  	$sql_field="select * from  aicrm_field where block=? and (aicrm_field.fieldlabel!='Total' and aicrm_field.fieldlabel!='Sub Total' and aicrm_field.fieldlabel!='Tax') and aicrm_field.displaytype IN (1,2,4,5) order by sequence"; 
				$sql_field_params = array($row["blockid"]);
			}
			/*print_r($row["blockid"])."<br>";
			echo $sql_field."<br>";*/
			...
			$query_fields_not_in_block ='select fieldid,fieldlabel,block from aicrm_field ' .
										'inner join aicrm_blocks on aicrm_field.block=aicrm_blocks.blockid ' .
										'where aicrm_field.block != ? and aicrm_blocks.blocklabel not in ("LBL_TICKET_RESOLUTION","LBL_COMMENTS","LBL_COMMENT_INFORMATION") ' .
										'AND aicrm_field.tabid = ? and aicrm_field.displaytype IN (1,2,4) order by aicrm_field.sequence';
...
{
	global $adb;
	$sql='select * from aicrm_convertleadmapping where cfmid = ?';
	...
	for($i =0;$i <$noofrows;$i++)
	{
		...
		$sql2="select fieldlabel from aicrm_field where fieldid = ? and aicrm_field.presence in (0,2)";
		$result2 = $adb->pquery($sql2, array($accountid));
		$accountfield = $adb->query_result($result2,0,'fieldlabel');
		$label['accountlabel'] = $accountfield;
		
		$sql3="select fieldlabel from aicrm_field where fieldid = ? and aicrm_field.presence in (0,2)";
		$result3 = $adb->pquery($sql3, array($contactid));
		$contactfield = $adb->query_result($result3,0,'fieldlabel');
		$label['contactlabel'] = $contactfield;
		$sql4="select fieldlabel from aicrm_field where fieldid = ? and aicrm_field.presence in (0,2)";
...
{
	global $adb;
	$sql="select distinct aicrm_field.tabid,name from aicrm_field inner join aicrm_tab on aicrm_field.tabid=aicrm_tab.tabid where aicrm_field.tabid not in(9,10,16,15,8,29)";
...
function getModuleBlocks($module){
	global $adb;
	$tabid = getTabid($module); 
	$blockquery = "select blocklabel,blockid from aicrm_blocks where tabid = ?";
...
function changeFieldOrder(){
	...
	if(!empty($_REQUEST['what_to_do'])){	
		if($_REQUEST['what_to_do']=='block_down'){
			$sql="select * from aicrm_blocks where blockid=?";
			$result = $adb->pquery($sql, array($_REQUEST['blockid']));
			$row= $adb->fetch_array($result);
			$current_sequence=$row[sequence];
			
			$sql_next="select * from aicrm_blocks where sequence > ? and tabid=? limit 0,1";
			...
			$sql_up_current="update aicrm_blocks  set sequence=? where blockid=?";
			$result_up_current = $adb->pquery($sql_up_current, array($next_sequence,$_REQUEST['blockid']));
			
			
			$sql_up_next="update aicrm_blocks  set sequence=? where blockid=?";
		...
		if($_REQUEST['what_to_do']=='block_up'){
			...
			$sql_previous="select * from aicrm_blocks where sequence < ? and tabid=?  order by sequence desc limit 0,1";
			...
			$sql_up_previous="update aicrm_blocks  set sequence=? where blockid=?";
		...
		if($_REQUEST['what_to_do']=='down' || $_REQUEST['what_to_do']=='Right'){
			$sql="select * from aicrm_field where fieldid=? and aicrm_field.presence in (0,2)";
			$result = $adb->pquery($sql, array($_REQUEST['fieldid']));
			$row= $adb->fetch_array($result);
			$current_sequence=$row['sequence'];
				if($_REQUEST['what_to_do']=='down'){
				$sql_next="select * from aicrm_field where sequence > ? and block = ? and aicrm_field.presence in (0,2) order by sequence limit 1,1";
				$sql_next_params = array($current_sequence, $_REQUEST['blockid']);
			}else{
				$sql_next="select * from aicrm_field where sequence > ? and block = ? and aicrm_field.presence in (0,2) order by sequence limit 0,1";
				$sql_next_params = array($current_sequence, $_REQUEST['blockid']);
			}
			...
			$sql_up_current="update aicrm_field  set sequence=? where fieldid=?";
			$result_up_current = $adb->pquery($sql_up_current, array($next_sequence,$_REQUEST['fieldid']));
			
			$sql_up_next="update aicrm_field  set sequence=? where fieldid=?";
			$result_up_next = $adb->pquery($sql_up_next, array($current_sequence,$next_id));
		...
		if($_REQUEST['what_to_do']=='up' || $_REQUEST['what_to_do']=='Left'){
			...
			if($_REQUEST['what_to_do']=='up'){
				$sql_previous="select * from aicrm_field where sequence < ? and block=? and aicrm_field.presence in (0,2) order by sequence desc limit 1,1";
				$sql_prev_params = array($current_sequence,$_REQUEST['blockid']);
			}else{
				$sql_previous="select * from aicrm_field where sequence < ? and block=? and aicrm_field.presence in (0,2) order by sequence desc limit 0,1";
				$sql_prev_params = array($current_sequence,$_REQUEST['blockid']);
			}
			...
			$sql_up_previous="update aicrm_field  set sequence=? where fieldid=?";
			$result_up_previous = $adb->pquery($sql_up_previous, array($current_sequence,$previous_id));
			$smarty->assign("COLORID",vtlib_purify($_REQUEST['fieldid']));
		}
	
		if($_REQUEST['what_to_do']=='show'){
			$sql_up_display="update aicrm_blocks  set display_status='1' where blockid=?";
			$result_up_display = $adb->pquery($sql_up_display, array($_REQUEST['blockid']));
		}
	
		if($_REQUEST['what_to_do']=='hide'){
			$sql_up_display="update aicrm_blocks  set display_status='0' where blockid=?";
			$result_up_display = $adb->pquery($sql_up_display, array($_REQUEST['blockid']));
		}
	}
}
...
function updateFieldProperties(){
	...
	$req_sql = "select * from aicrm_field where fieldid = ? and fieldname not in('salutationtype') and aicrm_field.presence in (0,2)";
	...
	if(!empty($_REQUEST['fld_module'])){
		if($_REQUEST['fld_module'] == 'Calendar'){
			$_REQUEST['fld_module']= 'Events';
		}
		$fld_module = vtlib_purify($_REQUEST['fld_module']);
	}else{
	...
	if($oldquickcreate != 3){
		if(($quickcreate_checked == 'true' || $quickcreate_checked == '' )){
			$qcdata = 2;
			$quickcreateseq_Query = 'select max(quickcreatesequence) as maxseq from aicrm_field where tabid = ?';
	...
	$mandatory_query = "update aicrm_field set typeofdata=? where fieldid=? and fieldname not in (?) AND displaytype != 2";
	$mandatory_params = array($datatype,$fieldid,$fieldname_list);
	$adb->pquery($mandatory_query, $mandatory_params);
	
	if(!empty($qcdata)){
		$quickcreate_query = "update aicrm_field set quickcreate = ? ,quickcreatesequence = ? where fieldid = ? and quickcreate not in (0,3) AND displaytype != 2";
		$quickcreate_params = array($qcdata,$maxseq+1,$fieldid);
		$adb->pquery($quickcreate_query,$quickcreate_params);
	}
	
	$presence_query = "update aicrm_field set presence = ? where fieldid = ? and presence not in (0,3) and quickcreate != 0";
	$quickcreate_params = array($presence,$fieldid);
	$adb->pquery($presence_query,$quickcreate_params);
	
	$massedit_query = "update aicrm_field set masseditable = ? where fieldid = ? and masseditable not in (0,3) AND displaytype != 2";
...
function editblock(){
	...
	$newblocklabel = str_replace("'","''",trim($_REQUEST['blocklabel']));
	$module = $_REQUEST['fld_module'];
	$edit_block = $_REQUEST['edit_blockid'];
	$tabid = getTabid($module);

	$dup_check_query = $adb->pquery("SELECT blocklabel from aicrm_blocks WHERE tabid = ? and blocklabel = ? and blockid != ?",array($tabid,$newblocklabel,$edit_block));
	...
	$editblock = 'UPDATE aicrm_blocks SET blocklabel = "'.$newblocklabel.'" where blockid = "'.$edit_block.'"  ';
	$res = $adb->pquery($editblock,array());	
}
...
function deleteCustomField(){
	...
	$fieldquery = 'select * from aicrm_field where fieldid = ?';
	...
	$query='delete from aicrm_field where fieldid = ? and aicrm_field.presence in (0,2)';
	...
	$dbquery = 'delete from aicrm_field where columnname= ? and fieldid=? and aicrm_field.presence in (0,2)';
	$adb->pquery($dbquery, array($colName, $id));
	//we have to remove the entries in customview and report related tables which have this field ($colName)
	$adb->pquery("delete from aicrm_cvcolumnlist where columnname = ? ", array($deletecolumnname));
	$adb->pquery("delete from aicrm_cvstdfilter where columnname = ?", array($column_cvstdfilter));
	$adb->pquery("delete from aicrm_cvadvfilter where columnname = ?", array($deletecolumnname));
	$adb->pquery("delete from aicrm_selectcolumn where columnname = ?", array($select_columnname));
	$adb->pquery("delete from aicrm_relcriteria where columnname = ?", array($select_columnname));
	$adb->pquery("delete from aicrm_reportsortcol where columnname = ?", array($select_columnname));
	$adb->pquery("delete from aicrm_reportdatefilter where datecolumnname = ?", array($column_cvstdfilter));
	$adb->pquery("delete from aicrm_reportsummary where columnname like ?", array('%'.$reportsummary_column.'%'));
...
function addblock(){
	...
	$flag = 0;
	$dup_check_query = $adb->pquery("SELECT blocklabel from aicrm_blocks WHERE tabid = ?",array($tabid));
	$norows = $adb->num_rows($dup_check_query);
	for($i=0;$i<$norows;$i++){
	...
	if($flag!=1){
		$sql_seq="select sequence from aicrm_blocks where blockid=?";
		...
		$sql_up="update aicrm_blocks set sequence=sequence+1 where tabid=? and sequence > ?";
		$adb->pquery($sql_up, array($tabid,$block_sequence));
		
		$max_blockid=$adb->getUniqueID('aicrm_blocks');
		$iscustom = 1;
		$sql="INSERT INTO aicrm_blocks (tabid, blockid, sequence, blocklabel,iscustom) values (?,?,?,?,?)";	
...
function deleteBlock(){
	...
	$deleteblock = 'delete from aicrm_blocks where blockid = ? and iscustom = 1';
...
$checkquery="select * from aicrm_field where tabid in (". generateQuestionMarks($dup_check_tab_id) .") and fieldlabel=?";
...
if($adb->num_rows($checkresult) > 0 )
{
	...
	$max_fieldid = $adb->getUniqueID("aicrm_field");
	$columnName = 'cf_'.$max_fieldid;
	$custfld_fieldid = $max_fieldid;
	//Assigning the aicrm_table Name
	if($fldmodule != '') {
		...
		if (isset($focus->customFieldTable)) {
			...
			if($fldmodule=="Faq"){
				...
				$tableName= 'aicrm_'.strtolower($fldmodule).'cf';
			}
		}
	}
...
//ekk========================================================================================	
	elseif($fldType == 'PickList_product'){
		$uitype = 59;
		$type="C(255) default ()"; //adodb type
		$uichekdata='V~O';
	}elseif($fldType == 'PickList_Account'){
		$uitype = 902;
		$type="C(255) default ()"; //adodb type
		$uichekdata='V~O';
	}elseif($fldType == 'PickList_User'){
		$uitype = 903;
		$type="C(255) default ()"; //adodb type
		$uichekdata='V~O';
	}
//ekk========================================================================================	
	     ...
	     if(is_numeric($blockid)){
			if($_REQUEST['fieldid'] == ''){
				$max_fieldsequence = "select max(sequence) as maxsequence from aicrm_field where block = ? ";
				...
				$query = "insert into aicrm_field (tabid,fieldid,columnname,tablename,generatedtype,uitype,fieldname,fieldlabel,readonly,presence,selected,maximumlength,sequence,block,displaytype,typeofdata,quickcreate,quickcreatesequence,info_type,masseditable) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
				...
				//Inserting values into aicrm_profile2field aicrm_tables
				$sql1 = "select * from aicrm_profile";
				...
				for($i=0; $i<$sql1_num; $i++){
					...
					$sql2 = "insert into aicrm_profile2field values(?,?,?,?,?)";
					$adb->pquery($sql2, array($profileid, $tabid, $custfld_fieldid, 0, 1));	 	
				}
	
				//Inserting values into def_org aicrm_tables
				$sql_def = "insert into aicrm_def_org_field values(?,?,?,?)";
			...
			if($fldType == 'Picklist' || $fldType == 'MultiSelectCombo'){
				...
				if($_REQUEST['fieldid'] == ''){
					$qur = "CREATE TABLE aicrm_".$columnName." (
				...
				if($mode != 'edit'){
					$picklistid = $adb->getUniqueID("aicrm_picklist");
					$sql="insert into aicrm_picklist values(?,?)";
					$adb->pquery($sql, array($picklistid,$columnName));
				}
				$roleid=$current_user->roleid;
				$qry="select picklistid from aicrm_picklist where  name=?";
				...
				for($i = 0; $i < $count; $i++)
				{
					...
					{
						$picklistcount=0;
						$sql ="select $columnName from aicrm_$columnName";
						...
						if($picklistcount == 0)
						{
							...
							$query = "insert into aicrm_".$columnName." values(?,?,?,?)";				
							$adb->pquery($query, array($adb->getUniqueID("aicrm_".$columnName),$pickArray[$i],1,$picklist_valueid));
							/*$sql="update aicrm_picklistvalues_seq set id = ?";
							$adb->pquery($sql, array(++$picklist_valueid));*/
						}
						$sql = "select picklist_valueid from aicrm_$columnName where $columnName=?";
						$pick_valueid = $adb->query_result($adb->pquery($sql, array($pickArray[$i])),0,'picklist_valueid');
						$sql = "insert into aicrm_role2picklist select roleid,$pick_valueid,$picklistid,$i from aicrm_role";
			...
			{
				$sql_def = "insert into aicrm_convertleadmapping (leadfid) values(?)";
...
function show_move_hiddenfields($submode){
	...
	$sequence = $adb->pquery('select max(sequence) as maxseq from aicrm_field where block = ?  and tabid = ?',array($_REQUEST['blockid'],$_REQUEST['tabid']));
	$max = $adb->query_result($sequence,0,'maxseq');
	$max_seq = $max + 1;
	
	if($submode == 'showhiddenfields'){
		for($i=0; $i< count($sel_arr);$i++){
			$res = $adb->pquery('update aicrm_field set presence = 2,sequence = ? where block = ? and fieldid = ?', array($max_seq,$_REQUEST['blockid'],$sel_arr[$i]));
			$max_seq++;
			}
		}
	else{
		for($i=0; $i< count($sel_arr);$i++){
			$res = $adb->pquery('update aicrm_field set sequence = ? , block = ? where fieldid = ?', array($max_seq,$_REQUEST['blockid'],$sel_arr[$i]));
			$max_seq++;
		}
	}
}
...
function getRelatedListInfo($module){
	global $adb;
	$tabid = getTabid($module);
	$related_query = 'select * from aicrm_relatedlists ' .
			'inner join aicrm_tab on aicrm_relatedlists.related_tabid = aicrm_tab.tabid and aicrm_tab.presence = 0 where aicrm_relatedlists.tabid = ? order by sequence';
...
function changeRelatedListOrder(){
	...
	if(!empty($_REQUEST['what_to_do'])){
		if($_REQUEST['what_to_do'] == 'move_up'){
			...
			$previous_relation = $adb->pquery('select relation_id,sequence from aicrm_relatedlists where sequence < ? and tabid = ? order by sequence desc limit 0,1',array($currentsequence,$tabid));
			...
			$adb->pquery('update aicrm_relatedlists set sequence = ? where relation_id = ? and tabid = ?',array($previous_sequence,$_REQUEST['id'],$tabid));
			$adb->pquery('update aicrm_relatedlists set sequence = ? where tabid = ? and relation_id = ?',array($currentsequence,$tabid,$previous_relationid));
		}elseif($_REQUEST['what_to_do'] == 'move_down'){
			
			$currentsequence = $_REQUEST['sequence'];
			
			$next_relation = $adb->pquery('select relation_id,sequence from aicrm_relatedlists where sequence > ? and tabid = ? order by sequence limit 0,1',array($currentsequence,$tabid));
			$next_sequence = $adb->query_result($next_relation,0,'sequence');
			$next_relationid = $adb->query_result($next_relation,0,'relation_id');
 			
			$adb->pquery('update aicrm_relatedlists set sequence = ? where relation_id = ? and tabid = ?',array($next_sequence,$_REQUEST['id'],$tabid));
			$adb->pquery('update aicrm_relatedlists set sequence = ? where tabid = ? and relation_id = ?',array($currentsequence,$tabid,$next_relationid));
