...
function getCFListEntries($module)
{
	...
	$theme_path="themes/".$theme."/";
	$image_path="themes/images/";
	$dbQuery = "select fieldid,columnname,fieldlabel,uitype,displaytype,block,aicrm_convertleadmapping.cfmid,tabid from aicrm_field inner join aicrm_convertleadmapping on  aicrm_convertleadmapping.leadfid = aicrm_field.fieldid where tabid in (". generateQuestionMarks($tabid) .") and aicrm_field.presence in (0,2) and generatedtype = 2 order by fieldid";
	/*echo "select fieldid,columnname,fieldlabel,uitype,displaytype,block,aicrm_convertleadmapping.cfmid,tabid from aicrm_field left join aicrm_convertleadmapping on  aicrm_convertleadmapping.leadfid = aicrm_field.fieldid where tabid in ('".$tabid."') and aicrm_field.presence in (0,2) and generatedtype = 2 order by sequence";*/
	/*echo $dbQuery;
	echo "<br>";
	print_r($tabid); */
	...
	if($row!='')
	{
		do
		{
			...
			if($module == 'Leads')
			{
				...
				$cf_element[]= $mapping_details['deallabel'];*/
			}
			if($module == 'Calendar')
			{
				if ($cf_tab_id == '9')
			...
			if ($module == 'Calendar') {
				$cf_element['tool']='&nbsp;<img style="cursor:pointer;" onClick="deleteCustomField('.$row["fieldid"].',\''.$module.'\', \''.$row["columnname"].'\', \''.$row["uitype"].'\')" src="'. aicrm_imageurl('delete.gif', $theme) .'" border="0"  alt="'.$app_strings['LBL_DELETE_BUTTON_LABEL'].'" title="'.$app_strings['LBL_DELETE_BUTTON_LABEL'].'"/></a>';
			}
			$cflist[] = $cf_element;
			$count++;
...
function getListLeadMapping($cfid)
{
	...
	for($i =0;$i <$noofrows;$i++)
	{
		...
		//$dealid = $adb->query_result($result,$i,'dealfid');
		$cfmid = $adb->query_result($result,$i,'cfmid');

		$sql2="select fieldlabel from aicrm_field where fieldid =?";
		...
		$sql3="select fieldlabel from aicrm_field where fieldid =?";
		...
		/*$sql4="select fieldlabel from aicrm_field where fieldid =?";
		...
		/*$sql5="select fieldlabel from aicrm_field where fieldid =?";
		$result5 = $adb->pquery($sql5, array($dealid));
		$dealfield = $adb->query_result($result5,0,'fieldlabel');
		$label['deallabel'] = getTranslatedString($dealfield,'Deal');*/
...
function getCustomFieldSupportedModules()
{
	...
	$sql="select distinct aicrm_field.tabid,name from aicrm_field inner join aicrm_tab on aicrm_field.tabid=aicrm_tab.tabid where aicrm_field.tabid not in(10,16,15,8,29) and aicrm_tab.presence != 1"; // 16 is still here as we do not want duplicates for Calendar - Both 9 and 16 point to Calendar itself
	// END
