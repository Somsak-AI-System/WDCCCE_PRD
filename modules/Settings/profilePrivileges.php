...
//Initially setting the secondary selected aicrm_tab
$mode=$_REQUEST['mode'];
if($mode == 'create')
{
...
//echo "<pre>"; print_r($privileges_tab); echo "</pre>"; exit;
...
$sql_disablefield = "select * from aicrm_def_org_field";
...
if($mode=='view')
{
	...
	for($i=0; $i<count($fieldListResult);$i++)
	{
		...
		for($j=0; $j<count($fieldListResult[$module_name]); $j++)
		{
			...
			{
				$visible = "<img src='".aicrm_imageurl('prvPrfSelectedTick.gif', $theme)."'>";
			}
			else
			{
				$visible = "<img src='".aicrm_imageurl('no.gif', $theme)."'>";
			}
...
//echo "<pre>"; print_r($privilege_field); echo "</pre>"; exit;
...
function getGlobalDisplayValue($id,$actionid)
{
	...
	elseif($id == 0)
	{
		$value = '<img src="' . aicrm_imageurl('prvPrfSelectedTick.gif', $theme) . '">';
	}
	elseif($id == 1)
	{
		$value = '<img src="' . aicrm_imageurl('no.gif', $theme) . '">';
	}
	else
	{
		$value = '&nbsp;';
