...
class AuditTrail{
	...
	var $table_name = "aicrm_audit_trial";
	...
	var $list_fields = Array(
			'Module'=>Array('aicrm_audit_trial'=>'module'), 
			'Action'=>Array('aicrm_audit_trial'=>'action'), 
			'Record'=>Array('aicrm_audit_trial'=>'recordid'),
		        'Action Date'=>Array('aicrm_audit_trial'=>'actiondate'), 
		);	
...
**/
	function getAuditTrailEntries($userid, $navigation_array, $sorder='', $orderby='')
	{
		...
		if($sorder != '' && $order_by != '')
			$list_query = "Select * from aicrm_audit_trial where userid =? order by ".$order_by." ".$sorder; 
		else
			$list_query = "Select * from aicrm_audit_trial where userid =? order by ".$this->default_order_by." ".$this->default_sort_order;
