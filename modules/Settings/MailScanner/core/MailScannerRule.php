...
class Vtiger_MailScannerRule {
	...
	function initialize($forruleid) {
		global $adb;
		$result = $adb->pquery("SELECT * FROM aicrm_mailscanner_rules WHERE ruleid=? ORDER BY sequence", Array($forruleid));
	...
	function initializeActions() {
		...
		if($this->ruleid) {
			$this->actions = Array();
			$actionres = $adb->pquery("SELECT actionid FROM aicrm_mailscanner_ruleactions WHERE ruleid=?",Array($this->ruleid));
	...
	static function resetSequence($ruleid1, $ruleid2) {
		global $adb;
		$ruleresult = $adb->pquery("SELECT ruleid, sequence FROM aicrm_mailscanner_rules WHERE ruleid = ? or ruleid = ?",
		...
		if($adb->num_rows($ruleresult) != 2) {
			...
			$adb->pquery("UPDATE aicrm_mailscanner_rules SET sequence = ? WHERE ruleid = ?", Array($rule_partinfo[$ruleid2], $ruleid1));
			$adb->pquery("UPDATE aicrm_mailscanner_rules SET sequence = ? WHERE ruleid = ?", Array($rule_partinfo[$ruleid1], $ruleid2));
		}
	}			
	...
	function update() {
		global $adb;
		if($this->ruleid) {
			$adb->pquery("UPDATE aicrm_mailscanner_rules SET scannerid=?,fromaddress=?,toaddress=?,subjectop=?,subject=?,bodyop=?,body=?,matchusing=?
			...
			$adb->pquery("INSERT INTO aicrm_mailscanner_rules(scannerid,fromaddress,toaddress,subjectop,subject,bodyop,body,matchusing,sequence) 
				VALUES(?,?,?,?,?,?,?,?,?)", 
	...
	function __nextsequence() {
		global $adb;
		$seqres = $adb->pquery("SELECT max(sequence) AS max_sequence FROM aicrm_mailscanner_rules", Array());
		$maxsequence = 0;
	...
	function delete() {
		global $adb;
		if($this->ruleid) {
			$adb->pquery("DELETE FROM aicrm_mailscanner_ruleactions WHERE ruleid = ?", Array($this->ruleid));
			$adb->pquery("DELETE FROM aicrm_mailscanner_rules WHERE ruleid=?", Array($this->ruleid));
		}
	}
