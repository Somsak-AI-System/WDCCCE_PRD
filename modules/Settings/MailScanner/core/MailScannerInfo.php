...
class Vtiger_MailScannerInfo {
	...
	function initialize($scannername) {
		global $adb;
		$result = $adb->pquery("SELECT * FROM aicrm_mailscanner WHERE scannername=?", Array($scannername));
	...
	function initializeFolderInfo() {
		...
		if($this->scannerid) {
			$this->lastscan = Array();
			$this->rescan   = Array();
			$lastscanres = $adb->pquery("SELECT * FROM aicrm_mailscanner_folders WHERE scannerid=?",Array($this->scannerid));
	...
	function clearLastscan() {
		global $adb;
		$adb->pquery("DELETE FROM aicrm_mailscanner_folders WHERE scannerid=?", Array($this->scannerid));
		$this->lastscan = false;
	}
	...
	function updateAllFolderRescan($rescanFlag=false) {
		...
		$adb->pquery("UPDATE aicrm_mailscanner_folders set rescan=? WHERE scannerid=?", 
	...
	function updateLastscan($folderName, $rescanFolder=false) {
		...
		$folderInfo = $adb->pquery("SELECT folderid FROM aicrm_mailscanner_folders WHERE scannerid=? AND foldername=?",
			Array($this->scannerid, $folderName));
		if($adb->num_rows($folderInfo)) {
			$folderid = $adb->query_result($folderInfo, 0, 'folderid');
			$adb->pquery("UPDATE aicrm_mailscanner_folders SET lastscan=?, rescan=? WHERE folderid=?", 
				Array($scannedOn, $needRescan, $folderid));
		} else {
			$enabledForScan = 1; // Enable folder for scan by default
			$adb->pquery("INSERT INTO aicrm_mailscanner_folders(scannerid, foldername, lastscan, rescan, enabled)
	...
	function getFolderInfo() {
		...
		if($this->scannerid) {
			global $adb;
			$fldres = $adb->pquery("SELECT * FROM aicrm_mailscanner_folders WHERE scannerid=?", Array($this->scannerid));
	...
	function updateFolderInfo($foldernames, $rescanFolder=false) {
		if($this->scannerid && !empty($foldernames)) {
			...
			$adb->pquery("DELETE FROM aicrm_mailscanner_folders WHERE scannerid=? AND foldername NOT IN
	...
	function enableFoldersForScan($folderinfo) {
		if($this->scannerid) {
			global $adb;
			$adb->pquery("UPDATE aicrm_mailscanner_folders set enabled=0 WHERE scannerid=?", Array($this->scannerid));
			foreach($folderinfo as $foldername=>$foldervalue) {
				$folderid = $foldervalue[folderid];
				$enabled  = $foldervalue[enabled];
				$adb->pquery("UPDATE aicrm_mailscanner_folders set enabled=? WHERE folderid=? AND scannerid=?",
	...
	function initializeRules() {
		...
		if($this->scannerid) {
			$this->rules = Array();
			$rulesres = $adb->pquery("SELECT * FROM aicrm_mailscanner_rules WHERE scannerid=? ORDER BY sequence",Array($this->scannerid));
	...
	function update($otherInstance) {
		...
		if($this->scannerid) { // This record exists in the database
			$adb->pquery("UPDATE aicrm_mailscanner SET scannername=?,server=?,protocol=?,username=?,password=?,ssltype=?,
			...
			$adb->pquery("INSERT INTO aicrm_mailscanner(scannername,server,protocol,username,password,ssltype,
	...
	function delete() {
		...
		if($this->scannerid) {
			$tables = Array(
				'aicrm_mailscanner',
				'aicrm_mailscanner_ids', 
				'aicrm_mailscanner_folders',
				'aicrm_mailscanner_rules');
			foreach($tables as $table) {
				$adb->pquery("DELETE FROM $table WHERE scannerid=?",
					Array($this->scannerid));
			}
			$adb->pquery("DELETE FROM aicrm_mailscanner_ruleactions
				WHERE actionid in (SELECT actionid FROM aicrm_mailscanner_actions WHERE scannerid=?)", Array($this->scannerid));
			$adb->pquery("DELETE FROM aicrm_mailscanner_actions WHERE scannerid=?", Array($this->scannerid));
		}
	}
}
?>
