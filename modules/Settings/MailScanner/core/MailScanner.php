...
class Vtiger_MailScanner {
	...
	function markMessageScanned($mailrecord, $crmid=false) {
		...
		$adb->pquery("INSERT INTO aicrm_mailscanner_ids(scannerid, messageid, crmid) VALUES(?,?,?)",
	...
	function isMessageScanned($mailrecord, $lookAtFolder) {
		global $adb;
		$messages = $adb->pquery("SELECT * FROM aicrm_mailscanner_ids WHERE scannerid=? AND messageid=?",
		...
		if($adb->num_rows($messages)) {
			...
			if($folderRescan && empty($relatedCRMId)) {
				$adb->pquery("DELETE FROM aicrm_mailscanner_ids WHERE scannerid=? AND messageid=?",
					Array($this->_scannerinfo->scannerid, $mailrecord->_uniqueid));
				$isScanned = false;
			}
		}
	...
	function LookupContact($email) {
		...
		$contactid = false;
		$contactres = $adb->pquery("SELECT contactid FROM aicrm_contactdetails WHERE email=?", Array($email));
		if($adb->num_rows($contactres)) {
			$contactid = $adb->query_result($contactres, 0, 'contactid');
			$crmres = $adb->pquery("SELECT deleted FROM aicrm_crmentity WHERE crmid=?", Array($contactid));
	...
	function LookupAccount($email) {
		...
		$accountid = false;
		$accountres = $adb->pquery("SELECT accountid FROM aicrm_account WHERE email1=? OR email2=?", Array($email, $email));
		if($adb->num_rows($accountres)) {
			$accountid = $adb->query_result($accountres, 0, 'accountid');
			$crmres = $adb->pquery("SELECT deleted FROM aicrm_crmentity WHERE crmid=?", Array($accountid));
	...
	function LookupTicket($subjectOrId) {
		...
		if(!$checkTicketId) {
			$ticketres = $adb->pquery("SELECT ticketid FROM aicrm_troubletickets WHERE title = ?", Array($subjectOrId));
		...
		$ticketid = false;
		if($checkTicketId) {
			$crmres = $adb->pquery("SELECT setype, deleted FROM aicrm_crmentity WHERE crmid=?", Array($checkTicketId));
