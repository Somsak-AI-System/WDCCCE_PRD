...
class Vtiger_MailScannerAction {
	...
	function initialize($foractionid) {
		global $adb;
		$result = $adb->pquery("SELECT * FROM aicrm_mailscanner_actions WHERE actionid=? ORDER BY sequence", Array($foractionid));
	...
	function update($ruleid, $actiontext) {
		...
		if($this->actionid) {
			$adb->pquery("UPDATE aicrm_mailscanner_actions SET scannerid=?, actiontype=?, module=?, lookup=? WHERE actionid=?",
				Array($this->scannerid, $this->actiontype, $this->module, $this->lookup, $this->actionid));
		} else {
			$this->sequence = $this->__nextsequence();
			$adb->pquery("INSERT INTO aicrm_mailscanner_actions(scannerid, actiontype, module, lookup, sequence) VALUES(?,?,?,?,?)",
				Array($this->scannerid, $this->actiontype, $this->module, $this->lookup, $this->sequence));
			$this->actionid = $adb->database->Insert_ID();
		}
		$checkmapping = $adb->pquery("SELECT COUNT(*) AS ruleaction_count FROM aicrm_mailscanner_ruleactions 
		...
		if($adb->num_rows($checkmapping) && !$adb->query_result($checkmapping, 0, 'ruleaction_count')) {
			$adb->pquery("INSERT INTO aicrm_mailscanner_ruleactions(ruleid, actionid) VALUES(?,?)", 
	...
	function delete() {
		global $adb;
		if($this->actionid) {
			$adb->pquery("DELETE FROM aicrm_mailscanner_actions WHERE actionid=?", Array($this->actionid));
			$adb->pquery("DELETE FROM aicrm_mailscanner_ruleactions WHERE actionid=?", Array($this->actionid));
		}
	}
	...
	function __nextsequence() {
		global $adb;
		$seqres = $adb->pquery("SELECT max(sequence) AS max_sequence FROM aicrm_mailscanner_actions", Array());
		$maxsequence = 0;
	...
	function __UpdateTicket($mailscanner, $mailrecord, $regexMatchInfo) {
		...
		if($this->lookup == 'SUBJECT') {
			...
			if($linkfocus) {
				...
				$adb->pquery("INSERT INTO aicrm_ticketcomments(ticketid, comments, ownerid, ownertype, createdtime) VALUES(?,?,?,?,?)",
					Array($linkfocus->id, $mailrecord->getBodyText(), $relatedid, 'customer', $timestamp));
				// Set the ticket status to Open if its Closed
				$adb->pquery("UPDATE aicrm_troubletickets set status=? WHERE ticketid=? AND status='Closed'", Array('Open', $linkfocus->id));
	...
	function __SaveAttachements($mailrecord, $basemodule, $basefocus) {
		...
		foreach($mailrecord->_attachments as $filename=>$filecontent) {
			$attachid = $adb->getUniqueId('aicrm_crmentity');
			$description = $filename;
			$usetime = $adb->formatDate($date_var, true);

			$adb->pquery("INSERT INTO aicrm_crmentity(crmid, smcreatorid, smownerid, 
			...
			if($issaved) {
				...
				$adb->pquery("INSERT INTO aicrm_seattachmentsrel(crmid, attachmentsid) VALUES(?,?)", 
					Array($document->id, $attachid));
				
				// Link document to base record
				$adb->pquery("INSERT INTO aicrm_senotesrel(crmid, notesid) VALUES(?,?)", 
	...
	function __SaveAttachmentFile($attachid, $filename, $filecontent) {
		...
		$adb->pquery("INSERT INTO aicrm_attachments SET attachmentsid=?, name=?, description=?, type=?, path=?",
