...
if(move_uploaded_file($_FILES["binFile"]["tmp_name"],$uploaddir.$_FILES["binFile"]["name"])) 
{
  ...
  $file_type_value = strtolower($filetype_array[1]);
    ...
    if($filesize != 0)	
    {
		...
		$genQueryId = $adb->getUniqueID("aicrm_wordtemplates");
		if($genQueryId != '') {
			if($result!=false && $savefile=="true") {
				$module = vtlib_purify($_REQUEST['target_module']);
				$sql = "INSERT INTO aicrm_wordtemplates ";
				...
				$result = $adb->updateBlob('aicrm_wordtemplates','data'," filename='". $adb->sql_escape_string($filename) ."'",$data);
