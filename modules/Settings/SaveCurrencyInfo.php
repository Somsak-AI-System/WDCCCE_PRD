...
{
	$cur_status_res = $adb->pquery("select currency_status from aicrm_currency_info where id=?", array($_REQUEST['record']));
	...
	$sql = "update aicrm_currency_info set currency_name =?, currency_code =?, currency_symbol =?, conversion_rate =?,currency_status=? where id =?";
	$params = array($currency_name, $currency_code, $currency_symbol, $conversion_rate, $currency_status, $_REQUEST['record']);
}
else
{
    $sql = "insert into aicrm_currency_info values(?,?,?,?,?,?,?,?)";
	$params = array($db->getUniqueID("aicrm_currency_info"), $currency_name, $currency_code, $currency_symbol, $conversion_rate, $currency_status,'0','0');
}
