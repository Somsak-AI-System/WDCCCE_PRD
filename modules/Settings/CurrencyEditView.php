...
if(isset($_REQUEST['record']) && $_REQUEST['record']!='')
{
	...
	$sql1 = "select * from aicrm_users where currency_id=?";
...
$currencies_query = $adb->pquery("SELECT currency_name from aicrm_currency_info WHERE deleted=0",array());
...
$currencies_query = $adb->pquery("SELECT currency_name,currency_code,currency_symbol from aicrm_currencies",array());
for($index = 0;$index<$adb->num_rows($currencies_query);$index++){
