...
//Deal Sharing
$custom_access['Deal'] = getSharingRuleList('Deal');
...
//Projects Sharing
$custom_access['Projects'] = getSharingRuleList('Projects');
...
//Servicerequest Sharing
$custom_access['Servicerequest'] = getSharingRuleList('Servicerequest');

//Job
$custom_access['Job'] = getSharingRuleList('Job');
...
// Claim
$custom_access['Claim'] = getSharingRuleList('Claim');

//$custom_access['Events'] = getSharingRuleList('Events');
...
function getSharingRuleList($module)
{
	...
	foreach($dataShareTableArray as $table_name => $colName)
	{
		...
		$query = "select ".$table_name.".* from ".$table_name." inner join aicrm_datashare_module_rel on ".$table_name.".shareid=aicrm_datashare_module_rel.shareid where aicrm_datashare_module_rel.tabid=?";
