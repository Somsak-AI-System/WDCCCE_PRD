 ...
 ********************************************************************************/
	...
	if($mode == 'addnew')
	{
		...
		{	
			...
			$picklist_id = $adb->getUniqueID("aicrm_".$tableName);
			
			$query = "insert into aicrm_".$tableName." values(?,?,?,?)";		
			...
			for($i=0;$i<$num_roles;$i++) {
				...
				$sql ="select max(sortid)+1 as sortid from aicrm_role2picklist left join aicrm_$tableName on aicrm_$tableName.picklist_valueid=aicrm_role2picklist.picklistvalueid where roleid=? and picklistid=?";
				$sortid = $adb->query_result($adb->pquery($sql, array($roleid, $picklistid)),0,'sortid');
				if ($sortid == '') {
					$sortid = 0;
				}
			
				$sql = "insert into aicrm_role2picklist values(?,?,?,?)";
	...
	if($mode == 'replace')
	{
		...
		{
			$qry="select tablename,columnname,uitype from aicrm_field where fieldname=?";
			...
			if($num > 0)
			{
				...
				{					
					...
					if ($mode_type == 'delete') {	
						...
						$dele_pick_val =" delete from aicrm_role2picklist where picklistvalueid in (select picklist_valueid from aicrm_$fieldName where $fieldName=?)";
						$adb->pquery($dele_pick_val, array($val));
	
						$del_qry = "delete from aicrm_$fieldName where $fieldName=?";
						$adb->pquery($del_qry, array($val));
					}
					if($mode_type == 'edit') {	
						$sel_qry = "select $fieldName from aicrm_$fieldName where $fieldName = ?";
						...
						if ($adb->num_rows($cnt_res) <= 0) {  // Avoid inserting duplicate entries for the new picklist value
							$picklistvalue_id = getUniquePicklistID();
							$picklist_id = $adb->getUniqueID("aicrm_$fieldName");						
							$query = "insert into aicrm_$fieldName values(?,?,?,?)";		
							$params = array($picklist_id, $replaceWith, 1, $picklistvalue_id);			
							$adb->pquery($query, $params);
							 
							$old_query = "select * from aicrm_$fieldName where $fieldName=?";
							...
							for($i=0;$i<$num_roles;$i++) {
								...
								$upd_qry = "update aicrm_role2picklist set picklistvalueid=? where picklistvalueid=? and picklistid=? and roleid=?";
	...
	if(isset($fieldName))
	{
		...
		$nonEdit ="select $fieldName from aicrm_$fieldName where presence=0";
		...
		if($mode == "transfer")
		{
			...
			$sql="select $fieldName from aicrm_$fieldName where $fieldName not in ($pick_str)";
			$res=$adb->query($sql);
			...
			$output="<table border=0 cellspacing=0 cellpadding=5 width=100%>
					<tr><td colspan=2 align='center'><strong> ".$mod_strings['LBL_PICKLIST_TRANSFER']."\"".$fieldLabel."\"</strong></td><td align='right'><img src='" . aicrm_imageurl('close.gif', $theme) . "' align='middle' border='0' onclick=hide('transferdiv');></td></tr>
