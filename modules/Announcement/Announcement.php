...
class Announcement extends CRMEntity {
// Account is used to store aicrm_account information.
	var $log;
	var $db;
	var $table_name = "aicrm_announcement";
	var $table_index= 'announcementid';
	var $_images_path= 'upload/announcement/';
	var $table_comment = "aicrm_announcementcomments";

	var $tab_name = Array('aicrm_crmentity','aicrm_announcement','aicrm_announcementcf');
	var $tab_name_index = Array('aicrm_crmentity'=>'crmid','aicrm_announcement'=>'announcementid','aicrm_announcementcf'=>'announcementid','aicrm_announcementcomments'=>'announcementid');
	/**
	 * Mandatory table for supporting custom fields.
	 */
	var $customFieldTable = Array('aicrm_announcementcf', 'announcementid');
	var $column_fields = Array();

	var $sortby_fields = Array('announcementid','announcement_name','smownerid');

	var $list_fields = Array(
		'หมายเลขประกาศ'=>Array('Announcement'=>'announcement_no'),
		'หัวข้อประกาศ'=>Array('Announcement'=>'announcement_name'),
		'ผู้รับผิดชอบ' => Array('crmentity'=>'smownerid')
	);

	var $list_fields_name = Array(
		'หมายเลขประกาศ'=>'announcement_no',
		'หัวข้อประกาศ'=>'announcement_name',
		'ผู้รับผิดชอบ'=>'assigned_user_id'
     );

	var $list_link_field= 'announcement_no';
	...
	var $search_fields = Array(
		'หมายเลขประกาศ'=>Array('aicrm_announcement'=>'announcement_no'),
		'หัวข้อประกาศ'=>Array('aicrm_announcement'=>'announcement_name'),
		'ผู้รับผิดชอบ' => Array('crmentity'=>'smownerid')
	);
	...
	var $mandatory_fields = Array('assigned_user_id','createdtime' ,'modifiedtime','announcementid');

	function Announcement()
	{
		$this->log =LoggerManager::getLogger('Announcement');
		$this->db = PearDatabase::getInstance();
		$this->column_fields = getColumnFields('Announcement');
	}
	function save_module()
	{
		global $adb;
		$this->insertIntoAttachment($this->id,'Announcement');
		$this->insertIntoCommentTable("aicrm_announcementcomments",'announcementid');
	}
	...
	function getCommentInformation($crmid)
	{
		...
		$sql = "select * from aicrm_announcementcomments where announcementid=? order by createdtime desc";
	...
	function getSortOrder()
	{
		...
		if(isset($_REQUEST['sorder']))
			...
			$sorder = (($_SESSION['ANNOUNCEMENT_SORT_ORDER'] != '')?($_SESSION['ANNOUNCEMENT_SORT_ORDER']):($this->default_sort_order));
	...
	function getOrderBy()
	{
		...
		if (isset($_REQUEST['order_by']))
			...
			$order_by = (($_SESSION['ANNOUNCEMENT_ORDER_BY'] != '')?($_SESSION['ANNOUNCEMENT_ORDER_BY']):($use_default_order_by));
	...
	function generateReportsSecQuery($module,$secmodule){
		$query = $this->getRelationQuery($module,$secmodule,"aicrm_announcement","announcementid");
		$query .=" left join aicrm_crmentity as aicrm_crmentityEmailtarget on aicrm_crmentityEmailtarget.crmid=aicrm_announcement.announcementid and aicrm_crmentityEmailtarget.deleted=0
				left join aicrm_announcementcf on aicrm_announcementcf.announcementid = aicrm_crmentityEmailtarget.crmid
	...
	function create_export_query($where)
	{
		...
		$sql = getPermittedFieldsQuery("Announcement", "detail_view");
		...
		$query = "SELECT $fields_list,case when (aicrm_users.user_name not like '') then aicrm_users.user_name else aicrm_groups.groupname end as user_name
				...
				INNER JOIN aicrm_announcement ON aicrm_announcement.announcementid = aicrm_crmentity.crmid
				...
				";
		$query .= setFromQuery("Announcement");
		...
		if($is_admin==false && $profileGlobalPermission[1] == 1 && $profileGlobalPermission[2] == 1 && $defaultOrgSharingPermission[6] == 3)
		{
			...
			$query = $query." ".getListViewSecurityParameter("Announcement");
		}
	...
	function setRelationTables($secmodule){
		$rel_tables = array (
			"Documents" => array("aicrm_senotesrel"=>array("crmid","notesid"),"aicrm_announcement"=>"announcementid"),
		);
	...
	function get_users($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		$button .="<input class='crmbutton small delete' type='button' value='Clear Data' onclick=\"JavaScript: void window.open('clear_list_data.php?crmid=".$id."&module=".$_REQUEST["module"]."&related_module=".$related_module."','Application','resizable=0,right=500,top=200,width=500,height=200,toolbar=no,scrollbars=no,menubar=no,location=no')\"  />";	
			
		$query = "SELECT aicrm_users.*, aicrm_role.*
					FROM aicrm_users      
					INNER JOIN aicrm_announcement_usersrel ON aicrm_announcement_usersrel.id=aicrm_users.id
					...
					WHERE aicrm_announcement_usersrel.announcementid = ".$id;
	...
	function insertIntoAttachment($id,$module){
		...
		//Remove the deleted aicrm_attachments from db - Products
	 	if($module == 'Announcement' && $_REQUEST['del_file_list'] != '')
		{
