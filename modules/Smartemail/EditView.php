...
if(isset($_REQUEST['record']) && $_REQUEST['record'] !='') 
{
	...
	$focus->name=$focus->column_fields['Smartemail_name'];		
}
if(isset($_REQUEST['isDuplicate']) && $_REQUEST['isDuplicate'] == 'true') 
{
	$focus->id = "";
    $focus->mode = ''; 	
	$query="SELECT * FROM aicrm_config_sendemail WHERE 1 and deleted=0  and email_type='bounce' and  	email_status='Active' limit 1 ";
	$result = $adb->pquery($query, array());
	$num_rows=$adb->num_rows($result);
	//echo $num_rows;
	if($num_rows>0){
		$focus->column_fields['email_bounce']=$adb->query_result($result,0,'email_user');
	}
	$query="SELECT * FROM aicrm_config_sendemail WHERE 1 and deleted=0  and email_type='edm' and  	email_status='Active' limit 1 ";
	$result = $adb->pquery($query, array());
	$num_rows=$adb->num_rows($result);
	//echo $num_rows;
	if($num_rows>0){
		$focus->column_fields['email_server']=$adb->query_result($result,0,'email_server');
	}
		
	$focus->column_fields['smartemail_name']="";
	//$focus->column_fields['email_template_type']="Email Marketing";
	$focus->column_fields['email_status']= "Prepare";
	$focus->column_fields['email_setup']= "0";
	$focus->column_fields['email_send']= "0";
	$focus->column_fields['email_start_date']=date('Y-m-d');
	$focus->column_fields['email_start_time']=date("H:i", strtotime((date("H:i")) . "+15 minutes"));
...
if(empty($_REQUEST['record']) && $focus->mode != 'edit'){
	...
	$focus->column_fields['email_start_time']=date("H:i", strtotime((date("H:i")) . "+15 minutes"));
	//$focus->column_fields['email_start_date']= date('Y-m-d');
	//$focus->column_fields['email_status']= "Active";
	$focus->column_fields['email_setup']= "0";
	$focus->column_fields['email_send']= "0";
...
$allOptions=getFieldTemplateVariables();
