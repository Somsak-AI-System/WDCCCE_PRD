...
class Smartemail extends CRMEntity {
// Account is used to store aicrm_account information.
	var $log;
	var $db;
	var $table_name = "aicrm_smartemail";
	var $table_index= 'smartemailid';

	var $tab_name = Array('aicrm_crmentity','aicrm_smartemail','aicrm_smartemailcf');
	var $tab_name_index = Array('aicrm_crmentity'=>'crmid','aicrm_smartemail'=>'smartemailid','aicrm_smartemailcf'=>'smartemailid');
	/**
	 * Mandatory table for supporting custom fields.
	 */
	var $customFieldTable = Array('aicrm_smartemailcf', 'smartemailid');
	var $column_fields = Array();

	var $sortby_fields = Array('smartemailid','smartemail_name','smownerid');

	var $list_fields = Array(
					'Smart Email No'=>Array('Smartemail'=>'smartemail_no'),
					'Smart Email Name'=>Array('Smartemail'=>'smartemail_name'),
					'Smart Email Status'=>Array('Smartemail'=>'email_status'),
					'Assigned To' => Array('crmentity'=>'smownerid')
				);

	var $list_fields_name = Array(
					'Smart Email No'=>'smartemail_no',
					'Smart Email Name'=>'smartemail_name',
					'Smart Email Status'=>'email_status',
					'Assigned To'=>'assigned_user_id'
				     );	  			

	var $list_link_field= 'smartemail_no';
	...
	var $search_fields = Array(
			'Smart Email No'=>Array('aicrm_smartemail'=>'smartemail_no'),
			'Smart Email Name'=>Array('aicrm_smartemail'=>'smartemail_name'),
			);
	...
	var $mandatory_fields = Array('assigned_user_id','createdtime' ,'modifiedtime','smartemailid');
	
	function Smartemail() 
	{
		$this->log =LoggerManager::getLogger('Smartemail');
		$this->db = PearDatabase::getInstance();
		$this->column_fields = getColumnFields('Smartemail');
	}
	function save_module()
	{
		global $adb;
		//in ajax save we should not call this function, because this will delete all the existing product values
		if($_REQUEST['action'] != 'SmartemailAjax' && $_REQUEST['ajxaction'] != 'DETAILVIEW' && $_REQUEST['action'] != 'MassEditSave')
		{
			//Based on the total Number of rows we will save the product relationship with this entity
			saveInventoryProductDetails($this, 'Smartemail');
		}
		
		if(isset($_REQUEST['email_status']) &&  $_REQUEST['email_status'] == 'Active'){
						
			$sql = " update aicrm_smartemail
						set email_setup = '0'
						,email_send = '0'
						where smartemailid = '".$this->id."' ";
...
//		$adb->pquery($update_query, $update_params);
	...
	function getSortOrder()
	{
		...
		if(isset($_REQUEST['sorder'])) 
			...
			$sorder = (($_SESSION['SMARTEMAIL_SORT_ORDER'] != '')?($_SESSION['SMARTEMAIL_SORT_ORDER']):($this->default_sort_order));
	...
	function getOrderBy()
	{
		...
		if (isset($_REQUEST['order_by'])) 
			...
			$order_by = (($_SESSION['SMARTEMAIL_ORDER_BY'] != '')?($_SESSION['SMARTEMAIL_ORDER_BY']):($use_default_order_by));
...
//	}
	...
	function get_emailtargets($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		//echo $id;
		$button .= $lhtml."<input title='Load List' class='crmbutton small edit' value='Load List' type='button' name='button' onclick='loadCvList_email(\"$related_module\",\"$id\")'>";
		$button .= '&nbsp;&nbsp;&nbsp;&nbsp';
		...
		$query = "SELECT aicrm_emailtargets.*,aicrm_emailtargetscf.*, aicrm_crmentity.crmid, 
					...
					INNER JOIN aicrm_smartemail_emailtargetrel ON aicrm_smartemail_emailtargetrel.emailtargetid=aicrm_emailtargets.emailtargetid
					...
					LEFT JOIN aicrm_groups ON aicrm_groups.groupid=aicrm_crmentity.smownerid
					WHERE aicrm_crmentity.deleted=0 AND aicrm_smartemail_emailtargetrel.smartemailid = ".$id;
		//echo 		$query;
		$return_value = GetRelatedList($this_module, $related_module, $other, $query, $button, $returnset); 
	...
	function get_contacts($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		$query = "SELECT aicrm_contactdetails.*,aicrm_contactscf.*, aicrm_crmentity.crmid, 
					...
					INNER JOIN aicrm_smartemail_contactsrel ON aicrm_smartemail_contactsrel.contactid=aicrm_contactdetails.contactid
					...
					WHERE aicrm_crmentity.deleted=0 AND aicrm_smartemail_contactsrel.smartemailid = ".$id;
		// echo $query; exit;
	...
	function get_users($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		$query = "SELECT aicrm_users.*, aicrm_role.*
					FROM aicrm_users      
					INNER JOIN aicrm_smartemail_usersrel ON aicrm_smartemail_usersrel.id=aicrm_users.id
					...
					WHERE aicrm_smartemail_usersrel.smartemailid = ".$id;
		// echo $query;
		$return_value = GetRelatedList($this_module, $related_module, $other, $query, $button, $returnset); 
	...
	function get_leads($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		$query = "SELECT aicrm_leaddetails.*,aicrm_leadscf.*, aicrm_crmentity.crmid, aicrm_leadaddress.*,
					...
					INNER JOIN aicrm_smartemail_leadsrel ON aicrm_smartemail_leadsrel.leadid=aicrm_leaddetails.leadid
					INNER JOIN aicrm_crmentity ON aicrm_crmentity.crmid = aicrm_leaddetails.leadid
					LEFT JOIN aicrm_users ON aicrm_crmentity.smownerid = aicrm_users.id
					LEFT JOIN aicrm_groups ON aicrm_groups.groupid=aicrm_crmentity.smownerid
					WHERE aicrm_crmentity.deleted=0 AND aicrm_smartemail_leadsrel.smartemailid = ".$id;
		// echo $query;
		$return_value = GetRelatedList($this_module, $related_module, $other, $query, $button, $returnset); 
	...
	function get_opportunity($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		$query = "SELECT aicrm_opportunity.*,aicrm_opportunitycf.*, aicrm_products.* , aicrm_branchs.*,aicrm_account.* , aicrm_crmentity.crmid, 
					CASE when (aicrm_users.user_name not like '') then aicrm_users.user_name else aicrm_groups.groupname end as user_name, 
					aicrm_crmentity.smownerid 
					FROM aicrm_opportunity      
					LEFT JOIN aicrm_opportunitycf on aicrm_opportunitycf.opportunityid=aicrm_opportunity.opportunityid
					INNER JOIN aicrm_smartemail_opportunityrel ON aicrm_smartemail_opportunityrel.opportunityid=aicrm_opportunity.opportunityid
					...
					LEFT JOIN aicrm_users ON aicrm_crmentity.smownerid = aicrm_users.id
					LEFT JOIN aicrm_groups ON aicrm_groups.groupid=aicrm_crmentity.smownerid
					WHERE aicrm_crmentity.deleted=0 AND aicrm_smartemail_opportunityrel.smartemailid = ".$id;
		//echo 		$query;
		$return_value = GetRelatedList($this_module, $related_module, $other, $query, $button, $returnset); 
	...
	function get_questionnaire($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		...
		$query = "SELECT aicrm_questionnaires.*,aicrm_questionnairescf.*, aicrm_crmentity.crmid, 
					...
					INNER JOIN aicrm_smartemail_questionnairerel ON aicrm_smartemail_questionnairerel.questionnaireid=aicrm_questionnaires.questionnaireid
					INNER JOIN aicrm_crmentity ON aicrm_crmentity.crmid = aicrm_questionnaires.questionnaireid
					LEFT JOIN aicrm_users ON aicrm_crmentity.smownerid = aicrm_users.id
					LEFT JOIN aicrm_groups ON aicrm_groups.groupid=aicrm_crmentity.smownerid
					WHERE aicrm_crmentity.deleted=0 AND aicrm_smartemail_questionnairerel.smartemailid = ".$id;
		//echo 		$query;
		$return_value = GetRelatedList($this_module, $related_module, $other, $query, $button, $returnset); 
...
//                $button .= "<input title='Advance Select' class='crmbutton small create' type='button' onclick=\"advanceSelect('$related_module','$id');\" value='Advance Select'>&nbsp;";
		...
		if($actions) {
			...
			if(is_string($actions)) $actions = explode(',', strtoupper($actions));
			if(in_array('SELECT', $actions) ) {
				$button .= "<input title='Select Accounts' class='crmbutton small create' type='button' onclick=\"return window.open('index.php?module=$related_module&return_module=$currentModule&action=Popup&popuptype=detailview&select=enable&form=EditView&form_submit=false&recordid=$id&parenttab=$parenttab','test','width=640,height=602,resizable=1,scrollbars=0');\" value='Select Accounts'>&nbsp;";
			}
		...
		$query = "SELECT aicrm_account.*,aicrm_accountscf.*, aicrm_crmentity.crmid, aicrm_accountbillads.*,
					...
					INNER JOIN aicrm_smartemail_accountsrel ON aicrm_smartemail_accountsrel.accountid=aicrm_account.accountid
					INNER JOIN aicrm_crmentity ON aicrm_crmentity.crmid = aicrm_account.accountid
					LEFT JOIN aicrm_users ON aicrm_crmentity.smownerid = aicrm_users.id
					LEFT JOIN aicrm_groups ON aicrm_groups.groupid=aicrm_crmentity.smownerid
					WHERE aicrm_crmentity.deleted=0 AND aicrm_smartemail_accountsrel.smartemailid = ".$id;
//		 echo $query;
		$return_value = GetRelatedList($this_module, $related_module, $other, $query, $button, $returnset); 
		...
		if($actions) {
			...
			if(in_array('ADD', $actions) ) {
				$button .= "<input title='Add SMS Marketing' class='crmbutton small create'" .
					" onclick=\"window.open('sms_marketing.php?smartemailid=".$id."&userid=".$_SESSION["user_id"]."','test','resizable=no,left=200,top=150,width=600,height=320,toolbar=no,menubar=no,resizable=no,scrollbars=yes,status=no,location=no')\" type='button' name='button'" .
					" value='Add SMS Marketing'>&nbsp;";
			}
		}
		$query = "SELECT *
					FROM aicrm_campaign_sms_marketing  
					WHERE aicrm_campaign_sms_marketing.deleted=0 AND aicrm_campaign_sms_marketing.smartemailid = ".$id." 
					order by id desc
					";
		//echo 		$query;
		$return_value = GetRelatedList($this_module, $related_module, $other, $query, $button, $returnset); 
	...
	function generateReportsSecQuery($module,$secmodule){
		$query = $this->getRelationQuery($module,$secmodule,"aicrm_smartemail","smartemailid");
		$query .=" left join aicrm_crmentity as aicrm_crmentitySmartemail on aicrm_crmentitySmartemail.crmid=aicrm_smartemail.smartemailid and aicrm_crmentitySmartemail.deleted=0 
				left join aicrm_smartemailcf on aicrm_smartemailcf.smartemailid = aicrm_crmentitySmartemail.crmid 
				left join aicrm_groups as aicrm_groupsSmartemail on aicrm_groupsSmartemail.groupid = aicrm_crmentitySmartemail.smownerid
				left join aicrm_users as aicrm_usersSmartemail on aicrm_usersSmartemail.id = aicrm_crmentitySmartemail.smownerid
	...
	function setRelationTables($secmodule){
		// echo "7787";
		$rel_tables = array (
			//"SalesOrder" =>array("aicrm_salesorder"=>array("quoteid","salesorderid"),"aicrm_smartemail"=>"quoteid"),
			//"Calendar" =>array("aicrm_seactivityrel"=>array("crmid","activityid"),"aicrm_smartemail"=>"smartemailid"),
			"Documents" => array("aicrm_senotesrel"=>array("crmid","notesid"),"aicrm_smartemail"=>"smartemailid"),
			"Accounts" => array("aicrm_smartemail_accountsrel"=>array("smartemailid","accountid"),"aicrm_smartemail"=>"smartemailid"),
			"Contacts" => array("aicrm_smartemail_contactsrel"=>array("smartemailid","contactid"),"aicrm_smartemail"=>"smartemailid"),
			"Leads" => array("aicrm_smartemail_leadsrel"=>array("smartemailid","leadid"),"aicrm_smartemail"=>"smartemailid"),
			"Users" => array("aicrm_smartemail_usersrel"=>array("smartemailid","id"),"aicrm_smartemail"=>"smartemailid"),
			"Opportunity" => array("aicrm_smartemail_opportunityrel"=>array("smartemailid","opportunityid"),"aicrm_smartemail"=>"smartemailid"),
			"Questionnaire" => array("aicrm_smartemail_questionnairerel"=>array("smartemailid","questionnaireid"),"aicrm_smartemail"=>"smartemailid"),
			//"Contacts" => array("aicrm_quotes"=>array("aicrm_smartemail","contactid")),
	...
	function unlinkRelationship($id, $return_module, $return_id) {//echo "ffff";
		...
		if($return_module == 'Accounts' ) {
			...
			$relation_query = 'DELETE FROM aicrm_campaignmaillistrel WHERE smartemailid=? AND campaignid=?';
