...
$_SESSION['SERVICE_ORDER_BY'] = $order_by;
$_SESSION['SERVICE_SORT_ORDER'] = $sorder;
//<<<<<<<<<<<<<<<<<<< sorting - stored in session >>>>>>>>>>>>>>>>>>>>
...
if(isPermitted('Service','Delete','') == 'yes')
{
	$other_text['del'] = $app_strings[LBL_MASS_DELETE];
}
if(isPermitted('Service','EditView','') == 'yes')
{
...
if($viewid != "0")
{
	$listquery = getListQuery("Service");
	$query = $oCustomView->getModifiedCvListQuery($viewid,$listquery,"Service");
}else
{
	$query = getListQuery("Service");
}
//<<<<<<<<customview>>>>>>>>>
if(isset($where) && $where != '')
{
...
if(isset($order_by) && $order_by != '')
{
	if($order_by == 'smownerid'){
			...
			$tablename = getTableNameForField('Service',$order_by);
...
$listview_header = getListViewHeader($focus,"Service",$url_string,$sorder,$order_by,"",$oCustomView);
$smarty->assign("LISTHEADER", $listview_header);

$listview_header_search = getSearchListHeaderValues($focus,"Service",$url_string,$sorder,$order_by,"",$oCustomView);
$smarty->assign("SEARCHLISTHEADER",$listview_header_search);

$listview_entries = getListViewEntries($focus,"Service",$list_result,$navigation_array,"","","EditView","Delete",$oCustomView);
...
$navigationOutput = getTableHeaderSimpleNavigation($navigation_array, $url_string,"Service","index",$viewid);
