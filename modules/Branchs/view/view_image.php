...
<div  id="basicTab" >
	...
	<tr>
        ...
        <input class="crmbutton small edit" type="button" value="เพิ่มรูปภาพสินค้า" style="height:30px;" onclick="JavaScript: void window.open('<?=$path_pic_std?>/upload_img_job.php?crmid=<?=$_GET["record"]?>&module=<?=$_GET["module"]?>&setype=Image Products','Application','resizable=yes,left=200,top=50,width=350,height=250,toolbar=no,scrollbars=no,menubar=no,location=no')"/>
		</td>
	</tr>
...
<!-- <div  id="basicTab" >
	<tr>
		<td class="detailedViewHeader" colspan="4">
        <input class="crmbutton small edit" type="button" value="เพิ่มรูปภาพใบเสร็จ" style="height:30px;" onclick="JavaScript: void window.open('<?=$path_pic_std?>/upload_img_job.php?crmid=<?=$_GET["record"]?>&module=<?=$_GET["module"]?>','Application','resizable=yes,left=200,top=50,width=350,height=250,toolbar=no,scrollbars=no,menubar=no,location=no')"/>
		</td>
	</tr>
	<?php
	if(isset($data) && $data['Type'] == 'S' && !empty($data['data'][0]['image'])){
