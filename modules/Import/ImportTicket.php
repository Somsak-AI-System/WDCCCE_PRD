	...
	function add_product()
        {
                ...
                //Modified to remove the spaces at first and last in aicrm_product name
                $pro_name = trim($pro_name);

                //Modified the query to get the available product only ie., which is not deleted
                $query = "select aicrm_products.* ,aicrm_crmentity.deleted from aicrm_products,aicrm_crmentity  WHERE productname=? and aicrm_crmentity.crmid = aicrm_products.productid and aicrm_crmentity.deleted=0";
