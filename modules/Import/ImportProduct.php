...
class ImportProduct extends Products {
	...
	function assign_user()
	{
		...
		if( $ass_user != $current_user->id)
		{
			...
			$result = $this->db->pquery("select id from aicrm_users where id = ?", array($ass_user));
	...
	function map_vendor_name()
	{
		...
		$query = "select vendorid from aicrm_vendor inner join aicrm_crmentity on aicrm_crmentity.crmid=aicrm_vendor.vendorid WHERE aicrm_vendor.vendorname=? and aicrm_crmentity.deleted=0";
    ...
    */
	function map_member_of()
	{
		...
		$query = "select productid from aicrm_products inner join aicrm_crmentity on aicrm_crmentity.crmid=aicrm_products.productid WHERE aicrm_products.productname=? and aicrm_crmentity.deleted=0";
