...
require_once('modules/Import/ImportCompetitor.php');
require_once('modules/Import/ImportErrors.php');
require_once('modules/Import/ImportSerial.php');
require_once('modules/Import/ImportSparepart.php');
require_once('modules/Import/ImportCampaigns.php');
require_once('modules/Import/ImportTicket.php');
require_once('modules/Import/ImportPriceList.php');
require_once('modules/Import/ImportPlant.php');
require_once('modules/Import/ImportDeal.php');
require_once('modules/Import/ImportVoucher.php');
require_once('modules/Import/ImportPromotion.php');
require_once('modules/Import/ImportCompetitorproduct.php');
require_once('modules/Import/ImportPremuimproduct.php');
require_once('modules/Import/ImportPromotionvoucher.php');
require_once('modules/Import/ImportService.php');
require_once('modules/Import/ImportFaq.php');
require_once('modules/Import/ImportKnowledgeBase.php');
require_once('include/database/PearDatabase.php');
require_once('include/CustomFieldUtil.php');
require_once('include/utils/CommonUtils.php');

unset($_SESSION["column_position_to_field"]); 
unset($_SESSION["totalrows"]); 
unset($_SESSION["recordcount"]); 
unset($_SESSION["startval"]); 
unset($_SESSION["return_field_count"]); 
...
//echo "<pre>"; print_r($ret_value); echo "</pre>"; exit;
$rows = $ret_value['rows'];
...
$import_object_array = Array(
	...
	"PriceList"=>"ImportPriceList",
	"Competitor"=>"ImportCompetitor",
	"Errors"=>"ImportErrors",
	"Serial"=>"ImportSerial",
	"Sparepart"=>"ImportSparepart",
	"Campaigns"=>"ImportCampaigns",
	"Plant"=>"ImportPlant",
	"Deal"=>"ImportDeal",
	"Promotion"=>"ImportPromotion",
	"Voucher"=>"ImportVoucher",
	"Competitorproduct"=>"ImportCompetitorproduct",
	"Premuimproduct"=>"ImportPremuimproduct",
	"Promotionvoucher"=>"ImportPromotionvoucher",
...
$map_list_combo = '<select class="small" name="source" id="saved_source" disabled onchange="getImportSavedMap(this)" style="border-radius:3px;">';
...
$translated_column_fields = getImportFieldsList($_REQUEST['module']);
$cnt=1;
for($field_count = 0; $field_count < $ret_field_count; $field_count++){
	...
	if($_REQUEST['module']=='PriceList'){
		$tablename='PriceList';
		$focus1=new PriceList();
	}if($_REQUEST['module']=='Competitor'){
        $tablename='Competitor';
        $focus1=new Competitor();
    }if($_REQUEST['module']=='Errors'){
        $tablename='Errors';
        $focus1=new Errors();
    }if($_REQUEST['module']=='Sparepart'){
        $tablename='Sparepart';
        $focus1=new Sparepart();
    }if($_REQUEST['module']=='Serial'){
        $tablename='Serial';
        $focus1=new Serial();
    }if($_REQUEST['module']=='Campaigns'){
        $tablename='Campaigns';
        $focus1=new Campaigns();
    
    }if($_REQUEST['module']=='Sparepartlist'){
        $tablename='Sparepartlist';
        $focus1=new Sparepartlist();
    
    }if($_REQUEST['module']=='Plant'){
        $tablename='Plant';
        $focus1=new Plant();
    ...
    }if($_REQUEST['module']=='Promotion'){
        $tablename='Promotion';
        $focus1=new Promotion();
    
    }if($_REQUEST['module']=='Voucher'){
        $tablename='Voucher';
        $focus1=new Voucher();
    }
    if($_REQUEST['module']=='Competitorproduct'){
        $tablename='Competitorproduct';
        $focus1=new Competitorproduct();
    }
    if($_REQUEST['module']=='Premuimproduct'){
        $tablename='Premuimproduct';
        $focus1=new Premuimproduct();
    }
    if($_REQUEST['module']=='Promotionvoucher'){
        $tablename='Promotionvoucher';
        $focus1=new Promotionvoucher();
    }
    if($_REQUEST['module']=='Service'){
        $tablename='Service';
        $focus1=new Service();
    }
    if($_REQUEST['module']=='Faq'){
        $tablename='Faq';
        $focus1=new Faq();
    }
    if($_REQUEST['module']=='KnowledgeBase'){
        $tablename='KnowledgeBase';
        $focus1=new KnowledgeBase();
    }
	...
	// END
	//echo "<pre>"; print_r($focus->importable_fields); echo "</pre>"; exit;
	//echo "<pre>"; print_r($translated_column_fields); echo "</pre>"; exit;
	//echo "<pre>"; print_r($firstrow); echo "</pre>"; exit;
	...
	foreach($rows as $row ){
		if( isset($row[$field_count]) && $row[$field_count] != ''){
			//echo htmlspecialchars($row[$field_count])."<br>";
...
unset($_SESSION['import_has_header']);
unset($_SESSION['import_firstrow']);
unset($_SESSION['import_field_map']);
unset($_SESSION['import_module_object_column_fields']);
unset($_SESSION['import_module_field_count']);
unset($_SESSION['import_module_object_required_fields']);
unset($_SESSION['import_module_translated_column_fields']);
...
$_SESSION['import_module_object_required_fields'] = $focus->required_fields;
...
function validate_import_map()
{
	...
	for(loop_count = 0; loop_count<field_count;loop_count++)
	{
		...
		if(optionData != -1)
		{
			...
			{
				//if a aicrm_field mapped more than once, alert the user and return
	...
	//check whether the mandatory aicrm_fields have been mapped.
