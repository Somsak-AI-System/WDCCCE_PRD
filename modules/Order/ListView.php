...
$_SESSION['ORDER_ORDER_BY'] = $order_by;
$_SESSION['ORDER_SORT_ORDER'] = $sorder;
//<<<<<<<<<<<<<<<<<<< sorting - stored in session >>>>>>>>>>>>>>>>>>>>
...
if(isPermitted('Order','Delete','') == 'yes')
{
	$other_text['del'] = $app_strings[LBL_MASS_DELETE];
}
if(isPermitted('Order','EditView','') == 'yes')
{
...
if($viewid != "0")
{
	$listquery = getListQuery("Order");
	$query = $oCustomView->getModifiedCvListQuery($viewid,$listquery,"Order");
}else
{
	$query = getListQuery("Order");
}
//<<<<<<<<customview>>>>>>>>>
//echo $query; exit;
if(isset($where) && $where != '')
...
if(isset($order_by) && $order_by != '')
{
	if($order_by == 'smownerid')
        ...
        {
		$tablename = getTableNameForField('Order',$order_by);
...
$listview_header = getListViewHeader($focus,"Order",$url_string,$sorder,$order_by,"",$oCustomView);
$smarty->assign("LISTHEADER", $listview_header);

$listview_header_search = getSearchListHeaderValues($focus,"Order",$url_string,$sorder,$order_by,"",$oCustomView);
$smarty->assign("SEARCHLISTHEADER",$listview_header_search);

$listview_entries = getListViewEntries($focus,"Order",$list_result,$navigation_array,"","","EditView","Delete",$oCustomView);
...
$navigationOutput = getTableHeaderSimpleNavigation($navigation_array, $url_string,"Order","index",$viewid);
