...
class Expense extends CRMEntity {
// Account is used to store aicrm_account information.
	var $log;
	var $db;
	var $table_name = "aicrm_expense";
	var $table_index= 'expenseid';
	var $_images_path= 'upload/expense/';
	var $table_comment = "aicrm_expensecomments";

	var $tab_name = Array('aicrm_crmentity','aicrm_expense','aicrm_expensecf');
	var $tab_name_index = Array('aicrm_crmentity'=>'crmid','aicrm_expense'=>'expenseid','aicrm_expensecf'=>'expenseid','aicrm_expensecomments'=>'expenseid');
	/**
	 * Mandatory table for supporting custom fields.
	 */
	var $customFieldTable = Array('aicrm_expensecf', 'expenseid');
	var $column_fields = Array();

	var $sortby_fields = Array('expenseid','expense_name','smownerid');

	var $list_fields = Array(
		'Expense No'=>Array('Expense'=>'expense_no'),
		'Expense Name'=>Array('Expense'=>'expense_name'),
	...
	var $search_fields = Array(
		'Expense No'=>Array('aicrm_expense'=>'expense_no'),
		'Expense Name'=>Array('aicrm_expense'=>'expense_name'),
		'Assigned To' => Array('crmentity'=>'smownerid')
	);
	...
	var $mandatory_fields = Array('assigned_user_id','createdtime' ,'modifiedtime','expenseid');

	function Expense()
	{
		$this->log =LoggerManager::getLogger('Expense');
		$this->db = PearDatabase::getInstance();
		$this->column_fields = getColumnFields('Expense');
	}
	function save_module()
	{
		global $adb;
		$this->insertIntoAttachment($this->id,'Expense');
		$this->insertIntoCommentTable("aicrm_expensecomments",'expenseid');
	...
	function getCommentInformation($crmid)
	{
		...
		$sql = "select * from aicrm_expensecomments where expenseid=? order by createdtime desc";
	...
	function getSortOrder()
	{
		...
		if(isset($_REQUEST['sorder']))
			...
			$sorder = (($_SESSION['EXPENSE_SORT_ORDER'] != '')?($_SESSION['EXPENSE_SORT_ORDER']):($this->default_sort_order));
	...
	function getOrderBy()
	{
		...
		if (isset($_REQUEST['order_by']))
			...
			$order_by = (($_SESSION['EXPENSE_ORDER_BY'] != '')?($_SESSION['EXPENSE_ORDER_BY']):($use_default_order_by));
	...
	function generateReportsSecQuery($module,$secmodule){
		$query = $this->getRelationQuery($module,$secmodule,"aicrm_expense","expenseid");
		$query .=" left join aicrm_crmentity as aicrm_crmentityEmailtarget on aicrm_crmentityEmailtarget.crmid=aicrm_expense.expenseid and aicrm_crmentityEmailtarget.deleted=0
				left join aicrm_expensecf on aicrm_expensecf.expenseid = aicrm_crmentityEmailtarget.crmid
	...
	function create_export_query($where)
	{
		...
		$sql = getPermittedFieldsQuery("Expense", "detail_view");
		...
		$query = "SELECT $fields_list,case when (aicrm_users.user_name not like '') then aicrm_users.user_name else aicrm_groups.groupname end as user_name
				...
				INNER JOIN aicrm_expense ON aicrm_expense.expenseid = aicrm_crmentity.crmid
		...
		if($is_admin==false && $profileGlobalPermission[1] == 1 && $profileGlobalPermission[2] == 1 && $defaultOrgSharingPermission[6] == 3)
		{
			...
			$query = $query." ".getListViewSecurityParameter("Expense");
		}
	...
	function setRelationTables($secmodule){
		$rel_tables = array (
			"Documents" => array("aicrm_senotesrel"=>array("crmid","notesid"),"aicrm_expense"=>"expenseid"),
		);
	...
	function insertIntoAttachment($id,$module){
		...
		if($module == 'Expense' && $_REQUEST['del_file_list'] != '')
		{
