...
if($ajaxaction == "DETAILVIEW")
{
     ...
     if($crmid != "")
	 {
		 ...
		 $email_res = $adb->pquery("select email from aicrm_contactdetails where contactid=?", array($cntObj->id));
		 $email = $adb->query_result($email_res,0,'email');

		 $check_available = $adb->pquery("select * from aicrm_portalinfo where id=?", array($cntObj->id));
		 $update = '';
		 if($fieldname =='email')
		 {
			 ...
			 if($active != '' && $active == 1)
			 {
				$sql = "update aicrm_portalinfo set user_name=?,isactive=? where id=?";
				$adb->pquery($sql, array($fieldvalue, $active, $crmid));
				$email = $fieldvalue;
				$result = $adb->pquery("select user_password from aicrm_portalinfo where id=?", array($cntObj->id));
		 ...
		 if($fieldname == "portal")
		 {
			if($email != '')
			{
				...
				if($confirm == '' && $fieldvalue == 1)
				{
					...
					$sql = "insert into aicrm_portalinfo (id,user_name,user_password,type,isactive) values(?,?,?,?,?)";
				...
				{
					$sql = "update aicrm_portalinfo set isactive=0 where id=?";
