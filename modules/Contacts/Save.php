 ...
 ********************************************************************************/
/*********************************************************************************
 * $Header: /advent/projects/wesat/aicrm_crm/sugarcrm/modules/Contacts/Save.php,v 1.9 2005/03/15 09:58:21 shaw Exp $
...
$image_name_val=$image_upload_array['picture'];
$image_error="false";
...
if($image_error=="true") //If there is any error in the file upload then moving all the data to EditView.
{
    ...
    foreach($focus->column_fields as $fieldname => $val)
    {
        ...
        {
			$log->debug("Assigning the previous values given for the contact to respective aicrm_fields ");
            $field_values_passed.="&";
...
if($saveimage=="true")
{
    $focus->column_fields['picture']=$image_name_val;
    $log->debug("Assign the Image name to the aicrm_field name ");
}
...
if($_FILES['picture']['name'] != '')
{
	if(isset($_REQUEST['picture'])) {
		$focus->column_fields['picture'] = vtlib_purify($_REQUEST['picture_hidden']);
	} else {
		$focus->column_fields['picture'] = $_FILES['picture']['name'];
	}
}
elseif($focus->id != '')
{
	$result = $adb->pquery("select picture from aicrm_contactdetails where contactid = ?", array($focus->id));
	$focus->column_fields['picture'] = $adb->query_result($result,0,'picture');
}
...
if($image_error=="false")
{
	...
	if(isset($_REQUEST['return_module']) && $_REQUEST['return_module'] == "Campaigns")
	{
		...
		{
			$sql = "delete from aicrm_campaigncontrel where contactid = ?";
			$adb->pquery($sql, array($focus->id));
			$sql = "insert into aicrm_campaigncontrel values (?,?)";
	...
	{
		$sql = "update aicrm_portalinfo set user_name=?,isactive=0 where id=?";
	...
	elseif($_REQUEST['portal'] != '' && $_REQUEST['email'] != '')// && $_REQUEST['mode'] != 'edit')
	{
		...
		$sql = "select id,user_name,user_password,isactive from aicrm_portalinfo";
		...
		for($i=0;$i<$adb->num_rows($result);$i++)
		{
			if($id == $adb->query_result($result,$i,'id'))
			{
				...
				{
					$sql = "update aicrm_portalinfo set user_name=?, isactive=1 where id=?";
		...
		if($insert == 'true')
		{
			...
			$sql = "insert into aicrm_portalinfo values(?,?,?,?,?,?,?,?)";
	...
	if(isset($_REQUEST['return_module']) && $_REQUEST['return_module'] == "EmailTargetList")
	{
		if(isset($_REQUEST['return_id']) && $_REQUEST['return_id'] != "")
		{
			 
			 $sql = "delete from aicrm_emailtargetlist_contactsrel where contactid = ?";
			 $adb->pquery($sql, array($focus->id));
			 $sql = "insert into aicrm_emailtargetlist_contactsrel   values (?,?)";
			 $adb->pquery($sql, array($_REQUEST['return_id'], $focus->id));
		}
	}
...
if(isset($_REQUEST['return_module']) && $_REQUEST['return_module'] == "Smartemail")
{
	if(isset($_REQUEST['return_id']) && $_REQUEST['return_id'] != "")
	{
		 $sql = "insert into aicrm_smartemail_contactsrel values (?,?)";
		 $adb->pquery($sql, array($_REQUEST['return_id'], $focus->id));
	}
}
...
$jsonMaster = [
	[
		'cardcode' => $focus->column_fields['contact_no'],
		'name' => $focus->column_fields['contactname'],
		'position' => $focus->column_fields['position'],
		'title' => '',
		'firstname' => $focus->column_fields['firstname'],
		'lastname' => $focus->column_fields['lastname'],
		'tel_1' => $focus->column_fields['mobile'],
		'email' => $focus->column_fields['email'],
	]
];
$logID = logUniqID();
// $urlMaster = 'http://localhost/WDCCCE/WB_Service_AI/batch/testApi';
if($focus->mode == 'edit'){
	$urlMaster = $serviceAPI.'SAPDI/Update/DIBPCnt?usercode=API&userpass=1234';
	logInfo(['module'=>'Contacts', 'title'=>'SAP_UPDATE_MASTER_DATA', 'logID'=>$logID, 'type'=>'Request', 'time'=>date('Y-m-d H:i:s'), 'url'=>$urlMaster, 'data'=>json_encode($jsonMaster, JSON_UNESCAPED_UNICODE)]);
	$rsMaster = postApi($urlMaster, $jsonMaster);
	logInfo(['module'=>'Contacts', 'title'=>'SAP_UPDATE_MASTER_DATA', 'logID'=>$logID, 'type'=>'Response', 'time'=>date('Y-m-d H:i:s'), 'url'=>$urlMaster, 'data'=>$rsMaster]);
} else {
	$urlMaster = $serviceAPI.'SAPDI/DIBPCnt?usercode=API&userpass=1234';
	logInfo(['module'=>'Contacts', 'title'=>'SAP_ADD_MASTER_DATA', 'logID'=>$logID, 'type'=>'Request', 'time'=>date('Y-m-d H:i:s'), 'url'=>$urlMaster, 'data'=>json_encode($jsonMaster, JSON_UNESCAPED_UNICODE)]);
	$rsMaster = postApi($urlMaster, $jsonMaster);
	logInfo(['module'=>'Contacts', 'title'=>'SAP_ADD_MASTER_DATA', 'logID'=>$logID, 'type'=>'Response', 'time'=>date('Y-m-d H:i:s'), 'url'=>$urlMaster, 'data'=>$rsMaster]);
}
/******************** SAP API ********************/
