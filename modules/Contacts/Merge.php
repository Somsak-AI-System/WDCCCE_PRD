...
{
	$query1="select aicrm_tab.name,aicrm_field.tablename,aicrm_field.columnname,aicrm_field.fieldlabel from aicrm_field inner join aicrm_tab on aicrm_tab.tabid = aicrm_field.tabid where aicrm_field.tabid in (4,6) and aicrm_field.block <> 75 order by aicrm_field.tablename and aicrm_field.presence in (0,2)";
	$params1 = array();
}
else
{
...
if(count($querycolumns) > 0)
{
	...
	$query = "select $selectcolumns from aicrm_contactdetails
				...
				inner join aicrm_customerdetails on aicrm_contactdetails.contactid = aicrm_customerdetails.customerid 
				...
				left join aicrm_account as accountAccounts on accountAccounts.accountid = aicrm_account.parentid
				...
				where aicrm_crmentity.deleted=0 and (crmentityAccounts.deleted is NULL or crmentityAccounts.deleted <> 1) and aicrm_contactdetails.contactid in(". generateQuestionMarks($mass_merge) .")";
