...
if(!empty($_REQUEST['idlist'])) {
	...
	$forCRMRecord = $record;
	$sql = "insert into aicrm_contpotentialrel values(?,?)";
...
foreach($storearray as $id)
{
	if($id != '')
	{
		if($dest_mod == 'Products')
			$adb->pquery("insert into aicrm_seproductsrel values (?,?,?)", array($forCRMRecord, $id, 'Contacts'));	
		elseif($dest_mod == 'Campaigns')
			$adb->pquery("insert into aicrm_campaigncontrel values(?,?)", array($id, $forCRMRecord));
