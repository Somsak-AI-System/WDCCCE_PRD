...
global $adb;
	...
	if(isset($_REQUEST['savemode']) && $_REQUEST['savemode'] == 'Save')
	{
		...
		{
			$params=array();
			$sqlfid="select max(folderid) from aicrm_attachmentsfolder";
			$fid=$adb->query_result($adb->pquery($sqlfid,$params),0,'max(folderid)')+1;
			$params=array();
			$sqlseq="select max(sequence) from aicrm_attachmentsfolder";
			...
			if($flag == 0)
			{
				$sql="insert into aicrm_attachmentsfolder (folderid,foldername,description,createdby,sequence)values ($fid,'".$foldername."','".$folderdesc."',".$current_user->id.",$sequence)";
		...
		{			
			...
			if($flag == 0)
			{
				$sql="update aicrm_attachmentsfolder set foldername= ? where folderid= ? ";
