...
$noteQuery = $adb->pquery("select crmid from aicrm_seattachmentsrel where attachmentsid = ?",array($fileid));
$noteid = $adb->query_result($noteQuery,0,'crmid');
$dbQuery = "SELECT * FROM aicrm_notes WHERE notesid = ? and folderid= ?";
...
if($adb->num_rows($result) == 1)
{
	...
	$pathQuery = $adb->pquery("select path from aicrm_attachments where attachmentsid = ?",array($fileid));
	...
	if($fileContent != '')
	{
		...
		$sql = "select filedownloadcount from aicrm_notes where notesid= ?";
		$download_count = $adb->query_result($adb->pquery($sql,array($fileid)),0,'filedownloadcount') + 1;
		$sql="update aicrm_notes set filedownloadcount= ? where notesid= ?";
