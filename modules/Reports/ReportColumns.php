...
/** Function to formulate the aicrm_fields for the primary modules 
 *  This function accepts the module name 
 *  as arguments and generates the aicrm_fields for the primary module as
 *  a HTML Combo values
 */
...
function getPrimaryColumnsHTML($module)
{
	...
	foreach($ogReport->module_list[$module] as $key=>$value)
	{
		if(isset($ogReport->pri_module_columnslist[$module][$value]) && !$block_listed[$value])
		{
			...
			if($id_added==false){
				$shtml .= "<option value=\"aicrm_crmentity:crmid:".$module."_ID:crmid:I\">".getTranslatedString(getTranslatedString($module).' ID')."</option>";
				$id_added=true;
			}
...
/** Function to formulate the aicrm_fields for the secondary modules
 *  This function accepts the module name
 *  as arguments and generates the aicrm_fields for the secondary module as
 *  a HTML Combo values
 */
