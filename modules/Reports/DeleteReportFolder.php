...
if($rfid != "")
{
	$records_in_folder = $adb->pquery("SELECT * from aicrm_report WHERE folderid=?",array($rfid));
	if($adb->num_rows($records_in_folder)>0){
		echo getTranslatedString('LBL_FLDR_NOT_EMPTY',"Reports");
	} else {
		$sql .= "delete from aicrm_reportfolder where folderid=?";
		$result = $adb->pquery($sql, array($rfid));
		if($result!=false)
        {
                $pquery = "delete from aicrm_report where folderid=?";
