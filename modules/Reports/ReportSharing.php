...
function getVisibleCriteria($recordid='')
{
	...
	if($recordid!='')
	{
		$result = $adb->pquery("select sharingtype from aicrm_report where reportid=?",array($recordid));
		$selcriteria=$adb->query_result($result,0,"sharingtype");
	}
	if($selcriteria == ""){
		$selcriteria = 'Public';
	}
	$filter_result = $adb->query("select * from aicrm_reportfilters");
...
function getShareInfo($recordid=''){
	global $adb;
	$member_query = $adb->pquery("SELECT aicrm_reportsharing.setype,aicrm_users.id,aicrm_users.user_name FROM aicrm_reportsharing INNER JOIN aicrm_users on aicrm_users.id = aicrm_reportsharing.shareid WHERE aicrm_reportsharing.setype='users' AND aicrm_reportsharing.reportid = ?",array($recordid));
	...
	$member_query = $adb->pquery("SELECT aicrm_reportsharing.setype,aicrm_groups.groupid,aicrm_groups.groupname FROM aicrm_reportsharing INNER JOIN aicrm_groups on aicrm_groups.groupid = aicrm_reportsharing.shareid WHERE aicrm_reportsharing.setype='groups' AND aicrm_reportsharing.reportid = ?",array($recordid));
	$noofrows = $adb->num_rows($member_query);
	if($noofrows > 0){
		for($i=0;$i<$noofrows;$i++){
