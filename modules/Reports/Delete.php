...
if(isset($_REQUEST['idlist']) && $_REQUEST['idlist']!= '')
{
	...
	{
		$own_query = $adb->pquery("SELECT reportname,owner FROM aicrm_report WHERE reportid=?",array($id_array[$i]));
...
{
	global $adb;
	$idelreportsql = "delete from aicrm_selectquery where queryid=?";
	$idelreportsqlresult = $adb->pquery($idelreportsql, array($reportid));

	$ireportsql = "delete from aicrm_report where reportid=?";
