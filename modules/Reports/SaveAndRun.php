...
$sql = "select * from aicrm_report where reportid=?";
...
if($numOfRows > 0)
{
	...
	if(isPermitted($primarymodule,'index') == "yes" && $modules_permitted == true)
	{
		...
		//echo "<pre>";print_r($sshtml[1]);echo "</pre>"; exit;
		if(is_array($sshtml))$totalhtml = $sshtml[1];
		...
		//commented to omit dashboards for aicrm_reports
...
{
	global $adb;
	$query = 'select reportid,reportname from aicrm_report where folderid=?';
