...
function getPrimaryColumns_GroupingHTML($module,$selected="")
{
    ...
    foreach($ogReport->module_list[$module] as $key=>$value)
    {
        if(isset($ogReport->pri_module_columnslist[$module][$value]) && !$block_listed[$value])
        {
			...
			if($id_added==false){
				$is_selected ='';
				if($selected == "aicrm_crmentity:crmid:".$module."_ID:crmid:I"){
					$is_selected = 'selected';
				}
				$shtml .= "<option value=\"aicrm_crmentity:crmid:".$module."_ID:crmid:I\" {$is_selected}>".getTranslatedString(getTranslatedString($module).' ID')."</option>";
				$id_added=true;
			}
