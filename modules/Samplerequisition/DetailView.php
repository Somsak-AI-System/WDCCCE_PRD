...
$log->info("Samplerequisition detail view");
...
if(isPermitted("Samplerequisition","EditView",$_REQUEST['record']) == 'yes')
...
if(isPermitted("Quotes","EditView",$_REQUEST['record']) == 'yes')
	$smarty->assign("CONVERTQUOTES","permitted");

if(isPermitted("Samplerequisition","Delete",$_REQUEST['record']) == 'yes')
	$smarty->assign("DELETE","permitted");

$smarty->assign("CONVERTMODE",'protoquote');
$smarty->assign("MODULE", $currentModule);

//Get the associated Products and then display above Terms and Conditions
$smarty->assign("ASSOCIATED_PRODUCTS",getDetailAssociatedSamplerequisition('Samplerequisition',$focus));
...
/*$display_acc = isPermitted('Accounts','DetailView',@$focus->column_fields['account_id']);
$display_con = isPermitted('Contacts','DetailView',@$focus->column_fields['contact_id']);
$smarty->assign("display_acc", $display_acc);
$smarty->assign("display_con", $display_con);*/

$approveflg = false;
$levelflg = '';
$samplerequisition_status = $focus->column_fields["samplerequisition_status"];

if($samplerequisition_status !='Approved'){
    $watermark = 1;
}else{
    $watermark = 0;
}
...
if($samplerequisition_status=="Request for Approve"){
...
$flagassign = false;
if($samplerequisition_status=="Approved" || $samplerequisition_status=="Rejected"  ){
...
//convert
/*$cquery = "SELECT convertflag FROM aicrm_quotes WHERE quoteid = '".$focus->id."' ";
...
$smarty->assign("samplerequisition_status", $samplerequisition_status);
...
$smarty->display("Samplerequisition/InventoryDetailView.tpl");
