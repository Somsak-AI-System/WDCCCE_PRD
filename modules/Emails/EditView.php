 ...
 ********************************************************************************/
/*********************************************************************************
 * $Header: /advent/projects/wesat/aicrm_crm/sugarcrm/modules/Emails/EditView.php,v 1.25 2005/04/18 10:37:49 samk Exp $
...
if(isset($_REQUEST['record']) && $_REQUEST['record'] !='')
{
	...
	$query = 'select idlists,from_email,to_email,cc_email,bcc_email from aicrm_emaildetails where emailid =?';
...
if($_REQUEST["internal_mailer"] == "true") {
	...
	if($rec_type == "record_id") {
		...
		$normal_tabs = Array('Users'=>'aicrm_users', 'Leads'=>'aicrm_leaddetails', 'Contacts'=>'aicrm_contactdetails', 'Accounts'=>'aicrm_account', 'Calendar'=>'aicrm_activity', 'Job'=>'aicrm_jobs','Quotes'=>'aicrm_quotes','HelpDesk'=>'aicrm_troubletickets');
		$cf_tabs = Array('Accounts'=>'aicrm_accountscf', 'Campaigns'=>'aicrm_campaignscf', 'Contacts'=>'aicrm_contactscf', 'Invoice'=>'aicrm_invoicecf', 'Leads'=>'aicrm_leadscf', 'Potentials'=>'aicrm_potentialscf', 'Products'=>'aicrm_productcf',  'PurchaseOrder'=>'aicrm_purchaseordercf', 'Quotes'=>'aicrm_quotescf', 'SalesOrder'=>'aicrm_salesordercf', 'HelpDesk'=>'aicrm_ticketcf');
		...
		elseif ($type == "Calendar")
			$q = "select $fieldname from $tablename where activityid=?";
		elseif ($type == "Job")
			$q = "select $fieldname from $tablename where jobid=?";
		elseif ($type == "Quotes")
			$q = "select $fieldname from $tablename where quoteid=?";
		elseif ($type == "HelpDesk")
			$q = "select $fieldname from $tablename where ticketid=?";
		else {
			// vtlib customization: Support for email-type custom field for other modules.
			$module_focus = CRMEntity::getInstance($type);
			//echo "<pre>";print_r($module_focus); echo "</pre>";
...
if($_REQUEST['reply'] == "true")
{
		...
		$query = "select from_email,idlists,cc_email,bcc_email from aicrm_emaildetails where emailid =?";
...
//echo "<pre>"; print_r($app_strings); echo "</pre>"; exit;
...
// Unimplemented until jscalendar language aicrm_files are fixed
