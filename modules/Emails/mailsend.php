...
if($_REQUEST['assigntype' == 'T'] && $_REQUEST['assigned_group_id']!='')
{
	...
	if (count($users_list) > 0) {
		$sql = "select first_name, last_name, email1, email2, yahoo_id from aicrm_users where id in (". generateQuestionMarks($users_list) .")";
		$params = array($users_list);
	} else {
		$sql = "select first_name, last_name, email1, email2, yahoo_id from aicrm_users";
		$params = array();
	}
...
{
	$query1 = "select email1 from aicrm_users where id =?";
...
//	$mail_status = send_mail('Emails',$to_email,$current_user->user_name,'',$_REQUEST['subject'],$_REQUEST['description'],$cc,$bcc,'all',$focus->id);
	...
	$query = 'update aicrm_emaildetails set email_flag ="SENT",from_email =? where emailid=?';
	...
	if($realid[1] == -1)
        {
                ...
                $emailadd = $adb->query_result($adb->pquery("select email1 from aicrm_users where id=?", array($mycrmid)),0,'email1');
                $pmodule = 'Users';
        ...
        {
		//Send mail to aicrm_account or lead or contact based on their ids
		$pmodule=getSalesEntityType($mycrmid);
		for ($j=1;$j<$nemail;$j++)
		{
			$temp=$realid[$j];
			$myquery='Select columnname from aicrm_field where fieldid = ? and aicrm_field.presence in (0,2)';
...
else
{
	global $adb;
	$date_var = date('Ymd');
	$query = 'update aicrm_activity set date_start =? where activityid = ?';
