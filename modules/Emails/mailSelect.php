...
$querystr = "select fieldid, fieldname, fieldlabel, columnname from aicrm_field where tabid=? and uitype=13 and aicrm_field.presence in (0,2)";
...
if($single_record && count($columnlists) > 0)
{
	...
	{
		case 'Accounts':
			$query = 'select accountname,'.implode(",",$columnlists).' from aicrm_account left join aicrm_accountscf on aicrm_accountscf.accountid = aicrm_account.accountid where aicrm_account.accountid = ?';
		...
		case 'Leads':
			$query = 'select concat(firstname," ",lastname) as leadname,'.implode(",",$columnlists).' from aicrm_leaddetails left join aicrm_leadscf on aicrm_leadscf.leadid = aicrm_leaddetails.leadid where aicrm_leaddetails.leadid = ?';
			$result=$adb->pquery($query, array($idlist));
		...
		case 'Contacts':
			$query = 'select concat(firstname," ",lastname) as contactname,'.implode(",",$columnlists).' from aicrm_contactdetails left join aicrm_contactscf on aicrm_contactscf.contactid = aicrm_contactdetails.contactid where aicrm_contactdetails.contactid = ?';
		    $result= $adb->pquery($query,array($idlist));
		    foreach($columnlists as $columnname)	
			{	
...
if($single_record && count($columnlists) > 0 && $val_cnt > 0){
	...
	$reponse['text'] = 'No Mail Ids';
    echo json_encode($reponse);*/

}else{
	echo 'Mail Ids not permitted';
	/*$reponse['status'] = false;	
	$reponse['text'] = 'Mail Ids not permitted';	
    echo json_encode($reponse);*/
}
