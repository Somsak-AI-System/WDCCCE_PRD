 ...
 ********************************************************************************/
/*********************************************************************************
 * $Header: /advent/projects/wesat/aicrm_crm/sugarcrm/modules/Emails/ListView.php,v 1.12 2005/04/18 10:37:49 samk Exp $
...
//to get the search aicrm_field if exists
if(isset($_REQUEST['search']) && $_REQUEST['search'] != '' && $_REQUEST['search_text'] != '')
{
	...
	if($_REQUEST['search_field'] != 'join')
		...
		$where = "(subject like '". formatForSqlLike($_REQUEST['search_text']) ."' OR aicrm_users.user_name like '". formatForSqlLike($_REQUEST['search_text']) ."')";	
}
...
{
	$list_query .= " AND aicrm_seactivityrel.crmid in (select contactid from aicrm_contactdetails) AND aicrm_emaildetails.email_flag !='WEBMAIL'";
}
if($_REQUEST['folderid'] =='3')
{
	$list_query .= " AND aicrm_seactivityrel.crmid in (select accountid from aicrm_account)";	
}
if($_REQUEST['folderid'] =='4')
{
	$list_query .= " AND aicrm_seactivityrel.crmid in (select leadid from aicrm_leaddetails)";	
}
if($_REQUEST['folderid'] =='5')
{
	$list_query .= " AND aicrm_salesmanactivityrel.smid in (select id from aicrm_users)";	
}
if($_REQUEST['folderid'] =='6')
{
	$list_query .= " AND aicrm_emaildetails.email_flag ='WEBMAIL'";	
}
