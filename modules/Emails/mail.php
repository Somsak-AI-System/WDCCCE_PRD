...
require("modules/Emails/phpmailer_607/src/PHPMailer.php");
require("modules/Emails/phpmailer_607/src/SMTP.php");
/**   Function used to send email 
  *   $module 		-- current module 
  *   $to_email 	-- to email address 
  *   $from_name	-- currently loggedin user name
  *   $from_email	-- currently loggedin aicrm_users's email id. you can give as '' if you are not in HelpDesk module
  ...
  *   $attachment	-- whether we want to attach the currently selected file or all aicrm_files.[values = current,all] - optional
  *   $emailid		-- id of the email object which will be used to get the aicrm_attachments
  */
function send_mail($module,$to_email,$from_name,$from_email,$subject,$contents,$cc='',$bcc='',$attachment='',$emailid='',$logo='')
{
	...
	//Get the email id of assigned_to user -- pass the value and name, name must be "user_name" or "id"(field names of aicrm_users aicrm_table)
	...
	//echo "<pre>"; print_r($mail_status); echo "</pre>"; exit;
	if($mail_status != 1)
	{
...
/**	Function to get the user Email id based on column name and column value
  *	$name -- column name of the aicrm_users aicrm_table 
  *	$val  -- column value 
  */
function getUserEmailId($name,$val)
{
	...
	if($val != '')
	{
		//$sql = "select email1, email2, yahoo_id from aicrm_users where ".$name." = '".$val."'";
		//done to resolve the PHP5 specific behaviour
		$sql = "SELECT email1, email2, yahoo_id ,user_name from aicrm_users WHERE status='Active' AND ". $adb->sql_escape_string($name)." = ?";
...
function addSignature($contents, $fromname)
{
	...
	$sign = nl2br($adb->query_result($adb->pquery("select signature from aicrm_users where user_name=?", array($fromname)),0,"signature"));
	if($sign != '')
	{
		$contents .= '<br><br>'.$sign;
...
/**	Function to set all the Mailer properties
  ...
  *	$attachment	-- whether we want to attach the currently selected file or all aicrm_files.
  				[values = current,all] - optional
  *	$emailid	-- id of the email object which will be used to get the aicrm_attachments - optional
  */
function setMailerProperties($mail,$subject,$contents,$from_email,$from_name,$to_email,$attachment='',$emailid='',$module='',$logo='')
{
	...
	$rs = $adb->pquery("select first_name,last_name from aicrm_users where user_name=?", array($from_name));
...
function setMailServerProperties($mail)
{
	...
	$res = $adb->pquery("select * from aicrm_systems where server_type=?", array('email'));
...
/**     Function to add all the aicrm_files as attachment with the mail object
  *     $mail -- reference of the mail object
  *     $record -- email id ie., record id which is used to get the all aicrm_attachments from database
  */
function addAllAttachments($mail,$record)
{
	...
	//Retrieve the aicrm_files from database where avoid the file which has been currently selected
	$sql = "select aicrm_attachments.* from aicrm_attachments inner join aicrm_seattachmentsrel on aicrm_attachments.attachmentsid = aicrm_seattachmentsrel.attachmentsid inner join aicrm_crmentity on aicrm_crmentity.crmid = aicrm_attachments.attachmentsid where aicrm_crmentity.deleted=0 and aicrm_seattachmentsrel.crmid=?";
	/*echo $sql;
	echo $record;
	exit;*/
...
/**	Function to get the Parent email id from HelpDesk to send the details about the ticket via email
  ...
  *	$parentid -- id of the parent ie., contact or aicrm_account
  */
...
function parseEmailErrorString($mail_error_str)
{
	...
	foreach($mail_status as $key => $val)
	{
		...
		if($status_str[1] != 1 && $status_str[1] != '')
		{
			...
			{
				$adb->println("first elseif part - status will be 0 which is the case of assigned to aicrm_users's email is empty.");
