 ...
 ********************************************************************************/
/*********************************************************************************
 * $Header: /advent/projects/wesat/aicrm_crm/sugarcrm/modules/Emails/Save.php,v 1.27 2005/04/29 08:54:38 rank Exp $
...
{
    $sql="select * from aicrm_systems where server_type = ?";
...
//Added on 09-11-2005 to avoid loading the webmail aicrm_files in Email process
if($_REQUEST['smodule'] != '')
{
    define('SM_PATH','modules/squirrelmail-1.4.4/');
    /* SquirrelMail required aicrm_files. */
...
function checkIfContactExists($mailid)
{
    ...
    global $adb;
    $sql = "select contactid from aicrm_contactdetails inner join aicrm_crmentity on aicrm_crmentity.crmid=aicrm_contactdetails.contactid where aicrm_crmentity.deleted=0 and email= ?";
...
$focus->column_fields["date_start"]= date(getNewDisplayDate());//This will be converted to db date format in save
    ...
    $ex = explode('|', $_REQUEST['parent_id']);

    foreach($ex as $i => $parent){
        ...
        if($parent != ''){
            ...
            if($_REQUEST['parent_type'] == 'Contacts'){
                $query = "SELECT * FROM aicrm_contactdetails WHERE contactid=?";
            }else if($_REQUEST['parent_type'] == 'Leads'){
                $query = "SELECT * FROM aicrm_leaddetails WHERE leadid=?";
            }else if($_REQUEST['parent_type'] == 'Accounts'){
                $query1 = "SELECT * FROM aicrm_account WHERE accountid=?";
            }

            if($_REQUEST['parent_type'] == 'Accounts'){
                $result = $adb->pquery($query1, array($a[0]));
            }else{
                $result = $adb->pquery($query, array($a[0]));
            }
            ...
            $sendmail_status = send_mail('Emails',$email,$current_user->user_name,'',$_REQUEST['subject'],$_REQUEST['description'],$_REQUEST['ccmail'],$_REQUEST['bccmail'],'all');
        }
    }

    $alert = "<script>alert('Send Email Complete!')</script>";
    echo  $alert;
...
if(isset($_REQUEST['send_mail']) && $_REQUEST['send_mail'] && $_REQUEST['parent_id'] != '')
{
    ...
    if($user_mail_status != 1){
        $query  = "select crmid,attachmentsid from aicrm_seattachmentsrel where crmid=?";
        ...
        for($i=0; $i<$numOfRows; $i++)
        {
            ...
            if($attachmentsid > 0)
            {
                $query1="delete from aicrm_crmentity where crmid=?";
                $adb->pquery($query1, array($attachmentsid));
            }

            $crmid=$adb->query_result($result,0,"crmid");
            $query2="delete from aicrm_crmentity where crmid=?";
            $adb->pquery($query2, array($crmid));
        }

        $query = "delete from aicrm_emaildetails where emailid=?";
