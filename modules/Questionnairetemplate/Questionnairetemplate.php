...
class Questionnairetemplate extends CRMEntity {
// Account is used to store aicrm_account information.
	var $log;
	var $db;
	var $table_name = "aicrm_questionnairetemplate";
	var $table_index= 'questionnairetemplateid';
	var $_images_path= 'upload/questionnairetemplate/';
	var $table_comment = "aicrm_questionnairetemplatecomments";

	var $tab_name = Array('aicrm_crmentity','aicrm_questionnairetemplate','aicrm_questionnairetemplatecf');
	var $tab_name_index = Array('aicrm_crmentity'=>'crmid','aicrm_questionnairetemplate'=>'questionnairetemplateid','aicrm_questionnairetemplatecf'=>'questionnairetemplateid','aicrm_questionnairetemplatecomments'=>'questionnairetemplateid');
	/**
	 * Mandatory table for supporting custom fields.
	 */
	var $customFieldTable = Array('aicrm_questionnairetemplatecf', 'questionnairetemplateid');
	var $column_fields = Array();

	var $sortby_fields = Array('questionnairetemplateid','questionnairetemplate_name','smownerid');

	var $list_fields = Array(
		'หมายเลขเทมเพลตแบบสอบถาม	'=>Array('Questionnairetemplate'=>'questionnairetemplate_no'),
		'ชื่อเทมเพลตแบบสอบถาม'=>Array('Questionnairetemplate'=>'questionnairetemplate_name'),
		'ผู้รับผิดชอบ' => Array('crmentity'=>'smownerid')
	);

	var $list_fields_name = Array(
		'หมายเลขเทมเพลตแบบสอบถาม	'=>'questionnairetemplate_no',
		'ชื่อเทมเพลตแบบสอบถาม'=>'questionnairetemplate_name',
		'ผู้รับผิดชอบ'=>'assigned_user_id'
     );

	var $list_link_field= 'questionnairetemplate_no';
	...
	var $search_fields = Array(
		'หมายเลขเทมเพลตแบบสอบถาม	'=>Array('aicrm_questionnairetemplate'=>'questionnairetemplate_no'),
		'ชื่อเทมเพลตแบบสอบถาม'=>Array('aicrm_questionnairetemplate'=>'questionnairetemplate_name'),
		'ผู้รับผิดชอบ' => Array('crmentity'=>'smownerid')
	);
	...
	var $mandatory_fields = Array('assigned_user_id','createdtime' ,'modifiedtime','questionnairetemplateid');

	function Questionnairetemplate()
	{
		$this->log =LoggerManager::getLogger('Questionnairetemplate');
		$this->db = PearDatabase::getInstance();
		$this->column_fields = getColumnFields('Questionnairetemplate');
	}
	function save_module()
	{
		global $adb;
		$this->insertIntoAttachment($this->id,'Questionnairetemplate');
		$this->insertIntoCommentTable("aicrm_questionnairetemplatecomments",'questionnairetemplateid');
	...
	function getCommentInformation($crmid)
	{
		...
		$sql = "select * from aicrm_questionnairetemplatecomments where questionnairetemplateid=? order by createdtime desc";
	...
	function getSortOrder()
	{
		...
		if(isset($_REQUEST['sorder']))
			...
			$sorder = (($_SESSION['QUESTIONNAIRETEMPLATE_SORT_ORDER'] != '')?($_SESSION['QUESTIONNAIRETEMPLATE_SORT_ORDER']):($this->default_sort_order));
	...
	function getOrderBy()
	{
		...
		if (isset($_REQUEST['order_by']))
			...
			$order_by = (($_SESSION['QUESTIONNAIRETEMPLATE_ORDER_BY'] != '')?($_SESSION['QUESTIONNAIRETEMPLATE_ORDER_BY']):($use_default_order_by));
	...
	function generateReportsSecQuery($module,$secmodule){
		$query = $this->getRelationQuery($module,$secmodule,"aicrm_questionnairetemplate","questionnairetemplateid");
		$query .=" left join aicrm_crmentity as aicrm_crmentityQuestionnairetemplate on aicrm_crmentityQuestionnairetemplate.crmid=aicrm_Questionnairetemplate.Questionnairetemplateid and aicrm_crmentityQuestionnairetemplate.deleted=0 
			left join aicrm_campaign on aicrm_campaign.campaignid = aicrm_questionnairetemplate.campaignid
			left join aicrm_questionnairetemplatecf on aicrm_questionnairetemplatecf.questionnairetemplateid = aicrm_crmentityQuestionnairetemplate.crmid 
			left join aicrm_groups as aicrm_groupsQuestionnairetemplate on aicrm_groupsQuestionnairetemplate.groupid = aicrm_crmentityQuestionnairetemplate.smownerid
			left join aicrm_users as aicrm_usersQuestionnairetemplate on aicrm_usersQuestionnairetemplate.id = aicrm_crmentityQuestionnairetemplate.smownerid"; 
		return $query;
	}
	...
	function create_export_query($where)
	{
		...
		$sql = getPermittedFieldsQuery("Questionnairetemplate", "detail_view");
		...
		$query = "SELECT $fields_list,case when (aicrm_users.user_name not like '') then aicrm_users.user_name else aicrm_groups.groupname end as user_name
				...
				INNER JOIN aicrm_questionnairetemplate ON aicrm_questionnairetemplate.questionnairetemplateid = aicrm_crmentity.crmid
				...
				";
		$query .= setFromQuery("Questionnairetemplate");
		...
		if($is_admin==false && $profileGlobalPermission[1] == 1 && $profileGlobalPermission[2] == 1 && $defaultOrgSharingPermission[6] == 3)
		{
			...
			$query = $query." ".getListViewSecurityParameter("Questionnairetemplate");
		}
	...
	function get_questionnaireanswer($id, $cur_tab_id, $rel_tab_id, $actions=false) {
        ...
        $query = "
			...
			AND aicrm_questionnaireanswer.questionnairetemplateid = ".$id;
	...
	function setRelationTables($secmodule){
		$rel_tables = array (
			"Documents" => array("aicrm_senotesrel"=>array("crmid","notesid"),"aicrm_questionnairetemplate"=>"questionnairetemplateid"),
		);
	...
	function unlinkRelationship($id, $return_module, $return_id) {
		...
		if($return_module == 'Accounts' ) {
			$this->trash('Questionnairetemplate',$id);
		} else {
	...
	function insertIntoAttachment($id,$module){
		...
		foreach($_FILES as $fileindex => $files)
		{
			if($files['name'] != '' && $files['size'] > 0)
			{
				...
				if (strpos($fileindex, 'image_footer') !== false) {
				    $files['flag'] = 'S';
		...
		if($module == 'Questionnairetemplate' && $_REQUEST['del_file_list'] != '')
		{
