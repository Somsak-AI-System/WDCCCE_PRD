...
$data_template = array();
//echo $_REQUEST['data_template']; exit;
$data_template = json_decode($_REQUEST['data_template'], TRUE); 
//echo $_REQUEST['data_template']; exit;
...
$focus->save("Questionnairetemplate");

$return_id = $focus->id;
sleep(5);
require_once("library/Questionnairetemplate.php");
include("config.inc.php");
$Questionnaire = new libQuestionnairetemplate($dbconfig);
$Questionnaire->save_template($data_template,$focus->id,$_REQUEST['mode']);

$parenttab = getParentTab();
if(isset($_REQUEST['return_module']) && $_REQUEST['return_module'] != "") $return_module = vtlib_purify($_REQUEST['return_module']);
else $return_module = "Questionnairetemplate";
...
if(isset($_REQUEST['return_module']) && $_REQUEST['return_module'] == "QuestionnairetemplateList")
{
	if(isset($_REQUEST['return_id']) && $_REQUEST['return_id'] != "")
	{
		 
		 $sql = "delete from aicrm_questionnairetemplatelist_questionnairetemplaterel  where questionnairetemplateid  = ?";
		 $adb->pquery($sql, array($focus->id));
		 $sql = "insert into aicrm_questionnairetemplatelist_questionnairetemplaterel  values (?,?)";
