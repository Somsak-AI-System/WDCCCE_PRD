...
//get email====================================================================================
	...
	if($crmid!=""){
		...
		and aicrm_smartemail.email_status='Schedule'
	...
	//Select แบบไม่มี crmid ส่งเข้ามา เช็ค active (Run Schedule)
		...
		$sql="
		  ...
		  from aicrm_smartemail
		  left join aicrm_crmentity on aicrm_crmentity.crmid = aicrm_smartemail.smartemailid
		  where aicrm_smartemail.email_status='Schedule'
		  and aicrm_crmentity.deleted =0
		  and aicrm_smartemail.email_setup=0
	...
	foreach ($data as $key => $value) {
		//Set ตัวแฟร
		$new_table="tbt_email_log_smartemailid_".$value['smartemailid'];
		$date_start=$value['email_start_date']." ".$value['email_start_time'];
		$campaignid=$value['smartemailid'];
		$campaign_name=$value['smartemail_name'];
		...
		//Check Path Folder
		$folder_name=date('Ymd',strtotime($value['email_start_date']))."_".$value['smartemailid'];
		if (!file_exists($path."/EDM/".$folder_name)){
			if (!mkdir($path."/EDM/".$folder_name, 0777, true)){}
		}

		//Set ตัวแปร Body
		$email_body=$value['email_message'];
		//Check ตัวแปรที่ส่งแบบ Presonal ($-------$) ครอบหน้าหลัง
		$str_arr = getInbetweenStrings($systax, $systax, $value['email_message']);
		$replace_arr = array();

		/*--/////////////////เพิ่ม data table แบ่ง group-personal //////////////////////////////--*/
		 $massage=$value['email_message'];                 
		 if(strstr($massage,$systax))
		 {
		 	//personal
		  	$sql="update aicrm_smartemail set email_type = 'personal' where smartemailid=".$value['smartemailid']."";
			$myLibrary_mysqli->Query($sql);
		 }else{
		 	//group
		   	$sql="update aicrm_smartemail set email_type = 'group' where smartemailid=".$value['smartemailid']."";
		 	$myLibrary_mysqli->Query($sql);          
		}
		/*--////////////////////////////////////////////////////////////////////////////////////--*/
		...
		{	
			...
			'0'
			,'".$email_body."'
			,CASE WHEN b.email_consent = '0' THEN 1 ELSE 0 END
			...
			where 1
			and d.deleted=0	
			and a.smartemailid='".$value['smartemailid']."' ";
			$myLibrary_mysqli->Query($sql); 
			...
			TRIM(REPLACE(email1,'\n','')) as email,
			'Accounts' as module
			,CASE WHEN b.email_consent = '0' THEN 1 ELSE 0 END AS email_consent
			...
			and a.smartemailid='".$value['smartemailid']."'
			";
			$dataacc = $myLibrary_mysqli->select($sql);

			foreach ($dataacc as $ka => $va) {
				$email=split(",",$va['email']);

				foreach ($email as $ke) {
					if($ke !=""){
						$sql="
						insert into ".$new_table."(campaignid,email_marketingid,emailtargetlistid,from_id,to_name,to_email,from_module,status,message,unsubscribe)
						values('".$value['smartemailid']."','0','0','".$va['id']."','".$va['name']."','".trim(str_replace("\n", '', $ke))."','".$va['module']."','0','".$email_body."','".$va['email_consent']."')";
						$myLibrary_mysqli->Query($sql); 
					}else{
						$sql="
						insert into ".$new_table."(campaignid,email_marketingid,emailtargetlistid,from_id,to_name,to_email,from_module,status,message,unsubscribe)
			...
			foreach($str_arr as $arr){
				...
				if(in_array($ex[0], array('aicrm_account','aicrm_accountscf','aicrm_crmentity'))){
					$sql = 'select aicrm_account.accountid, '. $arr .'
							...
							and aicrm_smartemail_accountsrel.smartemailid='.$value['smartemailid'];
	
					$res = $myLibrary_mysqli->select($sql);
					foreach($res as $rs){
					...
					$sql = 'select aicrm_contactdetails.contactid, '. $arr .'
							...
							where 1 and aicrm_crmentity.deleted=0 and aicrm_contactdetails.email<>"" and aicrm_smartemail_contactsrel.smartemailid='.$value['smartemailid'];
					...
					$sql = 'select aicrm_leaddetails.leadid, '. $arr .'
							...
							where 1
							and aicrm_crmentity.deleted=0
							and aicrm_smartemail_leadsrel.smartemailid='.$value['smartemailid'];
			...
			foreach($replace_arr as $id => $a){
					...
					$new_string = str_ireplace($find, $replace, $value['email_message']);
					...
					switch($a['module']){
						case'Accounts':
							...
							$dataacc = $myLibrary_mysqli->select($sql_acc);

							foreach ($dataacc as $ka => $va) {
								$email=split(",",$va['email1']);
								foreach ($email as $ke) {
									if($ke != ""){
										$sql="
										insert into ".$new_table."(campaignid,email_marketingid,emailtargetlistid,from_id,to_name,to_email,from_module,status,message,unsubscribe)
										values('".$value['smartemailid']."','0','0','".$va['id']."','".$va['accountname']."','".trim(str_replace("\n", '', $ke))."','".$va['module']."','0','".$new_string."','".$va['email_consent']."')";
										$myLibrary_mysqli->Query($sql); 
									}
								}
							}
		...
		$record_smartemail = $myLibrary_mysqli->select($new_sql);

		foreach($record_smartemail as $k => $v){
			$id=$v['id'];
			$campaignid=$v['campaignid'];
			$email_marketingid=$v['email_marketingid'];
			$emailtargetlistid=$v['emailtargetlistid'];
			...
			$url_click_view.=$url_path.'EDM/'.$folder_name.'/'.$folder_name.'.php?id='.$id.'*campaignid='.$campaignid;
			...
			$unsubscribe_url = $url_path.'/unsubscribe.php?accountid='.$from_id.'&campaignid='.$campaignid;
	        ...
	        $unsubscribe_link.='</td>
						...
						<tr>
							<td height="50" align="center" style="font-size: 0px;" mce_style="font-size:0px;">
								...
								หากท่านไม่ต้องการรับข่าวสารจากทางเราอีก <a href="'.$unsubscribe_url.'" style="color:#fd8103; font-family: tahoma; font-size: 12px;">กรุณายกเลิกการรับข่าวสารที่นี่</a>
								<br mce_bogus="1" /></div>
							</td>
						</tr>
					</tbody>
			...
			$body_html=$body_html.$unsubscribe_link.$url_load_open;
		    //Set Email Body
			
			//Update message เก็บ code html
			$sql_update="update tbt_email_log_smartemailid_".$val[1]." set message = '".$body_html."' where id='".$id."' ";
			$myLibrary_mysqli->Query($sql_update);
			//echo $sql_update; exit;

		}//foreach $record_smartemail
		
		$subject = $value['smartemail_name'];
		...
		function doQuery(click_link) {
			...
			<?
				...
				if (isset($_GET["campaignid"])){
					$varcampaignid = $_GET["campaQuestionnairetemplateignid"];
				}else{
					$varcampaignid = "0";
				}
		...
		foreach ($data_d as $k_d => $v_d) {
			$sql="update ".$new_table." set active=1,report=1 ,status = 0 ,duplicate = 0 where 1 and id='".$v_d['id']."';";
			$myLibrary_mysqli->Query($sql);
		}
		
		//แก้ตัวอักษรแปลกๆๆ //เช็ค format ก่อนส่ง	
		$sql="
		select 
		id,
		to_email 
		...
		foreach ($data_d as $k_re => $v_re) {
			$to_email=$v_re['to_email'];
			$to_email_old=$v_re['to_email'];
			//แก้ตัวอักษรแปลกๆๆ 
			...
			if(eregi("^[_a-z0-9-]+(\.[_a-z0-9-]+)*@[a-z0-9-]+(\.[a-z0-9-]+)+$", $to_email)){
				...
				$sql="update ".$new_table." set active=1,report=1,mistype_email=1,to_email_old='".$to_email_old."',to_email='".$to_email."' where 1 and id='".$v_re['id']."';";
				$myLibrary_mysqli->Query($sql);
			}
			//เช็ค format ก่อนส่ง
		...
		$sql="update aicrm_smartemail set email_setup =1,preparingtime= '".date('Y-m-d H:i:s')."' where smartemailid='".$campaignid."'";
		if($myLibrary_mysqli->Query($sql)){};
