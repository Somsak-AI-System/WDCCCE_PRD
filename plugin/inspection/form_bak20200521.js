...
function AddPmListValidate(){
	var jQuerycheckoutForm = jQuery('#add-pm-list-form').validate({
...
function save_work(){
	...
	// à¸•à¸£à¸§à¸ˆà¸ªà¸­à¸šà¸‚à¹‰à¸­à¸¡à¸¹à¸¥ PM à¸„à¸£à¸šà¸«à¸£à¸·à¸­à¹„à¸¡à¹ˆ
	...
	if(SaveWorkValidate()){
		...
		jQuery.bigBox({
			title : "Error",
			content : "à¸‚à¹‰à¸­à¸¡à¸¹à¸¥à¹„à¸¡à¹ˆà¸–à¸¹à¸à¸•à¹‰à¸­à¸‡ à¸•à¸£à¸§à¸ˆà¹€à¸Šà¹‡à¸„à¸‚à¹‰à¸­à¸¡à¸¹à¸¥à¸­à¸µà¸à¸„à¸£à¸±à¹‰à¸‡",
			color : "#C46A69",
			icon : "fa fa-warning shake animated"
		});
	}
...
function CheckToleranceMode(TbID){
	...
	if(ToleranceType == 6){
		...
		jQuery('#'+TbID+'-tolerance-error').text('Error');
		return false;
	}
}
...
function SaveFormValidate(){
	var $checkoutForm = jQuery('#design-form').validate({
		rules : {
		},
		errorPlacement : function(error, element) {
			error.insertAfter(element);
	...
	jQuery(".uncersource_settings").each(function( index ){
		if( jQuery(this).val() == 0 ){
			...
			if( jQuery("#"+TbID+"_report_type").val() == 'average' ){
				...
				count_error++;
				error_content += '- "' + jQuery('#'+TbID+'_parameter').val() + '" à¹„à¸¡à¹ˆà¹„à¸”à¹‰à¸•à¸±à¹‰à¸‡à¸„à¹ˆà¸² Uncertainty Source<br />';
			}
		}
	});
...
function FormUnitBox(TbID, ParamID){
	...
	jQuery.get( "plugin/inspection/load.php?data=form-unit-box&param_id="+ParamID+"&tbid="+TbID , function( data ) {
		...
		if(retobj.result == 'complete'){
			...
			jQuery('#'+TbID+'_unit_box').html('<div style="color:#FF0000; text-align:center">Error...</div>');
		}
