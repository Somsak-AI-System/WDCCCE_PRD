<?php
	...
	if($_REQUEST["myaction"]=="search"){
		if($_REQUEST["search_pname"]!=""){
			...
			$where.=" and aicrm_premuimproduct.premuimproduct_no like '%".$_REQUEST["search_pname"]."%'";
		}else{
			$_SESSION["search_pname"]="";
		}
		if($_REQUEST["search_pno"]!=""){
			$_SESSION["search_pno"]=$_REQUEST["search_pno"];
			$where.=" and aicrm_premuimproduct.premuimproduct_no like '%".$_REQUEST["search_pno"]."%'";
		}else{
			$_SESSION["search_pno"]="";
		}
	}else{
		if($_SESSION["search_pname"]!=""){
			...
			$where.=" and aicrm_premuimproduct.premuimproduct_name like '%".$_SESSION["search_pname"]."%'";
		}else{
			$_SESSION["search_pname"]="";
		}
		if($_SESSION["search_pno"]!=""){
			$_SESSION["search_pno"]=$_SESSION["search_pno"];
			$where.=" and aicrm_premuimproduct.premuimproduct_no like '%".$_SESSION["search_pno"]."%'";
		}else{
			$_SESSION["search_pno"]="";
		}
	}
	...
	<tr>
		<td>
			<table width="100%" border="0" cellpadding="0" cellspacing="0">
				<tr>
					<td class="moduleName" width="80%" style="padding-left:10px;">Premium</td>
					<td  width=30% nowrap class="componentName" align=right>AICRM</td>
				</tr>
			</table>
			...
			<table width="100%" cellpadding="5" cellspacing="0" border="0"  class="homePageMatrixHdr">
				<tr>
					<td style="padding:0px;" >
                   	  <form name="basicSearch" method="post" enctype="multipart/form-data" action="search_pop_premium_mySetup_Tab2.php">
						...
						<tr>
						  <td width="20%" class="dvtCellLabel small">หมายเลขสินค้าพรีเมี่ยม</td>
						  <td width="30%" class="dvtCellLabel"><input type="text" name="search_pno" id="search_pno" class="txtBox" value="<?=$_SESSION["search_pno"]?>"></td>
						  <td width="20%" class="dvtCellLabel small">ชื่อสินค้าพรีเมี่ยม</td>
						  <td width="30%" class="dvtCellLabel"><input type="text" name="search_pname2" id="search_pname2" class="txtBox" value="<?=$_SESSION["search_pname"]?>"></td>
						  </tr>
						<tr>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="small">
    <!--<tr>
        <td style="padding-left:5px;" align="left"><input class="crmbutton small save" type="button" value="Select Premium" onClick="if(InventorySelectAll_Premium('Promotion',image_pth))window.close();"/></td>
   	...
   	<tr>
		...
		left join aicrm_premuimproductcf on aicrm_premuimproduct.premuimproductid=aicrm_premuimproductcf.premuimproductid
		left join aicrm_crmentity on aicrm_crmentity.crmid=aicrm_premuimproduct.premuimproductid
		";
		$sql.=" where
		...
		$sql.="
		order by aicrm_premuimproduct.premuimproductid
		";
		...
		//echo $sql;
		//echo $_REQUEST["param_page"];
		?>
		<table style="background-color: rgb(204, 204, 204);" class="small" border="0" cellpadding="5" cellspacing="1" width="100%">
		<tbody>
		<tr>
							<!-- <td class="lvtCol" width="5%"><input type="checkbox" name="select_all" value="" onClick=toggleSelect(this.checked,"selected_id")></td> -->
							<td width="10%" class="lvtCol"><a href='javascript:;' onClick="getListViewSorted_js('Products','&curr_row=1&popuptype=inventory_prod&select=enable&return_module=Premiumproduct&order_by=premiumproduct_no&sorder=DESC')" class='listFormHeaderLinks'>หมายเลขสินค้าพรีเมี่ยม&nbsp;</a></td>

							<td width="25%" class="lvtCol"><a href='javascript:;' onClick="getListViewSorted_js('Products','&curr_row=1&popuptype=inventory_prod&select=enable&return_module=Premiumproduct&order_by=premiumproduct_name&sorder=DESC')" class='listFormHeaderLinks'>ชื่อสินค้าพรีเมี่ยม</a><a href='javascript:;' onClick="getListViewSorted_js('Products','&curr_row=1&popuptype=inventory_prod&select=enable&return_module=Premiumproduct&order_by=premiumproduct_name&sorder=DESC')" class='listFormHeaderLinks'> &nbsp;<img src ='../../themes/softed/images/arrow_down.gif' border='0'></a></td>
                            
                            <td width="10%" class="lvtCol"><a href='javascript:;' onClick="getListViewSorted_js('Products','&curr_row=1&popuptype=inventory_prod&select=enable&return_module=Premiumproduct&order_by=unit&sorder=DESC')" class='listFormHeaderLinks'>หน่วยการนับ&nbsp;</a></td>

							<td width="10%" class="lvtCol"><a href='javascript:;' onClick="getListViunitewSorted_js('Products','&curr_row=1&popuptype=inventory_prod&select=enable&return_module=Premiumproduct&order_by=stock_quantity&sorder=DESC')" class='listFormHeaderLinks'>จำนวนสินค้า&nbsp;</a></td>

							<td width="10%" class="lvtCol"><a href='javascript:;' onClick="getListViewSorted_js('Products','&curr_row=1&popuptype=inventory_prod&select=enable&return_module=Premiumproduct&order_by=premium_price&sorder=DESC')" class='listFormHeaderLinks'>ราคาสินค้า&nbsp;</a></td>

                            <!-- <td width="60%" class="lvtCol"><a href='javascript:;' onClick="getListViewSorted_js('Products','&curr_row=1&popuptype=inventory_prod&select=enable&return_module=Opportunity&order_by=cf_1804&sorder=DESC')" class='listFormHeaderLinks'>Premium Detail&nbsp;</a></td> -->
			        ...
			        <tr bgcolor=white onMouseOver="this.className='lvtColDataHover'" onMouseOut="this.className='lvtColData'"  >
			   			<!-- <td valign="top"><input type="checkbox" name="selected_id" value="<?=$value['premiumid']?>" onClick=toggleSelectAll(this.name,"select_all")></td> -->
			   			<td valign="top"><?=$value['premuimproduct_no']?></td>
			   			<td valign="top"><a href="javascript:window.close();" id='popup_product_<?=$value['premuimproductid']?>' onclick='return_premium_tab2("<?=$value['premuimproductid']?>", "<?=$value['premuimproductid']?>", "<?=$value['premuimproduct_name']?>","<?=$value['unit']?>","<?=$value['stock_quantity']?>","<?=$value['premium_price']?>","<?=$row_id?>","mySetup_Tab2","<?=$value['premuimproduct_name']?>","<?=$value['premium_product_code']?>");' vt_prod_arr='{"premuimproductid":"<?=$value['premuimproductid']?>","premuimproductid":"<?=$value['premuimproductid']?>","premuimproduct_name":"<?=$value['premuimproduct_name']?>","uom":"<?=$value['unit']?>","quantity":"<?=$value['stock_quantity']?>","listprice":"<?=$value['premium_price']?>","row_id":"<?=$row_id?>","tab_name":"mySetup_Tab2","premuimproduct_name":"<?=$value['premuimproduct_name']?>","premium_product_code":"<?=$value['premium_product_code']?>"}'  ><?=$value['premuimproduct_name']?></a></td>

			   			<td valign="top" align="center"><?=$value['unit']?></td>
						<td valign="top" align="center"><?=number_format($value['stock_quantity'],0)?></td>
						<td valign="top" align="right"><?=number_format($value['premium_price'],2)?></td>
                    </tr>
                    <?
					}
					?>
