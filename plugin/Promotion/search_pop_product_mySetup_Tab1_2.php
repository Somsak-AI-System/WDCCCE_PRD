			...
			<table width="100%" cellpadding="5" cellspacing="0" border="0"  class="homePageMatrixHdr">
				<tr>
					<td style="padding:0px;" >
                   	  <form name="basicSearch" method="post" enctype="multipart/form-data" action="search_pop_product_mySetup_Tab1_2.php">
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="small">
	<tr>
					<td style="padding-left:5px;" align="left"><input class="crmbutton small save" type="button" value="Select Products" onClick="if(InventorySelectAll_mySetup_Tab1_2('Quotation',image_pth))window.close();"/></td>
   	...
   	<tr>
		...
		";
		/*,cf_1642 as std_price
		,cf_1644 as member_price
		,cf_1124
		,cf_1641*/
		$sql.=" where  
			...
			<tr>
		    	...
		    	<td class="lvtCol"><a href='javascript:;' onClick="getListViewSorted_js('Products','&curr_row=1&popuptype=inventory_prod&select=enable&return_module=Opportunity&order_by=cf_1804&sorder=DESC')" class='listFormHeaderLinks'>ราคาขาย&nbsp;</a></td>
			        ...
			        <tr bgcolor=white onMouseOver="this.className='lvtColDataHover'" onMouseOut="this.className='lvtColData'"  >
		   			...
		   			<td><a href="javascript:window.close();" id='popup_product_<?=$value['productid']?>' onclick='set_return_inventory_mySetup_Tab1_2("<?=$value['productid']?>", "<?=str_replace('"','',$value['productname'])?>", "<?=$value['sellingprice']?>", "<?=$value['stockqty']?>","tax1=7.000,tax2=10.000,tax3=12.500","<?=$_REQUEST["curr_row"]?>","","::","<?=$value['unit']?>");' vt_prod_arr='{"entityid":"<?=$value['productid']?>","prodname":"<?=str_replace('"','',$value['productname'])?>","unitprice":"<?=$value['sellingprice']?>","qtyinstk":"<?=$value['stockqty']?>","taxstring":"tax1=7.000,tax2=10.000,tax3=12.500","rowid":"<?=$_REQUEST["curr_row"]?>","desc":"","subprod_ids":"::","usageunit":"<?=$value['unit']?>"}' ><?=$value['productname']?></a></td>
