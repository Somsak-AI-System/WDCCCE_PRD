		...
		<html>
		<head>
		<title>Test send smart email</title>
		...
		function doQuery(click_link) {
			...
			var strURL = "http://localhost/moai//EDM/update_click_html.php?id="+ id+"&click_link="+click_link+"&campaignid="+campaignid+"&marketid="+marketid;
			//alert(strURL);
				if (req){
				...
				<tbody>
					...
					<tr>
						<td height="50" align="center" style="font-size: 0px;" mce_style="font-size:0px;">
							...
							If you prefer not to receive email. Please  <a style="color:#fd8103;" target="_blank" href="http://localhost/moai//EDM/unsub.php?id=<?=$_GET["id"]?>&campaignid=<?=$_GET["campaignid"]?>&marketid=<?=$_GET["marketid"]?>&module=<?=$_GET["module"]?>&crmid=<?=$_GET["crmid"]?>"  onClick="doQuery(16);">Click Here</a> to unsubscribe<br mce_bogus="1" /></div>
						</td>
					</tr>
				</tbody>
			</table>	
