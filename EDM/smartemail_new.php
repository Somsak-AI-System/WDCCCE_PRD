...
//get email====================================================================================
	...
	for($i=0;$i<count($campaign);$i++){
		...
		$str_arr = getInbetweenStrings('#', '#', $campaign[$i][7]);

		$replace_arr = array();
		foreach($str_arr as $arr){
			$ex = explode('.', $arr);

			if(in_array($ex[0], array('aicrm_account'))){
				...
				foreach($res as $rs){
					...
					$replace_arr[$rs[0]]['#'.$arr.'#'] = $rs[1];
				}
			}else if(in_array($ex[0], array('aicrm_contactdetails','aicrm_contactaddress'))){
				$sql = 'select aicrm_contactdetails.contactid, '. $arr .'
						from aicrm_smartemail_contactsrel
						left join aicrm_contactdetails on aicrm_smartemail_contactsrel.contactid = aicrm_contactdetails.contactid
			...
			}else if(in_array($ex[0], array('aicrm_leaddetails','aicrm_leadscf'))){
			...
			}else if(in_array($ex[0], array('aicrm_opportunity'))){
...
//			echo $new_string;
			switch($a['module']){
				case'Accounts':
					$sql="insert into ".$new_table."(campaignid,email_marketingid,emailtargetlistid,from_id,to_name,to_email,to_message,from_module,status)
					select
					'". $campaign[$i][0] ."',
					0, 0,
					'". $id ."',
