...
$sql.= "now(),'".$link."',".$id.",'".$click_type."','".$device."','".$useragent."','".$data[0][0]."','".$data[0][1]."','".$data[0][2]."','".$data[0][3]."')";
//echo $sql;exit;
...
$genarate->query($sql);		
//เก็บการ click 
$sql="select * from tbt_report_tab_2 where campaign_id='".$campaignid."' and link_id='".$link."'";
//echo $sql;exit;
$data=$genarate->process($sql,"all");
if(count($data)>0){
}else{
	if($link=="1"){
		$sql="insert into tbt_report_tab_2 (campaign_id,link_id,link_name,total_click,status)values('".$campaignid."','".$link."','หากท่านไม่สามารถอ่านอีเมล์ฉบับนี้ได้ กรุณา คลิกที่นี่','0','Active');";
		$genarate->query($sql);	
	}else if($link=="3"){
		$sql="insert into tbt_report_tab_2 (campaign_id,link_id,link_name,total_click,status)values('".$campaignid."','".$link."','หากท่านไม่ต้องการรับข่าวสารจากทางเราอีก กรุณายกเลิกการรับข่าวสารที่นี่','0','Active');";
		$genarate->query($sql);	
	}else{
		$sql="insert into tbt_report_tab_2 (campaign_id,link_id,link_name,total_click,status)values('".$campaignid."','".$link."','".$url."','0','Active');";
		$genarate->query($sql);	
	}
}
//echo $sql;exit;
