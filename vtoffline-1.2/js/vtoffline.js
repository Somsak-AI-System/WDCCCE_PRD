	...
	Init : function() {
		...
		if(!vtoffline.checkLogin()) {
			...
			if(!vtoffline.modules) {
				...
				vtws.doListTypes(vtoffline.IndexPage.Init1);
			}
			else vtoffline.IndexPage.Init2(null);
		}
	},
	Init1 : function(result, args) {
		var modules = [];
		for(modulekey in result) {
			if(typeof(result[modulekey]['isEntity']) == 'undefined' ||
				result[modulekey]['isEntity']) {
				modules.push(result[modulekey]['name']);
			}
		}
		vtoffline.IndexPage.Init2(modules, args);
	},
