            ...
            <div class="row" style="margin-top: 5px;">
                <div class="col-sm-12"> 
                    ...
                    <div class="col-sm-12" style="margin-left: -14px;" id="fieldUL">
                        {foreach key=headerfield item=detailfield from=$FILTERBYFIELD}

						    <label class="container2">
						        <span>{$detailfield.fieldlabel}</span>
						        <input type="checkbox" name="namefield" value="{$detailfield.fieldid}" checked/>
            ...
            <div class="row" style="margin-top: 5px;">
                <div class="col-sm-12">
                    ...
                    <div class="col-sm-12" style="margin-left: -14px;" id="usersUL">
                		{foreach key=headeruser item=detailuser from=$FILTERBYUSER}

						    <label class="container2">
						        <span>{$detailuser.user_name}</span>
						        <input type="checkbox" name="nameuser" value="{$detailuser.user_name}" checked/>
						        <span class="checkmark"></span>
						    </label>
...
{foreach key=header item=detail from=$TIMELINELIST}

{assign var=rel_mod value=$header}
{assign var="HEADERLABEL" value=$header|@getTranslatedString:$rel_mod}

<table width=100% style="background-color: rgb(247, 247, 247);border-radius: 1em;margin-bottom: 10px; display: block;" class="small font-12 mix timelinetable {$detail.fieldid} {$detail.user_name}" data-filter="{$header}">
	<tr style="height:25px;line-height: 2;background-color: unset;" bgcolor=white>
		<td align="left" class="timeline_" style="padding-left: 20px;">
			...
			<label class="format_date">{$detail.format_date}</label>
			
			{if $detail.profile_image neq ''}
		 		<img src="{$detail.profile_image}" style="width: 25px; height: 25px;border-radius: 50%;vertical-align: text-bottom;">
		 	{else}
		 		<img src="themes/softed/images/profile.png" style="width: 25px; height: 25px;border-radius: 50%;vertical-align: text-bottom;">
		 	{/if}
			
			<label class="user_name font-weight">&nbsp;{$detail.user_name}</label>

			{if $detail.action eq 'create'}
				<label class="create_the"> Created the</label>&nbsp;<label class="font-weight">{$MODULE}</label>
			{else}
				<label class="edit_the"> Updated the</label>&nbsp;<label class="font-weight">{$detail.fieldlabel}</label>
			{/if}
