...
{if $MODULE neq 'Products' && $MODULE neq 'Services' && $MODULE neq 'Vendors' && $MODULE neq 'PriceList'}
...
{if $MODULE eq 'PurchaseOrder' || $MODULE eq 'SalesOrder' || $MODULE eq 'Quotes' || $MODULE eq 'Reserve' || $MODULE eq 'PriceList'}
    ...
    {php} 
        global $reserveid_id,$cf_1016;
        if($cf_1016=="PriceList"){
    {/php} 
            {if $MODULE eq 'PriceList'}    
               <tr>
                <td align="left" style="padding-left:10px;">
                    <a class="webMnu" href="#" onclick="JavaScript: void window.open('report_reserve_01.php?reserve_id={php} echo $reserveid_id;{/php}
            ','Acount','resizable=yes,left=20,top=10,width=990,height=800,toolbar=no,scrollbars=yes,menubar=no,location=no');"><img src="{'actionGeneratePDF.gif'|@aicrm_imageurl:$THEME}" hspace="5" align="absmiddle" border="0"/></a>
                   <a class="webMnu" href="#" onclick="JavaScript: void window.open('report_reserve_01.php?reserve_id={php} echo $reserveid_id;{/php}
            ','Acount','resizable=yes,left=20,top=10,width=990,height=800,toolbar=no,scrollbars=yes,menubar=no,location=no');">ใบจองสินค้าภายใน</a>
             {else}
                    <a class="webMnu" href="#" onclick="JavaScript: void window.open('report_reserve_01.php?reserve_id={php} echo $reserveid_id;{/php}
                ','Acount','resizable=yes,left=20,top=10,width=990,height=800,toolbar=no,scrollbars=yes,menubar=no,location=no');">Export PDF</a>
                </td>
               </tr>
            {/if}   
{php}
