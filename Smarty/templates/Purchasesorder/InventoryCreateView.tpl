...
function get_valuevender(){ldelim}

  	var e = document.getElementById("vender");
    var option= e.options[e.selectedIndex];
    var dataid = option.getAttribute("data-id");
    var datatype = 'Vender';

    if(dataid != ''){ldelim}
    	jQuery.post('WB_Service_AI/index.php/special/setting/venderbuyer',{ldelim} id:dataid ,type:datatype {rdelim}, function(rs){ldelim}
    		document.getElementById("vender_address").value = rs.data[0].address;
    		document.getElementById("vender_phone").value = rs.data[0].phone;
    		document.getElementById("vender_email").value = rs.data[0].email;
    		document.getElementById("payment_method").value = rs.data[0].payment_method;
    		document.getElementById("payment_term").value = rs.data[0].payment_term;
    	{rdelim},'json');

    {rdelim}else{ldelim}
    	
    	document.getElementById("vender_address").value = '';
    	document.getElementById("vender_phone").value = '';
    	document.getElementById("vender_email").value = '';
...
function get_valuebuyer(){ldelim}
    ...
    if(dataid != ''){ldelim}
    	jQuery.post('WB_Service_AI/index.php/special/setting/venderbuyer',{ldelim} id:dataid ,type:datatype {rdelim}, function(rs){ldelim}
    		document.getElementById("buyer_address").value = rs.data[0].address;
    		document.getElementById("buyer_phone").value = rs.data[0].phone;
    		document.getElementById("buyer_email").value = rs.data[0].email;
    	{rdelim},'json');
    {rdelim}else{ldelim}
						   ...
						   <tr>
							...
							<!-- General details -->
								<table border=0 cellspacing=0 cellpadding=0 width=100% class="small">
								   ...
								   {if $MODULE eq 'Purchasesorder' || $MODULE eq 'SalesOrder' || $MODULE eq 'Quotes' || $MODULE eq 'Invoice'}
									...
									{else}
										{include file="Purchasesorder/ProductDetails.tpl"}
									{/if}
