...
function confirm_approve(approve_status,id,level){
    ...
    if(approve_status == 'Approve'){
        ...
        url = 'salesorder_status.php';
    }else if ( approve_status =='Cancel_Salesorder') {
        msg = 'ต้องการยกเลิกรายการ';
        url = 'salesorder_cancel.php?crmid='+id+'&approve_status='+approve_status;
    }else if(approve_status =='Payments'){
        msg = 'ชำระแล้ว';
        url = 'salesorder_status.php?crmid='+id+'&approve_status='+approve_status;
    }else{
        msg = 'ต้องการไม่อนุมัติรายการ';
        url = 'salesorder_cancel.php?crmid='+id+'&level='+level+'&approve_status='+approve_status;
    }
            ...
            onClick: function(){
                ...
                if(approve_status == 'Cancel_Approve'){
                    ...
                    jQuery.ajax({
                       ...
                       url: url,
                       cache: false,
                       data: {crmid:id,salesorderstatus:approve_status,level:level},
                       dataType: "json",
            ...
            <td>
                ...
                <tr>
                    ...
                    <td class="dvtTabCache" align="right" style="width:100%">
                            ...
                            {if $flagassign eq true && $salesorder_status eq 'เปิดใบขาย' }
                            ...
                            {if $salesorder_status eq 'ขออนุมัติใบขาย' && $approveflg eq true }
                                <button title="Approve" class="crmbutton small edit Approve" onclick="confirm_approve('Approve','{$ID}','{$levelflg}');" type="button" name="approvelevel" value="Approve">
                                ...
                                <button title="Cancel Approve" class="crmbutton small cancel Cancel_Approve" onclick="confirm_approve('Cancel_Approve','{$ID}','{$levelflg}')">
                                    <img src="themes/softed/images/cancel_o.png" border="0" style="width: 17px;height: 17px; vertical-align: middle;">&nbsp;Cancel Approve
                                </button>&nbsp;
                            {/if}
                            
                            {if $flagaccount eq true && ($salesorder_status eq 'อนุมัติใบขาย')}
                                 <button title="Payments" class="crmbutton small success Payments" onclick="confirm_approve('Payments','{$ID}','');" type="button" name="payments" value="Payments">
                                    <img src="themes/softed/images/approval.png" border="0" style="width: 17px;height: 19px; vertical-align: middle;">&nbsp;Payments
                                </button>&nbsp;
                            {/if}

                            {if $flagassign eq true && ($salesorder_status eq 'เปิดใบขาย' || $salesorder_status eq 'อนุมัติใบขาย')}
                                <button title="Cancel Sales Order" class="crmbutton small cancel Cancel_Salesorder" onclick="confirm_approve('Cancel_Salesorder','{$ID}','')">
                                    <img src="themes/softed/images/cancel_o.png" border="0" style="width: 17px;height: 17px; vertical-align: middle;">&nbsp;Cancel Sales Order
                                </button>&nbsp;
                            {/if}

                            {if $EDIT_DUPLICATE eq 'permitted' }

                                {if $salesorder_status eq 'เปิดใบขาย'}    
                            ...
                            {if $EDIT_DUPLICATE eq 'permitted' || $is_permmited eq true}
                                {if $IS_ADMIN eq 'on' || $salesorder_status eq 'เปิดใบขาย' || $salesorder_status eq 'ยกเลิกใบขาย' || $salesorder_status eq 'ชำระแล้ว'}
                    ...
                    <table border=0 cellspacing=0 cellpadding=0 width=100%>
                            ...
                            {/if}
                                ...
                                {if $header neq 'Comments' && $header neq 'Comment Information'}
                                    ...
                                    {/foreach}
                                    {if $header=="Remark" && $MODULE=="Salesorder" }
                                        {include_php file='modules/Salesorder/log_change_status.php'}
                                    {/if}
                                    </table>
                                    </div>
                                {/if}
