...
<head>
    ...
    <link REL="SHORTCUT ICON" HREF="themes/images/AICRM.ico">
    <style type="text/css">@import url("themes/{$THEME}/style.css");</style>
    <script language="javascript" type="text/javascript" src="include/scriptaculous/prototype.js"></script>
    <script src="include/scriptaculous/scriptaculous.js" type="text/javascript"></script>
    <script src="include/js/general.js" type="text/javascript"></script>
    <script language="JavaScript" type="text/javascript"
            src="include/js/{php} echo $_SESSION['authenticated_user_language'];{/php}.lang.js?{php} echo $_SESSION['aicrm_version'];{/php}"></script>
    <!-- <script type="text/javascript" src="include/ckeditor/ckeditor.js"></script> -->
    <script type="text/javascript" src="modules/Products/multifile.js"></script>
</head>
<body marginheight="0" marginwidth="0" topmargin="0" bottommargin="0" leftmargin="0" rightmargin="0">
{literal}
<form name="EditView" id="EditView" method="POST" ENCTYPE="multipart/form-data" action="index.php" onSubmit="if(email_validate(this.form,'')) {VtigerJS_DialogBox.block();} else { return false; }">
    {/literal}
    ...
    <table class="small mailClient" border="0" cellpadding="0" cellspacing="0" width="100%">
		...
		<img src="{'select.gif'|@aicrm_imageurl:$THEME}" alt="{$APP.LBL_SELECT}" title="{$APP.LBL_SELECT}"
             LANGUAGE=javascript
             onclick='return window.open("index.php?module="+ document.EditView.parent_type.value +"&action=Popup&html=Popup_picker&form=HelpDeskEditView&popuptype=set_return_emails","test","width=640,height=602,resizable=0,scrollbars=0,top=150,left=200");'
             align="absmiddle" style='cursor:hand;cursor:pointer'>&nbsp;
		</span><span class="mailClientCSSButton"><img src="{'clear_field.gif'|@aicrm_imageurl:$THEME}"
                                                      alt="{$APP.LBL_CLEAR}" title="{$APP.LBL_CLEAR}"
                                                      LANGUAGE=javascript
                                                      onClick="$('parent_id').value=''; $('hidden_toid').value='';$('parent_name').value=''; return false;"
                                                      align="absmiddle" style='cursor:hand;cursor:pointer'>
		</span>
                        </td>
                    </tr>
                    <tr>
                        {if 'ccmail'|@emails_checkFieldVisiblityPermission eq '0'}
                            ...
                            <td class="cellText" style="padding: 5px;">
                                <input name="ccmail" id="cc_name" class="txtBox" type="text" value="{$CC_MAIL}"
                                       style="width:99%">&nbsp;
                            </td>
                        {else}
                            <td>&nbsp;</td>
                            <td>&nbsp;</td>
                        {/if}
                        <td valign="top" class="cellLabel" rowspan="4">
                            <div id="attach_cont" class="addEventInnerBox"
                                 style="overflow:auto;height:100px;width:100%;position:relative;left:0px;top:0px;"></div>
                    </tr>
                    {if 'bccmail'|@emails_checkFieldVisiblityPermission eq '0'}
                        <tr>
                            <td class="mailSubHeader" style="padding: 5px;" align="right">{$MOD.LBL_BCC}</td>
                            <td class="cellText" style="padding: 5px;">
                                <input name="bccmail" id="bcc_name" class="txtBox" type="text" value="{$BCC_MAIL}"
                                       style="width:99%">&nbsp;
                            </td>
                        </tr>
                    {/if}
                {elseif $elements.2.0 eq 'subject'}
                    <tr>
                        <td class="mailSubHeader" style="padding: 5px;" align="right" nowrap><font
                                    color="red">*</font>{$elements.1.0} :
                        </td>
                        {if $WEBMAIL eq 'true' or $RET_ERROR eq 1}
                            <td class="cellText" style="padding: 5px;"><input type="text" class="txtBox"
                                                                              name="{$elements.2.0}" value="{$SUBJECT}"
                                                                              id="{$elements.2.0}" style="width:99%">
                            </td>
                        {else}
                            <td class="cellText" style="padding: 5px;"><input type="text" class="txtBox"
                                                                              name="{$elements.2.0}"
                ...
                {elseif $elements.2.0 eq 'filename'}
                    <tr>
                        ...
                        <td class="cellText" style="padding: 5px;">
                            ...
                            <div id="files_list"
                                 style="border: 1px solid grey; width: 500px; padding: 5px; background: rgb(255, 255, 255) none repeat scroll 0%; -moz-background-clip: initial; -moz-background-origin: initial; -moz-background-inline-policy: initial; font-size: x-small">{$APP.Files_Maximum_6}
                            ...
                            <div id="attach_temp_cont" style="display:none;">
                                <table class="small" width="100% ">
                                    ...
                                    {if $smarty.request.attachment != ''}
                                        <tr>
                                            <td width="100%" colspan="2">{$smarty.request.attachment|@vtlib_purify}
                                                <input type="hidden" value="{$smarty.request.attachment|@vtlib_purify}"
                                                       name="pdf_attachment"></td>
                                        </tr>
                                    {else}
                                        ...
                                        {foreach item="attach_files" key="attach_id" from=$elements.3}
                                            <tr id="row_{$attach_id}">
                                                ...
                                                <td><img src="{'no.gif'|@aicrm_imageurl:$THEME}"
                                                         onClick="delAttachments({$attach_id})"
                                                         alt="{$APP.LBL_DELETE_BUTTON}" title="{$APP.LBL_DELETE_BUTTON}"
                    ...
                    <tr>
                        <td colspan="3" align="center" valign="top" height="320">
                            {if $WEBMAIL eq 'true' or $RET_ERROR eq 1}
                                ...
                                <textarea style="display: none;" class="detailedViewTextBox" id="description"
                                          name="description" cols="90" rows="8">{$DESCRIPTION}</textarea>
                            {else}
                                <textarea style="display: none;" class="detailedViewTextBox" id="description"
                                          name="description" cols="90" rows="16">{$elements.3.0}</textarea>
                            {/if}
                        </td>
                    </tr>
                {/if}
...
{literal}
    CKEDITOR.replace( 'description',{filebrowserUploadUrl:'include/ckeditor/plugins/imgupload/imgupload.php'})
    var description = CKEDITOR.instances['description'];
