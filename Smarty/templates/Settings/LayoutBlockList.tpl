...
function EditCustomBlockForm(modulename,mode)
{
	
	var blocklabel = document.getElementById('edit_blocklabel');
	var val = trim(blocklabel.value);
	if(val == "")
	{
		alert(alert_arr.BLOCK_NAME_CANNOT_BE_BLANK);
	...
	var blockid = document.getElementById('edit_blockid').value;
	...
	$('vtbusy_info').style.display = "block";
		...
		{queue: {position: 'end', scope: 'command'},
			method: 'post',
			postBody: 'module=Settings&action=SettingsAjax&file=LayoutBlockList&sub_mode=editBlock&fld_module='+modulename+'&parenttab=Settings&ajax=true&mode='+mode+'&blocklabel='+
			encodeURIComponent(val)+'&edit_blockid='+blockid,
			onComplete: function(response) {
				$('vtbusy_info').style.display = "none";
				var str = response.responseText;
				if(str == 'ERROR'){
...
<table align="center" border="0" cellpadding="0" cellspacing="0" width="98%">
	<tr>
        ...
        <td class="showPanelBg" style="padding: 10px;" valign="top" width="100%"><br>	
			<table class="settingsSelUITopLine" border="0" cellpadding="5" cellspacing="0" width="100%">
				<tr>
					<td rowspan="2" valign="top" width="50"><img src="{'orgshar.gif'|@aicrm_imageurl:$THEME}" alt="Users" title="Users" border="0" height="48" width="48"></td>
				...
				<tr>
					...
					<td align="right" width="5%"><input type="button" class="crmButton create small" onclick="fnvshobj(this,'addblock');" alt="{$MOD.ADD_BLOCK}" title="{$MOD.ADD_BLOCK}" value="{$MOD.ADD_BLOCK}"/>
					</td>
					&nbsp; <img src="{'vtbusy.gif'|@aicrm_imageurl:$THEME}" id="vtbusy_info" style="display:none;position:absolute;top:180px;right:100px;" border="0" />
				</tr>
			</table>
...
{else}
	...
	<div style='border: 3px solid rgb(153, 153, 153); background-color: rgb(255, 255, 255); width: 55%; position: relative; z-index: 10000000;'>
		...
		<tbody><tr>
		<td rowspan='2' width='11%'><img src="{'denied.gif'|@aicrm_imageurl:$THEME}" ></td>
