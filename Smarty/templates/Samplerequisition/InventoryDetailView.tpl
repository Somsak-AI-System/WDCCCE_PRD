...
function confirm_approve(approve_status,id,level){
    ...
    if(approve_status == 'Approve'){
        ...
        url = 'samplerequisition_status.php';
    }else if ( approve_status =='Cancel_Samplerequisition'){
        msg = 'ต้องการยกเลิกรายการ';
        url = 'samplerequisition_cancel.php?crmid='+id+'&approve_status='+approve_status;
    }else{
        msg = 'ต้องการไม่อนุมัติรายการ';
        url = 'samplerequisition_cancel.php?crmid='+id+'&level='+level+'&approve_status='+approve_status;
    }
        ...
        buttons:[{
            ...
            onClick: function(){
                ...
                if(approve_status == 'Cancel_Approve'){
                    ...
                    dlg.dialog("close");
                    jQuery('#dialog').window({
                        title: 'Cancel Sample Requisition',
                        width: 550,
                        height: 250,
                        closed: false,
                        cache: false,
                    ...
                    jQuery.ajax({
                       ...
                       url: url,
                       cache: false,
                       data: {crmid:id,samplerequisitionstatus:approve_status,level:level},
                       dataType: "json",
						   ...
						   <tr>
								...
								<td class="dvtTabCache" align="right" style="width:100%">
									...
									{if $EDIT_DUPLICATE eq 'permitted' || $is_permmited eq true}
		                                {if $flagassign eq true && $samplerequisition_status eq 'Create' }
		                                    <button title="Request Approve" class="crmbutton small edit Request_Approve" onclick="confirm_approve('Request_Approve','{$ID}','');" type="button" name="requestapprove" value="Request Approve">
		                                        <img src="themes/softed/images/approval.png" border="0" style="width: 17px;height: 19px; vertical-align: middle;">&nbsp;Request Approve
		                                    </button>
		                                {/if}

		                                {if $samplerequisition_status eq 'Request for Approve' && $approveflg eq true }
		                                    {if $levelflg eq '4'}
		                                ...
		                                {if $flagassign eq true && ($samplerequisition_status eq 'Create' || $samplerequisition_status eq 'Request for Approve')}
		                                    <button title="Cancel Samplerequisition" class="crmbutton small cancel Cancel" onclick="confirm_approve('Cancel_Samplerequisition','{$ID}','')">
		                                ...
		                                {if $samplerequisition_status eq 'Approved' ||  $samplerequisition_status eq 'Rejected' || ($is_permmited eq true && $samplerequisition_status eq 'Approved' ||  $samplerequisition_status eq 'Rejected')}   
		                                ...
		                                {*$flagassign eq true &&*}
		                                {if ( $samplerequisition_status eq 'Approved')}
		                                ...
		                                {*if $flagassign eq true && ($convertflag eq '0' && $samplerequisition_status eq 'ปิดการขาย')}
                                    ...
                                    {if $EDIT_DUPLICATE eq 'permitted' || $is_permmited eq true}
                                    	{if $samplerequisition_status eq 'Create'}
									...
									{if $EDIT_DUPLICATE eq 'permitted' || $is_permmited eq true && $MODULE neq 'Documents' && $MODULE neq 'Products'}
                                        {if $IS_ADMIN eq 'on' || $samplerequisition_status eq 'Create' || $samplerequisition_status eq 'Cancelled'}
...
{foreach key=header item=detail from=$BLOCKS}
	...
	{if $header neq 'Comments' && $header neq 'Comment Information'}
		...
		{/if}
		    ...
		    {if $header=="Remark" && $MODULE=="Samplerequisition" }
		   		{include_php file='modules/Samplerequisition/log_change_status.php'}
		    {/if}
			</table>
		  ...
		  <tr>
			...
			{/if}
									...
									<td width=22% valign=top style="padding:10px;">
										...
										{*include file="Samplerequisition/InventoryActions.tpl"*}
