...
<table border=0 cellspacing=0 cellpadding=0 width=100% class="small" style="border-bottom:1px solid #999999;padding:5px;">
	<tr>
		<td  valign=bottom ><b>{$HEADERLABEL}</b> 
			{if $MODULE eq 'Campaigns' && ($rel_mod eq 'Contacts')}
		...
		<td align=right>
			...
			{if $header eq 'Contacts' && $MODULE neq 'Campaigns' && $MODULE neq 'Accounts' && $MODULE neq 'Potentials' && $MODULE neq 'Products'}
				...
				{elseif $MODULE neq 'SmartSms' && $MODULE neq 'Smartemail' && $MODULE neq 'Smartquestionnaire'}
		...
		{if $header eq 'header'}
			<table border=0 cellspacing=1 cellpadding=3 width=100% style="background-color:#eaeaea;" class="small">
				...
				{foreach key=header item=headerfields from=$detail}
					<td class="lvtCol" align="left">{$headerfields}</td>
				{/foreach}
                                </tr>
		{elseif $header eq 'entries'}
			{foreach key=header item=detail from=$detail}
				...
				{foreach key=header item=listfields from=$detail}
                     ...
                     <td align="left" onmouseover="vtlib_listview.trigger('cell.onmouseover', $(this))" onmouseout="vtlib_listview.trigger('cell.onmouseout', $(this))">{$listfields}</td>
                     {* END *}
				{/foreach}
				</tr>
			{/foreach}
