...
{literal}
	function editexpressionscript($){
		...
		function jsonget(operation, params, callback){
			var obj = {
					module:'com_aicrm_workflow', 
					action:'com_aicrm_workflowAjax',	
		...
		function editpopup(){
				...
				$('#editpopup_save').bind('click', function(){
					...
					$.get('index.php', {
							module:'com_aicrm_autofillfields', 
							action:'com_aicrm_autofillfieldsAjax',	
					...
					function(result){
						...
						$.each(result, function(fieldName, expression){
							editLink = format('<img border="0" title="Edit" alt="Edit" \
													...
													src="{'editfield.gif'|@aicrm_imageurl:$THEME}"/>', fieldName);
		...
		<tr>
			...
			<td width="40%" align="right">
				<a href="javascript:void;" id="editpopup_close">
					<img border="0" align="absmiddle" src="{'close.gif'|@aicrm_imageurl:$THEME}"/>
				</a>
			</td>
		</tr>
	</table>
