...
abstract class REST_Controller extends MX_Controller
{
    ...
    */
    ...
    public function response($data = null, $http_code = null, $continue = false)
    {
		...
		// Maew Comment to change Content-Length for thai language
     /*  if ( ! $this->_zlib_oc && ! $this->config->item('compress_output')) {
            header('Content-Length: ' . strlen($output)*8);
        }*/
		/*	if ( ! $this->_zlib_oc && ! $this->config->item('compress_output')) {
            header('Content-Length:214748364700');
        }*/
		header('Access-Control-Allow-Origin: *');
		header("Content-Type: application/json; charset=UTF-8");
        if($continue){
            //echo($output);
			echo(preg_replace('/[\x00-\x1F\x80-\xFF]/', '', $output));
            ob_end_flush();
            ob_flush();
            flush();
        }else{
            //exit($output);
			exit (preg_replace('/[\x00-\x1F\x80-\xFF]/', '', $output));
        }
    }
	 ...
	 protected function _detect_api_key()
    {
		...
		$api_key_variable_dotnet = config_item('rest_key_name_dotnet');
        // Work out the name of the SERVER entry based on config
        $key_name = 'HTTP_'.strtoupper(str_replace('-', '_', $api_key_variable));
		$key_name_dotnet = 'HTTP_'.strtoupper(str_replace('-', '_', $api_key_variable_dotnet));
		//echo $key_name; exit;
        $this->rest->key = null;
		...
		}else if (($key = isset($this->_args[$api_key_variable_dotnet]) ? $this->_args[$api_key_variable_dotnet] : $this->input->server($key_name_dotnet))) {
