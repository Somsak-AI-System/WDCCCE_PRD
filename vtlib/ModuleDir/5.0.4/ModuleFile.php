...
class ModuleClass extends CRMEntity {
	...
	var $groupTable = Array('aicrm_payslipgrouprel', 'payslipid');
	...
	function getListQuery($module, $usewhere=false) {
		...
		if(!empty($this->groupTable)) {
			$query .= "
				...
				LEFT JOIN aicrm_groups ON aicrm_groups.groupname = " . $this->groupTable[0] . '.groupname';
		}
		...
		if($is_admin==false && $profileGlobalPermission[1] == 1 && $profileGlobalPermission[2] == 1 
			&& $defaultOrgSharingPermission[$tabid] == 3) {
				...
				$sec_query .= " AND (aicrm_crmentity.smownerid in($current_user->id) OR aicrm_crmentity.smownerid IN 
					...
					OR 
					(
						aicrm_crmentity.smownerid in (0)";
	...
	function create_export_query($where)
	{
		...
		$query = "SELECT $fields_list, ". $this->groupTable[0].'.'.$this->groupTable[1] ." as 'Assigned To Group', 
				CASE WHEN (aicrm_users.user_name NOT LIKE '') THEN aicrm_users.user_name ELSE aicrm_groups.groupname END 
				AS user_name FROM aicrm_crmentity INNER JOIN $this->table_name ON aicrm_crmentity.crmid=$this->table_name.$this->table_index";
	...
	function create_import_query($module) {
		global $current_user;
		$query = "SELECT aicrm_crmentity.crmid, aicrm_users.user_name, $this->table_name.* FROM $this->table_name
	...
	function set_import_assigned_user()
	{
		...
		if($record_user != $current_user->id){
			$sqlresult = $adb->pquery("select id from aicrm_users where id = ?", array($record_user));
