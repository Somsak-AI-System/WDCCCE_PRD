...
class Vtiger_Access {
	...
	static function __getDefaultSharingAccessId() {
		global $adb;
		return $adb->getUniqueID('aicrm_def_org_share');
	}
	...
	static function allowSharing($moduleInstance, $enable=true) {
		global $adb;
		$ownedby = $enable? 0 : 1;
		$adb->pquery("UPDATE aicrm_tab set ownedby=? WHERE tabid=?", Array($ownedby, $moduleInstance->id));
	...
	static function initSharing($moduleInstance) {
		...
		$result = $adb->query("SELECT share_action_id from aicrm_org_share_action_mapping WHERE share_action_name in
		...
		for($index = 0; $index < $adb->num_rows($result); ++$index) {
			...
			$adb->pquery("INSERT INTO aicrm_org_share_action2tab(share_action_id,tabid) VALUES(?,?)", Array($actionid, $moduleInstance->id));
		}
	...
	static function deleteSharing($moduleInstance) {
		global $adb;
		$adb->pquery("DELETE FROM aicrm_org_share_action2tab WHERE tabid=?", Array($moduleInstance->id));
	...
	static function setDefaultSharing($moduleInstance, $permission_text='Public_ReadWriteDelete') {
		...
		$result = $adb->pquery("SELECT * FROM aicrm_def_org_share WHERE tabid=?", Array($moduleInstance->id));
		if($adb->num_rows($result)) {
			$ruleid = $adb->query_result($result, 0, 'ruleid');
			$adb->pquery("UPDATE aicrm_def_org_share SET permission=? WHERE ruleid=?", Array($permission, $ruleid));
		} else {
			$ruleid = self::__getDefaultSharingAccessId();
			$adb->pquery("INSERT INTO aicrm_def_org_share (ruleid,tabid,permission,editstatus) VALUES(?,?,?,?)", 
	...
	static function updateTool($moduleInstance, $toolAction, $flag, $profileid=false) {
		...
		$result = $adb->pquery("SELECT actionid FROM aicrm_actionmapping WHERE actionname=?", Array($toolAction));
		if($adb->num_rows($result)) {
			...
			foreach($profileids as $useprofileid) {
				$result = $adb->pquery("SELECT permission FROM aicrm_profile2utility WHERE profileid=? AND tabid=? AND activityid=?", 
				...
				if($adb->num_rows($result)) {
					...
					if($curpermission != $permission) {
						$adb->pquery("UPDATE aicrm_profile2utility set permission=? WHERE profileid=? AND tabid=? AND activityid=?", 
							Array($permission, $useprofileid, $moduleInstance->id, $actionid));
					}
				} else {
					$adb->pquery("INSERT INTO aicrm_profile2utility (profileid, tabid, activityid, permission) VALUES(?,?,?,?)",
	...
	static function deleteTools($moduleInstance) {
		global $adb;
		$adb->pquery("DELETE FROM aicrm_profile2utility WHERE tabid=?", Array($moduleInstance->id));
