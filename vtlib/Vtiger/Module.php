...
class Vtiger_Module extends Vtiger_ModuleBasic {
	...
	function __getRelatedListUniqueId() {
		global $adb;
		return $adb->getUniqueID('aicrm_relatedlists');
	}
	...
	function __getNextRelatedListSequence() {
		global $adb;
		$max_sequence = 0;
		$result = $adb->pquery("SELECT max(sequence) as maxsequence FROM aicrm_relatedlists WHERE tabid=?", Array($this->id));
	 ...
	 *
	 * @internal Creates table aicrm_crmentityrel if it does not exists
	 */
	function setRelatedList($moduleInstance, $label='', $actions=false, $function_name='get_related_list') {
		...
		// Add column to aicrm_relatedlists to save extended actions
		Vtiger_Utils::AddColumn('aicrm_relatedlists', 'actions', 'VARCHAR(50)');

		$adb->pquery("INSERT INTO aicrm_relatedlists(relation_id,tabid,related_tabid,name,sequence,label,presence,actions) VALUES(?,?,?,?,?,?,?,?)",
	...
	function unsetRelatedList($moduleInstance, $label='', $function_name='get_related_list') {
		...
		$adb->pquery("DELETE FROM aicrm_relatedlists WHERE tabid=? AND related_tabid=? AND name=? AND label=?", 
	...
	static function getInstance($value) {
		...
		if(Vtiger_Utils::isNumber($value)) {
			$query = "SELECT * FROM aicrm_tab WHERE tabid=?";
		} else {
			$query = "SELECT * FROM aicrm_tab WHERE name=?";
		}
