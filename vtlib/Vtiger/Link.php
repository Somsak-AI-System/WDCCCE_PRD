...
class Vtiger_Link {
	...
	static function __getUniqueId() {
		global $adb;
		return $adb->getUniqueID('aicrm_links');
	}
	...
	static function __initSchema() {
		if(empty(self::$__cacheSchemaChanges['aicrm_links'])) {
			if(!Vtiger_Utils::CheckTable('aicrm_links')) {
				...
				Vtiger_Utils::ExecuteQuery(
					'CREATE INDEX link_tabidtype_idx on aicrm_links(tabid,linktype)');
			}
			self::$__cacheSchemaChanges['aicrm_links'] = true;
		}
	}
	...
	static function addLink($tabid, $type, $label, $url, $iconpath='',$sequence=0) {
		global $adb;
		self::__initSchema();
		$checkres = $adb->pquery('SELECT linkid FROM aicrm_links WHERE tabid=? AND linktype=? AND linkurl=? AND linkicon=? AND linklabel=?',
			Array($tabid, $type, $url, $iconpath, $label));
		if(!$adb->num_rows($checkres)) {
			$uniqueid = self::__getUniqueId();
			$adb->pquery('INSERT INTO aicrm_links (linkid,tabid,linktype,linklabel,linkurl,linkicon,sequence) VALUES(?,?,?,?,?,?,?)',
	...
	static function deleteLink($tabid, $type, $label, $url=false) {
		...
		self::__initSchema();
		if($url) {
			$adb->pquery('DELETE FROM aicrm_links WHERE tabid=? AND linktype=? AND linklabel=? AND linkurl=?',
				Array($tabid, $type, $label, $url));
			self::log("Deleting Link ($type - $label - $url) ... DONE");
		} else {
			$adb->pquery('DELETE FROM aicrm_links WHERE tabid=? AND linktype=? AND linklabel=?',
	...
	static function deleteAll($tabid) {
		global $adb;
		self::__initSchema();
		$adb->pquery('DELETE FROM aicrm_links WHERE tabid=?', Array($tabid));
		...
		if($type) {
			...
			if(is_array($type)) { 
				...
				if($tabid === self::IGNORE_MODULE) {
					$result = $adb->pquery('SELECT * FROM aicrm_links WHERE linktype IN ('. 
						Vtiger_Utils::implodestr('?', count($type), ',') .')', 
						Array($adb->flatten_array($type)));
				} else {
					$result = $adb->pquery('SELECT * FROM aicrm_links WHERE tabid=? AND linktype IN ('.
						Vtiger_Utils::implodestr('?', count($type), ',') .')',
				...
				if($tabid === self::IGNORE_MODULE) {
					$result = $adb->pquery('SELECT * FROM aicrm_links WHERE linktype=?', Array($type));
				} else {
					$result = $adb->pquery('SELECT * FROM aicrm_links WHERE tabid=? AND linktype=?', Array($tabid, $type));				
				}
			}
		} else {
			$result = $adb->pquery('SELECT * FROM aicrm_links WHERE tabid=?', Array($tabid));
		}
