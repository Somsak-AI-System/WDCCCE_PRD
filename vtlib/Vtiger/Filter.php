...
class Vtiger_Filter {
	...
	function __getUniqueId() {
		global $adb;
		return $adb->getUniqueID('aicrm_customview');
	}
	...
	function __create($moduleInstance) {
		...
		$adb->pquery("INSERT INTO aicrm_customview(cvid,viewname,setdefault,setmetrics,entitytype) VALUES(?,?,?,?,?)", 
		...
		if(!$this->status) {
			...
			$adb->pquery("UPDATE aicrm_customview SET status=? WHERE cvid=?", Array($this->status, $this->id));
	...
	function __delete() {
		global $adb;
		$adb->pquery("DELETE FROM aicrm_cvadvfilter WHERE cvid=?", Array($this->id));
		$adb->pquery("DELETE FROM aicrm_cvcolumnlist WHERE cvid=?", Array($this->id));
		$adb->pquery("DELETE FROM aicrm_customview WHERE cvid=?", Array($this->id));
	}
	...
	function addField($fieldInstance, $index=0) {
		...
		$adb->pquery("UPDATE aicrm_cvcolumnlist SET columnindex=columnindex+1 WHERE cvid=? AND columnindex>=? ORDER BY columnindex DESC", 
			Array($this->id, $index));
		$adb->pquery("INSERT INTO aicrm_cvcolumnlist(cvid,columnindex,columnname) VALUES(?,?,?)", Array($this->id, $index, $cvcolvalue));
	...
	function addRule($fieldInstance, $comparator, $comparevalue, $index=0) {
		...
		$adb->pquery("UPDATE aicrm_cvadvfilter set columnindex=columnindex+1 WHERE cvid=? AND columnindex>=? ORDER BY columnindex DESC",
			Array($this->id, $index));		
		$adb->pquery("INSERT INTO aicrm_cvadvfilter(cvid, columnindex, columnname, comparator, value) VALUES(?,?,?,?,?)",
	...
	static function getInstance($value, $moduleInstance=false) {
		...
		if(Vtiger_Utils::isNumber($value)) {
			$query = "SELECT * FROM aicrm_customview WHERE cvid=?";
			$queryParams = Array($value);
		} else {
			$query = "SELECT * FROM aicrm_customview WHERE viewname=? AND entitytype=?";
	...
	static function getAllForModule($moduleInstance) {
		...
		$query = "SELECT * FROM aicrm_customview WHERE entitytype=?";
	...
	static function deleteForModule($moduleInstance) {
		...
		$cvidres = $adb->pquery("SELECT cvid FROM aicrm_customview WHERE entitytype=?", Array($moduleInstance->name));
		if($adb->num_rows($cvidres)) {
			...
			if(!empty($cvids)) {
				$adb->query("DELETE FROM aicrm_cvadvfilter WHERE cvid  IN (" . implode(',', $cvids) . ")");
				$adb->query("DELETE FROM aicrm_cvcolumnlist WHERE cvid IN (" . implode(',', $cvids) . ")");
				$adb->query("DELETE FROM aicrm_customview WHERE cvid   IN (" . implode(',', $cvids) . ")");
			}
		}
	}
}
