...
class Vtiger_Profile {
	...
	static function initForField($fieldInstance) {
		...
		$adb->pquery("INSERT INTO aicrm_def_org_field (tabid, fieldid, visible, readonly) VALUES(?,?,?,?)",
			Array($fieldInstance->getModuleId(), $fieldInstance->id, '0', '1'));

		$profileids = self::getAllIds();
		foreach($profileids as $profileid) {
			$adb->pquery("INSERT INTO aicrm_profile2field (profileid, tabid, fieldid, visible, readonly) VALUES(?,?,?,?,?)",
	...
	static function deleteForField($fieldInstance) {
		...
		$adb->pquery("DELETE FROM aicrm_def_org_field WHERE fieldid=?", Array($fieldInstance->id));
		$adb->pquery("DELETE FROM aicrm_profile2field WHERE fieldid=?", Array($fieldInstance->id));
	}
	...
	static function getAllIds() {
		global $adb;
		$profileids = Array();
		$result = $adb->query('SELECT profileid FROM aicrm_profile');
	...
	static function initForModule($moduleInstance) {
		...
		$actionids = Array();
		$result = $adb->query("SELECT actionid from aicrm_actionmapping WHERE actionname IN 
		...
		foreach($profileids as $profileid) {			
			$adb->pquery("INSERT INTO aicrm_profile2tab (profileid, tabid, permissions) VALUES (?,?,?)",
				Array($profileid, $moduleInstance->id, 0));

			if($moduleInstance->isentitytype) {
				foreach($actionids as $actionid) {
					$adb->pquery(
						"INSERT INTO aicrm_profile2standardpermissions (profileid, tabid, Operation, permissions) VALUES(?,?,?,?)",
	...
	static function deleteForModule($moduleInstance) {
		global $adb;
		$adb->pquery("DELETE FROM aicrm_profile2tab WHERE tabid=?", Array($moduleInstance->id));
		$adb->pquery("DELETE FROM aicrm_profile2standardpermissions WHERE tabid=?", Array($moduleInstance->id));
	}
}
?>
