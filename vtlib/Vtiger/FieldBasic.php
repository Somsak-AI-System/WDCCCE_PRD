...
class Vtiger_FieldBasic {
	...
	function __handleVtigerCoreSchemaChanges() {
		// Add helpinfo column to the aicrm_field table
		if(empty(self::$__cacheSchemaChanges['aicrm_field.helpinfo'])) {
			Vtiger_Utils::AddColumn('aicrm_field', 'helpinfo', ' TEXT');
			self::$__cacheSchemaChanges['aicrm_field.helpinfo'] = true;
		}
	}
	...
	function __getUniqueId() {
		global $adb;
		return $adb->getUniqueID('aicrm_field');
	}
	...
	function __getNextSequence() {
		global $adb;
		$result = $adb->pquery("SELECT MAX(sequence) AS max_seq FROM aicrm_field WHERE tabid=? AND block=?",
	...
	function __getNextQuickCreateSequence() {
		global $adb;
		$result = $adb->pquery("SELECT MAX(quickcreatesequence) AS max_quickcreateseq FROM aicrm_field WHERE tabid=?",
	...
	function __create($blockInstance) {
		...
		$adb->pquery("INSERT INTO aicrm_field (tabid, fieldid, columnname, tablename, generatedtype,
		...
		$adb->pquery('UPDATE aicrm_field SET masseditable=? WHERE fieldid=?', Array($this->masseditable, $this->id));
	...
	function __delete() {
		...
		$adb->pquery("DELETE FROM aicrm_field WHERE fieldid=?", Array($this->id));
	...
	function setHelpInfo($helptext) {
		...
		global $adb;
		$adb->pquery('UPDATE aicrm_field SET helpinfo=? WHERE fieldid=?', Array($helptext, $this->id));
	...
	function setMassEditable($value) {
		global $adb;
		$adb->pquery('UPDATE aicrm_field SET masseditable=? WHERE fieldid=?', Array($value, $this->id));
