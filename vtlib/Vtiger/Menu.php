...
class Vtiger_Menu {
	...
	function __getNextRelSequence() {
		global $adb;
		$result = $adb->pquery("SELECT MAX(sequence) AS max_seq FROM aicrm_parenttabrel WHERE parenttabid=?", 
			Array($this->id));
	...
	function addModule($moduleInstance) {
		if($this->id) {
			...
			$adb->pquery("INSERT INTO aicrm_parenttabrel (parenttabid,tabid,sequence) VALUES(?,?,?)",
	...
	function removeModule($moduleInstance) {
		...
		if($this->id) {
			global $adb;
			$adb->pquery("DELETE FROM aicrm_parenttabrel WHERE parenttabid = ? AND tabid = ?",
	...
	static function detachModule($moduleInstance) {
		global $adb;
		$adb->pquery("DELETE FROM aicrm_parenttabrel WHERE tabid=?", Array($moduleInstance->id));
	...
	static function getInstance($value) {
		...
		if(Vtiger_Utils::isNumber($value)) {
			$query = "SELECT * FROM aicrm_parenttab WHERE parenttabid=?";
		} else {
			$query = "SELECT * FROM aicrm_parenttab WHERE parenttab_label=?";
		}
