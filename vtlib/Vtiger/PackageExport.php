...
class Vtiger_PackageExport {
	...
	function export_Dependencies() {
		...
		$this->outputNode($aicrm_current_version, 'aicrm_version');
	...
	function export_Module($moduleInstance) {
		...
		$sqlresult = $adb->query("SELECT * FROM aicrm_parenttabrel WHERE tabid = $moduleid");
		$parenttabid = $adb->query_result($sqlresult, 0, 'parenttabid');
		$menu = Vtiger_Menu::getInstance($parenttabid);
		$parent_name = $menu->label;

		$sqlresult = $adb->query("SELECT * FROM aicrm_tab WHERE tabid = $moduleid");
	...
	function export_Blocks($moduleInstance) {
		global $adb;
		$sqlresult = $adb->pquery("SELECT * FROM aicrm_blocks WHERE tabid = ?", Array($moduleInstance->id));
	...
	function export_Fields($moduleInstance, $blockid) {
		...
		$fieldresult = $adb->pquery("SELECT * FROM aicrm_field WHERE tabid=? AND block=?", Array($moduleInstance->id, $blockid));
		$fieldcount = $adb->num_rows($fieldresult);

		if(empty($fieldcount)) return;

		$entityresult = $adb->pquery("SELECT * FROM aicrm_entityname WHERE tabid=?", Array($moduleInstance->id));
		...
		for($index = 0; $index < $fieldcount; ++$index) {
			...
			if($uitype == '10') {
				$relatedmodres = $adb->pquery("SELECT * FROM aicrm_fieldmodulerel WHERE fieldid=?", Array($fieldid));
	...
	function export_CustomViews($moduleInstance) {
		...
		$customviewres = $adb->pquery("SELECT * FROM aicrm_customview WHERE entitytype = ?", Array($moduleInstance->name));
		...
		for($cvindex = 0; $cvindex < $customviewcount; ++$cvindex) {
			...
			$cvcolumnres = $adb->query("SELECT * FROM aicrm_cvcolumnlist WHERE cvid=$cvid");
			...
			for($index = 0; $index < $cvcolumncount; ++$index) {
				...
				$cvcolumnruleres = $adb->pquery("SELECT * FROM aicrm_cvadvfilter WHERE cvid=? AND columnname=?",
	...
	function export_SharingAccess($moduleInstance) {
		...
		$deforgshare = $adb->pquery("SELECT * FROM aicrm_def_org_share WHERE tabid=?", Array($moduleInstance->id));
	...
	function export_Actions($moduleInstance) {
		...
		global $adb;
		$result = $adb->pquery('SELECT distinct(actionname) FROM aicrm_profile2utility, aicrm_actionmapping 
			WHERE aicrm_profile2utility.activityid=aicrm_actionmapping.actionid and tabid=?', Array($moduleInstance->id));
	...
	function export_RelatedLists($moduleInstance) {
		...
		global $adb;
		$result = $adb->pquery("SELECT * FROM aicrm_relatedlists WHERE tabid = ?", Array($moduleInstance->id));
		if($adb->num_rows($result)) {
