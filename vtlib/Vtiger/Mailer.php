...
class Vtiger_Mailer extends PHPMailer {
	...
	function __getUniqueId() {
		global $adb;
		return $adb->getUniqueID('aicrm_mailer_queue');
	}
	...
	function initialize() {
		...
		global $adb;
		$result = $adb->pquery("SELECT * FROM aicrm_systems WHERE server_type=?", Array('email'));
	...
	function initFromTemplate($emailtemplate) {
		global $adb;
		$result = $adb->pquery("SELECT * from aicrm_emailtemplates WHERE templatename=? AND foldername=?", 
	...
	function __initializeQueue() {
		if(!$this->_queueinitialized) {
			if(!Vtiger_Utils::CheckTable('aicrm_mailer_queue')) {
				Vtiger_Utils::CreateTable('aicrm_mailer_queue',
			...
			if(!Vtiger_Utils::CheckTable('aicrm_mailer_queueinfo')) {
				Vtiger_Utils::CreateTable('aicrm_mailer_queueinfo',
	...
	function __AddToQueue($linktoid) {
		if($this->__initializeQueue()) {
			...
			$adb->pquery('INSERT INTO aicrm_mailer_queue(id,fromname,fromemail,content_type,subject,body,mailer,relcrmid) VALUES(?,?,?,?,?,?,?,?)',
			...
			foreach($this->to as $toinfo) {
				...
				$adb->pquery('INSERT INTO aicrm_mailer_queueinfo(id, name, email, type) VALUES(?,?,?,?)',
	...
	static function dispatchQueue() {
		global $adb;
		if(!Vtiger_Utils::CheckTable('aicrm_mailer_queue')) return;

		$mailer = new self();
		$queue = $adb->query('SELECT * FROM aicrm_mailer_queue WHERE failed != 1');
		if($adb->num_rows($queue)) {
			for($index = 0; $index < $adb->num_rows($queue); ++$index) {
				...
				$emails = $adb->pquery('SELECT * FROM aicrm_mailer_queueinfo WHERE id=?', Array($queueid));
				...
				if($sent) {
					...
					$adb->pquery('DELETE FROM aicrm_mailer_queue WHERE id=?', Array($queueid));
					$adb->pquery('DELETE FROM aicrm_mailer_queueinfo WHERE id=?', Array($queueid));
				} else {
					$adb->pquery('UPDATE aicrm_mailer_queueinfo SET failed=?, failreason=? WHERE id=?', Array(1, $mailer->ErrorInfo, $queueid));
				}
			}
		}
	}
