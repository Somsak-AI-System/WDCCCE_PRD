...
class Vtiger_Event {
	...
	static function hasSupport() {
		if(self::$is_supported === '') {
			self::$is_supported = Vtiger_Utils::checkTable('aicrm_eventhandlers');
		}
	...
	static function trigger($eventname, $crmid) {
		...
		global $adb;
		$checkres = $adb->pquery("SELECT setype, crmid, deleted FROM aicrm_crmentity WHERE crmid=?", Array($crmid));
	...
	static function getAll($moduleInstance) {
		...
		if(self::hasSupport()) {
			...
			$records = $adb->pquery("SELECT * FROM aicrm_eventhandlers WHERE handler_class IN 
				(SELECT handler_class FROM aicrm_eventhandler_module WHERE module_name=?)", Array($moduleInstance->name)); 
			if($records) {
