...
class Vtiger_Block {
	...
	function __getUniqueId() {
		...
		$maxblockid = $adb->getUniqueID('aicrm_blocks');
		return $maxblockid;
	}
	...
	function __getNextSequence() {
		global $adb;
		$result = $adb->pquery("SELECT MAX(sequence) as max_sequence from aicrm_blocks where tabid = ?", Array($this->module->id));
		$maxseq = 0;
	...
	function __create($moduleInstance) {
		...
		$adb->pquery("INSERT INTO aicrm_blocks(blockid,tabid,blocklabel,sequence,show_title,visible,create_view,edit_view,detail_view) 
	...
	function __delete() {
		...
		$adb->pquery("DELETE FROM aicrm_blocks WHERE blockid=?", Array($this->id));
		self::log("DONE");
	}
	...
	static function getInstance($value, $moduleInstance=false) {
		...
		if(Vtiger_Utils::isNumber($value)) {
			$query = "SELECT * FROM aicrm_blocks WHERE blockid=?";
			$queryParams = Array($value);
		} else {
			$query = "SELECT * FROM aicrm_blocks WHERE blocklabel=? AND tabid=?";
	...
	static function getAllForModule($moduleInstance) {
		...
		$query = "SELECT * FROM aicrm_blocks WHERE tabid=?";
	...
	static function deleteForModule($moduleInstance, $recursive=true) {
		...
		$adb->pquery("DELETE FROM aicrm_blocks WHERE tabid=?", Array($moduleInstance->id));
