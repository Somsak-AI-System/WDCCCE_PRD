...
class Vtiger_Field extends Vtiger_FieldBasic {
	...
	function __getPicklistUniqueId() {
		global $adb;
		return $adb->getUniqueID('aicrm_picklist');
	}
	...
	function setPicklistValues($values) {
		...
		$picklist_table = 'aicrm_'.$this->name;
		...
		if(!Vtiger_Utils::CheckTable($picklist_table)) {
			...
			$adb->pquery("INSERT INTO aicrm_picklist (picklistid,name) VALUES(?,?)",Array($new_picklistid, $this->name));
			self::log("Creating table $picklist_table ... DONE");
		} else {
			$new_picklistid = $adb->query_result(
				$adb->pquery("SELECT picklistid FROM aicrm_picklist WHERE name=?", Array($this->name)), 0, 'picklistid');
		}
		...
		foreach($values as $value) {
			...
			$adb->query("INSERT INTO aicrm_role2picklist(roleid, picklistvalueid, picklistid, sortid) SELECT roleid, 
				$new_picklistvalueid, $new_picklistid, $sortid FROM aicrm_role");
		}
	}
	 ...
	 *
	 * @internal Creates table aicrm_fieldmodulerel if it does not exists
	 */
	function setRelatedModules($moduleNames) {
		...
		foreach($moduleNames as $relmodule) {
			$checkres = $adb->pquery('SELECT * FROM aicrm_fieldmodulerel WHERE fieldid=? AND module=? AND relmodule=?',
				Array($this->id, $this->getModuleName(), $relmodule));

			// If relation already exist continue
			if($adb->num_rows($checkres)) continue;

			$adb->pquery('INSERT INTO aicrm_fieldmodulerel(fieldid, module, relmodule) VALUES(?,?,?)', 
				Array($this->id, $this->getModuleName(), $relmodule));
	...
	function unsetRelatedModules($moduleNames) {
		global $adb;
		foreach($moduleNames as $relmodule) {
			$adb->pquery('DELETE FROM aicrm_fieldmodulerel WHERE fieldid=? AND module=? AND relmodule = ?', 
	...
	static function getInstance($value, $moduleInstance=false) {
		...
		if(Vtiger_Utils::isNumber($value)) {
			$query = "SELECT * FROM aicrm_field WHERE fieldid=?";
			$queryParams = Array($value);
		} else {
			$query = "SELECT * FROM aicrm_field WHERE fieldname=? AND tabid=?";
	 ...
	 */
	 static function getAllForBlock($blockInstance, $moduleInstance=false) {
		...
		$queryParams = false;
		if($moduleInstance) {
			$query = "SELECT * FROM aicrm_field WHERE block=? AND tabid=?";
			$queryParams = Array($blockInstance->id, $moduleInstance->id);
		} else {
			$query = "SELECT * FROM aicrm_field WHERE block=?";
	...
	static function getAllForModule($moduleInstance) {
		...
		$query = "SELECT * FROM aicrm_field WHERE tabid=?";
	...
	static function deleteForModule($moduleInstance) {
		global $adb;
		$adb->pquery("DELETE FROM aicrm_field WHERE tabid=?", Array($moduleInstance->id));
