...
class Vtiger_ModuleBasic {
	...
	function initialize2() {
		global $adb;
		$result = $adb->pquery("SELECT tablename,entityidfield FROM aicrm_entityname WHERE tabid=?", 
			Array($this->id));
	...
	function __getUniqueId() {
		global $adb;
		$result = $adb->query("SELECT MAX(tabid) AS max_seq FROM aicrm_tab");
	...
	function __getNextSequence() {
		global $adb;
		$result = $adb->query("SELECT MAX(tabsequence) AS max_tabseq FROM aicrm_tab");
	...
	function __handleVtigerCoreSchemaChanges() {
		...
		Vtiger_Utils::AddColumn('aicrm_tab', 'version', ' VARCHAR(10)');
	}
	...
	function __create() {
		...
		$adb->pquery("INSERT INTO aicrm_tab (tabid,name,presence,tabsequence,tablabel,modifiedby,
		...
		$adb->pquery('UPDATE aicrm_tab set isentitytype=? WHERE tabid=?',Array($useisentitytype, $this->id));
	...
	function __delete() {
		...
		$adb->pquery("DELETE FROM aicrm_tab WHERE tabid=?", Array($this->id));
	...
	function __updateVersion($newversion) {
		$this->__handleVtigerCoreSchemaChanges();
		global $adb;
		$adb->pquery("UPDATE aicrm_tab SET version=? WHERE tabid=?", Array($newversion, $this->id));
	...
	function initTables($basetable=false, $basetableid=false) {
		...
		if(!$this->basetable) $this->basetable = "aicrm_$lcasemodname";
	...
	function setEntityIdentifier($fieldInstance) {
		...
		if($this->entityidfield && $this->entityidcolumn) {
			$adb->pquery("INSERT INTO aicrm_entityname(tabid, modulename, tablename, fieldname, entityidfield, entityidcolumn) VALUES(?,?,?,?,?,?)",
	...
	function unsetEntityIdentifier() {
		global $adb;
		$adb->pquery("DELETE FROM aicrm_entityname WHERE tabid=?", Array($this->id));
	...
	function deleteRelatedLists() {
		global $adb;
		$adb->pquery("DELETE FROM aicrm_relatedlists WHERE tabid=?", Array($this->id));
