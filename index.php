 ...
 ********************************************************************************/
/*********************************************************************************
 * $Header: /advent/projects/wesat/aicrm_crm/sugarcrm/index.php,v 1.93 2005/04/21 16:17:25 ray Exp $
...
//$randomStr = md5($_SERVER['REMOTE_ADDR']); 
...
global $adb, $aicrm_current_version;
if(isset($_SESSION['VTIGER_DB_VERSION']) && isset($_SESSION['authenticated_user_id'])) {
    if(version_compare($_SESSION['VTIGER_DB_VERSION'], $aicrm_current_version, '!=')) {
        unset($_SESSION['VTIGER_DB_VERSION']);
        header("Location: install.php");
        exit();
    }
} else {
    $result = $adb->query("SELECT * FROM aicrm_version");
    $dbversion = $adb->query_result($result, 0, 'current_version');
    if(version_compare($dbversion, $aicrm_current_version, '=')) {
...
if($use_current_login){
	...
	if(!in_array("internal_mailer", $arr))
	{
		...
		$adb->pquery("alter table aicrm_users add column tagcloud_view int(1) default 1", array());
	}
	//End*/

	//getting the internal_mailer flag
	if(!isset($_SESSION['internal_mailer'])){
		$qry_res = $adb->pquery("select internal_mailer from aicrm_users where id=?", array($_SESSION["authenticated_user_id"]));
...
if($use_current_login)
{
	...
	if($audit_trail == 'true')
	{
		...
		if (!$skip_auditing) {
			...
			$qparams = array($adb->getUniqueID('aicrm_audit_trial'), $current_user->id, $module, $action, $auditrecord, $date_var);
...
if(isset($_SESSION['aicrm_authenticated_user_theme']) && $_SESSION['aicrm_authenticated_user_theme'] != '')
{
	$theme = $_SESSION['aicrm_authenticated_user_theme'];
}
else 
{
	$theme = $default_theme;
...
if (isset($_SESSION['aicrm_authenticated_user_theme'])) {
        $log->debug("setting cookie ck_login_theme_vtiger to ".$_SESSION['aicrm_authenticated_user_theme']);
        setcookie('ck_login_theme_vtiger', $_SESSION['aicrm_authenticated_user_theme']);
}
...
if($_SESSION['authenticated_user_id']!=""){
	$uuid = time();
	$fileName = 'logs/autologout/log-'.date('Y-m-d').'.txt';
	$fileHandle = fopen($fileName, 'a+') or die("can't open file");

	$sql="SELECT count(*) as count FROM ai_check_user_login where end_time>'".date("Y-m-d H:i:s")."' ";
	$result = $adb->query($sql);
	$dbversion = $adb->query_result($result, 0, 'count');
	$chk=$dbversion;
	if($chk==$list_max_user){
		fwrite($fileHandle, "(logID: ".$uuid.") ".date('Y-m-d H:i:s')." => SessionData: ".json_encode($_SESSION, JSON_UNESCAPED_UNICODE)."\r\n");
		fwrite($fileHandle, "(logID: ".$uuid.") ".date('Y-m-d H:i:s')." => LoginData: ".json_encode(['max_user'=>$list_max_user, 'active_user'=>$chk], JSON_UNESCAPED_UNICODE)."\r\n");
		fwrite($fileHandle, "(logID: ".$uuid.") ".date('Y-m-d H:i:s')." => ErrorCase: ".json_encode(['case'=>'max_user เท่ากับ active_user', 'message'=>$mod_strings['ERR_USER_LOGIN']], JSON_UNESCAPED_UNICODE)."\r\n");

		unset($_SESSION['authenticated_user_id']);
		$_SESSION['login_error1'] = $mod_strings['ERR_USER_LOGIN'];
		echo "<script type='text/javascript'> window.location.replace('./index.php?action=Timeout');</script>";
		exit;
	}else{
		$sql="SELECT count(*) as count FROM ai_check_user_login where  user_id='".$_SESSION["user_id"]."' 
		and id='".$_SESSION["login_id"]."' and status =1
		";	
		$result = $adb->query($sql);
		...
		if($count>0){
			fwrite($fileHandle, "(logID: ".$uuid.") ".date('Y-m-d H:i:s')." => SessionData: ".json_encode($_SESSION, JSON_UNESCAPED_UNICODE)."\r\n");
			fwrite($fileHandle, "(logID: ".$uuid.") ".date('Y-m-d H:i:s')." => LoginData: ".json_encode(['max_user'=>$list_max_user, 'active_user'=>$chk], JSON_UNESCAPED_UNICODE)."\r\n");
			fwrite($fileHandle, "(logID: ".$uuid.") ".date('Y-m-d H:i:s')." => ErrorCase: ".json_encode(['case'=>'มีข้อมูล ai_check_user_login status เท่ากับ 1', 'message'=>$mod_strings['ERR_USER_LOGIN1']], JSON_UNESCAPED_UNICODE)."\r\n");

			unset($_SESSION['authenticated_user_id']);
			$_SESSION['login_error1'] = $mod_strings['ERR_USER_LOGIN1'];
			echo "<script type='text/javascript'> window.location.replace('./index.php?action=Timeout');</script>";
			exit;
		}else{
			$sql="SELECT count(*) as count FROM ai_check_user_login where  user_id='".$_SESSION["user_id"]."' 
			...
			if($count>0){
				fwrite($fileHandle, "(logID: ".$uuid.") ".date('Y-m-d H:i:s')." => SessionData: ".json_encode($_SESSION, JSON_UNESCAPED_UNICODE)."\r\n");
				fwrite($fileHandle, "(logID: ".$uuid.") ".date('Y-m-d H:i:s')." => LoginData: ".json_encode(['max_user'=>$list_max_user, 'active_user'=>$chk], JSON_UNESCAPED_UNICODE)."\r\n");
				fwrite($fileHandle, "(logID: ".$uuid.") ".date('Y-m-d H:i:s')." => ErrorCase: ".json_encode(['case'=>'มีข้อมูล ai_check_user_login status เท่ากับ 0 end_time น้อยกว่าปัจจุบัน', 'message'=>$mod_strings['ERR_USER_LOGIN1']], JSON_UNESCAPED_UNICODE)."\r\n");
				
				$query = "update ai_check_user_login set status=? where user_id=? and id=? and ipaddress=?";
				$params = array('1',$_SESSION["user_id"],$_SESSION["login_id"],$_SESSION["ipaddress"]);				
				...
				$query = "update ai_check_user_login set start_time=?,end_time=? where user_id=? and ipaddress=? and id=?";
				$params = array($start_date,$end_time,$_SESSION["user_id"],$_SESSION["ipaddress"],$_SESSION["login_id"]);			
				$adb->pquery($query, $params);
			}
		}
	}
...
//echo 'display: '.$display.' module: '.$module;
if($display == "no" && $module!='Quotes' && $module!='Calendar')
{
	...
	
	{
		...
		if($currentModule == 'Users' && empty($current_user->id)) {
			...
			if(isset($disable_stats_tracking) && !empty($disable_stats_tracking)) {
				...
				$statimage = "<img src='http://stats.vtiger.com/stats.php?uid=$application_unique_key&v=$aicrm_current_version&type=U' 
				alt='|' title='' border=0 width='1px' height='1px'>";
			}
		}
		// END
		echo "<script language = 'JavaScript' type='text/javascript' src = 'include/js/popup.js'></script>";
		echo "<br><br><br><table border=0 cellspacing=0 cellpadding=5 width=100% class=footer >";
echo "<tr><td class=small align=left><span style='color: #2B2B2B; font-family: PromptMedium; font-weight: 400;'>AI-CRM v.2.0</span></td>";
		echo "<td class=small align=right><span style='color: #2B2B2B; font-family: PromptMedium; font-weight: 400;'>&copy; ".date('Y')." <a href='https://www.ai-crm.com' target='_blank' style='color: #2B2B2B; font-family: PromptMedium;'>https://www.ai-crm.com</a></span></td></tr></table>";
