...
$cloudtag = Array ('SO_vendtl', 'X-CEED', 'X-CEED', 'aicrm_50usr');
...
for($i = 0; $i < $company_name_count; $i++) {
	...
	if ($i > 3) {
		$freetag = $adb->getUniqueId('aicrm_freetags');
		$query = "insert into aicrm_freetags values (?,?,?)";
		$qparams = array($freetag, $cloudtag[1], $cloudtag[1]);
		$res = $adb->pquery($query, $qparams);

		$date = $adb->formatDate(date('YmdHis'), true); 
		$query_tag = "insert into aicrm_freetagged_objects values (?,?,?,?,?)";
...
for($i=0; $i<10; $i++)
{
	...
	$query = "insert into aicrm_contpotentialrel ( contactid, potentialid ) values (?,?)";
...
$isubj_array = array ("aicrm_invoice201", "zoho_inv7841", "vtiger5usrp_invoice71134", "vt100usrpk_inv113", "vendtl_inv214");
...
$adb->query("UPDATE aicrm_activity_reminder_popup set status = 1");

$adb->pquery("update aicrm_crmentity set smcreatorid=?", array($assigned_user_id));
...
for($i=0;$i<5;$i++)
{
	$portalid = $adb->getUniqueId('aicrm_portal');
	$portal_qry = "insert into aicrm_portal values (?,?,?,?,?)";
...
for($i=0;$i<2;$i++)
{
	$rssid = $adb->getUniqueId('aicrm_rss');
	$rss_qry = "insert into aicrm_rss values (?,?,?,?,?)";
