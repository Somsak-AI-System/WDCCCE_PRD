webshims.register("form-shim-extend2", function(a, b, c, d, e, f) {
    ...
    "use strict";
    var g, h, i, j = 'input[type="file"].ws-filereader, input[type="file"].ws-capture',
        ...
        m = function() {
            var c, d, e, f, h = this;
            b.implement(h, "filepicker") && (h = this, c = a(this), e = c.parent(), f = function() {
                        ...
                        return b.info("xdomain transport used."), {
                            send: function(b, c) {
                                ...
                                i = new XDomainRequest, /^\d+$/.test(h.timeout) && (i.timeout = h.timeout), i.ontimeout = function() {
                                    ...
                                    try {
                                        if ("html" === e || /text\/html/i.test(i.contentType)) f.html = i.responseText;
                                        else if ("json" === e || "text" !== e && /\/json/i.test(i.contentType)) try {
    ...
    b.loader.addModule("moxie", {
        ...
        URL.createObjectURL = function(b) {
            var c = b;
            if (b._wsimgDataURL) c = b._wsimgDataURL;
            else if (a) return a.apply(this, arguments);
            return c
        ...
        h = c.moxie, g = c.mOxie, g.Env.swf_url = f.swfpath, c.FileReader = g.FileReader, c.FormData = function(c) {
            var d, e, f, h, i, j, k, l = new g.FormData;
            if (c && a.nodeName(c, "form")) {
                for (d = a(c).serializeArray(), e = 0; e < d.length; e++) Array.isArray(d[e].value) ? d[e].value.forEach(function(a) {
                    l.append(d[e].name, a)
                }) : l.append(d[e].name, d[e].value);
                for (d = c.querySelectorAll('input[type="file"][name]'), e = 0, f = d.length; e < d.length; e++)
                    if (k = d[e].name, k && !a(d[e]).is(":disabled") && (h = a.prop(d[e], "files") || [], h.length))
                        for ((h.length > 1 || l.hasBlob && l.hasBlob()) && b.error("FormData shim can only handle one file per ajax. Use multiple ajax request. One per file."), i = 0, j = h.length; j > i; i++) l.append(k, h[i])
            }
            return l
        }, o = m, w.moxie = v, f.mimeTypes = f.mimeTypes ? d + "," + f.mimeTypes : d;
        try {
        ...
        a(b.querySelectorAll(j)).add(c.filter(j)).each(o)
