 ...
 * if any call is found, it just inserts the values into the aicrm_asteriskincomingcalls table
 * 
		...
		while(true){
			...
			if(($response['Event'] == 'Newexten') && (strstr($response['AppData'],"__DIALED_NUMBER") || strstr($response['AppData'],"EXTTOCALL"))){
				...
				if(checkExtension($extension, $adb)){
					...
					$sql = "insert into aicrm_asteriskincomingcalls values (?,?,?,?,?,?)";
					$flag= 0;
					$timer = time();
...
function checkExtension($ext, $adb){
	$sql = "select * from aicrm_asteriskextensions where asterisk_extension='$ext'";
